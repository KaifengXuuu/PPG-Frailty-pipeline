# pttppg_pipeline_v7_2_noleak.py
# ------------------------------------------------------------
# v7.2 (NO-LEAK) — subject-split first, then train/eval
#
# Key guarantees:
#  - Holdout subjects are NEVER used for training, early stopping, threshold fitting, or hyperparam selection.
#  - CV is performed ONLY within train-subjects with GroupKFold subject splits (no subject overlap).
#  - Holdout evaluation uses a model trained on inner-train subjects and early-stopped on inner-val subjects (both from train partition).
#  - AE detector threshold is fit ONLY on inner-val (train partition), then fixed for all eval (CV-val and holdout).
#
# Requirements implemented:
# 1) Strict learning by filename activity: sit / walk / run
#    - AE trained only on sit-clean windows (acc_rms <= motion_thresh) from training subjects
#    - Denoiser trained separately for walk and run
# 2) ECG/peaks supervision-only, not input:
#    - Setup1: no ECG supervision
#    - Setup2: peaks-derived HR used as supervision loss term (soft HR estimator on model output)
# 3) Both CV and holdout, with no leakage, and output plots/CSVs/JSON.
#
# Progress bars:
#  - Overall stages
#  - Per-fold CV
#  - Per-epoch training (AE + denoise)
# ------------------------------------------------------------
...
class DatasetAE(Dataset):
    ...
    - label is still returned for evaluation (motion vs no-motion by IMU threshold)
    """
...
class SoftHRFromFFT(nn.Module):
    """
    Differentiable HR estimate from output yhat via soft-argmax over rFFT magnitudes in [fmin,fmax].
    Returns hr_hat_bpm: (B,)
    """
...
def make_subject_splits(
    ...
    """
    Make splits BEFORE any training:
      - train subjects
      - holdout subjects
      - inner-train / inner-val subjects (within train partition) for early stopping and threshold fitting
      - CV folds (GroupKFold) within train subjects
    """
    ...
    # inner split for early stopping + threshold (still within train partition)
...
def fit_detector_threshold_from_inner_val(
    ...
    # threshold MUST be derived only from train partition (inner-val), not from holdout
...
def eval_detector_fixed_thr(model: nn.Module, ds: DatasetAE, thr: float) -> dict:
    device = next(model.parameters()).device
    L = DataLoader(ds, batch_size=128, shuffle=False, num_workers=0, pin_memory=torch.cuda.is_available())
    ...
    if not losses:
        return {"threshold": thr, "pr_auc": float("nan"), "roc_auc": float("nan"), "f1": float("nan"), "bal_acc": float("nan"), "n_windows": 0}
    ...
    if len(np.unique(pred)) > 1:
        ...
        bal = float(metrics.balanced_accuracy_score(labels, pred))
    else:
        f1 = float("nan"); bal = float("nan")

    return {"threshold": float(thr), "pr_auc": pr_auc, "roc_auc": roc_auc, "f1": f1, "bal_acc": bal, "n_windows": int(losses.size)}

def eval_detector_with_arrays(model: nn.Module, ds: DatasetAE, thr: float) -> dict:
    """
    Like eval_detector_fixed_thr, but also returns arrays:
      - y_true (0/1)
      - y_pred (0/1)
      - scores (recon loss)
    """
    ...
    # metrics (safe when single-class)
...
def train_denoiser(
    ...
    for ep in tqdm(range(1, epochs + 1), desc=progress_desc, leave=False):
        ...
        for x, y_proxy, is_clean, hr_ecg in TL:
            ...
            # proxy rec only on clean windows
            if (is_clean > 0.5).any():
...
def run_ae_cv_and_holdout_noleak(
    ...
    # Prepare holdout training records
    ...
    # Holdout model (trained once):
    # Train AE on sit-clean windows of inner-train subjects, validate on sit-clean of inner-val subjects
    ...
    if len(tr_ds) == 0 or len(va_ds) == 0:
        # can't train AE reliably; still record this
    ...
    # Fit threshold ONLY on inner-val sit-clean
    ...
    # Holdout eval per domain (with fixed thr)
    for dom in eval_domains:
        ...
        if len(ho_ds) == 0:
            ...
            continue
        m = eval_detector_fixed_thr(ae_hold, ho_ds, thr=thr)
        m.update({"domain": dom, "best_val_loss_inner": float(best_val)})
        res["holdout"][dom] = m

    # CV: within train subjects only, for each fold:
    # For each fold, fit threshold from that fold's inner-val (the fold val sit-clean), and evaluate on fold val domain.
    for dom in eval_domains:
        fold_rows = []
        for fold_id, (fold_train_subs, fold_val_subs) in enumerate(tqdm(cv_folds, desc=f"AE CV folds (eval={dom})", leave=False), start=1):
            ...
            m = eval_detector_fixed_thr(ae, fold_va_dom, thr=fold_thr)
            m.update({"fold": fold_id, "domain": dom, "best_val_loss": float(fold_best), "threshold": float(fold_thr)})
            fold_rows.append(m)
...
def run_denoise_cv_and_holdout_noleak(
    ...
    # HOLDOUT training (once): inner-train vs inner-val, then eval on holdout subjects
    inner_train_recs = records_from_subjects(sub2recs, inner_train_subjects)
    inner_val_recs = records_from_subjects(sub2recs, inner_val_subjects)
    holdout_recs = records_from_subjects(sub2recs, holdout_subjects)
    ...
    # CV folds within train subjects
    fold_rows = []
    for fold_id, (fold_train_subs, fold_val_subs) in enumerate(tqdm(cv_folds, desc=f"Denoise CV folds ({activity}, setup{setup})", leave=False), start=1):
        fold_train_recs = records_from_subjects(sub2recs, fold_train_subs)
        fold_val_recs = records_from_subjects(sub2recs, fold_val_subs)
...
def run_experiment_v72_noleak(
    ...
    # setup2: ECG HR supervision via peaks (still no ECG input)
    for act in denoise_domains:
    ...
    print(f"[v7.2 NO-LEAK] Done in {(time.time()-t0)/60:.1f} min. Results: {outroot}")
    return out
...
def parse_cli():
    ...
    ap.add_argument("--outdir", type=str, default="results_v72_noleak")
