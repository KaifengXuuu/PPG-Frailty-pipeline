...
def cnn_cache_name(config: RunConfig) -> Path:
    ...
    return out_dir / (
        f"frailty3_cnn_windows_{role_cache_tag(config)}_fs{config.fs:g}_"
...
def extract_cnn_windows_from_file(path: Path, config: RunConfig) -> np.ndarray:
    ...
    seq_samples = max(8, int(round(config.cnn_seq_sec * config.fs)))
    hop_samples = max(1, int(round(config.cnn_hop_sec * config.fs)))
...
def run_auto_sweep(args: argparse.Namespace) -> None:
    ...
    for window_sec in window_secs:
        for overlap in overlap_fracs:
            hop_sec = max(1.0 / base_config.fs, float(window_sec) * (1.0 - float(overlap)))
