...
# ===========================
# Deep models for AE fusion in Detector (v7.4)
# ===========================
class CNNBiLSTMAE(nn.Module):
    """CNN + BiLSTM autoencoder for window-level reconstruction.
    Input shape: (B, C, T). Output same shape.
    """
    def __init__(self, in_ch: int = 2, hidden: int = 256, bottleneck: int = 128):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        B, C, T = x.shape
        z = self.encoder(x).permute(0, 2, 1)  # (B, T', 64)
        z, _ = self.bi_lstm(z)                # (B, T', hidden)
        z = self.bottleneck(z)                # (B, T', bottleneck)
        z, _ = self.decoder_lstm(z)           # (B, T', hidden)
        z = z.permute(0, 2, 1)                # (B, hidden, T')
        z = F.interpolate(z, size=T, mode="linear", align_corners=False)
        return self.to_channels(z)

class AEDataset(Dataset):
    """Window dataset for AE training and scoring (subject-level split is handled outside).
    Uses bandpassed pleth_1/pleth_2 and optional DWT compression.
    """
    def __init__(self, records: List[Dict[str,Any]], win_cfg: 'WinConfig', use_dwt: bool = True):
        self.records = records
        self.win_cfg = win_cfg
        self.use_dwt = use_dwt
        self.index: List[Tuple[int,int,int,int]] = []  # (rec_i, s, e, y)
        for i, r in enumerate(records):
            N = len(r["pleth_2"])
            y = 0 if r["_activity"] == "sit" else 1
    ...
    def __getitem__(self, idx: int):
        i, s, e, y = self.index[idx]
        r = self.records[i]
        p1 = r["pleth_1"][s:e].astype(np.float32)
        p2 = r["pleth_2"][s:e].astype(np.float32)

        if self.use_dwt:
            p1 = dwt_compress(p1, level=2).astype(np.float32)
            p2 = dwt_compress(p2, level=2).astype(np.float32)

        x = np.stack([zscore_np(p1), zscore_np(p2)], axis=0).astype(np.float32)  # (2,T)
...
def train_ae(
    ...
    batch_size: int = 64,
    use_dwt: bool = True,
) -> Tuple[CNNBiLSTMAE, Dict[str,Any]]:
    """Train AE only on SIT windows from train_records, early-stop on SIT windows from val_records."""
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = CNNBiLSTMAE(in_ch=2).to(device)
    opt = torch.optim.Adam(model.parameters(), lr=lr)

    # Filter records: only SIT for training/validation (unsupervised baseline).
    tr = [r for r in train_records if r["_activity"] == "sit"]
    va = [r for r in val_records if r["_activity"] == "sit"]

    tr_ds = AEDataset(tr, win_cfg, use_dwt=use_dwt)
    va_ds = AEDataset(va, win_cfg, use_dwt=use_dwt)
    tr_ld = DataLoader(tr_ds, batch_size=batch_size, shuffle=True, drop_last=False)
    va_ld = DataLoader(va_ds, batch_size=batch_size*2, shuffle=False, drop_last=False)

    best = {"val": float("inf"), "state": None}
    pbar = tqdm(range(1, epochs + 1), desc="AE epochs", leave=False)
    for ep in pbar:
        model.train()
        tr_loss = 0.0
        for x, _ in tr_ld:
        ...
        tr_loss = tr_loss / max(1, len(tr_ld.dataset))
        ...
        with torch.no_grad():
            for x, _ in va_ld:
                x = x.to(device)
                yhat = model(x)
                va_loss += float(F.mse_loss(yhat, x).item()) * x.size(0)
        va_loss = va_loss / max(1, len(va_ld.dataset))

        pbar.set_postfix(train=f"{tr_loss:.4f}", val=f"{va_loss:.4f}")
        if va_loss < best["val"]:
            best = {"val": va_loss, "state": {k: v.detach().cpu() for k, v in model.state_dict().items()}}

    if best["state"] is not None:
        model.load_state_dict(best["state"])
    return model, {"best_val": float(best["val"]), "use_dwt": bool(use_dwt)}

@torch.no_grad()
def ae_scores(model: CNNBiLSTMAE, records: List[Dict[str,Any]], win_cfg: 'WinConfig', use_dwt: bool = True) -> Tuple[np.ndarray,np.ndarray]:
    """Return (scores, y_true) over all windows (sit=0, walk/run=1)."""
    device = next(model.parameters()).device
    ds = AEDataset(records, win_cfg, use_dwt=use_dwt)
    ld = DataLoader(ds, batch_size=256, shuffle=False, drop_last=False)
    scores = []
    ys = []
    model.eval()
    for x, y in ld:
        x = x.to(device)
        yhat = model(x)
        l = F.mse_loss(yhat, x, reduction="none").mean(dim=[1,2]).detach().cpu().numpy()
        scores.append(l)
        ys.append(y.numpy())
    if not scores:
        return np.asarray([], dtype=np.float32), np.asarray([], dtype=int)
    return np.concatenate(scores).astype(np.float32), np.concatenate(ys).astype(int)

def pick_thr_max_balacc(scores: np.ndarray, y: np.ndarray) -> float:
    """Pick an AE anomaly threshold (on training fold only) by maximising balanced accuracy."""
    if len(scores) == 0 or len(np.unique(y)) < 2:
        return float(np.nan)
    qs = np.quantile(scores, np.linspace(0.50, 0.99, 50))
    best_thr = float(qs[-1])
    best_ba = -1.0
    for thr in qs:
...
try:
    ...
    project_funcs = None
bandpass_filter = project_funcs.bandpass_filter
aboypp_peak_hr = project_funcs.aboypp_peak_hr 
# ===========================
...
def dwt_compress(x: np.ndarray, wavelet: str = "db4", level: int = 2) -> np.ndarray:
    """
    DWT low-frequency approximation compression, then resample back to original length.
    Used to stabilize AE training on long windows (optional).
    If PyWavelets is unavailable, returns x unchanged.
    """
    x = np.asarray(x, dtype=np.float32)
    if pywt is None or level <= 0:
        return x
    try:
        coeffs = pywt.wavedec(x, wavelet, level=level)
        a = coeffs[0]  # approximation
        # resample back to original length via linear interpolation
        xp = np.linspace(0.0, 1.0, num=len(a), dtype=np.float32)
        xq = np.linspace(0.0, 1.0, num=len(x), dtype=np.float32)
...
def load_physionet_csv(root: Path, fs: float) -> Dict[str, List[Dict[str, Any]]]:
    ...
    for p in paths:
        ...
        if "pleth_1" in rec:
            rec["pleth_1"] = bandpass_filter(rec["pleth_1"], 0.5, 8.0, fs=fs, order=3)

        if "pleth_2" in rec:
            rec["pleth_2"] = bandpass_filter(rec["pleth_2"], 0.5, 8.0, fs=fs, order=3)
...
def build_window_matrices(
    ...
    for r in tqdm(records, desc="Feature extraction", leave=True):
        N = len(r["pleth_2"])
        act = r["_activity"]
...
def run_pipeline(
    ...
    for k, (tr_idx, va_idx) in enumerate(tqdm(folds, desc="Detector CV folds", leave=True), start=1):
        ...
        # ---------- AE fusion (scheme 1+2): train on SIT windows only, score all windows ----------
        ae_model, ae_info = train_ae(tr_recs, va_recs, win_cfg, epochs=15, lr=lr, batch_size=128, use_dwt=True)

        # scores on train/val (order consistent with split_windows iteration)
        sc_tr, y_tr_ae = ae_scores(ae_model, tr_recs, win_cfg, use_dwt=ae_info["use_dwt"])
        sc_va, y_va_ae = ae_scores(ae_model, va_recs, win_cfg, use_dwt=ae_info["use_dwt"])

        # Align AE scores to the same trimmed window sequence used for IMU lag compensation.
        def align_scores(scores: np.ndarray, y: np.ndarray, Fp: np.ndarray, Fi: np.ndarray, lag: int):
            T = min(len(Fp), len(Fi), len(scores), len(y))
            scores = scores[:T]
            y = y[:T]
            if lag >= 0:
                return scores[:T-lag], y[:T-lag]
            kk = -lag
            return scores[kk:T], y[kk:T]

        sc_tr2, y_tr_ae2 = align_scores(sc_tr, y_tr_ae, Fp_tr, Fi_tr, lag_best)
        sc_va2, y_va_ae2 = align_scores(sc_va, y_va_ae, Fp_va, Fi_va, lag_best)

        # Threshold selection: maximise balanced accuracy on the TRAIN fold only (no leakage).
        ae_thr = pick_thr_max_balacc(sc_tr2, y_tr_ae2)
        pred_ae = (sc_va2 > ae_thr).astype(int)

        # Fused predictions (keep original threshold+lag intact; add AE as an additional vote).
        pred_fused_or = ((pred_or > 0) | (pred_ae > 0)).astype(int)
        pred_fused_and = ((pred_and > 0) & (pred_ae > 0)).astype(int)
        m_or = detector_metrics(y_va2, pred_or)
        m_and = detector_metrics(y_va2, pred_and)

        m_ae = detector_metrics(y_va2, pred_ae, y_score=sc_va2)
        m_fused_or = detector_metrics(y_va2, pred_fused_or)
        m_fused_and = detector_metrics(y_va2, pred_fused_and)

        cm_ae = metrics.confusion_matrix(y_va2, pred_ae)
        cm_fused_or = metrics.confusion_matrix(y_va2, pred_fused_or)
        cm_fused_and = metrics.confusion_matrix(y_va2, pred_fused_and)
        ...
        plot_confusion(cm_ae, fold_out / "confusion_ae.png", f"Detector CV fold{k} (AE only)")
        plot_confusion(cm_fused_or, fold_out / "confusion_fused_or.png", f"Detector CV fold{k} (FUSED OR)")
        plot_confusion(cm_fused_and, fold_out / "confusion_fused_and.png", f"Detector CV fold{k} (FUSED AND)")
        # lag curve
        ...
        det_cv_results.append({
            ...
            "metrics_and": m_and,
            "metrics_ae": m_ae,
            "metrics_fused_or": m_fused_or,
            "metrics_fused_and": m_fused_and,
            "ae": {"thr": float(ae_thr), "best_val": float(ae_info["best_val"]), "use_dwt": bool(ae_info["use_dwt"])} ,
            "n_val": int(len(y_va2)),
    ...
    # ---------- AE fusion (train on train_subjects SIT only; score holdout windows) ----------
    ae_model, ae_info = train_ae(train_records, train_records, win_cfg, epochs=15, lr=lr, batch_size=128, use_dwt=True)
    sc_tr, y_tr_ae = ae_scores(ae_model, train_records, win_cfg, use_dwt=ae_info["use_dwt"])
    sc_ho, y_ho_ae = ae_scores(ae_model, hold_records, win_cfg, use_dwt=ae_info["use_dwt"])

    def align_scores2(scores: np.ndarray, y: np.ndarray, Fp: np.ndarray, Fi: np.ndarray, lag: int):
        T = min(len(Fp), len(Fi), len(scores), len(y))
        scores = scores[:T]; y = y[:T]
        if lag >= 0:
            return scores[:T-lag], y[:T-lag]
        kk = -lag
        return scores[kk:T], y[kk:T]

    sc_tr2, y_tr_ae2 = align_scores2(sc_tr, y_tr_ae, Fp_tr, Fi_tr, lag_best)
    sc_ho2, y_ho_ae2 = align_scores2(sc_ho, y_ho_ae, Fp_ho, Fi_ho, lag_best)

    ae_thr = pick_thr_max_balacc(sc_tr2, y_tr_ae2)
    pred_ae = (sc_ho2 > ae_thr).astype(int)

    pred_fused_or = ((pred_or > 0) | (pred_ae > 0)).astype(int)
    pred_fused_and = ((pred_and > 0) & (pred_ae > 0)).astype(int)
    m_or = detector_metrics(y_ho2, pred_or)
    m_and = detector_metrics(y_ho2, pred_and)
    m_ae = detector_metrics(y_ho2, pred_ae, y_score=sc_ho2)
    m_fused_or = detector_metrics(y_ho2, pred_fused_or)
    m_fused_and = detector_metrics(y_ho2, pred_fused_and)

    cm_or = metrics.confusion_matrix(y_ho2, pred_or)
    cm_and = metrics.confusion_matrix(y_ho2, pred_and)
    cm_ae = metrics.confusion_matrix(y_ho2, pred_ae)
    cm_fused_or = metrics.confusion_matrix(y_ho2, pred_fused_or)
    cm_fused_and = metrics.confusion_matrix(y_ho2, pred_fused_and)
    ...
    plot_confusion(cm_ae, hold_dir / "confusion_ae.png", "Detector HOLDOUT (AE only)")
    plot_confusion(cm_fused_or, hold_dir / "confusion_fused_or.png", "Detector HOLDOUT (FUSED OR)")
    plot_confusion(cm_fused_and, hold_dir / "confusion_fused_and.png", "Detector HOLDOUT (FUSED AND)")
    xs = np.array(sorted(lag_scores.keys()), dtype=float) * win_cfg.hop_sec
    ys = np.array([lag_scores[int(l)] for l in sorted(lag_scores.keys())], dtype=float)
    ...
    detector_artifact = {
        ...
        "metrics_holdout_ae": m_ae,
        "metrics_holdout_fused_or": m_fused_or,
        "metrics_holdout_fused_and": m_fused_and,
        "ae": {"thr": float(ae_thr), "best_val": float(ae_info["best_val"]), "use_dwt": bool(ae_info["use_dwt"])},
        "win_cfg": win_cfg.__dict__,
...
# Persist reusable Detector parameters (CPU-friendly):
#  - rule-based thresholds and lag (NumPy NPZ)
#  - AE weights + metadata (PyTorch .pt + JSON)
    np.savez_compressed(
    det_dir / "detector_rule_params.npz",
    ppg_feature_names=np.array(names_ppg, dtype=object),
    imu_feature_names=np.array(names_imu, dtype=object),
    taus_ppg=np.asarray(taus_ppg, dtype=np.float32),
    dirs_ppg=np.asarray(dirs_ppg, dtype=np.int32),
    taus_imu=np.asarray(taus_imu, dtype=np.float32),
    dirs_imu=np.asarray(dirs_imu, dtype=np.int32),
    lag_best_steps=np.asarray([lag_best], dtype=np.int32),
    lag_best_sec=np.asarray([lag_best * win_cfg.hop_sec], dtype=np.float32),
)
    torch.save({"state_dict": ae_model.state_dict(), "thr": float(ae_thr), "meta": ae_info}, det_dir / "detector_ae.pt")
