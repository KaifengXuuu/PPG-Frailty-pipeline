...
try:
    ...
    raise ImportError("LegacyEffectSizeShapeFormer requires the optional deep dependency") from exc
...
def _selection_hash(
    ...
    payload = "\n".join(
        ...
        for participant, file_id, window_id in zip(participant_ids, file_ids, window_ids)).encode("utf-8")
    return hashlib.sha256(payload).hexdigest()
...
class LegacyEffectSizeShapelets:
    ...
    def __post_init__(self) -> None:
        ...
        count = len(values)
        if count == 0 or any(
                value.ndim != 1 or value.shape[0] != int(self.shapelet_length_samples) or not np.isfinite(value).all()
                for value in values):
        ...
        if not np.isfinite(arrays["source_scores"]).all() or not np.isfinite(arrays["source_weights"]).all():
        ...
        if np.any(arrays["source_ends"] - arrays["source_starts"] != int(self.shapelet_length_samples)):
            raise ValueError("legacy source endpoints disagree with shapelet length")
        if np.any(arrays["source_starts"] < 0) or np.any(arrays["source_ends"] > int(self.sequence_length_samples)):
        ...
        if not set(arrays["source_sample_indices"].tolist()) <= set(discovery_indices.tolist()):
        ...
        classes, class_counts = np.unique(arrays["source_classes"], return_counts=True)
        ...
        if not fitted or self.outer_train_participant_hash != _participant_roster_hash(fitted):
...
def _balanced_indices(y: np.ndarray, maximum: int, seed: int) -> np.ndarray:
    ...
    for class_value in classes:
        ...
        chosen.extend(rng.choice(indices, size=take, replace=False).astype(int).tolist())
    if len(chosen) < maximum:
        remaining = np.setdiff1d(np.arange(len(y)), np.asarray(chosen, dtype=np.int64))
        if remaining.size:
            fill = min(maximum - len(chosen), remaining.size)
            chosen.extend(rng.choice(remaining, size=fill, replace=False).astype(int).tolist())
...
def _overlaps_existing(
    ...
    *,
    ...
    return any(
        max(0,
            min(end, old_end) - max(start, old_start)) / length > maximum_overlap for old_start, old_end in occupied)
...
def discover_legacy_effect_size_shapelets(
    ...
    *,
    ...
    if any(isinstance(value, (bool, np.bool_)) or int(value) <= 0 for value in controls.values()):
    ...
    if int(shapelet_length_samples) < 4:
        raise ValueError("legacy shapelet length must be at least four effective samples")
    ...
    for class_value in sorted(np.unique(discovery_y).tolist()):
        ...
        for channel in range(discovery_x.shape[1]):
            ...
            for start in range(0, discovery_x.shape[-1] - length + 1, stride):
                ...
                channel_candidates.append((float(np.mean(effect[channel, start:end])), channel, start, end))
        ...
        occupied: dict[int, list[tuple[int, int]]] = {channel: [] for channel in range(discovery_x.shape[1])}
        ...
        while selected_for_class < int(shapelets_per_class):
            ...
            start = int(rng.integers(0, discovery_x.shape[-1] - length + 1))
            end = start + length
            source_index = int(discovery_indices[local_source])
            selected.append((
                source_index,
    ...
    return LegacyEffectSizeShapelets(
        ...
        discovery_selection_hash=_selection_hash(discovery_participants, discovery_files, discovery_windows),
...
class LegacyShapeFormerAttention(nn.Module):
    ...
    def forward(self, values: torch.Tensor) -> torch.Tensor:
        ...
        keys = self.key(values).reshape(batch, tokens, self.heads, -1).permute(0, 2, 3, 1)
        projected_values = self.value(values).reshape(batch, tokens, self.heads, -1).transpose(1, 2)
        queries = self.query(values).reshape(batch, tokens, self.heads, -1).transpose(1, 2)
        attention = functional.softmax(torch.matmul(queries, keys) * self.scale, dim=-1)
...
class LegacyShapeBlock(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.selected_projection = nn.Linear(self.kernel_size, shape_embedding_channels)
        self.shapelet_projection = nn.Linear(self.kernel_size, shape_embedding_channels)
        complexity = np.sqrt(np.sum(np.square(np.diff(array)))) + (1.0 / self.complexity_norm)
        self.register_buffer("shapelet_complexity", torch.tensor(complexity, dtype=torch.float32))

    def forward(self, values: torch.Tensor) -> torch.Tensor:
        series = values[:, self.source_channel, self.search_start:self.search_end]
        ...
        complexities = (differences.unfold(1, self.kernel_size - 1,
                                           1).contiguous().reshape(-1, self.kernel_size - 1).sum(dim=1))
        complexities = complexities + (1.0 / self.complexity_norm)
        shapelet_complexity = self.shapelet_complexity.to(values.device)
        ratios = torch.maximum(complexities, shapelet_complexity) / torch.minimum(complexities, shapelet_complexity)
        ...
        distances = torch.where(torch.isfinite(distances), distances,
                                torch.ones_like(distances)).reshape(values.shape[0], -1)
        best = torch.argmin(distances, dim=1)
        selected = windows[torch.arange(values.shape[0], device=values.device), best]
        return (self.selected_projection(selected) - self.shapelet_projection(self.shapelet.unsqueeze(0))).unsqueeze(1)
...
class LegacyEffectSizeShapeFormer(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        if input_fs_hz is None or not np.isclose(input_fs_hz, shapelets.input_fs_hz, rtol=0.0, atol=1e-12):
        ...
        if int(local_embedding_channels) % int(attention_heads) or int(shape_embedding_channels) % int(attention_heads):
        ...
        if set(shapelets.source_classes.tolist()) != set(range(int(n_classes))):
            raise ValueError("legacy shapelet classes must match the classifier label space")
        ...
        self.shapelet_search_window_samples = int(shapelet_search_window_samples)
        self.complexity_norm = float(complexity_norm)
        self.max_complexity_ratio = float(max_complexity_ratio)
        ...
        self.candidates_per_class_channel = int(shapelets.candidates_per_class_channel)
        self.discovery_window_count = int(shapelets.discovery_indices.size)
        self.enumerated_candidate_count = int(shapelets.enumerated_candidate_count)
        ...
        self.shapelet_weights = nn.Parameter(torch.as_tensor(shapelets.source_weights, dtype=torch.float32))
        ...
        self.channel_projection = nn.Linear(channel_positions.shape[1], self.shape_embedding_channels)
        self.start_projection = nn.Linear(start_positions.shape[1], self.shape_embedding_channels)
        self.end_projection = nn.Linear(end_positions.shape[1], self.shape_embedding_channels)
        ...
        self.feature_dim = self.shape_embedding_channels + self.local_embedding_channels
    ...
    def forward_features(
        ...
        if mask is not None:
            ...
            if not bool(mask.to(dtype=torch.bool).all()):
                raise ValueError("legacy ShapeFormer requires complete, unpadded windows")
        ...
        shape = torch.cat([block(values) for block in self.shape_blocks], dim=1)
        positions = (self.channel_projection(self.channel_positions) + self.start_projection(self.start_positions) +
