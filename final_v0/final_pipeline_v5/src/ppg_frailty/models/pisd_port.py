...
DISCOVERY_BALANCES = frozenset({DISCOVERY_BALANCE, "class_window_balanced"})
...
class PisdShapelets:
    ...
    def __post_init__(self) -> None:
        ...
        if not np.isfinite(scores).all() or np.any(scores < -1.0) or np.any(scores > 1.0):
            raise ValueError("information gains must be finite in [-1,1]; -1 is the exact "
        ...
        if self.discovery_balance not in DISCOVERY_BALANCES:
            raise ValueError("PISD discovery_balance must be one of " f"{sorted(DISCOVERY_BALANCES)}")
        if not np.isfinite(self.input_fs_hz) or self.input_fs_hz <= 0.0:
            raise ValueError("input_fs_hz must be finite and positive")
        if not np.isfinite(self.num_pip_ratio) or not 0.0 < float(self.num_pip_ratio) <= 1.0:
            raise ValueError("num_pip_ratio must be finite in (0,1]")
        if (isinstance(self.shapelets_per_class, (bool, np.bool_))
                or not isinstance(self.shapelets_per_class, (int, np.integer)) or int(self.shapelets_per_class) <= 0):
            raise ValueError("shapelets_per_class must be a positive integer")
        if (isinstance(self.max_discovery_windows,
                       (bool, np.bool_)) or not isinstance(self.max_discovery_windows, (int, np.integer))
                or int(self.max_discovery_windows) <= 0):
            raise ValueError("max_discovery_windows must be a positive integer")
        if (isinstance(self.position_search_neighbourhood_samples, (bool, np.bool_))
                or not isinstance(self.position_search_neighbourhood_samples,
                                  (int, np.integer)) or int(self.position_search_neighbourhood_samples) <= 0):
            raise ValueError("position_search_neighbourhood_samples must be a positive integer")
        ...
        if self.candidate_generation_rule != CANDIDATE_GENERATION_RULE:
            raise ValueError(f"candidate_generation_rule must be {CANDIDATE_GENERATION_RULE}")
        if self.candidate_enumeration_rule != CANDIDATE_ENUMERATION_RULE:
            raise ValueError(f"candidate_enumeration_rule must be {CANDIDATE_ENUMERATION_RULE}")
        if self.candidate_ranking_rule != CANDIDATE_RANKING_RULE:
            raise ValueError(f"candidate_ranking_rule must be {CANDIDATE_RANKING_RULE}")
        if self.selected_bank_order_rule != SELECTED_BANK_ORDER_RULE:
            raise ValueError(f"selected_bank_order_rule must be {SELECTED_BANK_ORDER_RULE}")
        if self.discovery_position_search_boundary_rule != DISCOVERY_POSITION_SEARCH_BOUNDARY_RULE:
            raise ValueError("discovery_position_search_boundary_rule must be "
                             f"{DISCOVERY_POSITION_SEARCH_BOUNDARY_RULE}")
        if self.information_gain_split_rule != INFORMATION_GAIN_SPLIT_RULE:
            raise ValueError("information_gain_split_rule must be " f"{INFORMATION_GAIN_SPLIT_RULE}")
        ...
        source_class_values, class_counts = np.unique(integer_arrays["source_classes"], return_counts=True)
        if (tuple(source_class_values.tolist()) != (0, 1, 2) or np.any(class_counts != self.shapelets_per_class)
                or count != 3 * int(self.shapelets_per_class)):
            raise ValueError("Frailty3 OSD/PISD bank requires source_classes {0,1,2} "
        ...
        if any(name != channel_schema[int(index)] for name, index in zip(self.source_channel_names, source_channels)):
            raise ValueError("source channel names/indices disagree with channel_schema")
        selection_payload = "\n".join(f"{participant}\t{file_id}\t{window_id}"
    ...
    def candidate_records(self) -> tuple[dict[str, object], ...]:
        ...
        return tuple({
            ...
            "position_search_neighbourhood_samples": int(self.position_search_neighbourhood_samples),
...
def _perpendicular_distances(
    ...
    return np.abs(-values[positions] + slope * time_axis[positions] + intercept) / np.sqrt(slope**2 + 1.0)
    ...
    *,
    ...
    for class_offset, class_value in enumerate(classes):
        ...
        for index in np.flatnonzero(y == class_value).tolist():
            groups.setdefault(participant_ids[index], {}).setdefault(file_ids[index], []).append(index)
...
def _information_gain(distances: np.ndarray, positive: np.ndarray) -> float:
    ...
    if distance.ndim != 1 or target.shape != distance.shape or distance.size < 2 or not np.isfinite(distance).all():
    ...
    for label in labels[:-1]:
        ...
        if round(left_positive / total_positive, 1) == recall_marker:
            ...
            gain = parent - ((left_count / labels.size) * entropy_counts(left_positive, left_negative) +
                             (right_count / labels.size) * entropy_counts(right_positive, right_negative))
...
def _select_nonoverlapping_candidate_rows(
    ...
    for position, row in enumerate(ranked):
        ...
        overlaps = any(other_channel == channel and max(start, other_start) < min(end, other_end)
...
def _rank_candidate_rows_upstream(
    candidate_rows: list[tuple[float, int, int, int, int, int]], ) -> list[tuple[float, int, int, int, int, int]]:
...
def _order_selected_candidate_rows_upstream(
    selected_rows: list[tuple[float, int, int, int, int, int]], ) -> list[tuple[float, int, int, int, int, int]]:
...
def discover_pisd_shapelets(
    ...
    *,
    ...
    if discovery_method != PISD_DISCOVERY_METHOD:
        raise ValueError(f"the reference requires discovery_method={PISD_DISCOVERY_METHOD}; no fallback is allowed")
    if not np.isfinite(num_pip_ratio) or not 0.0 < float(num_pip_ratio) <= 1.0:
        raise ValueError("num_pip_ratio must be finite in (0,1]")
    if (isinstance(shapelets_per_class, (bool, np.bool_)) or not isinstance(shapelets_per_class, (int, np.integer))
            or int(shapelets_per_class) <= 0):
        raise ValueError("shapelets_per_class must be a positive integer")
    if (isinstance(max_discovery_windows, (bool, np.bool_)) or not isinstance(max_discovery_windows, (int, np.integer))
            or int(max_discovery_windows) <= 0):
        raise ValueError("max_discovery_windows must be a positive integer")
    if discovery_balance not in DISCOVERY_BALANCES:
        raise ValueError(f"discovery_balance must be one of {sorted(DISCOVERY_BALANCES)}")
    if (isinstance(position_search_neighbourhood_samples,
                   (bool, np.bool_)) or not isinstance(position_search_neighbourhood_samples, (int, np.integer))
    ...
    lengths = (np.full(x.shape[0], x.shape[-1], dtype=np.int64)
               if sequence_lengths is None else np.asarray(sequence_lengths, dtype=np.int64))
    ...
    discovery_indices = (_balanced_discovery_indices(
        ...
        seed=seed,
    ) if discovery_balance == DISCOVERY_BALANCE else _class_window_balanced_discovery_indices(
        y,
        maximum=max_discovery_windows,
        seed=seed,
    ))
    ...
    for class_value in sorted(np.unique(discovery_y).tolist()):
        ...
        for channel in range(x.shape[1]):
            for local_index in positive_indices:
                if should_cancel is not None and should_cancel():
                    raise InterruptedError(f"PISD discovery cancelled after {evaluated_candidates} candidates")
                ...
                for start, end in intervals:
                    ...
                    candidate_rows.append((score, global_index, channel, start, end, pip_count))
        ...
        if len(ranked) < shapelets_per_class:
            raise RuntimeError(f"PISD failure: class {class_value} produced {len(ranked)} candidates; "
    ...
    selection_payload = "\n".join(f"{participant}\t{file_id}\t{window_id}" for participant, file_id, window_id in zip(
        discovery_participants, discovery_files, discovery_windows)).encode("utf-8")
    return PisdShapelets(
        ...
        position_search_neighbourhood_samples=int(position_search_neighbourhood_samples),
