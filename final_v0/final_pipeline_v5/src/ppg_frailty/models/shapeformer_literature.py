...
try:
    ...
    raise ImportError("LiteratureShapeFormer requires the optional deep dependency: pip install .[deep]") from exc
...
class _ChunkedShapeFormerAttention(nn.Module):
    ...
    def forward(
        ...
        keys = (self.key(tokens).reshape(batch, token_count, self.attention_heads,
                                         self.head_channels).permute(0, 2, 3, 1))
        values = (self.value(tokens).reshape(batch, token_count, self.attention_heads,
                                             self.head_channels).transpose(1, 2))
        queries = (self.query(tokens).reshape(batch, token_count, self.attention_heads,
                                              self.head_channels).transpose(1, 2))
        ...
        merged = torch.cat(outputs, dim=2).transpose(1, 2).reshape(batch, token_count, self.embedding_channels)
        if valid_mask is not None:
...
class ChannelSpecificShapeBlock(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        if (not np.isfinite(complexity_norm) or not np.isfinite(max_complexity_ratio) or complexity_norm <= 0.0
        ...
        self.position_search_neighbourhood_samples = int(position_search_neighbourhood_samples)
        self.search_start = max(0, self.source_start - self.position_search_neighbourhood_samples)
        self.search_end = min(
        ...
        self.selected_projection = nn.Linear(self.shapelet_length, int(shape_embedding_channels))
        self.shapelet_projection = nn.Linear(self.shapelet_length, int(shape_embedding_channels))
    ...
    def _best_segments(
        ...
        if valid_mask is not None:
            ...
            window_valid = region_mask.to(dtype=torch.float32).unfold(-1, self.shapelet_length, 1).amin(-1) > 0
        ...
        best_position = torch.zeros(values.shape[0], dtype=torch.long, device=values.device)
        for offset in range(0, windows.shape[1], self.distance_position_chunk_size):
            ...
            complexity = chunk.diff(dim=-1).square().sum(dim=-1) + 1.0 / self.complexity_norm
            shapelet_complexity = self.shapelet_complexity.to(dtype=complexity.dtype, device=complexity.device)
            correction = torch.maximum(complexity, shapelet_complexity) / torch.minimum(
                complexity, shapelet_complexity).clamp_min(1e-12)
            ...
            best_position = torch.where(replace, chunk_position + offset, best_position)
        if not bool(torch.isfinite(best_distance).all()):
            raise ValueError("ShapeBlock failed to select a finite best-fit segment")
        return windows[torch.arange(values.shape[0], device=values.device), best_position, ]
    ...
    def forward(
        ...
        token = self.selected_projection(selected) - self.shapelet_projection(self.shapelet.unsqueeze(0))
...
class LiteratureShapeFormerChannelSpecificOSD(nn.Module):
    ...
    literature_parity_scope = "reviewed_shapeblock_token_fusion_with_v2_fold_local_channel_specific_osd"
    ...
    def __init__(
        ...
        *,
        ...
        invalid_controls = [name for name, value in positive_controls.items() if int(value) <= 0]
        if invalid_controls:
            raise ValueError(f"ShapeFormer numeric controls must be positive: {invalid_controls}")
        ...
        if not np.isfinite(dropout) or not 0.0 <= float(dropout) < 1.0:
            raise ValueError("dropout must be finite in [0,1)")
        if (not np.isfinite(complexity_norm) or float(complexity_norm) <= 0.0 or not np.isfinite(max_complexity_ratio)
                or float(max_complexity_ratio) < 1.0):
            raise ValueError("complexity_norm must be positive and max_complexity_ratio at least one")
        ...
        self.candidate_enumeration_rule = str(shapelets.candidate_enumeration_rule)
        ...
        self.discovery_position_search_boundary_rule = str(shapelets.discovery_position_search_boundary_rule)
        self.information_gain_split_rule = str(shapelets.information_gain_split_rule)
        ...
        self.local_position = nn.Parameter(torch.empty(self.sequence_length, self.local_embedding_channels))
        ...
        self.shape_blocks = nn.ModuleList([
            ChannelSpecificShapeBlock(
                ...
                position_search_neighbourhood_samples=(self.position_search_neighbourhood_samples),
        ...
        self.shape_position_embedding_width_policy = "upstream_observed_max_plus_1_per_axis"
        self.shape_channel_position_width = _upstream_observed_position_width(source_channels)
        self.shape_start_position_width = _upstream_observed_position_width(source_starts)
        self.shape_end_position_width = _upstream_observed_position_width(source_ends)
        self.attention_probability_dropout_applied = False
        self.register_buffer(
            "shape_channel_position",
            torch.nn.functional.one_hot(source_channels,
                                        num_classes=self.shape_channel_position_width).to(torch.float32),
            persistent=True,
        )
        self.register_buffer(
            "shape_start_position",
            torch.nn.functional.one_hot(source_starts, num_classes=self.shape_start_position_width).to(torch.float32),
            persistent=True,
        )
        self.register_buffer(
            "shape_end_position",
            torch.nn.functional.one_hot(source_ends, num_classes=self.shape_end_position_width).to(torch.float32),
            persistent=True,
        )
        self.channel_position_projection = nn.Linear(self.shape_channel_position_width, self.shape_embedding_channels)
        self.start_position_projection = nn.Linear(self.shape_start_position_width, self.shape_embedding_channels)
        self.end_position_projection = nn.Linear(self.shape_end_position_width, self.shape_embedding_channels)
        ...
        self.feature_dim = self.local_embedding_channels + self.shape_embedding_channels
    ...
    def _generic_features(
        ...
        positioned = self.local_position_dropout(local + self.local_position[:local.shape[1]].unsqueeze(0))
        attended = self.local_norm1(local + self.local_attention(positioned, valid_mask))
    ...
    def _shape_features(
        ...
        positions = (self.channel_position_projection(self.shape_channel_position) +
        ...
        output = self.shape_norm2(attended + self.shape_feedforward(attended)) * weights
    ...
    def forward_features(
        ...
        if x.ndim != 3 or x.shape[1:] != (self.n_channels, self.sequence_length):
            raise ValueError("literature ShapeFormer expects [batch,n_channels,sequence_length]")
        valid_mask = None
        if mask is not None:
    ...
    def provenance(self) -> dict[str, object]:
        return {
            ...
            "discovery_position_search_boundary_rule": (self.discovery_position_search_boundary_rule),
            ...
            "shape_position_embedding_width_policy": (self.shape_position_embedding_width_policy),
            ...
            "attention_probability_dropout_applied": (self.attention_probability_dropout_applied),
