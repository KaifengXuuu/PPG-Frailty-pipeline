...
The formal V2 tensor is the ordered axes-only 8-channel schema exported by
representations.motion: RED, IR, three dynamic-acceleration axes, and three
gyroscope axes. The three derived magnitudes exist only in a separately named
11-channel motion augmentation constructor and are never frailty inputs.
...
if nn is not None:
    ...
    class LightCnnMotionDetector(nn.Module):
        ...
        def forward(self, values: Any) -> Any:
            ...
            if int(values.shape[1]) != self.architecture.in_channels:
                raise ValueError("motion CNN channel count does not match its frozen ordered schema")
    ...
    class LightCnnMotionDetector:  # type: ignore[no-redef]
        ...
        def __init__(self, architecture: LightCnnArchitecture):
            architecture.validate()
            raise ImportError("LightCnnMotionDetector requires the optional 'deep' PyTorch profile")
...
def build_formal_motion_cnn() -> LightCnnMotionDetector:
    """Construct only the frozen 8-channel, 3200-sample V2 motion reference.
    ...
    model.motion_input_contract = {
        ...
        "derived_signal_augmentation": False,
    }
    return model

def build_motion_derived_augmentation_cnn() -> LightCnnMotionDetector:
    """Construct the named 11-channel motion-only augmentation ablation."""

    model = build_parameterized_light_cnn(MOTION_AUGMENTED_CHANNEL_SCHEMA)
    model.model_id = MOTION_DERIVED_AUGMENTATION_MODEL_ID
    model.motion_input_contract = {
        "channel_schema": MOTION_AUGMENTED_CHANNEL_SCHEMA,
        "schema_sha256": MOTION_AUGMENTED_SCHEMA_SHA256,
        "window_samples": MOTION_WINDOW_SAMPLES,
        "profile_id": MOTION_DERIVED_AUGMENTATION_PROFILE_ID,
        "derived_signal_augmentation": True,
