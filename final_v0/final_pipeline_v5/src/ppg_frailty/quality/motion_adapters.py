...
import numpy as np
from ..models.motion import build_formal_motion_cnn, count_trainable_parameters
...
from ..signal.motion_imu import CALIBRATED_ROLL_PITCH_EKF_PROFILE_ID, PTT_STATIC_CALIBRATION_ROLE, MotionImuResult
from .motion import FORMAL_MOTION_TRAINING_CONFIG, motion_activity_label
from .motion_runner import MotionFitContext, MotionFittedArtifact, MotionPredictionInput, MotionWindowExample

FORMAL_MOTION_ARTIFACT_SCHEMA = 'ppg_frailty.formal_motion_model_artifact.imu_iqr_over_1p349.v3'
FORMAL_MOTION_TRAINER_SCHEMA = 'ppg_frailty.formal_motion_trainer.v2'
_CUDA_DEVICE = re.compile('^cuda(?::([0-9]+))?$')

# These adapters are the sole bridge between recording tensors and runner callbacks.
...
def materialize_motion_window_examples(recordings: Sequence[MotionRecordingInput], *,
                                       dataset_kind: str) -> tuple[MotionWindowExample, ...]:
    ...
    for recording in recordings:
        ...
        if kind == 'internal':
            ...
            if recording.motion_imu.diagnostics.get('calibration_source_role') != PTT_STATIC_CALIBRATION_ROLE:
                raise ValueError('PTT motion recording requires declared same-participant sit-static calibration')
        ...
        for index, start in enumerate(windows.start_samples.tolist()):
            examples.append(
                MotionWindowExample(window_id=f'{recording.record_id}:{int(start):010d}',
                                    participant_id=recording.participant_id, file_id=recording.record_id,
                                    role_or_activity=recording.role_or_activity, activity_label=label,
                                    values=np.asarray(windows.values[index],
                                                      dtype=np.float32), dataset_id=recording.dataset_id))
    if not examples:
...
def write_formal_motion_input_schema(path: str | Path) -> tuple[str, str]:
    ...
    payload = {**motion_network_schema_payload(), 'semantic_sha256': MOTION_NETWORK_SCHEMA_SHA256}
    ...
    def validate(self) -> None:
        ...
        if set(observed) != set(FORMAL_MOTION_TRAINING_CONFIG) or any(
            (type(observed[name]) is not type(expected) or observed[name] != expected
             for name, expected in FORMAL_MOTION_TRAINING_CONFIG.items() if name != 'device')):
        ...
        if not isinstance(self.device, str) or (self.device != 'cpu' and (not _CUDA_DEVICE.fullmatch(self.device))):
            raise ValueError('formal motion artifact device must be cpu or explicit CUDA (cuda or cuda:N)')
...
def _training_arrays(examples: Sequence[MotionWindowExample],
                     context: MotionFitContext) -> tuple[np.ndarray, np.ndarray, MotionFoldImuTransform]:
    if not examples:
    ...
    if values.shape[1:] != (len(MOTION_NETWORK_CHANNEL_SCHEMA), MOTION_WINDOW_SAMPLES):
    ...
    transform = fit_motion_fold_imu_transform(values, participant_ids,
...
def _balanced_sampler_weights(examples: Sequence[MotionWindowExample]) -> np.ndarray:
    ...
    weights = np.asarray([
        1.0 / class_counts[dataset, label] / np.sqrt(participant_counts[participant])
...
def fit_formal_motion_model(examples: Sequence[MotionWindowExample], context: MotionFitContext, *,
    ...
    sampler = torch.utils.data.WeightedRandomSampler(torch.as_tensor(weights, dtype=torch.double),
                                                     num_samples=len(examples), replacement=True, generator=generator)
    dataset = torch.utils.data.TensorDataset(torch.as_tensor(scaled, dtype=torch.float32),
    ...
    evaluation_loader = torch.utils.data.DataLoader(dataset, batch_size=config.batch_size, shuffle=False,
                                                    num_workers=config.num_workers, drop_last=False)
    for epoch_index in range(config.fixed_epochs):
        ...
        with torch.no_grad():
            for batch_values, batch_labels in evaluation_loader:
                ...
                observed_predictions.append((torch.sigmoid(logits) >= 0.5).to(torch.int64).cpu().numpy())
        ...
        recalls = [float(np.mean(predicted[truth == label] == label)) for label in (0, 1) if np.any(truth == label)]
        training_history.append({
            'epoch': epoch_index + 1,
            'training_loss': float(final_loss),
            'training_balanced_accuracy': float(np.mean(recalls)),
            'data_scope': f'all_{len(context.training_participant_ids)}_{context.training_dataset_kind}_participants'
            if context.final_fit else f'outer_training_{context.training_dataset_kind}_participants_only',
    ...
    runtime = load_formal_motion_model(artifact_path, metadata, runtime_device=config.device)
    return MotionFittedArtifact(runtime_model=runtime, model_id=FORMAL_MOTION_MODEL_ID,
                                ...
                                parameter_count=int(metadata['parameter_count']), inference_cost=inference_cost)

def predict_formal_motion_probability(runtime_model: FormalMotionRuntime,
                                      rows: Sequence[MotionPredictionInput]) -> np.ndarray:
    ...
    with torch.inference_mode():
        for start in range(0, len(rows), runtime_model.batch_size):
            ...
            values = np.stack([np.asarray(row.values, dtype=np.float32) for row in batch])
            scaled = apply_motion_fold_imu_transform(values, runtime_model.imu_transform)
            logits = runtime_model.model(torch.as_tensor(scaled, dtype=torch.float32, device=runtime_model.device))
            outputs.append(torch.sigmoid(logits).detach().cpu().numpy())
    return np.asarray(np.concatenate(outputs), dtype=np.float64)

def load_formal_motion_model(artifact_path: Path, metadata: Mapping[str, Any], *,
                             runtime_device: str | None = None) -> FormalMotionRuntime:
    required_metadata = {
        'artifact_sha256', 'training_participant_ids', 'parameter_count', 'inference_cost', 'model_input_schema_sha256'
    }
    ...
    expected_payload_fields = {
        'schema_version', 'model_id', 'model_input_schema_sha256', 'trainer_config', 'training_participant_ids',
        'final_training_loss', 'state_dict', 'imu_transform', 'inference_cost'
    }
    if set(payload) != expected_payload_fields:
        raise ValueError('formal motion artifact field schema drift')
    if payload.get('schema_version') != FORMAL_MOTION_ARTIFACT_SCHEMA or payload.get(
            'model_id') != FORMAL_MOTION_MODEL_ID or payload.get(
                'model_input_schema_sha256') != MOTION_NETWORK_SCHEMA_SHA256:
    ...
    transform = MotionFoldImuTransform(
        center=np.asarray(scaler['center'], dtype=np.float64), scale=np.asarray(scaler['scale'], dtype=np.float64),
        valid_count=np.asarray(scaler['valid_count'],
                               dtype=np.int64), fitted_on_participant_ids=tuple(scaler['fitted_on_participant_ids']),
        artifact_sha256=str(scaler['artifact_sha256']), profile_id=str(scaler['profile_id']),
        schema_version=str(scaler['schema_version']), channel_schema=tuple(scaler['channel_schema']))
    transform.validate()
