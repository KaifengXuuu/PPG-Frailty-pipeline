...
class QualityModeOutcome:
    ...
    def validate(self) -> None:
        ...
        if self.mode is QualityMode.DIAGNOSTICS_ONLY and (not self.computed or self.result is None):
    ...
    **evaluation_kwargs: Any,
    ...
    if resolved is QualityMode.OFF:
        ...
        if not isinstance(result, QualityResult):
            raise TypeError("quality route evaluator must return an endpoint QualityResult")
        outcome = QualityModeOutcome(
            mode=resolved,
            computed=True,
            result=result,
