...
import numpy as np
from ..data.external_folds import PTT_FORMAL_FOLD_SIZES, PTT_FORMAL_REGISTRY_ID, PTT_FORMAL_REPEAT_SEEDS
...
from ..data.folds import M2_SPLIT_FILE_SHA256, M2_SPLIT_PAYLOAD_SHA256, FrozenFoldRegistry
...
HISTORICAL_LIGHT_CNN_CHANNELS = ('ppg_single_historical_loader_pleth_2_first', 'acc_dynamic_x', 'acc_dynamic_y',
                                 'acc_dynamic_z', 'gyro_x', 'gyro_y', 'gyro_z', 'acc_magnitude', 'gyro_magnitude',
                                 'jerk_magnitude')
MOTION_CONTRACT_SCHEMA = 'ppg_frailty.motion_detector_contract.sensor_lpf_order3_imu_iqr_over_1p349.v5'
MOTION_INTERNAL_EVIDENCE_SCHEMA = 'ppg_frailty.motion_internal_evidence.imu_iqr_over_1p349.v3'
MOTION_MIDPOINT_THRESHOLD_SCHEMA = 'ppg_frailty.motion_midpoint_threshold.imu_iqr_over_1p349.v3'
...
MOTION_SPLIT_CSV_SHA256 = '130b2887eb29a5a534397b4ce4dc7032f9de30ae46533fa0b2c41559ff4a1284'
PTT_SPLIT_CSV_SHA256 = '138f999da334e2280c9e7df3304f9f017496f0ed6b60562a8d33e3c69c502dec'
...
MOTION_DEPLOYMENT_THRESHOLD_SCHEMA = 'ppg_frailty.motion_deployment_midpoint_threshold.strict_oof.v1'
MOTION_DEPLOYMENT_THRESHOLD_SCORE_ORIGIN = 'strict_outer_oof_model_predictions_only'
MOTION_DEPLOYMENT_THRESHOLD_FIT_SCOPE = 'all_training_dataset_participants_once_as_outer_oof_no_external_test'
...
FORMAL_RAW_INPUT_SOURCES = ('RED_optical_raw_source', 'IR_optical_raw_source', 'AX_accelerometer_raw_source',
                            'AY_accelerometer_raw_source', 'AZ_accelerometer_raw_source', 'GX_gyroscope_raw_source',
                            'GY_gyroscope_raw_source', 'GZ_gyroscope_raw_source')
FORMAL_NETWORK_TENSOR_STATUS = 'frozen_before_training'
MOTION_MAJOR_METRIC_FIELDS = ('participant_macro_balanced_accuracy', 'worst_fold_balanced_accuracy',
                              'participant_macro_f1', 'ece', 'parameter_count', 'inference_cost')
MOTION_REPORT_METRIC_FIELDS = ('participant_macro_balanced_accuracy', 'participant_macro_f1',
                               'participant_macro_sensitivity', 'participant_macro_specificity',
                               'participant_macro_roc_auc', 'participant_macro_pr_auc', 'worst_fold_balanced_accuracy',
                               'ece', 'parameter_count', 'inference_cost')
MOTION_INFERENCE_COST_FIELDS = ('device', 'batch_size', 'window_samples', 'warmup_iterations', 'timed_iterations',
                                'latency_ms_per_window_p50', 'latency_ms_per_window_p95',
...
MOTION_OPTIONS = (MotionOptionDescriptor(
    option_id=MotionOptionId.SQI_ONLY, formal_default=True,
    execution_status='external_audit_control_not_classifier_runtime',
    classifier_effect='none_core_pipeline_uses_quality_mode', supervised_training='not_applicable',
    external_evaluation='not_applicable',
    notes=('V2-010 default; SQI itself is currently off by V2-009d unless separately requested',
           'does not retain, drop, aggregate, route, or predict from motion scores')),
                  MotionOptionDescriptor(
                      option_id=MotionOptionId.SQI_PLUS_MOTION_OVERRIDE, formal_default=False,
                      ...
                      notes=('activity/motion supervision is B/R=static and S/W=motion',
                  ...
                  MotionOptionDescriptor(
                      option_id=MotionOptionId.HISTORICAL_LIGHT_CNN_BACKUP, formal_default=False,
                      ...
                      notes=('not the removed V2 IR-only 10-channel ablation',
...
HISTORICAL_LIGHT_CNN_EVIDENCE = HistoricalLightCnnEvidence(
    ...
    source_run='.CNN_results/20260427-01_peak_hr_gate_balanced_v2', model_id=HISTORICAL_LIGHT_CNN_MODEL_ID,
    source_detector_config_base_channels=24, resolved_network_base_channels=12,
    input_channels=HISTORICAL_LIGHT_CNN_CHANNELS, fs_hz=256.0, window_s=8.0, hop_s=2.0, threshold=0.05,
    external_dataset='simultaneous_measurements', external_window_count=12032,
    external_balanced_accuracy=0.7802173951757236, external_f1=0.7633982422884715, external_roc_auc=0.8642102117410313,
    ...
    benchmark_summary_sha256='0a34ae2656aac00a3cdeb789e582981745c5322ba90d3cf36d108ae925c8f7a0', parameter_count=6181,
    claim_boundary='pooled overlapping-window historical external-SIM result; no detector 5x5 subject OOF, '
    'no participant bootstrap CI, and never a V2/PTT result')
...
# Split loading and threshold fitting encode the leakage-sensitive scientific contract.
def load_motion_fold_jobs(split_csv: str | Path) -> tuple[MotionFoldJob, ...]:
    ...
    expected_identity = {(row.source_registry_id, row.source_registry_file_sha256, row.source_registry_payload_sha256,
                          row.dataset_version_id)
                         for row in rows}
    if expected_identity != {
        (MOTION_SOURCE_SPLIT_REGISTRY_ID, M2_SPLIT_FILE_SHA256, M2_SPLIT_PAYLOAD_SHA256, M2_DATASET_VERSION_ID)
    }:
    ...
    for repeat_index, expected_seed in enumerate((MOTION_SPLIT_SEED, )):
        ...
        for fold_index in range(MOTION_FOLD_COUNT):
            ...
            jobs.append(
                MotionFoldJob(repeat_index=repeat_index, fold_index=fold_index, split_seed=expected_seed,
                              training_seed=MOTION_TRAINING_SEED, train_participant_ids=train, oof_participant_ids=oof))
...
def _roster_sha256(participant_ids: Sequence[str]) -> str:
    encoded = json.dumps(sorted(set(participant_ids)), ensure_ascii=False, sort_keys=True, separators=(',', ':'),
                         allow_nan=False).encode('utf-8')
    return hashlib.sha256(encoded).hexdigest()

def fit_train_only_midpoint_threshold(scores: Sequence[float], labels: Sequence[int], participant_ids: Sequence[str], *,
                                      ...
                                      score_origin: str = MOTION_THRESHOLD_SCORE_ORIGIN) -> MidpointThresholdArtifact:
    ...
    forbidden = {str(value) for value in (*tuple(forbidden_oof_participant_ids), *tuple(forbidden_ptt_participant_ids))}
    ...
    for participant in sorted(training):
        ...
        for class_id in (0, 1):
            ...
            if values.size == 0:
                raise ValueError(f'training participant {participant!r} lacks motion class {class_id}')
    ...
    artifact = MidpointThresholdArtifact(
        schema_version=MOTION_MIDPOINT_THRESHOLD_SCHEMA, threshold_rule_id=MOTION_THRESHOLD_RULE_ID,
        score_origin=MOTION_THRESHOLD_SCORE_ORIGIN, fit_scope=MOTION_THRESHOLD_FIT_SCOPE,
        participant_ids=tuple(sorted(training)), participant_roster_sha256=_roster_sha256(tuple(training)),
        ...
        score_space='calibrated_p_active_probability', class_ids=(0, 1), observed_row_count=int(score_array.size))
    artifact.validate()
    return artifact
...
def validate_motion_major_metrics(payload: Mapping[str, Any]) -> None:
    ...
    for name in ('latency_ms_per_window_p50', 'latency_ms_per_window_p95', 'throughput_windows_per_second'):
        value = float(inference[name])
        if not math.isfinite(value) or value <= 0.0:
            raise ValueError(f'motion inference {name} must be finite and positive')
    if float(inference['latency_ms_per_window_p95']) < float(inference['latency_ms_per_window_p50']):
        raise ValueError('motion inference p95 latency cannot be below p50')

def _audit_ptt_external_readiness_payload(internal_evidence: Mapping[str, Any]) -> PttExternalReadinessAudit:
    reasons: list[str] = []
    evidence = dict(internal_evidence)
    if evidence.get('formal_entry_id') != 'formal_internal_motion_reference_source_bound_v2':
        reasons.append('canonical_internal_motion_entry_missing')
    source_evidence = evidence.get('formal_source_evidence')
    if not isinstance(
            source_evidence,
            Mapping) or source_evidence.get('formal_entry_id') != 'formal_internal_motion_reference_source_bound_v2':
    ...
    if evidence.get('upstream_split_registry_payload_sha256') != M2_SPLIT_PAYLOAD_SHA256:
    ...
    if not _SHA256.fullmatch(str(evidence.get('model_input_schema_file_sha256', ''))):
    ...
    if evidence.get('threshold_score_origin') not in {
            MOTION_THRESHOLD_SCORE_ORIGIN, MOTION_DEPLOYMENT_THRESHOLD_SCORE_ORIGIN
    }:
    ...
    if not isinstance(cell_evidence, list):
        ...
        expected_cells = {(repeat_index, fold_index)
                          for repeat_index in range(MOTION_REPEAT_COUNT) for fold_index in range(MOTION_FOLD_COUNT)}
    ...
    return PttExternalReadinessAudit(
        ready=not reasons, reasons=tuple(dict.fromkeys(reasons)) if reasons else
        ('complete_internal_readiness_evidence', ))

# The payload is a serializable registry view; it performs no training or evaluation.
def motion_contract_payload() -> dict[str, Any]:
    return {
        ...
        'formal_input': {
            ...
            'imu_scaling': (
                'six_physical_axes_outer_training_participant_only_median_iqr_over_1p349_'
            ...
            'ptt_calibration': 'same_participant_PTT_SIT_STATIC_CALIBRATION_explicit_external_preprocessing',
            'cross_participant_calibration': 'forbidden',
            'ekf_failure_fallback': 'forbidden_fail_closed',
            'silent_channel_derivation': 'forbidden'
        },
        'derived_signal_augmentation_ablation': {
            'status': 'constructible_named_ablation_not_formal_default_not_run',
            'profile_id': MOTION_DERIVED_AUGMENTATION_PROFILE_ID,
            'network_channel_schema': list(MOTION_AUGMENTED_CHANNEL_SCHEMA),
            ...
            'imu_scaling': (
                'all_nine_motion_channels_outer_training_participant_only_median_iqr_over_1p349_'
                'population_sd_then_one'),
        ...
        'external_ptt_readiness_audit': {
            'status': 'read_only_complete_internal_formal_evidence_and_exact_v2_036_unit_artifact',
            'execution_authority': 'none_audit_does_not_control_evaluation',
            'canonical_entry_id': 'formal_ptt_motion_reference_source_bound_v2',
            'injected_examples_or_callbacks_allowed': False,
            'imu_unit_evidence_check': 'exact_v2_036_path_sha_schema_and_66_source_hashes_before_conversion',
            'unit_evidence_schema': 'ppg_frailty.ptt_imu_unit_evidence.v3',
            'unit_evidence_relative_path': PTT_IMU_UNIT_EVIDENCE_RELATIVE_PATH.as_posix(),
