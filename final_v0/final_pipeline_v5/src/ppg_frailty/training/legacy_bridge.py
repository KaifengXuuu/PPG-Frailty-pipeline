...
class LegacyBridgeTrainingConfig:
    ...
    def class_count_basis(self) -> str:
        ...
        return "row" if self.class_weighting == "outer_train_window_inverse_frequency" else "participant"
...
def sampling_diagnostics(
    ...
    *,
    ...
    return {
        ...
        "draw_counts_by_participant": _count_strings([str(value.participant_id) for value in selected]),
        "draw_counts_by_class": _count_strings([str(int(value.label)) for value in selected]),
        "draw_counts_by_B_R_family": _count_strings([canonical_role_family(str(value.role)) for value in selected]),
        "draw_counts_by_file": _count_strings([str(value.file_id) for value in selected]),
...
class LegacyBridgeTrainer(UnifiedTrainer):
    ...
    def _draw_indices(self, dataset: Dataset, *, epoch_seed: int) -> np.ndarray:
        ...
        return torch.multinomial(weights, size, replacement=True, generator=generator).cpu().numpy().astype(np.int64)
    ...
    def _train_epochs(
        ...
        *,
        ...
        for epoch in range(1, int(epochs) + 1):
            ...
            probabilities, labels, identities = self._predict_probabilities_with_identities(model, dataset)
