...
MOTION_DERIVED_AUGMENTATION_PROFILE_ID = "motion_11ch_derived_augmentation_ablation_v2"
...
MOTION_REFERENCE_SCALER_SCHEMA = ("ppg_frailty.motion_axes6_outer_train_median_"
                                  "iqr_over_1p349_population_sd_then_one.v3")
MOTION_AUGMENTED_SCALER_SCHEMA = ("ppg_frailty.motion_derived9_outer_train_median_"
                                  "iqr_over_1p349_population_sd_then_one.v3")
MOTION_SCALER_SCHEMA = MOTION_REFERENCE_SCALER_SCHEMA


def _profile_components(profile_id: str, ) -> tuple[tuple[str, ...], tuple[str, ...], tuple[str, ...], str]:
...
def motion_network_schema_payload(profile_id: str = MOTION_REFERENCE_PROFILE_ID, ) -> dict[str, object]:
    ...
    return {
        ...
        "profile_role": ("named_derived_signal_augmentation_ablation" if is_augmentation else "canonical_reference"),
        ...
        "imu_normalization": ("outer_training_participant_only_median_"
...
def _profile_schema_sha256(profile_id: str) -> str:
    ...
    return MOTION_NETWORK_SCHEMA_SHA256 if profile_id == MOTION_REFERENCE_PROFILE_ID else MOTION_AUGMENTED_SCHEMA_SHA256
...
class MotionWindowTensors:
    ...
    def validate(self) -> None:
        ...
        if tensor.ndim != 3 or tensor.shape[1:] != expected_shape:
            raise ValueError("motion tensor shape does not match its declared reference/augmentation profile")
        ...
        if self.channel_schema != channel_schema or self.schema_sha256 != _profile_schema_sha256(self.profile_id):
...
class MotionFoldImuTransform:
    ...
    def validate(self) -> None:
        ...
        if (self.schema_version != expected_schema or self.channel_schema != expected_channels
                or self.artifact_sha256 != _scaler_hash(
                    center,
                    scale,
                    count,
...
def fit_motion_fold_imu_transform(
    ...
    *,
    ...
    inferred_profile = (MOTION_REFERENCE_PROFILE_ID
                        if tensor.ndim == 3 and tensor.shape[1] == len(MOTION_NETWORK_CHANNEL_SCHEMA) else
                        MOTION_DERIVED_AUGMENTATION_PROFILE_ID
                        if tensor.ndim == 3 and tensor.shape[1] == len(MOTION_AUGMENTED_CHANNEL_SCHEMA) else None)
    ...
    for channel in range(imu_count):
        ...
        source_samples = tensor[:, channel + 2, :] if bool(np.all(selected)) else tensor[selected, channel + 2, :]
        ...
        scale[channel] = candidate if np.isfinite(candidate) and candidate > 1e-12 else 1.0
