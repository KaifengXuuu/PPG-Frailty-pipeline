...
def _implementation_dependency_sha256(
    ...
    try:
        ...
        raise RecordingCacheAccessError("cannot hash preprocessing implementation dependency manifest") from exc


_PACKAGE_ROOT = Path(__file__).resolve().parents[1]
_IMPLEMENTATION_FILES = {
    "cache_domain_adapter": "data/preprocessing_cache.py",
    "contracts": "contracts.py",
    "data_schema": "data/schema.py",
    "experiment_callsite": "experiment.py",
    "module_registry": "module_registry.py",
    "motion_bundle_adapter": "quality/motion_bundle_adapter.py",
    "motion_input_adapter": "quality/motion_adapters.py",
    "motion_representation": "representations/motion.py",
    "normalization": "normalization.py",
    "physical_qc": "data/qc.py",
    "provenance_hashing": "provenance.py",
    "raw_representation": "representations/raw.py",
    "recording_loader": "pipeline.py",
    "signal_imu": "signal/imu.py",
    "signal_imu_units": "signal/imu.py",
    "signal_motion_imu": "signal/motion_imu.py",
    "signal_preprocess": "signal/preprocess.py",
    "signal_preprocess_config": "signal/preprocess.py",
    "signal_resample": "signal/resample.py",
    "signal_views": "signal/views.py",
    "window_contract": "data/windows.py",
}

def _implementation_digest(*names: str) -> str:
    """Hash source modules used by one cacheable preprocessing stage."""

    return _implementation_dependency_sha256({name: _PACKAGE_ROOT / _IMPLEMENTATION_FILES[name] for name in names})
...
def _strict_clone(value: Any) -> Any:
    return to_strict_json_value(value)
...
class PreprocessingCacheSession:
    ...
    def from_mapping(
        ...
        namespaces = frozenset(str(item) for item in payload.get("namespaces", sorted(SUPPORTED_NAMESPACES)))
        ...
        if mode == "off" and not hasattr(paths, "pipeline_root"):
            ...
            return cls(
                ...
                repository_root=Path(getattr(paths, "repository_root", inert_root)).resolve(),
    ...
    def _source(self, row: Any, *, name: str) -> NamedSourceDependency:
        try:
            ...
            raise RecordingCacheSourceError(
                f"cannot verify recording source before cache access: " f"{getattr(row, 'record_id', '<unknown>')}"
            ) from exc
        if observed != expected:
            raise RecordingCacheSourceError(f"source hash drift before cache access: {row.record_id}")
        ...
        self.events.append(
            {
                ...
                "logical_array_bytes": int(sum(np.asarray(item).nbytes for item in arrays.values())),
                "entry_path": (None if path is None else Path(path).relative_to(self.root).as_posix()),
                "module_chain": [module.module_id for module in identity.module_chain],
    ...
    def calibration(
        ...
        try:
            ...
            value = MotionImuCalibration(
                ...
                acceleration_bias_mps2=np.asarray(arrays["acceleration_bias_mps2"]),
                ...
                calibration_start_sample=int(attributes["calibration_start_sample"]),
            ...
            raise RecordingCacheCorruptionError(f"cannot unpack cached IMU calibration: {key}") from exc
    ...
    def canonical_views(
        ...
        *,
        ...
        if calibration is not None:
            ...
            dependencies.append(
                NamedSourceDependency(
                    ...
                    sha256=(calibration.artifact_sha256 if calibration_key is None else calibration_key),
                    properties={
        ...
        loader_sha = _implementation_digest(*loader_names)
        preprocess_sha = _implementation_digest(*preprocess_names)
        identity = RecordingCacheIdentity(
            ...
            module_chain=(
                ...
                _module(
                    ...
                    preprocess_sha,
                    {
                        ...
                        "calibration_artifact_sha256": (None if calibration is None else calibration.artifact_sha256),
                    },
                ),
                _module(
                    "canonical_signal_views",
            ...
            producer_sha256=_implementation_digest(*dict.fromkeys((*loader_names, *preprocess_names))),
            output_schema={
        ...
        arrays, attributes, key = self._operate(identity, pack)
        try:
            imu = {str(name): np.asarray(arrays[f"imu__{name}"]) for name in attributes["imu_keys"]}
            ...
            raise RecordingCacheCorruptionError(f"cannot unpack cached canonical signal views: {key}") from exc
        return (
            views,
            qc,
            profile,
    ...
    def raw_windows(
        ...
        *,
        ...
        raw_sha = _implementation_digest(
            "cache_domain_adapter",
            "contracts",
            "normalization",
            "raw_representation",
        ...
        def pack() -> RecordingCacheBuild:
            value = builder()
            if value.window_quality_scores is not None or value.window_aggregation_mask is not None:
        ...
        try:
            ...
            raise RecordingCacheCorruptionError(f"cannot unpack cached raw windows: {key}") from exc
        return value, key
    ...
    def motion_windows(
        ...
        *,
        ...
        implementation = _implementation_digest(
            "cache_domain_adapter",
            "contracts",
            "motion_bundle_adapter",
            "motion_input_adapter",
        ...
        try:
            ...
            raise RecordingCacheCorruptionError(f"cannot unpack cached motion windows: {key}") from exc
        return value, key
    ...
    def audit_payload(self) -> dict[str, Any]:
        ...
        for event in self.events:
            ...
            counts.setdefault(namespace, {})[disposition] = counts.setdefault(namespace, {}).get(disposition, 0) + 1
        return {
            ...
            "counts": counts,
            "logical_array_bytes": int(sum(int(event["logical_array_bytes"]) for event in self.events)),
            "elapsed_seconds": float(sum(float(event["elapsed_seconds"]) for event in self.events)),
