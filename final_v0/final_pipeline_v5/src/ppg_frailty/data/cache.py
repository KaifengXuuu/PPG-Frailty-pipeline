...
class CacheIdentity:
    ...
    def to_payload(self) -> dict[str, Any]:
        ...
        if not self.namespace or any(character not in "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_-"
                                     for character in self.namespace):
            raise ValueError("cache namespace contains unsafe characters")
        hashes = (tuple(self.source_sha256) + (self.config_sha256, ) + tuple(self.schema_sha256) +
                  (self.producer_sha256, ) + (() if self.fold_file_sha256 is None else (self.fold_file_sha256, )))
