...
ORDERED_WINDOW_MATRIX_SCHEMA_VERSION = "ordered_window_feature_matrix_d146_variable_k_v1"

MORPHOLOGY_WINDOW_NAMES = tuple(f"local_morphology.{name}.{statistic}" for name in MORPHOLOGY_NAMES
...
def validate_window_feature_extraction(extraction: WindowFeatureExtraction, *, transformed: bool) -> None:
    ...
    expected = WINDOW_FEATURE_SCHEMA_VERSION + ("+outer_train_robust_v1" if transformed else "")
    if (tuple(sequence.channel_schema) != names or sequence.schema_version != expected or values.ndim != 2
            or values.shape[1] != len(names) or validity.shape != values.shape
...
def _eligible_event_segment(
    ...
    *,
    ...
    if left is None or right is None or left.cell_id != right.cell_id:
        return False
    return left.source_route == source_route and left.final_tier in {"excellent", "acceptable"}
...
def build_route_eligible_rate_pulse(
    ...
    for pulse in pulses:
        ...
        for index, (left_index, right_index) in enumerate(zip(starts, stops)):
            ...
            right_cell = _cell_for_sample(timeline, max(left_sample, right_sample - 1e-9))
            if (left_cell is not None and right_cell is not None and left_cell.cell_id == right_cell.cell_id
                    and left_cell.source_route == source_route and left_cell.final_tier in {"excellent", "acceptable"}):
                candidate_rows.append((index, left_cell.cell_id))
        current: list[int] = []
        current_cell: str | None = None
        for index, cell_id in candidate_rows:
            contiguous = bool(current and index == current[-1] + 1 and cell_id == current_cell
                              and int(stops[current[-1]]) == int(starts[index])
                              and str(pulse.interval_run_ids[current[-1]]) == str(pulse.interval_run_ids[index]))
            if current and not contiguous:
                segments.append((float(times[starts[current[0]]]), pulse, tuple(current), source_route))
                current = []
    ...
    for _, pulse, indices, source_route in segments:
        ...
        original_starts = np.asarray(pulse.interval_start_peak_indices, dtype=np.int64)
        original_stops = np.asarray(pulse.interval_stop_peak_indices, dtype=np.int64)
        ...
        for index in indices:
            ...
            interval_routes.append(SignalRoute.DIRECT.value if source_route ==
                                   "direct" else SignalRoute.ARTIFACT_RATE_ONLY.value)
            local_left = local_right
        previous_peak_index = local_left

    route_set = set(interval_routes) - {"routing_boundary"}
    composite_route = (SignalRoute.ARTIFACT_RATE_ONLY
                       if route_set == {SignalRoute.ARTIFACT_RATE_ONLY.value} else SignalRoute.DIRECT)
...
def route_eligible_morphology_aggregates(
    ...
    values, validity = _morphology_features(pulse, morphology, timeline, 0, timeline.n_samples)
    names = tuple(f"{name}_{statistic}" for name in MORPHOLOGY_NAMES for statistic in ("median", "mad"))
    return (
        {name: float(value)
...
def _pulse_intervals(
    ...
    if not (start_indices.shape == stop_indices.shape == ppi.shape == valid.shape == adjacency.shape):
    ...
    allowed = np.asarray(
        [
            bool(valid[index] and np.isfinite(ppi[index]) and ppi[index] > 0.0
                 and start_sample <= mids[index] < stop_sample and _eligible_event_segment(
                     timeline,
...
def _rate_features(
    ...
    for pulse in pulses:
        ...
        for left in sorted(selected):
            right = left + 1
            if (right in selected and bool(adjacency[left]) and bool(adjacency[right])
                    and int(pulse.interval_stop_peak_indices[left]) == int(pulse.interval_start_peak_indices[right])
                    and str(pulse.interval_run_ids[left]) == str(pulse.interval_run_ids[right])):
                pair_start = float(pulse.peak_timestamps_s[pulse.interval_start_peak_indices[left]]) * CANONICAL_FS_HZ
                pair_stop = float(pulse.peak_timestamps_s[pulse.interval_stop_peak_indices[right]]) * CANONICAL_FS_HZ
                pair_midpoint = (0.5 * (float(pulse.peak_timestamps_s[pulse.interval_start_peak_indices[left]]) +
                                        float(pulse.peak_timestamps_s[pulse.interval_stop_peak_indices[right]])) *
                                 CANONICAL_FS_HZ)
                if start_sample <= pair_midpoint < stop_sample and _eligible_event_segment(
                        timeline,
...
def _morphology_features(
    ...
    for name_index, name in enumerate(MORPHOLOGY_NAMES):
        ...
        for ordinal in range(1, peaks.size - 1):
            ...
            eligible[ordinal] = bool(beat_validity[ordinal] and start_sample <= peaks[ordinal] < stop_sample
                                     and cell is not None and cell.final_tier == "excellent"
                                     and cell.source_route == "direct" and _eligible_event_segment(
                                         timeline,
...
def extract_window_features(
    ...
    *,
    ...
    mixed_rate_pulses = direct_rate_pulses if processed_pulse is None else (direct_pulse, processed_pulse)
    for row_index, item in enumerate(planned):
        ...
        if tier == "excellent":
            ...
            morph_values, morph_validity = _morphology_features(direct_pulse, direct_morphology, timeline, start, stop)
...
def fit_fold_window_feature_transform(
    ...
    *,
    ...
    for column in range(146):
        selected = matrix[:, column][row_valid & valid[:, column] & np.isfinite(matrix[:, column])]
...
def transform_window_features(
    ...
    if (transform.feature_names != window_feature_names() or np.asarray(transform.center).shape != (146, )
            or np.asarray(transform.scale).shape != (146, ) or not np.isfinite(transform.center).all()
            or not np.isfinite(transform.scale).all() or np.any(transform.scale <= 0.0)):
        raise ValueError("window feature transform schema drift")
    valid = np.asarray(extraction.value_validity, dtype=bool)
    values = (np.asarray(extraction.sequence.values, dtype=np.float64) - transform.center) / transform.scale
    values[~valid] = 0.0
