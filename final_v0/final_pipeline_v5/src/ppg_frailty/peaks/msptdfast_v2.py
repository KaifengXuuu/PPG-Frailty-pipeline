...
AUTHOR_SOURCE_SHA256 = "39f5010f1d485f2dc180bffbbe662dc9bd16e8116bd1ae630761f1e2b58bcabd"
...
def resolve_parameters(value: Mapping[str, Any] | None = None) -> dict[str, float]:
    ...
    resolved["overlap_fraction"] = _finite(resolved["overlap_fraction"], "MSPTDfast.overlap_fraction")
...
def _msptd_window_peaks(
    ...
    *,
    ...
    for scale_index, k in enumerate(range(1, max_scale + 1)):
        ...
        local_max[scale_index, centers] = (x[centers] > x[centers - k]) & (x[centers] > x[centers + k])
    ...
    *,
    ...
    for channel, label in enumerate(labels):
        ...
        if run_id is not None:
            ...
            digest = hashlib.sha256(np.ascontiguousarray(matrix[:, channel], dtype="<f8").tobytes()).hexdigest()[:20]
        ...
        result = PulseResult(
            ...
            interval_rejection_reasons=tuple("" if item else "outside_project_35_210_bpm" for item in valid.tolist()),
