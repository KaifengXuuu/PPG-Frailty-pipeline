...
IMPLEMENTATION_PATH = "ppg_frailty.peaks.aboy_project.detect_pulses_per_wavelength_aboy_project"
BLOCK_SECONDS = 10.0
INITIAL_HRI = 0.0
...
def _clean_intervals(
    ...
    *,
    ...
    reference = float(np.median(ppi[cleaned])) if np.any(cleaned) else None
    ...
    *,
    ...
    if preliminary_ppi.size:
        ...
        if retained.size >= 2:
            ...
            if 0.5 * preliminary_median <= retained_mean <= 1.5 * preliminary_median and retained_mean > 0.0:
                hri_out = float(10.0 * np.std(retained, ddof=0) / retained_mean)
                update_reason = "updated_population_sd_ddof0"
            else:
                update_reason = "retained_mean_outside_0p5_to_1p5_preliminary_median"
    ...
    provenance: dict[str, Any] = {
        ...
        "preliminary_ppi_30th_percentile_s": (float(threshold) if preliminary_ppi.size else None),
        "preliminary_ppi_median_s": (float(np.median(preliminary_ppi)) if preliminary_ppi.size else None),
        ...
        "upper_30_percent_preliminary_amplitude_mean": (None if not np.isfinite(upper_mean) else float(upper_mean)),
...
def _merge_block_peaks(
    ...
    prominence_parts = [np.asarray(row.prominence, dtype=np.float64) for row in blocks]
...
def _result_for_wavelength(
    ...
    *,
    ...
    if run_id is None:
        if record_id:
            resolved_run_id = f"{record_id}::{source_route.value}::{DETECTOR_ID}::" f"{label}::{provenance_hash[:16]}"
        else:
            digest = hashlib.sha256(np.ascontiguousarray(values, dtype="<f8").tobytes(order="C")).hexdigest()[:20]
            resolved_run_id = f"array::{source_route.value}::{DETECTOR_ID}::{label}::{digest}"
    else:
        resolved_run_id = str(run_id).strip()
    if not resolved_run_id:
        raise ValueError("pulse detection run_id cannot be empty")

    accepted_peaks, valid_intervals, rejection_reasons, _reference = _clean_intervals(best.peaks, fs_hz=fs_hz)
...
def detect_pulses_per_wavelength_aboy_project(
    ...
    *,
    ...
    if matrix.shape[0] < block_samples:
        raise ValueError(f"{DETECTOR_ID} requires at least one complete 10-second block")
