...
def resolve_detector_id(detector_id: str) -> str:
    ...
    if requested not in REGISTERED_DETECTOR_IDS:
        raise ValueError(
            f"pulse detector ID is not registered: {detector_id!r}; " f"expected one of {REGISTERED_DETECTOR_IDS}"
        )
    return requested
...
def validate_peak_detection_parameters(
    ...
    if isinstance(min_observation_sec, bool) or not isinstance(min_observation_sec, Real):
...
def resolve_detector_parameters(
    detector_id: str,
    value: Mapping[str, Any] | None = None,
) -> dict[str, float]:
    """Resolve only parameters consumed by the selected detector module."""

    resolved = resolve_detector_id(detector_id)
    declared = {} if value is None else dict(value)
    if resolved == MSPTDFAST_V2_DETECTOR_ID:
        from .msptdfast_v2 import resolve_parameters

        return resolve_parameters(declared)
    if declared:
        raise ValueError(f"{resolved} does not accept detector-specific parameters")
    return {}

def _available_labels(values: np.ndarray | CanonicalSignalViews) -> tuple[str, ...]:
    matrix = np.asarray(values.analysis_signal) if isinstance(values, CanonicalSignalViews) else np.asarray(values)
    channel_count = 1 if matrix.ndim == 1 else (int(matrix.shape[1]) if matrix.ndim == 2 else 0)
...
def detect_pulses_per_wavelength(
    ...
    *,
    ...
    if resolved == ABOY_PROJECT_V2_DETECTOR_ID:
        from .aboy_project_v2 import (
            detect_pulses_per_wavelength_aboy_project_v2,
        )

        return detect_pulses_per_wavelength_aboy_project_v2(
            values,
            fs_hz=fs_hz,
            min_observation_sec=min_observation_sec,
            min_peaks=min_peaks,
    ...
    if resolved == MSPTDFAST_V2_DETECTOR_ID:
        from .msptdfast_v2 import detect_pulses_per_wavelength_msptdfast_v2

        return detect_pulses_per_wavelength_msptdfast_v2(
            values,
            fs_hz=fs_hz,
            min_observation_sec=min_observation_sec,
            min_peaks=min_peaks,
