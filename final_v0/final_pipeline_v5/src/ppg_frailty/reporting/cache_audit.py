...
_BYPASS_DISPOSITIONS = frozenset({"cache_off", "namespace_bypassed", "read_only_miss_computed"})
...
def _empty_group(
    *,
    ...
    return {
        ...
        "fold_local_artifacts_cached": payload.get("fold_local_artifacts_cached"),
    ...
    for path in sorted(root.rglob("preprocessing_cache.json"), key=str):
        ...
        try:
            ...
            if payload.get("schema_version") != PREPROCESSING_CACHE_AUDIT_SCHEMA:
                raise ValueError("unsupported cache audit schema " f"{payload.get('schema_version')!r}")
            events = payload.get("events")
            if not isinstance(events, list):
                raise TypeError("cache audit events must be a list")
            repeat, fold = _cell_coordinates(path, payload)
            validated_events: list[tuple[str, str, str, float, int, str | None, tuple[str, ...] | None]] = []
            for event_index, event in enumerate(events):
                ...
                if not logical_bytes_float.is_integer():
                    raise ValueError(f"event {event_index} logical_array_bytes must be integral")
                logical_bytes = int(logical_bytes_float)
                cache_key = event.get("cache_key")
                cache_key = cache_key if isinstance(cache_key, str) and cache_key else None
                module_chain = event.get("module_chain")
                module_chain_value = (tuple(module_chain) if isinstance(module_chain, list)
                                      and all(isinstance(item, str) for item in module_chain) else None)
            ...
            for (
                ...
                row["disposition_counts"][disposition] = row["disposition_counts"].get(disposition, 0) + 1
            ...
            limitations.append(f"{case_id}: cannot summarize {relative.as_posix()}: "
