...
class FrozenModelInputAdapter:
    ...
    def __post_init__(self) -> None:
        ...
        canonical_roles = tuple(canonical_role_family(value) for value in self.allowed_role_families)
        if not canonical_roles or len(canonical_roles) != len(set(canonical_roles)):
            raise ValueError("adapter allowed_role_families must be non-empty and unique")
    ...
    def transform_record(self, record: Any) -> dict[str, np.ndarray]:
        if not isinstance(record, Mapping):
            raise TypeError(
                "deployment source preprocessing is unresolved; provide an already-" "preprocessed model-input mapping"
            )
        values = dict(record)
        ...
        if self.representation_mode == "feature_vector":
            ...
            if np.isinf(x).any():
                raise ValueError("feature_vector x may use NaN for unavailable physiology but not infinity")
        ...
        if not required <= set(values) or set(values) - (required | {"sample_mask"}):
            raise ValueError(
                "fusion file input requires window_bag, window_mask, file_features " "and optional sample_mask"
            )
        ...
        if bag.ndim != 3 or window_mask.shape != (bag.shape[0],) or features.ndim != 1:
...
def infer_participant(
    ...
    *,
    ...
    if selected_line not in {LINE_A_EQUAL_FILES, LINE_B_EQUAL_ROLE_FAMILIES}:
        raise RuntimeError("bundle aggregation_rule is not an executable V2 Line A/Line B identity")
    ...
    if not allowed_roles or len(allowed_roles) != len(set(allowed_roles)):
        raise RuntimeError("bundle adapter role-family contract must be non-empty and unique")
    for item in frozen:
        family = canonical_role_family(item.role)
        if family not in allowed_roles:
            raise ValueError(
                f"role family {family!r} is outside this bundle training scope " f"{','.join(allowed_roles)}"
            )
    ...
    if selected_line == LINE_A_EQUAL_FILES:
        ...
        for family in sorted(set(role_by_file.values())):
            selected = [file_probability[file_id] for file_id, role in role_by_file.items() if role == family]
