...
审查方式：静态源码、配置、测试定义和文档证据复核；本次未运行训练、完整 5×5、PTT benchmark、ablation 或科学指标计算。文档初稿完成后，总控补充报告 ensemble focused tests 87/87 与最终 conda-ml safe suite 258/258 通过；这属于实现合同验证，不是正式科学运行。
...
| IMU scaling | Frailty 和 motion 8ch reference 只缩放六个 IMU 轴；11ch motion augmentation 才缩放九个 IMU/derived channels。均只用 outer-training participants，median/IQR，IQR 退化时 population SD，再退化为 1；禁止逐窗幅度归一。 | 覆盖更早“全部九通道进入 frailty scaler”的决策。 | <code>configs/v2_decision_profile.yaml:78-110</code>；<code>representations/imu_transform.py::fit_fold_imu_transform</code>。 |
...
| S6 | 运行配置和 provenance 必须如实描述六轴 frailty scaler。 | MATCHED IDENTITY | <code>signal/preprocess.py</code> 与四个 canonical YAML 的 <code>normalization.raw_imu</code> 均为 <code>outer_training_participant_only_robust_scaler_axes6</code>；11ch motion augmentation 仍有独立 nine-channel profile。 | 数值实现、config hash、run summary 和论文方法使用同一 six-axis 身份。 | 保持 config/provenance consistency test；不得重新使用旧 <code>all_9</code> frailty 字符串。 |
...
| ID | 要求 | 状态 | 当前 V2 证据 | 科学后果 | 建议闭合 |
|---|---|---|---|---|---|
| F1 | PulseResult 必须保存真实时间、PPI validity、原始 adjacency 和 detection run identity。 | MATCHED | <code>signal/peaks.py::detect_dual_polarity_pulses</code>；<code>contracts.py::PulseResult</code>；<code>signal/prv.py</code> 使用 <code>detection_run_id</code> 与 adjacency。 | 拒绝 interval 不会把 gap 两侧错误拼成相邻 PPI。 | Q3 分段后确保不同 route runs 不跨 run 连接。 |
...
| F4 | Engineering 10 s/5 s 包含统计量、band powers、total power、dominant frequency、spectral centroid。 | MATCHED WITH USER-APPROVED CHANNEL DEVIATION | <code>features/engineering.py</code> 已包含 total power、dominant frequency、spectral centroid 和 bands；frailty engineering 只用 PPG+六轴。 | Frailty 不再含 A_mag/Omega/J 描述符；这是后续 thesis 8ch 裁决，不是遗漏。 | derived descriptors 只在 motion 11ch augmentation 评估，不能混入 frailty。 |
| F5 | FeatureVector 必须是显式 ordered allowlist；FeatureMatrix 为 D×32，含 validity channels、padding mask 和 schema hash。 | MATCHED | <code>features/registry.py::default_registry</code>、<code>::build_feature_vector</code>、<code>::build_ordered_feature_matrix</code>；schema/validity hash 在 329–429 行。 | 不同 fold/model 使用相同可审计特征顺序；缺失不被默认为生理零。 | 正式 bundle round-trip 继续校验 schema hash。 |
| F6 | Canonical peak detector 必须统一用于 internal/A3，版本和 polarity 固定。 | PARTIAL / DELIBERATE PROJECT PORT | 当前 <code>signal/peaks.py</code> 为 <code>dual_polarity_prominence_v1</code>；<code>peaks/aboy_project.py</code> 明确不是上游 exact Aboy。 | 内部一致性存在，但在 A3 前尚无 ECG endpoint accuracy 证据，不能声称原 Aboy parity。 | 在 A3 以同一 detector 做 raw/direct/reducer 比较；论文称 project detector，不称 exact Aboy。 |
...
| ID | 要求 | 状态 | 当前 V2 证据 | 科学后果 | 建议闭合 |
|---|---|---|---|---|---|
| T1 | 所有 scaler、imputer、selector、ROCKET、shapelet、calibration、epoch rule 只用 outer-train；trainer 不接受 outer-OOF labels。 | MATCHED AT CONTRACT LEVEL | <code>training/trainer.py::FrozenOuterSplit</code>、<code>::UnifiedTrainer</code>、fit provenance；representation/ROCKET/ShapeFormer 各有 train-roster hash；总控报告 conda-ml safe suite 258/258 通过。 | 阻断最主要的外层信息泄漏路径。 | 每个正式 cell 继续持久化 fitted participant roster/hash；后续修改保持 leakage tests 通过。 |
...
| AG9 | Workflow/spec 中逐项 acceptance 要求应有可追溯 test mapping。 | PARTIAL / MAPPING MISSING | 当前 safe suite 覆盖大量合同，但没有一份 workflow 条目→测试名的完整矩阵；静态搜索未找到明确命名并绑定 filter impulse/passband 与 35–210 bpm boundary 等全部要求的证据。 | 258/258 只能说明已纳入 safe suite 的测试通过，不能解释为两份要求文档的每条 acceptance 都已有自动测试。 | P1：建立 acceptance matrix；对缺项补 tiny deterministic tests，对已有但名称不直观的测试补精确链接。 |
...
1. 建立 CPU CI，并把当前已通过的 conda-ml safe suite（258/258）接入自动执行；本地通过不等于 CPU CI 已存在。
...
- 不把代码中存在的测试定义自动写成通过；本文只记录总控明确报告的 ensemble focused 87/87 与 conda-ml safe suite 258/258，且不将其解释为科学性能证据。
...
3. conda-ml safe suite 已由总控报告 258/258 通过，但本次仍未运行任何正式科学任务；MATCHED/VERIFIED CONTRACT 不等于真实数据性能验证，正式 run 前仍需 resolved-config audit。
