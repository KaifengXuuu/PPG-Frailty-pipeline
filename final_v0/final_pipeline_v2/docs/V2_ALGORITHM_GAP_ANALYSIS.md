...
| F6 | Canonical peak detector 必须统一用于 internal/A3，版本、wavelength 和 polarity 持久化。 | IMPLEMENTED / V3 RERUN PENDING | <code>msptdfast_v2_3_python_port</code> 已设为默认；七步 <code>aboy_project_v2</code> 作为显式 ablation 并在 Stage 输出中命名为 <code>aboy_project</code>；历史 v1 仅保留为需明确选择的隐式备用，禁止自动 fallback。Stage-ablation-01 v3 直接引用共享模块并采用 recording-level 300 s 漂移时延、+/-150 ms 一对一匹配、median/IQR、10/90 whiskers 及 rank-sum + Holm-Sidak。 | MSPTDfast 是 equation-level Python port，仍不声称 bitwise MATLAB parity；旧 v2 结果不满足新评估合同。 | 重新运行 Stage-ablation-01 v3，报告新合同结果；dynamic/denoised endpoint 继续复用同一 registry。 |
