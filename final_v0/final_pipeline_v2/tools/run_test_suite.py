...
English: pytest is not required. ``--suite`` selects one module suite or the
complete non-scientific CPU regression, with per-test status optionally written
as strict JSON.
"""
...
SUITES = {
    # 中文：V2 身份检查可单独执行；它不是运行时门禁。
    # English: V2 identity checks remain selectable and are not a runtime gate.
