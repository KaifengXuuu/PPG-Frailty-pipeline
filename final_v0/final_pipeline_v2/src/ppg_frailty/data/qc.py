    ...
    def validate(self, *, timestamps_present: bool | None = None) -> None:
        ...
        if self.device_limits_verified:
            if any(
                ...
                or float(value) <= 0.0
                for value in absolute_limits
            ):
                raise ValueError(
                    "verified device limits must be finite positive values"
                )
            if (
                self.saturation_fraction_limit is None
                or not np.isfinite(float(self.saturation_fraction_limit))
                or not 0.0 <= float(self.saturation_fraction_limit) <= 1.0
            ):
                raise ValueError(
                    "verified saturation limit must be finite in [0,1]"
                )
        elif any(value is not None for value in absolute_limits):
            raise ValueError(
                "deferred device limits must use explicit None for every channel"
            )
        ...
        if needs_timestamp_tolerance:
            if (
                self.timestamp_relative_tolerance is None
                or not np.isfinite(float(self.timestamp_relative_tolerance))
                or float(self.timestamp_relative_tolerance) < 0.0
            ):
                raise ValueError(
                    "timestamped QC requires a finite nonnegative tolerance"
                )
    ...
    *,
    ...
    metrics: dict[str, float | int | str | bool | None] = {
        "device_dependent_qc_status": (
            "enabled_verified_device_limits"
            if thresholds.device_limits_verified
            else "deferred_missing_device_metadata"
        )
    }
    ...
    for index, channel in enumerate(channels[: matrix.shape[1]]):
        ...
        if (
            thresholds.device_limits_verified
            and max_abs > float(thresholds.absolute_limit_by_channel[channel])
        ):
        ...
        if (
            thresholds.device_limits_verified
            and thresholds.saturation_fraction_limit is not None
    ...
    if timestamps_s is None:
        ...
        if timestamps.size != matrix.shape[0] or not np.isfinite(timestamps).all():
            ...
            if (
                ...
                > float(thresholds.timestamp_relative_tolerance) * expected
            ):
                reasons.add(QCReason.TIMESTAMP_FAILURE.value)
...
def assess_manifest_record(
    ...
    *,
    ...
    metrics.update(
        {
            ...
            "device_dependent_checks_executed": bool(
                thresholds.device_limits_verified
            ),
        }
    )
    ...
    evidence: dict[str, Any] = {
        ...
        "device_dependent_qc_status": assessment.metrics.get(
            "device_dependent_qc_status"
        ),
...
def require_recording_qc_pass(admission: RecordingQCAdmission) -> None:
    ...
    if not admission.admitted:
        raise ValueError(
            "recording failed manifest-bound QC: "
            + ";".join(admission.assessment.reasons)
        )
...
def physical_recording_qc_thresholds_v2() -> QCThresholds:
    ...
    return QCThresholds(
        ...
        flatline_std_floor_by_channel={channel: 0.0 for channel in channels},
        robust_span_floor_by_channel={channel: 0.0 for channel in channels},
        absolute_limit_by_channel={channel: None for channel in channels},
...
def physical_recording_qc_profile_v2() -> dict[str, Any]:
    ...
    return {
        ...
        "flatline_std_floor_by_channel": dict(
            thresholds.flatline_std_floor_by_channel
        ),
        "robust_span_floor_by_channel": dict(
            thresholds.robust_span_floor_by_channel
        ),
