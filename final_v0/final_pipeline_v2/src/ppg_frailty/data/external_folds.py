...
def _ptt_by_subject(
    ...
    ptt = [
        ...
        if row.dataset_id == PTT_DATASET_ID and row.inclusion_status == "included"
    ]
    ...
    for subject, rows in grouped.items():
        if len(rows) != 3 or {row.activity_raw for row in rows} != {
            ...
            raise ExternalManifestError(
                [f"formal PTT subject lacks sit/walk/run roster: {subject}"]
            )
    return grouped
            ...
            output.append(
                {
                    ...
                    "activity_raw": _strict_json(
                        sorted({row.activity_raw for row in rows})
                    ),
                    "activity_binary": _strict_json(
                        sorted({row.activity_binary for row in rows})
                    ),
...
def validate_formal_ptt_fold_rows(rows: Iterable[Mapping[str, str]]) -> None:
    ...
    keys = [
        (row.get("repeat_index", ""), row.get("subject_id", ""))
        for row in materialized
    ]
    ...
    for repeat_index, seed in enumerate(PTT_FORMAL_REPEAT_SEEDS):
        repeat = [
            row
            for row in materialized
            if row.get("repeat_index") == str(repeat_index)
        ]
        ...
        if observed != PTT_FORMAL_FOLD_SIZES:
            issues.append(
                f"repeat {repeat_index} fold sizes {observed} != {PTT_FORMAL_FOLD_SIZES}"
            )
    if issues:
...
def resolve_formal_ptt_split(
    ...
    *,
    ...
    repeat = [
        row for row in materialized if int(row["repeat_index"]) == repeat_index
    ]
    oof = tuple(
        sorted(row["subject_id"] for row in repeat if int(row["fold_index"]) == fold_index)
    )
    train = tuple(
        sorted(row["subject_id"] for row in repeat if int(row["fold_index"]) != fold_index)
    )
