...
class DecisionBiasOraclePlan:
    """Validated, fully configurable Stage 0 plan."""
...
def load_decision_bias_oracle_plan(
    path: str | Path,
    *,
    pipeline_root: str | Path | None = None,
) -> DecisionBiasOraclePlan:
    """Load the independent Stage 0 YAML without touching model configuration."""
    ...
    if root.get("schema_version") != SCHEMA_VERSION:
        raise ValueError(
            f"unsupported Stage 0 schema: {root.get('schema_version')!r}; "
    ...
    if repeat_aggregation != "arithmetic_mean_probability_per_participant":
        raise ValueError(
            "Stage 0 repeat_aggregation must be "
            "'arithmetic_mean_probability_per_participant'"
        )
    if str(oracle.get("bias_parameterization")) != "nonnegative_sum_one_simplex":
        raise ValueError(
            "Stage 0 bias_parameterization must be 'nonnegative_sum_one_simplex'"
        )
    ...
    if (
        str(oracle.get("prediction_tie_break"))
        != "first_class_in_declared_class_order"
    ):
        raise ValueError("unsupported Stage 0 prediction_tie_break")
    if (
        str(oracle.get("optimum_tie_break"))
        != "closest_to_equal_bias_then_lexicographic"
    ):
    ...
    substantial = float(interpretation.get("substantial_upper_bound", 0.70))
    limited = float(interpretation.get("limited_upper_bound", 0.65))
    if not 0.0 <= limited < substantial <= 1.0:
        raise ValueError(
            "interpretation thresholds must satisfy 0 <= limited < substantial <= 1"
        )
    ...
    return DecisionBiasOraclePlan(
        plan_path=plan_path,
        study_id=study_id,
        source_study_dir=resolve(source.get("study_dir"), field="source.study_dir"),
        case_id=case_id,
        prediction_file=resolve(
            source.get("prediction_file"),
            field="source.prediction_file",
            allow_none=True,
        ),
        prediction_level=prediction_level,
        prediction_kind=(
            None
            if source.get("prediction_kind") is None
            else str(source.get("prediction_kind"))
        ),
        ...
        bias_step=step,
        substantial_upper_bound=substantial,
        limited_upper_bound=limited,
...
def _resolve_prediction_file(plan: DecisionBiasOraclePlan) -> Path:
    ...
    if not candidates:
        ...
        suffix = (
            f"; found {len(staging)} fold/staging files, which Stage 0 refuses to use"
            if staging
            else ""
        )
        raise FileNotFoundError(
            "no completed root participant OOF file for "
            f"case {plan.case_id!r} under {plan.source_study_dir}{suffix}"
        )
    if len(candidates) > 1:
        raise ValueError(
            "multiple completed attempts exist; set source.prediction_file explicitly: "
            + ", ".join(str(path) for path in candidates)
        )
    return candidates[0]


def load_participant_oracle_dataset(
    plan: DecisionBiasOraclePlan,
) -> ParticipantOracleDataset:
    """Load and fail-closed validate the exact 29 x repeat OOF roster."""
    ...
    observed_orders = {
        tuple(int(value) for value in np.asarray(raw).tolist())
        for raw in selected["class_order"]
    }
    if observed_orders != {plan.expected_class_order}:
        raise ValueError(
            f"class order mismatch: observed={sorted(observed_orders)}, "
            f"expected={plan.expected_class_order}"
        )
    ...
    if observed_repeats != tuple(sorted(plan.expected_repeats)):
        raise ValueError(
            f"incomplete repeat roster: observed={observed_repeats}, "
            f"expected={tuple(sorted(plan.expected_repeats))}"
        )
    ...
    if len(participant_ids) != plan.expected_participants:
        raise ValueError(
            f"participant count mismatch: observed={len(participant_ids)}, "
            f"expected={plan.expected_participants}"
        )
    expected_rows = plan.expected_participants * len(plan.expected_repeats)
    if len(selected) != expected_rows:
        raise ValueError(
            f"participant-repeat row count mismatch: observed={len(selected)}, "
    ...
    aggregation_rules = tuple(
        sorted(str(value) for value in selected["aggregation_rule"].unique())
    )
    ...
    for participant_id in participant_ids:
        ...
        matrix = np.asarray(
            [np.asarray(value, dtype=np.float64) for value in rows["probabilities"]],
            dtype=np.float64,
        )
    ...
    if observed_labels != set(plan.expected_class_order):
        raise ValueError(
            f"all three classes are required: observed={sorted(observed_labels)}, "
            f"expected={plan.expected_class_order}"
        )
    return ParticipantOracleDataset(
        participant_ids=participant_ids,
        labels=np.asarray(labels, dtype=np.int64),
        probabilities=np.asarray(probabilities, dtype=np.float64),
        class_order=plan.expected_class_order,
        repeats=tuple(sorted(plan.expected_repeats)),
        source_rows=len(selected),
        prediction_file=prediction_file,
        prediction_file_sha256=_sha256(prediction_file),
        config_hash=config_hashes[0],
        aggregation_rule=aggregation_rules[0],
    )


def enumerate_simplex_biases(step: float) -> np.ndarray:
    """Return every non-negative three-class bias summing to one."""
...
def search_decision_bias_oracle(
    labels: np.ndarray,
    probabilities: np.ndarray,
    *,
    ...
    """Search BA on the same labels used for evaluation (intentional oracle)."""
    ...
    prediction_indices = np.argmax(
        probability[:, np.newaxis, :] + biases[np.newaxis, :, :], axis=2
    )
    ...
    tied = np.flatnonzero(
        np.isclose(balanced_accuracies, maximum, rtol=0.0, atol=_TOLERANCE)
    )
    equal = np.full(order.size, 1.0 / order.size, dtype=np.float64)
    best_index = min(
        (int(value) for value in tied),
        key=lambda index: (
            float(np.sum((biases[index] - equal) ** 2)),
            tuple(float(value) for value in biases[index]),
        ),
    )
    baseline_predictions = order[np.argmax(probability, axis=1)]
    oracle_predictions = predictions[:, best_index]
    return BiasOracleResult(
        biases=biases,
        balanced_accuracies=balanced_accuracies,
        baseline_balanced_accuracy=float(
            balanced_accuracy_score(y, baseline_predictions)
        ),
        baseline_macro_f1=float(f1_score(y, baseline_predictions, average="macro")),
        oracle_balanced_accuracy=maximum,
        oracle_macro_f1=float(f1_score(y, oracle_predictions, average="macro")),
        best_bias=tuple(float(value) for value in biases[best_index]),
        baseline_predictions=baseline_predictions,
        oracle_predictions=oracle_predictions,
        tied_optimum_count=int(tied.size),
    )
...
def _interpret(plan: DecisionBiasOraclePlan, oracle_ba: float) -> tuple[str, str]:
    if oracle_ba >= plan.substantial_upper_bound:
        return (
            ...
            "The leaked upper bound reaches the predeclared 0.70 threshold; "
            "decision-layer work is worth a leakage-free follow-up.",
        )
    if oracle_ba <= plan.limited_upper_bound:
        return (
            "limited_decision_layer_headroom",
            "The leaked upper bound is at or below the predeclared 0.65 ceiling; "
            "prioritise beat-level representation work.",
        )
    return (
        "intermediate_decision_layer_headroom",
        "The leaked upper bound lies between 0.65 and 0.70; the Stage 0 decision "
        "is inconclusive and any follow-up must use training-only bias fitting.",
    )
...
def _metric_rows(
    dataset: ParticipantOracleDataset,
    result: BiasOracleResult,
) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    for rule, prediction in (
        ("unmodified_argmax", result.baseline_predictions),
        ("leaked_oracle_bias_argmax", result.oracle_predictions),
    ):
        precision, recall, f1, support = precision_recall_fscore_support(
            dataset.labels,
            prediction,
            labels=dataset.class_order,
            zero_division=0,
        )
            ...
            rows.append(
                {
                    ...
                    "class_name": CANONICAL_CLASS_NAMES.get(class_label, str(class_label)),
...
def _confusion_rows(
    ...
    for rule, prediction in (
        ...
        matrix = confusion_matrix(
            dataset.labels, prediction, labels=dataset.class_order
        )
        for true_index, true_label in enumerate(dataset.class_order):
            for predicted_index, predicted_label in enumerate(dataset.class_order):
                rows.append(
                    {
                        "decision_rule": rule,
                        "true_class": CANONICAL_CLASS_NAMES.get(true_label, str(true_label)),
                        "predicted_class": CANONICAL_CLASS_NAMES.get(
...
def _plot_simplex(
    ...
    axis.scatter(
        [equal_x], [equal_y], marker="x", s=80, c="white", linewidths=2, label="equal-bias location"
    )
    vertices = ((0.0, 0.0), (1.0, 0.0), (0.5, math.sqrt(3.0) / 2.0))
    for (vx, vy), class_label in zip(vertices, class_order, strict=True):
        axis.text(
            vx,
            vy - 0.045 if vy == 0.0 else vy + 0.035,
            f"b[{CANONICAL_CLASS_NAMES.get(class_label, class_label)}]=1",
            ha="center",
            va="center",
            fontsize=9,
        )
...
def _plot_confusions(
    ...
    for axis, title, prediction in zip(
        axes,
        ("Unmodified argmax", "Leaked oracle bias"),
        (result.baseline_predictions, result.oracle_predictions),
        strict=True,
    ):
...
def _markdown_table(rows: Sequence[Mapping[str, Any]], fields: Sequence[str]) -> str:
    ...
    body = [
        "| " + " | ".join(render(row.get(field, "")) for field in fields) + " |"
        for row in rows
    ]
...
def _write_reports(
    ...
    for index, participant_id in enumerate(dataset.participant_ids):
        row: dict[str, Any] = {
            ...
            "true_class": CANONICAL_CLASS_NAMES.get(
                int(dataset.labels[index]), str(dataset.labels[index])
            ),
        }
        for class_index, class_label in enumerate(dataset.class_order):
            row[f"mean_oof_probability_class_{class_label}"] = float(
                dataset.probabilities[index, class_index]
            )
        row["baseline_prediction"] = int(result.baseline_predictions[index])
        row["oracle_prediction"] = int(result.oracle_predictions[index])
        row["decision_changed"] = bool(
            result.baseline_predictions[index] != result.oracle_predictions[index]
        )
        participant_rows.append(row)
    grid_rows = [
        {
            "bias_class_0": float(bias[0]),
            "bias_class_1": float(bias[1]),
            "bias_class_2": float(bias[2]),
            "balanced_accuracy_same_29_labels": float(score),
            "is_selected_optimum": bool(
                np.allclose(bias, np.asarray(result.best_bias), atol=_TOLERANCE)
            ),
        }
        for bias, score in zip(
            result.biases, result.balanced_accuracies, strict=True
        )
    ]
    component_rows = [
        {
            "stage": plan.study_id,
            "component_type": "source_model_probabilities",
            "component_id": plan.case_id,
            "input_data": (
                f"{plan.expected_participants} participants x "
                f"{len(plan.expected_repeats)} OOF repeats; participant-level "
                "three-class probabilities"
            ),
            "fixed_parameters": (
                f"level={plan.prediction_level}; prediction_kind={plan.prediction_kind}; "
                f"repeat_aggregation={plan.repeat_aggregation}; "
                f"aggregation_rule={dataset.aggregation_rule}"
            ),
        },
        {
            "stage": plan.study_id,
            "component_type": "decision_bias_oracle",
            ...
            "fixed_parameters": (
                f"b_c>=0; sum(b)=1; step={plan.bias_step:g}; "
                "objective=balanced_accuracy; rule=argmax(p_c+b_c); "
                "prediction_tie=first_class_order; "
                "optimum_tie=closest_to_equal_then_lexicographic"
            ),
        },
    ]
    table_figure_rows = [
        {
            "table": "stage0_summary.csv; bias_grid.csv",
            "figure": "bias_simplex_balanced_accuracy.png",
        ...
        {
            "table": "confusion_matrices.csv",
            "figure": "baseline_oracle_confusion_matrices.png",
            "status": "available" if plan.write_static_figures else "N/A_disabled_by_plan",
    ...
    for name, rows in (
        ("stage0_summary", summary_rows),
        ("per_class_metrics", per_class_rows),
        ("confusion_matrices", confusion_rows),
        ("participant_probabilities", participant_rows),
        ("bias_grid", grid_rows),
        ("test_components", component_rows),
        ("table_figure_pairs", table_figure_rows),
    ):
    ...
    component_markdown = "# Stage 0 test components\n\n" + _markdown_table(
        component_rows,
        ("component_type", "component_id", "input_data", "fixed_parameters"),
    )
    (output_dir / "TEST_COMPONENTS.md").write_text(
        component_markdown + "\n", encoding="utf-8"
    )
    ...
    result_payload = {
        ...
        "best_bias": list(result.best_bias),
        "class_order": list(dataset.class_order),
        "source": {
            "study_dir": str(plan.source_study_dir),
            "case_id": plan.case_id,
            "prediction_file": str(dataset.prediction_file),
            "prediction_file_sha256": dataset.prediction_file_sha256,
            "config_hash": dataset.config_hash,
            "aggregation_rule": dataset.aggregation_rule,
            "source_participant_repeat_rows": dataset.source_rows,
            "participant_count_after_repeat_mean": len(dataset.participant_ids),
            "repeats": list(dataset.repeats),
        },
        "oracle_contract": {
            "bias_parameterization": "nonnegative_sum_one_simplex",
            "step": plan.bias_step,
            "objective": "balanced_accuracy",
            "prediction_rule": "argmax_c(p_c + b_c)",
            "prediction_tie_break": "first_class_in_declared_class_order",
            "optimum_tie_break": "closest_to_equal_bias_then_lexicographic",
            "grid_points": int(result.biases.shape[0]),
            "tied_optimum_count": result.tied_optimum_count,
        },
        "leakage_guard": {
            "status": LEAKAGE_STATUS,
            "scientific_role": SCIENTIFIC_ROLE,
            "eligible_as_predictive_performance": False,
    ...
    fields = (
        ...
        "oracle_upper_bound_balanced_accuracy",
        "recoverable_margin_percentage_points",
        "decisions_changed",
        "best_bias",
        "interpretation_status",
    )
    table = _markdown_table(summary_rows, fields)
    ...
    if plan.write_static_figures:
        figure_lines = (
            "\n\n![Bias-simplex BA](figures/bias_simplex_balanced_accuracy.png)"
            "\n\n![Baseline and oracle confusion matrices]"
            "(figures/baseline_oracle_confusion_matrices.png)"
        )
    report = f"""# Stage 0: decision-layer recoverable-margin oracle

> **LEAKED UPPER BOUND — NOT PREDICTIVE PERFORMANCE.** The same 29 labels are
> used to choose the bias and score it. This result is ineligible for model
> selection, deployment, calibration, a formal performance table, CI, or P value.
...
The formal baseline remains `argmax_c(p_c)`. The diagnostic enumerates
non-negative three-class bias vectors with `sum(b)=1` at step
`{plan.bias_step:g}` and applies `argmax_c(p_c+b_c)`. A common additive constant
does not change argmax decisions; the simplex removes that translation
indeterminacy. Exact score ties use the first declared class order; tied optimum
vectors use the closest-to-equal then lexicographic rule. Five OOF probabilities
per participant were averaged first, so
the optimisation roster contains exactly {len(dataset.participant_ids)} people,
not {dataset.source_rows} pseudo-independent rows.

## Per-class decision results on the oracle roster

{per_class_table}

These are descriptive values on the same leaked 29-person optimisation roster,
not a second performance table.

{figure_lines}

## What this can and cannot show

The bias changes class decision boundaries but does not change the ordering of
participants within any one class score. A material BA increase therefore says
that the fitted probabilities contain class-separating information that the
unmodified multiclass argmax fails to convert into decisions. It does **not**
show that a deployable bias can achieve the oracle number. Any follow-up must fit
the bias on training participants only and evaluate it once on held-out people.

No CI or P value is calculated: resampling the same label-leaking optimisation
would quantify the oracle procedure's reuse of labels, not generalisation.

## Formulae

- Decision: `y_hat_i(b) = argmax_c (p_ic + b_c)`.
- Simplex: `b_c >= 0`, `sum_c b_c = 1`, lattice step `{plan.bias_step:g}`.
- Balanced accuracy: `BA = (1/3) * sum_c TP_c / (TP_c + FN_c)`.
- Recoverable margin: `100 * (BA_oracle - BA_unmodified)` percentage points.
- Macro-F1: arithmetic mean of the three one-vs-rest class F1 values.

## Provenance

- Source study: `{plan.source_study_dir}`
- Source case/model: `{plan.case_id}`
- Source OOF file: `{dataset.prediction_file}`
- Source OOF SHA-256: `{dataset.prediction_file_sha256}`
- Config hash: `{dataset.config_hash}`
- Aggregation: `{dataset.aggregation_rule}`
- Repeat aggregation: `{plan.repeat_aggregation}`
- Grid points: `{result.biases.shape[0]}`
- Leakage status: `{LEAKAGE_STATUS}`

Every displayed table has a CSV counterpart in `tables/`; the full 5,151-point
surface is `tables/bias_grid.csv`. `tables/TABLE_COLUMN_DEFINITIONS.md` defines
every column and formula, and the workbook contains one sheet per CSV.
"""
    (output_dir / "STUDY_SUMMARY.md").write_text(report, encoding="utf-8")

    html_header = "".join(f"<th>{html_escape(field)}</th>" for field in fields)
    html_table = "".join(
        "<tr>"
        + "".join(
    ...
    html_per_class_header = "".join(
        f"<th>{html_escape(field)}</th>" for field in per_class_fields
    )
    html_per_class_rows = "".join(
        "<tr>"
        + "".join(
            f"<td>{html_escape(str(row.get(field, '')))}</td>"
            for field in per_class_fields
        )
        + "</tr>"
    ...
    if plan.write_static_figures:
        html_figures = (
            '<h2>Paired figures</h2><img src="figures/bias_simplex_balanced_accuracy.png" '
            'alt="Bias simplex balanced accuracy"><img '
            'src="figures/baseline_oracle_confusion_matrices.png" '
            'alt="Baseline and oracle confusion matrices">'
        )
    html = f"""<!doctype html><html><head><meta charset="utf-8">
<title>Stage 0 decision-bias oracle</title><style>
body{{font-family:sans-serif;max-width:1100px;margin:2rem auto;line-height:1.5}}
.warning{{border:3px solid #a40000;background:#fff1f1;padding:1rem}}
table{{border-collapse:collapse;width:100%}}th,td{{border:1px solid #aaa;padding:.45rem}}
img{{max-width:100%;height:auto;margin:1rem 0}}
</style></head><body><h1>Stage 0: decision-layer recoverable-margin oracle</h1>
<div class="warning"><strong>LEAKED UPPER BOUND — NOT PREDICTIVE PERFORMANCE.</strong>
The same 29 labels choose and score the bias. No CI or P value is applicable.</div>
<h2>Result</h2><table><thead><tr>{html_header}</tr></thead>
<tbody>{html_table}</tbody></table><p>{html_escape(str(summary['interpretation']))}</p>
<h2>Per-class decision results on the oracle roster</h2>
<table><thead><tr>{html_per_class_header}</tr></thead><tbody>{html_per_class_rows}</tbody></table>
<p>Descriptive values on the same leaked 29-person optimisation roster.</p>
{html_figures}<h2>Method and formulae</h2>
<p>Five participant-level OOF probabilities were averaged per person. The 5,151-point
non-negative unit simplex was searched at step {plan.bias_step:g}. Decision:
argmax<sub>c</sub>(p<sub>c</sub>+b<sub>c</sub>); BA is the mean of three class recalls.</p>
<p>Recoverable margin = 100 × (oracle BA − unmodified BA) percentage points.
Any follow-up must fit a bias on training participants only.</p>
<h2>Provenance</h2><ul><li>Case: {html_escape(plan.case_id)}</li>
<li>Source: {html_escape(str(dataset.prediction_file))}</li>
<li>SHA-256: {dataset.prediction_file_sha256}</li><li>Config hash: {dataset.config_hash}</li></ul>
</body></html>"""
    (output_dir / "STUDY_SUMMARY.html").write_text(html, encoding="utf-8")


def run_decision_bias_oracle(
    plan_path: str | Path,
    *,
    ...
    """Execute the read-only oracle and write a new immutable report directory."""
    ...
    if source_study_dir is not None:
        ...
        overrides["source_study_dir"] = (
            candidate if candidate.is_absolute() else (Path(pipeline_root) / candidate).resolve()
        )
    if case_id is not None:
        overrides["case_id"] = str(case_id)
    if prediction_file is not None:
        candidate = Path(prediction_file)
        overrides["prediction_file"] = (
            candidate if candidate.is_absolute() else (Path(pipeline_root) / candidate).resolve()
        )
    if step is not None:
    ...
    summary = {
        ...
        "recoverable_margin_percentage_points": 100.0 * result.recoverable_margin,
        "decisions_changed": int(
            np.sum(result.baseline_predictions != result.oracle_predictions)
        ),
        "best_bias": "[" + ", ".join(f"{value:.2f}" for value in result.best_bias) + "]",
        "grid_step": plan.bias_step,
        "grid_points": int(result.biases.shape[0]),
        "tied_optimum_count": result.tied_optimum_count,
        ...
        "eligible_as_predictive_performance": False,
        "ci95": "N/A_same_labels_used_to_fit_and_score_oracle",
        "p_value": "N/A_same_labels_used_to_fit_and_score_oracle",
    }
