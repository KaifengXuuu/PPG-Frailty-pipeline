...
_REGISTERED_SOURCE_SPECIFICATION_SHA256 = (
    "7ad847a630f81e76304c8d7a38924a8c2b0ca9b16f9344488ade750cbee3c49b"
)
_EXPECTED_OUTPUTS = (
...
_STATIC_ROLE_RE = re.compile(r"^(?P<participant>.+)_(?P<role>B|R[1-4])$")
_ANY_ROLE_RE = re.compile(r"^(?P<participant>.+)_(?P<role>B|R[1-4]|S[12]|W[12])$")
_VISIT_SUFFIX_RE = re.compile(r"^(?P<alias>.+)_(?P<suffix>\d{2})$")
...
def _discover_static(
...
) -> tuple[list[_DiscoveryRow], list[dict[str, Any]], list[str]]:
    ...
    for directory_name, cohort in (("StudyData", "older"), ("TestDataYoungers", "young")):
        ...
        for path in sorted(directory.glob("*.csv"), key=lambda item: item.name.encode("utf-8")):
            ...
            if cohort == "young":
                # The historical reader retained the complete young identifier
                # before the role suffix (for example ``AB_01``).  Only the
                # older StudyData label join collapsed its visit suffix.
    ...
    for participant in sorted(by_participant):
        ...
        one_to_one = (
            len(aliases) == 1
            and len(alias_to_participants.get(alias, set())) == 1
            and len(class_ids) == 1
        )
        row_issues: list[str] = []
        if not one_to_one:
        ...
        alias_rows.append(
            {
                "historical_file_participant_id": participant,
                "historical_participant_alias": alias,
                "removed_suffix": rows[0].removed_suffix,
                "alias_rule": "remove_single_trailing_underscore_two_digit_visit_suffix",
                "cohort": rows[0].cohort,
                "label_source_id": rows[0].label_source_id,
                "class_id": next(iter(class_ids)) if len(class_ids) == 1 else "",
                "class_name": (
                    CANONICAL_CLASS_NAMES.get(next(iter(class_ids)), "")
                    if len(class_ids) == 1
                    else ""
                ),
                "roles": ";".join(sorted(roles)),
                "one_to_one": str(bool(one_to_one)).lower(),
...
def _constant_run_stats(values: np.ndarray, minimum: int) -> tuple[int, int]:
    ...
    # A run of k equal adjacencies represents k+1 constant samples.
...
def _audit_sources(
    ...
    limitations: list[str] = [
        "sampling_rate_not_independently_observable_no_timestamp_column"
    ]
    ...
    for row in rows:
        ...
        duration_observed = (
            float(observed_samples) / float(row.fs) if observed_samples != "" else None
        )
        if duration_observed is not None and not math.isclose(
            duration_observed,
            float(row.duration_s),
            abs_tol=max(1e-9, 0.51 / float(row.fs)),
        ):
    ...
    if (
        len(set(declared_hashes)) != len(declared_hashes)
        or len(set(record_ids)) != len(record_ids)
        or len(set(identities)) != len(identities)
        or len(set(observed_hashes)) != len(observed_hashes)
    ):
...
def _manifest_diff(
    ...
    manifest_static = {
        ...
        if row.role in {"B", "R1", "R2", "R3", "R4"}
        and ("/StudyData/" in f"/{row.source_path}" or "/TestDataYoungers/" in f"/{row.source_path}")
    }
    ...
    expected = (
        len(discovered) == 145
        and len(participants) == 29
        and class_counts == Counter({0: 9, 1: 12, 2: 8})
...
def _audit_imu(
    ...
    for row in sorted((item for item in rows if item.role == "B"), key=lambda item: item.participant_id):
        ...
        try:
            ...
            if not _declared_unit(
                row, ("GX", "GY", "GZ"), "degree_per_second_source_declared"
            ):
            ...
            result_row["raw_imu_nonfinite_count"] = int(
                np.sum(~np.isfinite(np.column_stack((acc_raw, gyro_raw))))
            )
            ...
            result_row.update(
                {f"converted_acc_norm_{key}": value for key, value in converted_acc.items()}
            )
            ...
            result_row.update(
                {
                    ...
                    "acceleration_bias_mps2": _json_cell(calibration.acceleration_bias_mps2.tolist()),
                    "gyroscope_bias_rads": _json_cell(calibration.gyroscope_bias_rads.tolist()),
                    "initial_roll_rad": calibration.initial_roll_rad,
                    "initial_pitch_rad": calibration.initial_pitch_rad,
                    "gravity_norm_median_mps2": gravity_stats["median"],
                    "gravity_norm_iqr_mps2": gravity_stats["iqr"],
                    "dynamic_acc_mean_mps2": _json_cell(np.mean(dynamic, axis=0).tolist()),
                    "dynamic_acc_rms_mps2": _json_cell(np.sqrt(np.mean(np.square(dynamic), axis=0)).tolist()),
                    "dynamic_acc_p99_abs_mps2": _json_cell(np.percentile(np.abs(dynamic), 99, axis=0).tolist()),
                    "ekf_status": "PASS",
                    "ekf_failure_reason": "",
                    "ekf_lineage_sha256": processed.diagnostics.get("lineage_sha256", ""),
                }
            )
            if float(np.percentile(np.linalg.norm(dynamic, axis=1), 99)) > 5.0:
                red_flags.append("large_residual_dynamic_acceleration_on_B")
        except Exception as exc:  # noqa: BLE001 - one B file must not suppress the other 28.
...
def _selected_npz_rows(
    path: Path,
    wanted_positions: Mapping[int, np.ndarray],
    ...
    with zipfile.ZipFile(path) as archive, archive.open("x.npy") as handle:
        ...
        for file_index, positions_raw in wanted_positions.items():
            ...
            selected[file_index] = np.empty(
                (len(positions), *shape[1:]), dtype=dtype
            )
        ...
        for row_index in range(int(shape[0])):
            ...
            if target is not None:
                ...
                selected[file_index][local_index] = np.frombuffer(
                    payload, dtype=dtype
                ).reshape(shape[1:])
        if handle.read(1):
...
def _audit_cache(
    ...
    for path in sorted(candidates):
        ...
        try:
            ...
            metadata_ok = (
                len(paths) == 145
                and set(paths.tolist()) == set(discovery_by_path)
            ...
            if metadata_ok:
                for index, source_path in enumerate(paths.tolist()):
                    ...
                    cached_labels = sorted(
                        int(value) for value in np.unique(labels[mask]).tolist()
                    )
                    cached_participants = sorted(
                        str(value) for value in np.unique(subjects[mask]).tolist()
                    )
                    ...
                    stored_file_metadata.append(
                        {
                            ...
                            "participant_id_from_path": (
                                "" if identity is None else identity[0]
                            ),
            ...
            entry["metadata_array_payload_sha256"] = {
                ...
                "labels": hashlib.sha256(
                    np.ascontiguousarray(labels).tobytes(order="C")
                ).hexdigest(),
                "participants": hashlib.sha256(
                    np.ascontiguousarray(subjects).tobytes(order="C")
                ).hexdigest(),
                "file_index": hashlib.sha256(
                    np.ascontiguousarray(file_index).tobytes(order="C")
                ).hexdigest(),
            }
            ...
            for index, source_path in enumerate(paths.tolist()):
                ...
                if row is None or row.participant_id != selected_participant_by_class.get(row.class_id) or row.role not in {"B", "R4"}:
                    continue
                selected_indices.append(index)
            wanted = {
                index: np.flatnonzero(file_index == index)
            ...
            for index in selected_indices:
                ...
                comparison: dict[str, Any] = {
                    ...
                    "cached_sha256": hashlib.sha256(
                        np.ascontiguousarray(cached_values).tobytes(order="C")
                    ).hexdigest(),
                }
                if fresh.shape == cached_values.shape:
                    difference = np.abs(
                        fresh.astype(np.float64) - cached_values.astype(np.float64)
                    )
                    comparison.update(
                        {
            ...
            entry["status"] = (
                "historical_cache_untraceable_to_current_source_bytes"
                if not entry["required_provenance_present"]
                else "historical_cache_provenance_present_numeric_comparison_required"
            )
            limitations.append(
                f"historical_cache_byte_equivalence_unproven:{relative}"
            )
        except Exception as exc:  # noqa: BLE001 - cache cannot be a training source.
...
def _audit_split(
    ...
    try:
        ...
        declarations = {
            "registry_id": sorted({row.source_registry_id for row in repeat0}),
            "file_sha256": sorted({row.source_registry_file_sha256 for row in repeat0}),
            "payload_sha256": sorted({row.source_registry_payload_sha256 for row in repeat0}),
        }
        class_consistent = all(
            participants.get(row.participant_id) == row.class_id for row in repeat0
        )
    ...
    except Exception as exc:  # noqa: BLE001 - convert all split failures into STOP evidence.
...
def _markdown_summary(
    ...
    if mismatches:
        ...
        lines.extend(
            f"- `{row.get('legacy_source_path') or row.get('v2_record_id')}`: {row.get('reasons')}"
            for row in mismatches
        )
    if source_mismatches:
        lines.extend(["", "## Current source mismatch details", ""])
        lines.extend(
            f"- `{row.get('record_id')}`: {row.get('issues')}" for row in source_mismatches
        )
    lines.extend(["", "## IMU unit / calibration / EKF red flags", ""])
    if imu_flags:
        lines.extend(
            f"- `{row.get('record_id')}`: {row.get('red_flags')}"
            for row in imu_flags
        )
    else:
        lines.append("- None")
    return "\n".join(lines) + "\n"
...
def run_legacy_v2_phase0(
    ...
    *,
    ...
    """Run the complete read-only Phase-0 audit and write its nine artifacts.
    ...
    evidence package is retained without controlling pipeline execution.
    """
    ...
    pipeline = (
        ...
        else repository / "final_v0/final_pipeline_v2"
    )
    ...
    if legacy_bridge is not None:
        ...
        bridge_source_sha = str(
            _bridge_field(legacy_bridge, "source_specification_sha256")
        )
        ...
        if (
            source_specification_sha256 is not None
            and source_specification_sha256 != bridge_source_sha
        ):
        ...
        # The registered Stage-3 schema currently freezes these exact values.
        # Runners should still pass them explicitly so a future schema cannot
        # silently inherit an old binding.
        source_specification = (
            source_specification or _REGISTERED_SOURCE_SPECIFICATION
        )
        source_specification_sha256 = (
            source_specification_sha256
            or _REGISTERED_SOURCE_SPECIFICATION_SHA256
        )
    ...
    output_paths = {
        Path(relative).name: _strict_path(artifacts, relative) for relative in outputs_raw
    }
    stops: list[str] = []
    limitations: list[str] = []
    ...
    data_root = (
        Path(historical_data_root).resolve()
        if historical_data_root is not None
        else repository / "PPG_Testing_05_01_2026"
    )
    label_path = (
        Path(label_table_path).resolve()
        if label_table_path is not None
        else data_root / "StudyData_frailtyScored/StudyData_V7_standard.csv"
    )
    ...
    # Extend independently reconstructed class identity to S/W roles without using
    # manifest labels as proof.
    ...
    diff_rows, diff_stops = _manifest_diff(
        discovered,
        manifest_rows,
        observed_by_path,
        tuple(str(value) for value in phase0["required_channel_order"]),
    ...
    imu_rows, imu_stops, imu_limitations = _audit_imu(
        repository,
        manifest_rows,
        imu_config or RollPitchEkfConfig(),
    )
    stops.extend(imu_stops)
    limitations.extend(imu_limitations)
    roots = (
        tuple(Path(value).resolve() for value in cache_search_roots)
        if cache_search_roots is not None
        else (repository / "datasets",)
    )
    ...
    _write_csv(
        ...
        diff_rows,
        (
            "legacy_source_path", "v2_record_id", "legacy_file_participant_id",
            "legacy_participant_alias", "v2_participant_id", "role",
            "legacy_class_id", "v2_class_id", "legacy_class_name", "v2_class_name",
            "observed_source_hash", "v2_source_hash", "v2_n_samples",
            "v2_channel_schema", "status", "reasons",
        ),
        artifacts,
    )
    ...
    _write_json(
        output_paths["legacy_v2_source_hash_audit.json"],
        {
            ...
            "manifest_version": sorted({row.manifest_version for row in manifest_rows}),
    ...
    imu_fields = tuple(dict.fromkeys(key for row in imu_rows for key in row)) or (
        "participant_id", "record_id", "ekf_status", "ekf_failure_reason", "red_flags"
    )
