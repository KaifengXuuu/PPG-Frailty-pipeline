...
def fixed_ppi_fixtures() -> dict[str, np.ndarray]:
    ...
    return {
        ...
        "dual_modulated": 800.0 + 35.0 * np.sin(2.0 * np.pi * index / 128.0) + 12.0 * np.sin(2.0 * np.pi * index / 23.0),
...
def _aura_features(ppi_ms: np.ndarray) -> dict[str, float | None]:
    ...
    combined.update({
        key.lower(): value
        for key, value in _numeric_values(module.get_csi_cvi_features(vector)).items()
    })
    combined.update({
    ...
    if isinstance(sample_entropy, Mapping):
        combined.update({
            f"sampen_{key.lower()}": value
            for key, value in _numeric_values(sample_entropy).items()
        })
    else:
...
def run_prv_backend_comparison(
    ...
    for fixture_id in selected_ids:
        ...
        backend_rows = [
            evaluate_prv_backend(vector, backend).to_dict()
    ...
    result_statuses = tuple(
        str(backend_row["status"])
        for fixture in rows
        for backend_row in fixture["backends"]
    )
    ...
    return {
        ...
        "status": (
            ...
            if diagnostic_success else "failed_closed"
        ),
        "execution_authority": {
