...
class ClassificationDiagnosticConfig:
    ...
    def __post_init__(self) -> None:
        if not math.isfinite(float(self.tsne_perplexity)) or float(
    ...
    for index, row in enumerate(rows):
        ...
        try:
            class_order = (
                ...
                if raw_order is not None and not isinstance(raw_order, (str, bytes))
        ...
        if threshold_value is not None and len(class_order) == 2:
            predicted_label = (
                class_order[-1]
                if probabilities[-1] >= threshold_value
                else class_order[0]
            )
        else:
            predicted_label = class_order[int(np.argmax(probabilities))]
        group_classifier = str(
            classifier_id if classifier_id is not None else row.get("case_id", "classifier")
        )
        group_evaluation = str(
            evaluation_id
            if evaluation_id is not None
            else row.get("evaluation_id", "outer_oof")
        )
        group_level = str(
            aggregation_level
            if aggregation_level is not None
            else row.get("level", "participant")
        )
        sample_id = str(
            row.get(
                "sample_id",
        ...
        normalized = {
            "classifier_id": group_classifier,
            "evaluation_id": group_evaluation,
            ...
            "true_class_probability": float(probabilities[class_order.index(label)]),
            "decision_threshold": threshold_value,
            "threshold_source": (
                "persisted_frozen_threshold"
                if threshold_value is not None
                else "none_multiclass_argmax"
                if len(class_order) > 2
                else "binary_argmax_equivalent_0.5"
            ),
        }
    ...
    *,
    ...
    for key, group in sorted(_groups(rows).items()):
        ...
        for column, class_label in enumerate(class_order):
            ...
            for point_index, (x_value, y_value, threshold) in enumerate(
                ...
                output.append(
                    {
                        ...
                        "score_threshold": (
                            float(threshold) if math.isfinite(float(threshold)) else None
                        ),
                        "roc_auc": curve_auc,
        ...
        if class_curves:
            ...
            for point_index, (x_value, y_value) in enumerate(
                zip(grid, macro_tpr, strict=True)
            ):
                output.append(
                    {
                        "classifier_id": classifier_id,
    ...
    *,
    ...
    for key, raw_group in sorted(_groups(rows).items()):
        ...
        if indices.size > int(max_samples):
            indices = np.sort(
                rng.choice(indices, size=int(max_samples), replace=False)
            )
    ...
    return tuple(
        {
            ...
            "roc_auc_curve_status": (
                "available" if roc_counts.get(classifier_id, 0) else "N/A_requires_both_classes"
            ),
            "prediction_tsne_status": (
                "available" if tsne_counts.get(classifier_id, 0) else "N/A_requires_three_unique_probability_vectors"
            ),
            "prediction_row_count": normalized_counts.get(classifier_id, 0),
            "roc_curve_point_count": roc_counts.get(classifier_id, 0),
            "tsne_point_count": tsne_counts.get(classifier_id, 0),
        }
