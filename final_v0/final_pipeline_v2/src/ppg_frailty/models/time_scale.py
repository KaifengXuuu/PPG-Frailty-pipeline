...
class FixedKernelResamplingCase:
    ...
    def __post_init__(self) -> None:
        ...
        expected_kernels = (
            ...
            if self.model_name == "CompactCNN1D"
            else INCEPTION_KERNEL_SAMPLES
        )
...
def fixed_kernel_case(case_id: str) -> FixedKernelResamplingCase:
    ...
    matches = tuple(
        case for case in build_fixed_kernel_resampling_cases() if case.case_id == str(case_id)
    )
    if len(matches) != 1:
...
def materialize_fixed_kernel_case_config(
    ...
    return {
        ...
        "catalog_role": (
            "reference_condition"
            if selected.dl_fs_hz == 400.0
            and selected.raw_window_seconds == 5.0
            and selected.dilation == 1
...
def prepare_fixed_kernel_dl_input(
    ...
    *,
    ...
    expected_source_length = int(
        round(REFERENCE_FS_HZ * selected.raw_window_seconds)
    )
    if (
        array.ndim != 3
...
def build_physical_time_cases(*args, **kwargs):
    ...
    raise RuntimeError(
        "V1 physical-time-matched cases are not V2 evidence; use "
...
def create_time_scaled_model(*args, **kwargs):
    ...
    raise RuntimeError(
        "V1 physical-time model construction is disabled in V2; use "
