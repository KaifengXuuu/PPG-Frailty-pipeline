...
FIELD_DRIVEN_PROTOCOL_DESIGNS = frozenset(
...
class LegacyBridgeProfile:
    ...
    def __post_init__(self) -> None:
        ...
        if self.historical_retained_fraction is not None and not (
            0.0 < self.historical_retained_fraction <= 1.0
        ):
    ...
    def resolved_ppg_preprocessing(self) -> str:
        ...
        return (
            "legacy_detrend_bandpass_0p2_8"
            if self.preprocessing == "legacy_raw_filtered"
            else "canonical_v2"
        )
    ...
    def uses_legacy_preprocessing(self) -> bool:
        return (
            self.resolved_ppg_preprocessing == "legacy_detrend_bandpass_0p2_8"
    ...
    def requires_calibrated_imu_views(self) -> bool:
        return (
            self.builds_windows_from_raw_record
            and self.resolved_imu_preprocessing == "calibrated_ekf_adyn"
        )

    @property
    def uses_canonical_all_channel_window_scaling(self) -> bool:
        return (
            not self.builds_windows_from_raw_record
            and self.imu_normalization == "per_window_all_eight_channels"
        )
    ...
    *,
    ...
    if protocol_design in FIELD_DRIVEN_PROTOCOL_DESIGNS:
        ...
        if observed_keys != _STAR_CONTROL_KEYS:
            ...
            raise ValueError(
                "field-driven profile controls must be complete:"
        ...
        if any(
            ...
            or not isinstance(controls[key], numeric_types)
            for key in numeric_keys
        ):
            raise ValueError("field-driven numeric controls must be numbers")
        if any(
            isinstance(controls[key], bool)
            or not isinstance(controls[key], (int, np.integer))
        ...
        if retained_fraction is not None and (
        ...
        if window_cap is not None and (
            isinstance(window_cap, bool)
            or not isinstance(window_cap, (int, np.integer))
        ):
        ...
        if profile_definition_sha256 != observed_sha256:
            raise ValueError(
                "field-driven profile definition SHA mismatch:"
        ...
        return LegacyBridgeProfile(
            ...
            historical_retained_fraction=(
                None
                if controls["historical_retained_fraction"] is None
                else float(controls["historical_retained_fraction"])
            ),
            max_windows_per_file=(
                None
                if controls["max_windows_per_file"] is None
                else int(controls["max_windows_per_file"])
            ),
            ...
            expected_aggregation_rule=str(
                controls["training_metric_aggregation_rule"]
            ),
            ...
            allow_short_record_padding=bool(
                controls["allow_short_record_padding"]
            ),
            primary_report_aggregation_view=str(
...
def bridge_profile_from_case(
    ...
    *,
    ...
    return resolve_legacy_bridge_profile(
        ...
        protocol_design=protocol_design,
        profile_definition=(
            definition if protocol_design in FIELD_DRIVEN_PROTOCOL_DESIGNS else None
        ),
        profile_definition_sha256=(
            str(definition.get("controls_sha256"))
            if protocol_design in FIELD_DRIVEN_PROTOCOL_DESIGNS
            else None
        ),
    )
    ...
    *,
    ...
    for start in starts:
        ...
        if segment.shape[0] < window:
            if not profile.resolved_allow_short_record_padding:
                raise ValueError(
                    "this bridge profile forbids right-zero padding of a short record"
                )
            segment = np.pad(
                segment,
    ...
    return RawWindows(
        ...
        provenance={
            ...
            "normalization": (
                ...
                if profile.imu_normalization == "per_window_all_eight_channels"
                else "per_window_red_ir_only_then_outer_train_axes6_fold_robust"
            ),
            "short_record_policy": (
                (
                    "historical_L0_scale_available_rows_then_right_zero_pad"
                    if profile.protocol_design == "cumulative_chain_v1"
                    else "scale_available_rows_then_right_zero_pad"
                )
                if profile.resolved_allow_short_record_padding
    ...
    *,
    ...
    if profile.resolved_imu_preprocessing == "legacy_filtered_axes":
        ...
        valid_mask = np.asarray(
            calibrated_views.imu_processed.get(
                "imu_valid_mask", np.ones(ppg.shape[0], dtype=bool)
            ),
            dtype=bool,
        )
    ...
    if target_fs != source_fs:
        ...
        if valid_mask is not None:
            source_positions = np.minimum(
                np.rint(
                    np.arange(matrix.shape[0], dtype=np.float64)
                    * source_fs
                    / target_fs
                ).astype(np.int64),
                valid_mask.size - 1,
        ...
        provenance={
            ...
            "resampling": (
                ...
                if target_fs == source_fs
                    ...
                    "historical_scipy_resample_poly_default_constant_pad_400_to_64Hz"
                    if profile.protocol_design == "cumulative_chain_v1"
                    else (
                        "scipy_resample_poly_default_constant_pad_"
...
def build_v2_window_scaled_bridge_raw_windows(
    ...
    return _raw_windows_from_matrix(
        ...
        provenance={
            ...
            "channel_schema": profile.channel_schema,
            "imu_fold_transform": (
                "not_applied_L3_per_window_ablation"
                if profile.protocol_design == "cumulative_chain_v1"
                else "not_applied_per_window_all_eight_profile"
            ),
        },
    )
