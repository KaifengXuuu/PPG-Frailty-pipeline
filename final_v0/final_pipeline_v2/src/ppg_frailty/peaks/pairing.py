...
DUAL_WAVELENGTH_PAIRING_SCHEMA_VERSION = (
    "dual_wavelength_midpoint_cycle_pairing_v3"
)
...
class BeatPairingResult:
    ...
    reference_selection_rule: str = (
        "highest_detector_score_then_coverage_then_red"
    )
    cycle_interval_policy: str = "midpoint_left_inclusive_right_exclusive"
    ambiguity_tie_break: str = (
        "nearest_sample_then_earlier_sample_then_lower_peak_ordinal"
    )
...
def select_reference_wavelength(pulses: Mapping[str, PulseResult]) -> str:
    ...
    evidence = {
        wavelength: _score_and_coverage(pulse)
        for wavelength, pulse in normalized.items()
    }
    return max(
        ("RED", "IR"),
    ...
    *,
    ...
    def row_for(
        *,
        ...
        red_position = (
            reference_position
            if reference_wavelength == "RED"
            else secondary_position
        )
        ir_position = (
            reference_position
            if reference_wavelength == "IR"
            else secondary_position
        )
        red_pulse = normalized["RED"]
        ir_pulse = normalized["IR"]
        red_sample = (
            int(red_pulse.peaks[red_position]) if red_position is not None else None
        )
        ir_sample = (
            int(ir_pulse.peaks[ir_position]) if ir_position is not None else None
        )
        lag_samples = (
            int(ir_sample - red_sample)
            if red_sample is not None and ir_sample is not None
            else None
        )
        return BeatPairAudit(
            reference_wavelength=reference_wavelength,
            secondary_wavelength=secondary_wavelength,
            reference_peak_ordinal=(
                int(reference_ordinals[reference_position])
                if reference_position is not None
                else None
            ),
            reference_peak_sample=(
                int(reference_peaks[reference_position])
                if reference_position is not None
                else None
            ),
            red_peak_ordinal=(
                int(red_ordinals[red_position]) if red_position is not None else None
            ),
            red_peak_sample=red_sample,
            ir_peak_ordinal=(
                int(ir_ordinals[ir_position]) if ir_position is not None else None
            ),
            ir_peak_sample=ir_sample,
            lag_samples_ir_minus_red=lag_samples,
            lag_s_ir_minus_red=(
                float(lag_samples / fs_hz) if lag_samples is not None else None
            ),
    ...
    for reference_position in range(reference_peaks.size):
        ...
        left_midpoint = (
            float(reference_peaks[previous_position])
            + float(reference_peaks[reference_position])
        ) / 2.0
        right_midpoint = (
            float(reference_peaks[reference_position])
            + float(reference_peaks[following_position])
        ) / 2.0
        ...
        candidates = np.asarray(
            [
                ...
                if int(position) not in used_secondary_positions
            ],
            dtype=np.int64,
        )
        ...
        reasons = (
            ("paired", "multiple_secondary_candidates_nearest_selected")
    ...
    for secondary_position in range(secondary_peaks.size):
        if secondary_position in used_secondary_positions:
            continue
        reason = (
            "secondary_peak_unpaired"
            if secondary_accepted[secondary_position]
            else "secondary_peak_rejected"
        )
        rows.append(
            row_for(
    ...
    rows.sort(
        key=lambda row: (
            min(
                sample
                for sample in (row.red_peak_sample, row.ir_peak_sample)
    ...
    if any(
        current.reference_peak_sample >= following.reference_peak_sample
        or current.red_peak_sample >= following.red_peak_sample
        or current.ir_peak_sample >= following.ir_peak_sample
    ...
    return BeatPairingResult(
        ...
        red_block_hri_provenance_hash=str(
            normalized["RED"].block_hri_provenance_hash
        ),
        ir_block_hri_provenance_hash=str(
            normalized["IR"].block_hri_provenance_hash
        ),
        rows=tuple(rows),
    )
