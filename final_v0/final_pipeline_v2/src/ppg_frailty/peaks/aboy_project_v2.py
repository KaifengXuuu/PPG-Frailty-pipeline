...
IMPLEMENTATION_PATH = (
    "ppg_frailty.peaks.aboy_project_v2."
...
def _prepare_input(
    ...
    *,
    ...
    if (
        ...
        or valid_samples.shape != (matrix.shape[0],)
    ):
...
def _physiology_and_mad(
    peaks: np.ndarray, fs_hz: float
    ...
    if int(np.count_nonzero(physiological)) >= 5:
        ...
        if robust_sigma > 0.0:
            mad_valid[physiological] = (
                np.abs(ppi[physiological] - median) <= 4.0 * robust_sigma
            )
...
def _score(peaks: np.ndarray, fs_hz: float) -> tuple[float, int, float, float]:
    ...
    cv = (
        float(np.std(selected, ddof=0) / np.mean(selected))
...
def _candidate(
    ...
    *,
    ...
    prominence = np.asarray(
        properties.get("prominences", np.zeros(peaks.size)), dtype=np.float64
    )
    ...
    provenance = {
        ...
        "upper_30_percent_preliminary_amplitude_mean": (
            float(upper_mean) if np.isfinite(upper_mean) else None
        ),
    ...
    return _Candidate(
        polarity, peaks, prominence, hri_out, n_clean, coverage, cv, score, provenance
    )


def _remove_ratio_outlier_peaks(
    peaks: np.ndarray, prominence: np.ndarray, fs_hz: float
) -> tuple[np.ndarray, np.ndarray, float | None, tuple[int, ...]]:
...
def _result_for_channel(
    ...
    *,
    ...
    prominence = (
        np.concatenate(prominence_parts) if prominence_parts else np.empty(0)
    )
    peaks, prominence, ratio_reference, rejected = _remove_ratio_outlier_peaks(
        peaks, prominence, fs_hz
    )
