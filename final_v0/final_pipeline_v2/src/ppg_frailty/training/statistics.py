...
def _formal_authority_payload(
    ...
    reference_id = str(run_manifest.get("reference_config_id", ""))
    source_runs = run_manifest.get("source_runs")
    reference = (
        source_runs.get(reference_id)
        if isinstance(source_runs, Mapping) else None
    )
    if not reference_id or not isinstance(reference, Mapping):
        raise ValueError("formal comparison reference source-run identity missing")
    reference_hash = str(reference.get("config_hash", ""))
    source_runs_hash = stable_payload_sha256(source_runs)
    if len(reference_hash) != 64:
        raise ValueError("formal comparison reference config hash invalid")
    unsigned = {
        "schema_version":
            "ppg_frailty.formal_comparison_producer_authority.v2",
        "pipeline_generation": "final_pipeline_v2",
        "status": "formal_pipeline_writer",
        "manual_selection_eligible": True,
        "authority_config_id": reference_id,
        "authority_config_hash": reference_hash,
    ...
    return {
        **unsigned,
        "producer_authority_sha256": hashlib.sha256(encoded).hexdigest(),
    }
