...
def _require_torch() -> None:
    ...
    if torch is None:
        raise ImportError(
            "tensor dataset access requires optional dependency torch; "
...
def _validate_identities(identities: list[SampleIdentity] | tuple[SampleIdentity, ...], size: int) -> tuple[SampleIdentity, ...]:
...
class FileBagDataset(_IdentityDataset):
    ...
    def __init__(
        ...
        if sample_masks is None:
            self.sample_masks = tuple(
                np.ones((bag.shape[0], bag.shape[2]), dtype=bool) for bag in bags
            )
        else:
            masks = tuple(np.asarray(mask, dtype=bool) for mask in sample_masks)
            if len(masks) != len(bags) or any(
                mask.shape != (bag.shape[0], bag.shape[2]) for mask, bag in zip(masks, bags)
            ):
