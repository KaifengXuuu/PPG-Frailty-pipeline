...
def _strategy(value: Any, aliases: Mapping[str, str], *, name: str) -> str:
    ...
    try:
        ...
        raise ValueError(
            f"{name} must select one of {sorted(set(aliases.values()))}; "
            "motion-derived channels are a separate model input module"
        ) from exc
...
class RawNormalizationConfig:
    ...
    def from_mapping(
        ...
        if clip_raw is None:
            ...
            isinstance(clip_raw, (str, bytes))
            or not isinstance(clip_raw, (list, tuple))
            or len(clip_raw) != 2
        ):
            raise ValueError(
                "signal.normalization.clip_after_scale must be null or two bounds"
            )
        else:
            if any(isinstance(value, bool) for value in clip_raw):
                raise ValueError(
                    "signal.normalization.clip_after_scale bounds must be numeric"
                )
            try:
            ...
            if not np.isfinite(clip).all() or not clip[0] < clip[1]:
                raise ValueError(
                    "signal.normalization.clip_after_scale bounds must be finite "
                    "and strictly increasing"
                )
        scalar_values = (
            declared.get("robust_iqr_divisor", defaults.robust_iqr_divisor),
            declared.get(
                "mad_consistency_divisor", defaults.mad_consistency_divisor
            ),
        ...
        if not np.isfinite([iqr_divisor, mad_divisor, epsilon]).all() or not (
            iqr_divisor > 0.0 and mad_divisor > 0.0 and epsilon > 0.0
        ):
        ...
        if (
            isinstance(ddof, bool)
            or not isinstance(ddof, (int, np.integer))
            or int(ddof) < 0
        ):
            raise ValueError(
                "signal.normalization.standard_ddof must be a non-negative integer"
            )
        result = cls(
            raw_ppg=_strategy(
        ...
        consumes_standard_ddof = (
            result.raw_ppg == PPG_STANDARD_ZSCORE
            or result.raw_imu == IMU_MEAN_STD
            or (
                any_robust
                and result.iqr_fallback == FALLBACK_STANDARD_DEVIATION
            )
        )
        consumed = {
            ...
            "mad_consistency_divisor": (
                any_robust and result.iqr_fallback == FALLBACK_MAD
            ),
            "scale_epsilon": (
                result.raw_ppg != PPG_NONE or result.raw_imu != IMU_NONE
            ),
        ...
        default_values = {
            name: getattr(defaults, name) for name in resolved_values
        }
        for name, is_consumed in consumed.items():
            if (
                name in declared
                and not is_consumed
                and resolved_values[name] != default_values[name]
            ):
                raise ValueError(
                    f"signal.normalization.{name} has no runtime consumer for "
    ...
    def to_mapping(self) -> dict[str, Any]:
        ...
        return {
            "raw_ppg": self.raw_ppg,
            "raw_imu": self.raw_imu,
            "iqr_fallback": self.iqr_fallback,
            "clip_after_scale": (
                None
                if self.clip_after_scale is None
                else [float(value) for value in self.clip_after_scale]
            ),
            "robust_iqr_divisor": float(self.robust_iqr_divisor),
            "mad_consistency_divisor": float(self.mad_consistency_divisor),
            "scale_epsilon": float(self.scale_epsilon),
            "standard_ddof": int(self.standard_ddof),
        }
