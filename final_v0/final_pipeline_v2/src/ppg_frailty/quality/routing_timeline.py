...
def build_routing_windows(
    ...
    *,
    ...
    if not np.isclose(window_samples / fs_hz, window_s) or not np.isclose(
        hop_samples / fs_hz, hop_s
    ):
...
def resolve_routing_evidence(
    ...
    *,
    ...
    if evidence.motion_detector_enabled:
        motion_high = (
            False
            if evidence.motion_state == "low"
            else True
            if evidence.motion_state == "high"
            else None
        )
    else:
...
def build_routing_timeline(
    *,
    ...
    if any(
        left.start_sample_400 >= right.start_sample_400
    ...
    if not rows:
        ...
        centres = np.asarray(
            [
                (row.window.start_sample_400 + row.window.stop_sample_400) / 2.0
                for row in rows
            ],
            dtype=np.float64,
        )
        ...
        starts = np.concatenate(
            ([rows[0].window.start_sample_400], boundaries)
        ).astype(np.int64)
        stops = np.concatenate(
            (boundaries, [rows[-1].window.stop_sample_400])
        ).astype(np.int64)
        if starts[0] > 0:
            cells.append(
                _excluded_edge_cell(
...
def overlapping_cells(
    timeline: RoutingTimeline, start_sample: int, stop_sample: int
) -> tuple[RoutingCell, ...]:
    ...
    return tuple(
        ...
        if cell.start_sample_400 < stop_sample
        and cell.stop_sample_400 > start_sample
    )


def matrix_row_route(
    timeline: RoutingTimeline, start_sample: int, stop_sample: int
) -> tuple[str, bool]:
    ...
    if all(
        cell.final_tier == QualityTier.EXCELLENT.value
