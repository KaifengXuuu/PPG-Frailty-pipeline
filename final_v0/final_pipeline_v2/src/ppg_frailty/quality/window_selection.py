...
class WindowSelectionConfig:
    ...
    def __post_init__(self) -> None:
        if self.policy not in WINDOW_SELECTION_POLICIES:
            raise ValueError(
                "quality.window_selection.policy must be one of "
                f"{sorted(WINDOW_SELECTION_POLICIES)}"
            )
        if (
            isinstance(self.keep_fraction, bool)
            or not isinstance(self.keep_fraction, (int, float, np.integer, np.floating))
            or not np.isfinite(self.keep_fraction)
            or not 0.0 < float(self.keep_fraction) <= 1.0
        ):
            raise ValueError(
                "quality.window_selection.keep_fraction must be finite in (0,1]"
            )
        if self.score_algorithm != LEGACY_WINDOW_SCORE_ALGORITHM:
            raise ValueError(
                "quality.window_selection.score_algorithm must be "
                f"{LEGACY_WINDOW_SCORE_ALGORITHM}"
            )
        if self.application_scope not in WINDOW_SELECTION_APPLICATION_SCOPES:
            raise ValueError(
                "quality.window_selection.application_scope must be one of "
                f"{sorted(WINDOW_SELECTION_APPLICATION_SCOPES)}"
            )
        if self.policy == "none" and float(self.keep_fraction) != 1.0:
            raise ValueError(
                "quality.window_selection.keep_fraction must be 1 when policy=none"
            )
        if self.policy == "none" and self.application_scope != "outer_train_only":
            raise ValueError(
                "quality.window_selection.application_scope must remain "
    ...
    def from_mapping(cls, value: Mapping[str, Any] | None) -> "WindowSelectionConfig":
        ...
        allowed = {
            "policy", "keep_fraction", "score_algorithm", "application_scope"
        }
        unknown = sorted(set(value) - allowed)
        if unknown:
            raise ValueError(
                f"quality.window_selection contains unknown fields: {unknown}"
            )
...
def legacy_window_sqi_scores(
    ...
    *,
    ...
    for window, mask in zip(windows, masks):
        ...
        acc = (
            np.linalg.norm(legacy_window[2:5].astype(np.float64), axis=0)
            if window.shape[0] >= 5
            else np.zeros(valid_length, dtype=np.float64)
        )
        gyro = (
            np.linalg.norm(legacy_window[5:8].astype(np.float64), axis=0)
            if window.shape[0] >= 8
            else np.zeros(valid_length, dtype=np.float64)
        )
        ...
        spectral_ratio = (
            float(np.trapezoid(psd[cardiac], frequencies[cardiac]) / total)
            if np.any(cardiac)
            else 0.0
        )
        ...
        if peaks.size >= 3:
            ...
            ppi_stability = 1.0 / (
                1.0
                + float(np.std(intervals) / (np.mean(intervals) + 1e-8))
            )
            peak_density = min(
                1.0,
                float(peaks.size)
                / max(2.0, ppg.size / float(fs_hz) * 3.0),
            )
        else:
            ppi_stability = 0.0
            peak_density = 0.0
            ...
            **dict(scored.provenance),
            "window_quality_selection": {
                **config.to_mapping(),
                ...
                "retained_score_vector_sha256": hashlib.sha256(
                    np.asarray(scores[keep], dtype="<f4").tobytes()
                ).hexdigest(),
                "retained_start_samples_sha256": hashlib.sha256(
                    np.asarray(scored.start_samples[keep], dtype="<i8").tobytes()
                ).hexdigest(),
            },
        },
    )
