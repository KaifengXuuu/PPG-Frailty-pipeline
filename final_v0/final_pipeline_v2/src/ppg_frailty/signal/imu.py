"""Stateful IMU preprocessing with the frozen M3 quaternion MEKF.

中文：迁移已审核的 scalar-first quaternion、六维误差状态、切平面加速度更新、
Joseph covariance update 和无预校准状态机。ACC/Gyro 先经三阶 20/40 Hz 因果
滤波；LPF comparator 再经二阶 0.3 Hz 因果低通。无 M3 runtime import、无 fallback。

English: This independent V1 port implements the reviewed scalar-first quaternion,
six-state multiplicative error model, tangent-plane update, Joseph covariance update,
and no-precalibration state machine. Shared causal ACC20/Gyro40 filtering precedes
either the MEKF or the causal second-order 0.3 Hz LPF comparator.
"""
...
class NoPrecalibrationEskf:
    ...
    def _tilt_sigma_deg(self) -> float:
        ...
        return float(
            np.rad2deg(
                np.sqrt(max(np.max(np.linalg.eigvalsh(covariance)), 0.0))
            )
        )
    ...
    def step(
        ...
        if not initialized_now:
            self.q = quat_normalize(
                quat_multiply(self.q, quat_exp(rate * self.dt))
            )
        ...
        if sample_index % self.config.update_decimation == 0:
            ...
            measurement = (
                self.config.base_accel_angle_noise_rad**2
                * scale_r
                * np.eye(2)
            )
            innovation_covariance = (
                h_matrix @ self.p @ h_matrix.T + measurement
            )
            nis = float(
                residual
                @ np.linalg.solve(innovation_covariance, residual)
            )
            if (
                0.5 * STANDARD_GRAVITY <= acc_norm <= 1.5 * STANDARD_GRAVITY
                and nis <= self.config.nis_gate
            ):
                gain = np.linalg.solve(
                    innovation_covariance, h_matrix @ self.p
                ).T
        ...
        if float(np.min(eigenvalues)) < 1e-15:
            self.p = (
                eigenvectors
                @ np.diag(np.maximum(eigenvalues, 1e-15))
                @ eigenvectors.T
            )
        ...
        ready = (
            self.samples_since_initialization / self.fs_hz
            >= self.config.min_tracking_sec
        ...
        if divergent:
            ...
            not accepted
            and self.samples_since_update > self.config.update_decimation
        ):
...
def _timestamp_ok(
    ...
    if time.size > 1:
        ...
        if (
            ...
            or float(np.percentile(relative, 99)) > 0.05
        ):
...
def _repair_chunk(
    ...
    for column in range(source.shape[1]):
        ...
        if (
            not finite.any()
            or np.count_nonzero(~finite) / finite.size > 0.01
        ):
        ...
        for start, stop in _true_runs(~finite):
            ...
            if (
                start == 0
                or stop == source.shape[0]
                or length > maximum_gap
            ):
                raise ValueError(
                    f"channel_{column}:unrepairable_gap:{length}"
                )
            ...
            reasons.append(
                f"channel_{column}:short_gap_interpolated:{length}"
            )
...
def _jerk(
    ...
    if (
        previous_dynamic is not None
        and route_valid[0]
        and np.isfinite(previous_dynamic).all()
    ):
...
class CausalImuProcessor:
    ...
    def process_chunk(
        ...
        *,
        ...
        try:
            repaired, repair_mask, repair_reasons = _repair_chunk(
                np.column_stack((acc_raw, gyro_raw)), self.fs_hz
            )
            acc_si = convert_acceleration(
                repaired[:, :3], self.acceleration_unit
            )
            gyro_si = convert_gyro(
                repaired[:, 3:], self.gyroscope_unit
            )
            ...
            if self.profile.gravity_method == "no_precalibration_ekf":
                ...
                for local_index in range(count):
                    ...
                    if "quaternion" in sample:
                        ...
                        covariance_min[local_index] = sample[
                ...
                route_diagnostics.update(
                    {
                        ...
                        "physical_observability_status": (
                            "unverified_no_static_precalibration"
                        ),
                ...
                state_names = [
                    "tracking" if valid else "no_estimate"
                    for valid in route_valid
                ]
                confidence = route_valid.astype(np.float64)

            dynamic = acc_filtered - gravity
            dynamic[~route_valid] = np.nan
            jerk, common_valid = _jerk(
                dynamic, route_valid, self.fs_hz, self.last_dynamic
            )
            terminal_state = (
                state_names[-1] if state_names else "initialization_pending"
            )
            ...
            diagnostics = {
                ...
                "gravity_norm_mean_mps2": (
                    float(
                        np.mean(
                            np.linalg.norm(gravity[route_valid], axis=1)
                        )
                    )
                ...
                "first_valid_index": (
                    int(np.flatnonzero(common_valid)[0])
                    if np.any(common_valid)
                    else None
                ),
                "last_valid_index": (
                    int(np.flatnonzero(common_valid)[-1])
                    if np.any(common_valid)
                    else None
                ),
                "silent_fallback": False,
        ...
        self.last_dynamic = (
            dynamic[-1].copy() if route_valid[-1] else None
        )
        self.global_sample_index += acc_raw.shape[0]
        if timestamps_s is not None:
            self.previous_timestamp_s = float(
                np.asarray(timestamps_s).ravel()[-1]
            )
        return ImuPreprocessResult(
            processed,
            status,
...
def preprocess_imu(
    ...
    *,
    ...
    return processor.process_chunk(
        acc, gyro, timestamps_s=timestamps_s
    )
...
def estimate_gravity_lpf(
    ...
    *,
    ...
    gyro = (
        np.zeros_like(acc)
        if gyro_rads is None
        else np.asarray(gyro_rads, dtype=np.float64)
    )
    result = preprocess_imu(
        acc,
        gyro,
...
def compare_ekf_lpf_gravity(
    ...
    *,
    ...
    metrics: dict[str, Any] = {
        ...
        "ekf_gravity_norm_mean_mps2": float(
            np.mean(np.linalg.norm(ekf[common], axis=1))
        ),
        "lpf_gravity_norm_mean_mps2": float(
            np.mean(np.linalg.norm(lpf[common], axis=1))
        ),
        "ekf_no_static_precalibration": bool(
            ekf_diagnostics.get("no_static_precalibration", False)
        ),
