...
def _safe_ratio(numerator: float, denominator: float) -> float:
    ...
    if (
        ...
        or abs(denominator) <= OPTICAL_RATIO_EPSILON
    ):
        return float("nan")
...
def _standardized_waveform_agreement(
    ...
    *,
    ...
    zero_lag = float(
        np.dot(standardized_red, standardized_ir) / standardized_red.size
    )
...
def extract_dual_optical(
    ...
    *,
    ...
    if any(
        SignalRoute(pulse.source_route) is not route
        for pulse in normalized_pulses.values()
    ):
    ...
    beat_values = {
        name: np.full(len(pairing.rows), np.nan, dtype=np.float64) for name in names
    }
    beat_validity = {
        name: np.zeros(len(pairing.rows), dtype=bool) for name in names
    }
    ...
    for index, pair in enumerate(pairing.rows):
        ...
        if optical_valid:
            ...
            ratio_of_ratios = (
                ...
                if (
                    np.isfinite(red_pi)
                    and np.isfinite(ir_pi)
                    and abs(ir_pi) > OPTICAL_RATIO_EPSILON
                )
                else float("nan")
            )
            current = {
                ...
                "red_ir_dc_ratio": _safe_absolute_denominator_ratio(
                    abs(red_dc), ir_dc
                ),
    ...
    recording_medians = {
        name: (
            float(np.median(beat_values[name][common]))
            if common.size
            else float("nan")
        )
    ...
    aggregate = {
        f"{name}_median": value for name, value in recording_medians.items()
    }
    aggregate_validity = {
        f"{name}_median": bool(support_valid and np.isfinite(value))
        for name, value in recording_medians.items()
    }
    ...
    derived_ratios = {
        ...
        "red_ir_dc_ratio_median": _safe_absolute_denominator_ratio(
            abs(red_dc), ir_dc
        ),
    }
    ...
    aggregate_validity.update(
        {
            name: bool(support_valid and np.isfinite(value))
            for name, value in derived_ratios.items()
        }
    )
    ...
    waveform_values = {
        ...
        "red_ir_xcorr_lag_s": (
            float(lag / fs_hz) if lag is not None else float("nan")
        ),
    }
    aggregate.update(waveform_values)
    aggregate_validity.update(
        {name: bool(np.isfinite(value)) for name, value in waveform_values.items()}
    )
    diagnostics: dict[str, Any] = {
    ...
    reasons = (
        ()
        if any(aggregate_validity.values())
        else ("dual_optical_unavailable",)
    )
