...
class FoldImuChannelTransform:
    ...
    def validate(self) -> None:
        ...
        if (
            ...
            or self.strategy != resolved.raw_imu
            or self.iqr_fallback != resolved.iqr_fallback
    ...
    *,
    ...
    for channel in range(channel_count):
        ...
        if normalization_config.raw_imu == IMU_MEAN_STD:
            ...
            candidate = (
                float(np.std(samples, ddof=normalization_config.standard_ddof))
                if samples.size > normalization_config.standard_ddof
                else float("nan")
            )
            ...
            candidate = float(q75 - q25) / float(
                normalization_config.robust_iqr_divisor
            )
            if (
                not np.isfinite(candidate)
                or candidate <= normalization_config.scale_epsilon
            ):
                if normalization_config.iqr_fallback == FALLBACK_ONE:
                    candidate = 1.0
                elif normalization_config.iqr_fallback == FALLBACK_MAD:
                    candidate = float(
                        np.median(np.abs(samples - center[channel]))
                    ) / float(normalization_config.mad_consistency_divisor)
                else:
                    candidate = (
                        float(
                            np.std(
            ...
            raise ValueError(
                f"unsupported raw IMU normalization: {normalization_config.raw_imu}"
            )
        scale[channel] = (
            candidate
            if np.isfinite(candidate)
            and candidate > normalization_config.scale_epsilon
            else 1.0
        )
