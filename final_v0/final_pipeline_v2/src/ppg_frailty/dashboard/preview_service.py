"""Read-only stage previews backed by the canonical V2 algorithm modules."""
...
class PipelinePreviewService:
    ...
    def config_paths(self) -> tuple[Path, ...]:
        configs = self.pipeline_root / "configs"
        return tuple(
            path
            for name in self.CANONICAL_CONFIG_NAMES
            if (path := configs / name).is_file()
        )

    def study_directories(self) -> tuple[Path, ...]:
        root = self.pipeline_root / "artifacts" / "studies"
        if not root.is_dir():
            return ()
        studies = {
            manifest.parent.resolve()
            for manifest in root.rglob("study_manifest.json")
            if manifest.is_file()
        }
        return tuple(
            sorted(
    ...
    def study_plan_paths(self) -> tuple[str, ...]:
        ...
        return tuple(
            path.relative_to(self.pipeline_root).as_posix()
    ...
    def preview(
        ...
        *,
        ...
        values = np.column_stack(
            (
                np.asarray(loaded["ppg"], dtype=np.float64),
                np.asarray(loaded["acc"], dtype=np.float64),
                np.asarray(loaded["gyro"], dtype=np.float64),
            )
        )
        ...
        if str(imu_config["gravity_method"]) in {
            ...
            if not candidates:
                raise ValueError(
                    "canonical preview requires a same-participant role-B "
                    f"calibration record: {row.participant_id}"
                )
            calibration_row = candidates[0]
            calibration_loaded = _load_record(
                calibration_row,
                paths,
                max_samples=None,
            )
            calibration = fit_motion_imu_calibration(
                np.asarray(calibration_loaded["acc"], dtype=np.float64),
        ...
        traces = {
            name: np.asarray(all_traces[name][start_index:end_index], dtype=np.float64)
            for name in requested
        }
        spectra = {
            name: self._spectrum(trace, fs_hz)
        ...
        metadata = {
            ...
            "imu_calibration_record_id": (
                None if calibration_row is None else str(calibration_row.record_id)
            ),
        ...
        all_stage_rows = self._module_stage_rows(
            row=row,
            views=views,
        ...
        selected_stages = {
            ...
            for value in (
                ...
                else tuple(stage_names)
            )
        }
        stage_rows = tuple(
            item for item in all_stage_rows if str(item["stage"]) in selected_stages
        )
    ...
    def _study_dir(self, value: str | Path) -> Path:
        candidate = Path(value)
        target = candidate.resolve() if candidate.is_absolute() else (self.pipeline_root / candidate).resolve()
        studies = (self.pipeline_root / "artifacts" / "studies").resolve()
        target.relative_to(studies)
    ...
    def _trace_map(values: np.ndarray, views: Any) -> dict[str, np.ndarray]:
        ...
        return {
            "raw_red": values[:, 0],
            "raw_ir": values[:, 1],
            "filtered_red": np.asarray(views.x_filter[:, 0], dtype=np.float64),
            "filtered_ir": np.asarray(views.x_filter[:, 1], dtype=np.float64),
            "dynamic_acceleration_magnitude": first(
                "dynamic_magnitude", "dynamic_acc_magnitude",
                "acc_dynamic_magnitude", "a_dynamic_magnitude"
            ),
            "angular_velocity_magnitude": first(
                "gyro_magnitude", "angular_velocity_magnitude", "omega"
            ),
            "jerk_magnitude": first("jerk_magnitude", "jerk"),
        }

    @staticmethod
    def _module_stage_rows(
        *,
        row: Any,
        views: Any,
        ...
        add(
            ...
            "segment_state",
            (
                "not_applied_thresholds_deferred"
                if quality_mode == "off"
                else "see_formal_quality_diagnostics"
            ),
        ...
        try:
            ...
            add(
                "pulse_ppi",
                "accepted_peak_count",
                int(np.count_nonzero(pulse.accepted_peak_mask)),
            )
            add(
                "pulse_ppi",
            ...
            try:
                ...
                for name, value in sorted(morphology.aggregate_values.items()):
                    add(
                        ...
                        (
                            "available"
                            if morphology.aggregate_validity.get(name, False)
                            else "unavailable"
                        ),
                    )
        ...
        try:
            ...
            add(
                ...
                float(np.mean(validity)) if validity.size else None,
                "available" if validity.size else "N/A",
            )
            for index, name in enumerate(engineering.sequence.channel_schema):
                usable = (
                    validity[:, index] & np.isfinite(feature_values[:, index])
                    if validity.size
                ...
                add(
                    ...
                    f"median.{name}",
                    (
                        float(np.median(feature_values[usable, index]))
                        if np.any(usable)
                        else None
                    ),
        ...
        add(
            "representation_model",
            "preview_status",
            "pre_fit_inputs_only_model_fit_not_executed",
            "N/A",
        )
        add(
            "aggregation",
            "hierarchy",
            resolved_config.get("aggregation", {}).get("hierarchy"),
        )
        add(
            "aggregation",
            "preview_status",
            "requires_outer_oof_predictions_not_executed",
            "N/A",
        )
        return tuple(output)
