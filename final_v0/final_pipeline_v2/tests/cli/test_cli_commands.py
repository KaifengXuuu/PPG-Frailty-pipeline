...
class CliCommandTests(unittest.TestCase):
    ...
    def test_validate_all_formal_v2_configs_pass(self) -> None:
        """四种正式表示配置通过；motion合同不混入 / Four formal modes."""
    ...
    def test_validate_accepts_an_absolute_generated_study_config(self) -> None:
        source = (
            ROOT / "configs" / "reference_static_feature_vector_v2.yaml"
        ).read_text(encoding="utf-8")
        source = source.replace(
            "config_id: reference_static_feature_vector_lr_role_aware_v2",
            (
                "config_id: "
                "reference_static_feature_vector_lr_role_aware_v2__cli_external"
            ),
            1,
        )
        with tempfile.TemporaryDirectory() as directory:
            config_path = Path(directory) / "resolved_case.yaml"
        ...
        self.assertEqual(completed.returncode, 0, completed.stderr)
        self.assertEqual(payload["status"], "passed")
        self.assertEqual(payload["config"]["path_scope"], "external_archive")
        self.assertEqual(
