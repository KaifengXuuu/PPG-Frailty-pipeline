...
class FrozenFoldTests(unittest.TestCase):
    ...
    def test_all_25_outer_cells_share_their_repeat_seed(self) -> None:
        registry = FrozenFoldRegistry(tuple(self.assignments))
        splits = [
            registry.get_split(repeat_index=repeat, fold_index=fold)
            for repeat in range(5)
            for fold in range(5)
        ]
        self.assertEqual(
            {split["training_seed"] for split in splits},
            set(M2_SEEDS),
        )
        self.assertEqual(
            {split["split_seed"] for split in splits},
            set(M2_SEEDS),
        )
        for repeat, expected_seed in enumerate(M2_SEEDS):
            repeat_splits = [
                split for split in splits if split["repeat_index"] == repeat
            ]
            self.assertEqual(len(repeat_splits), 5)
            self.assertEqual(
                {split["training_seed"] for split in repeat_splits},
                {expected_seed},
            )
            self.assertTrue(
                all(
                    split["training_seed"] == split["split_seed"]
                    for split in repeat_splits
                )
            )

    def test_repeat_seed_helper_fails_closed_on_drift(self) -> None:
        self.assertEqual(outer_cv_single_model_training_seed(3, 30042), 30042)
        with self.assertRaises(ValueError):
            outer_cv_single_model_training_seed(3, 42)
        with self.assertRaises(ValueError):
            outer_cv_single_model_training_seed(5, 40042)
