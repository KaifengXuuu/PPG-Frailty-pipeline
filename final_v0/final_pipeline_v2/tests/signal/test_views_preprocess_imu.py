...
class SignalViewsTest(unittest.TestCase):
    ...
    def test_filter_reference_and_single_ablation_are_registered(self) -> None:
        reference = get_ppg_filter_profile(REFERENCE_PPG_FILTER_PROFILE_ID)
        ablation = get_ppg_filter_profile(ABLATION_PPG_FILTER_PROFILE_ID)
        self.assertEqual((reference.low_hz, reference.high_hz), (0.2, 8.0))
        self.assertEqual(reference.registry_role, "reference")
...
class ImuTest(unittest.TestCase):
    ...
    def test_ekf_lpf_comparison_is_paired_and_descriptive(self) -> None:
        ...
        comparison = compare_ekf_lpf_gravity(record["acc"], record["gyro"])
        self.assertEqual(
            comparison.ekf_gravity_mps2.shape,
            comparison.lpf_gravity_mps2.shape,
        )
        self.assertTrue(comparison.common_valid_mask.any())
        self.assertTrue(comparison.metrics["ekf_no_static_precalibration"])
        self.assertFalse(comparison.metrics["selection_performed"])
        self.assertGreaterEqual(comparison.metrics["rmse_mps2"], 0.0)
