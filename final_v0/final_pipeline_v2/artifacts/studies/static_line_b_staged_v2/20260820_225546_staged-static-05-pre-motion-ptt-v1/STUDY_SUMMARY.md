...
| fastica_bss | denoiser | PTT denoiser benchmark | configured_not_executed | audit_provenance_v1 | not_applicable |
| identity | denoiser | PTT denoiser benchmark | configured_not_executed | audit_provenance_v1 | not_applicable |
| nlms_imu_anc | denoiser | PTT denoiser benchmark | configured_not_executed | audit_provenance_v1 | not_applicable |
| nmf_bss | denoiser | PTT denoiser benchmark | configured_not_executed | audit_provenance_v1 | not_applicable |
| pca_bss | denoiser | PTT denoiser benchmark | configured_not_executed | audit_provenance_v1 | not_applicable |
| spectral_mask | denoiser | PTT denoiser benchmark | configured_not_executed | audit_provenance_v1 | not_applicable |
| ssa_decomposition | denoiser | PTT denoiser benchmark | configured_not_executed | audit_provenance_v1 | not_applicable |
...
| formal_local_supervised_motion_detector_v2 | motion_detector_reverse_ablation | PTT22 OOF + all-22 final → Frailty29 | executed | binary_motion_window_file_v1 | audit_provenance_v1 |
...
| fastica_bss | denoiser | 对同步 RED/IR 做固定随机种子的双源 FastICA，再按心率带集中度减 IMU 相关惩罚选分量并回投；内核：unit-variance whitening、FastICA 不动点迭代与线性 mixing 重建。 | N/A — component was not executed |
| identity | denoiser | 逐样本复制双波长 PPG，不估计或抑制伪影；内核：恒等映射与同时间网格校验，作为未去噪直接对照。 | N/A — component was not executed |
| nlms_imu_anc | denoiser | 以六轴物理单位 IMU 为参考，对 RED/IR 分别运行带泄漏的归一化 LMS 自适应噪声抵消；内核：多延迟 tapped-reference 线性滤波，参考 RMS 达阈值时按 NLMS 规则更新权重。 | N/A — component was not executed |
| nmf_bss | denoiser | 拼接 RED/IR 的非负 STFT 幅度并拟合共享谱基，选择心率带能量占比最高的基后复用原相位重建；内核：Hann STFT、NNDSVDA 初始化的坐标下降 NMF 与 ISTFT。 | N/A — component was not executed |
| pca_bss | denoiser | 对同步 RED/IR 做确定性双源 PCA，再按心率带集中度减 IMU 最大相关惩罚选择单一分量并投影回双通道；内核：full-SVD PCA、Welch 频谱评分和线性 mixing 重建。 | N/A — component was not executed |
| spectral_mask | denoiser | 由六轴 IMU 时频幅度构造软污染掩膜，在心率频带内抑制 RED/IR、带外置零；内核：Hann STFT/ISTFT、逐帧 IMU 分位数归一化和有下限的乘性谱增益。 | N/A — component was not executed |
| ssa_decomposition | denoiser | RED/IR 分别构造 Hankel 轨迹矩阵并做 SSA，选择心率频带能量占比达阈值的分量后重建；内核：全矩阵 SVD、逐分量对角平均与 Welch 0.5–3.5 Hz 浓度筛选。 | N/A — component was not executed |
...
| formal_local_supervised_motion_detector_v2 | motion_detector_reverse_ablation | Project LightCNN binary motion detector over RED/IR plus six processed physical IMU channels. | Project implementation: src/ppg_frailty/models/motion.py; no external architecture-equivalence claim |
...
| binary_motion_window_file_v1 | Binary motion detector | endpoint_or_module | motion_detector:formal_local_supervised_motion_detector_v2; motion_detector_reverse_ablation:formal_local_supervised_motion_detector_v2; motion_threshold:participant_balanced_midpoint | True | False |
...
|---|---|---|---|---|---|
| audit_provenance_v1 | Persisted resolved configuration, input data, seeds, splits, status and artifact inventory are projected without changing the experiment. |  |  |  | N/A — component was not executed; Project component-role audit binding: dataset_adapter; no separate external literature source claimed; Project component-role audit binding: motion_threshold; no separate external literature source claimed; Project implementation: src/ppg_frailty/models/motion.py; no external architecture-equivalence claim; Roll/pitch EKF context: Sabatini (2011), DOI:10.3390/s110201482; project equations/noise are persisted in resolved config |
...
| --- | --- | --- | --- | --- |
| motion_detector_endpoint::frailty29::frozen_cross_dataset::file | ptt22_trained_motion_detector | Reported endpoint; no within-stratum candidate family: model=ptt22_trained_motion_detector, evaluation_id=ptt22_trained_to_frailty29, BA=50.0%, macro-F1=30.2%, ROC-AUC=56.7%. No comparison is made across target datasets, evaluation scopes, or aggregation levels. | cross_dataset_benchmark_not_independent_untouched_test | none_automatic |
| motion_detector_endpoint::frailty29::frozen_cross_dataset::window | ptt22_trained_motion_detector | Reported endpoint; no within-stratum candidate family: model=ptt22_trained_motion_detector, evaluation_id=ptt22_trained_to_frailty29, BA=50.0%, macro-F1=2.3%, ROC-AUC=56.3%. No comparison is made across target datasets, evaluation scopes, or aggregation levels. | cross_dataset_benchmark_not_independent_untouched_test | none_automatic |
| motion_detector_endpoint::frailty29::source_grouped_oof::file | frailty29_trained_motion_detector | Reported endpoint; no within-stratum candidate family: model=frailty29_trained_motion_detector, evaluation_id=frailty29_outer_oof, BA=96.8%, macro-F1=96.8%, ROC-AUC=96.3%. No comparison is made across target datasets, evaluation scopes, or aggregation levels. | grouped_oof_descriptive | none_automatic |
| motion_detector_endpoint::frailty29::source_grouped_oof::window | frailty29_trained_motion_detector | Reported endpoint; no within-stratum candidate family: model=frailty29_trained_motion_detector, evaluation_id=frailty29_outer_oof, BA=94.9%, macro-F1=72.9%, ROC-AUC=96.1%. No comparison is made across target datasets, evaluation scopes, or aggregation levels. | grouped_oof_descriptive | none_automatic |
| motion_detector_endpoint::ptt22::frozen_cross_dataset::file | frailty29_trained_motion_detector | Reported endpoint; no within-stratum candidate family: model=frailty29_trained_motion_detector, evaluation_id=frailty29_trained_to_ptt22, BA=78.4%, macro-F1=71.2%, ROC-AUC=100.0%. No comparison is made across target datasets, evaluation scopes, or aggregation levels. | cross_dataset_benchmark_not_independent_untouched_test | none_automatic |
| motion_detector_endpoint::ptt22::frozen_cross_dataset::window | frailty29_trained_motion_detector | Reported endpoint; no within-stratum candidate family: model=frailty29_trained_motion_detector, evaluation_id=frailty29_trained_to_ptt22, BA=78.9%, macro-F1=71.8%, ROC-AUC=99.7%. No comparison is made across target datasets, evaluation scopes, or aggregation levels. | cross_dataset_benchmark_not_independent_untouched_test | none_automatic |
| motion_detector_endpoint::ptt22::source_grouped_oof::file | ptt22_trained_motion_detector | Reported endpoint; no within-stratum candidate family: model=ptt22_trained_motion_detector, evaluation_id=ptt22_outer_oof, BA=100.0%, macro-F1=100.0%, ROC-AUC=100.0%. No comparison is made across target datasets, evaluation scopes, or aggregation levels. | grouped_oof_descriptive | none_automatic |
| motion_detector_endpoint::ptt22::source_grouped_oof::window | ptt22_trained_motion_detector | Reported endpoint; no within-stratum candidate family: model=ptt22_trained_motion_detector, evaluation_id=ptt22_outer_oof, BA=100.0%, macro-F1=100.0%, ROC-AUC=100.0%. No comparison is made across target datasets, evaluation scopes, or aggregation levels. | grouped_oof_descriptive | none_automatic |
...
| denoiser | N/A | Denoiser benchmark was skipped; outputs are explicitly N/A with the execution-option reason. | not_available | none_automatic |
| motion_detector_training_source_inference | ptt22_trained_motion_detector | 17/24 endpoint comparisons reject after Holm correction. The analysis is participant-paired on identical target units but retrospective and asymmetric; it does not select a deployment model automatically. | retrospective_exploratory | none_automatic |
...
| --- | --- | --- | --- | --- | --- |
| frailty29_trained_motion_detector | frailty29_outer_oof (source_grouped_oof) | file | 96.9 ± 9.2 | [93.2, 99.6] | Reference |
| frailty29_trained_motion_detector | frailty29_outer_oof (source_grouped_oof) | window | 95.3 ± 10.7 | [90.8, 98.3] | Reference |
| frailty29_trained_motion_detector | frailty29_trained_to_ptt22 (frozen_cross_dataset) | file | 78.4 ± 16.0 | [71.6, 85.2] | Reference |
| frailty29_trained_motion_detector | frailty29_trained_to_ptt22 (frozen_cross_dataset) | window | 78.9 ± 12.9 | [73.5, 84.2] | Reference |
| ptt22_trained_motion_detector | ptt22_outer_oof (source_grouped_oof) | file | 100.0 ± 0.0 | [100.0, 100.0] | 0.0004 |
| ptt22_trained_motion_detector | ptt22_outer_oof (source_grouped_oof) | window | 100.0 ± 0.0 | [100.0, 100.0] | 6.00e-05 |
| ptt22_trained_motion_detector | ptt22_trained_to_frailty29 (frozen_cross_dataset) | file | 50.0 ± 0.0 | [50.0, 50.0] | 6.00e-05 |
| ptt22_trained_motion_detector | ptt22_trained_to_frailty29 (frozen_cross_dataset) | window | 50.0 ± 0.0 | [50.0, 50.0] | 6.00e-05 |
...
| model_id | evaluation | level | participant_macro_mean_sd | participant_bootstrap_ci95 | holm_p_vs_reference |
| --- | --- | --- | --- | --- | --- |
| frailty29_trained_motion_detector | frailty29_outer_oof (source_grouped_oof) | file | 96.9 ± 9.5 | [93.0, 99.6] | Reference |
| frailty29_trained_motion_detector | frailty29_outer_oof (source_grouped_oof) | window | 86.0 ± 18.5 | [79.0, 92.1] | Reference |
| frailty29_trained_motion_detector | frailty29_trained_to_ptt22 (frozen_cross_dataset) | file | 70.1 ± 23.5 | [60.2, 79.5] | Reference |
| frailty29_trained_motion_detector | frailty29_trained_to_ptt22 (frozen_cross_dataset) | window | 71.2 ± 18.3 | [63.5, 78.5] | Reference |
| ptt22_trained_motion_detector | ptt22_outer_oof (source_grouped_oof) | file | 100.0 ± 0.0 | [100.0, 100.0] | 0.0004 |
| ptt22_trained_motion_detector | ptt22_outer_oof (source_grouped_oof) | window | 100.0 ± 0.0 | [100.0, 100.0] | 6.00e-05 |
| ptt22_trained_motion_detector | ptt22_trained_to_frailty29 (frozen_cross_dataset) | file | 30.1 ± 1.9 | [29.3, 30.6] | 6.00e-05 |
| ptt22_trained_motion_detector | ptt22_trained_to_frailty29 (frozen_cross_dataset) | window | 2.3 ± 0.8 | [2.0, 2.6] | 6.00e-05 |
...
| model_id | evaluation | level | participant_macro_mean_sd | participant_bootstrap_ci95 | holm_p_vs_reference |
| --- | --- | --- | --- | --- | --- |
| frailty29_trained_motion_detector | frailty29_outer_oof (source_grouped_oof) | file | 96.6 ± 8.8 | [93.1, 99.1] | Reference |
| frailty29_trained_motion_detector | frailty29_outer_oof (source_grouped_oof) | window | 95.3 ± 11.4 | [90.8, 98.9] | Reference |
| frailty29_trained_motion_detector | frailty29_trained_to_ptt22 (frozen_cross_dataset) | file | 56.8 ± 32.0 | [43.2, 70.5] | Reference |
| frailty29_trained_motion_detector | frailty29_trained_to_ptt22 (frozen_cross_dataset) | window | 57.7 ± 25.9 | [47.0, 68.3] | Reference |
| ptt22_trained_motion_detector | ptt22_outer_oof (source_grouped_oof) | file | 100.0 ± 0.0 | [100.0, 100.0] | 0.0004 |
| ptt22_trained_motion_detector | ptt22_outer_oof (source_grouped_oof) | window | 100.0 ± 0.0 | [100.0, 100.0] | 6.00e-05 |
| ptt22_trained_motion_detector | ptt22_trained_to_frailty29 (frozen_cross_dataset) | file | 100.0 ± 0.0 | [100.0, 100.0] | 0.1284 |
| ptt22_trained_motion_detector | ptt22_trained_to_frailty29 (frozen_cross_dataset) | window | 100.0 ± 0.0 | [100.0, 100.0] | 0.0166 |
...
| model_id | evaluation | level | participant_macro_mean_sd | participant_bootstrap_ci95 | holm_p_vs_reference |
| --- | --- | --- | --- | --- | --- |
| frailty29_trained_motion_detector | frailty29_outer_oof (source_grouped_oof) | file | 97.2 ± 11.6 | [92.4, 100.0] | Reference |
| frailty29_trained_motion_detector | frailty29_outer_oof (source_grouped_oof) | window | 95.2 ± 12.0 | [90.2, 98.6] | Reference |
| frailty29_trained_motion_detector | frailty29_trained_to_ptt22 (frozen_cross_dataset) | file | 100.0 ± 0.0 | [100.0, 100.0] | Reference |
| frailty29_trained_motion_detector | frailty29_trained_to_ptt22 (frozen_cross_dataset) | window | 100.0 ± 0.0 | [100.0, 100.0] | Reference |
| ptt22_trained_motion_detector | ptt22_outer_oof (source_grouped_oof) | file | 100.0 ± 0.0 | [100.0, 100.0] | 1.0000 |
| ptt22_trained_motion_detector | ptt22_outer_oof (source_grouped_oof) | window | 100.0 ± 0.0 | [100.0, 100.0] | 1.0000 |
| ptt22_trained_motion_detector | ptt22_trained_to_frailty29 (frozen_cross_dataset) | file | 0.0 ± 0.0 | [0.0, 0.0] | 6.00e-05 |
| ptt22_trained_motion_detector | ptt22_trained_to_frailty29 (frozen_cross_dataset) | window | 0.0 ± 0.0 | [0.0, 0.0] | 6.00e-05 |
...
| model_id | evaluation | level | participant_macro_mean_sd | participant_bootstrap_ci95 | holm_p_vs_reference |
| --- | --- | --- | --- | --- | --- |
| frailty29_trained_motion_detector | frailty29_outer_oof (source_grouped_oof) | file | 97.2 ± 12.1 | [92.4, 100.0] | Reference |
| frailty29_trained_motion_detector | frailty29_outer_oof (source_grouped_oof) | window | 97.1 ± 12.6 | [92.1, 99.9] | Reference |
| frailty29_trained_motion_detector | frailty29_trained_to_ptt22 (frozen_cross_dataset) | file | 100.0 ± 0.0 | [100.0, 100.0] | Reference |
| frailty29_trained_motion_detector | frailty29_trained_to_ptt22 (frozen_cross_dataset) | window | 100.0 ± 0.0 | [100.0, 100.0] | Reference |
| ptt22_trained_motion_detector | ptt22_outer_oof (source_grouped_oof) | file | 100.0 ± 0.0 | [100.0, 100.0] | 1.0000 |
| ptt22_trained_motion_detector | ptt22_outer_oof (source_grouped_oof) | window | 100.0 ± 0.0 | [100.0, 100.0] | 0.7448 |
| ptt22_trained_motion_detector | ptt22_trained_to_frailty29 (frozen_cross_dataset) | file | 56.7 ± 12.3 | [52.7, 61.3] | 6.00e-05 |
| ptt22_trained_motion_detector | ptt22_trained_to_frailty29 (frozen_cross_dataset) | window | 56.6 ± 10.0 | [53.2, 60.3] | 6.00e-05 |
...
| model_id | evaluation | level | participant_macro_mean_sd | participant_bootstrap_ci95 | holm_p_vs_reference |
| --- | --- | --- | --- | --- | --- |
| frailty29_trained_motion_detector | frailty29_outer_oof (source_grouped_oof) | file | 97.4 ± 11.0 | [93.0, 100.0] | Reference |
| frailty29_trained_motion_detector | frailty29_outer_oof (source_grouped_oof) | window | 89.8 ± 22.2 | [81.0, 96.4] | Reference |
| frailty29_trained_motion_detector | frailty29_trained_to_ptt22 (frozen_cross_dataset) | file | 100.0 ± 0.0 | [100.0, 100.0] | Reference |
| frailty29_trained_motion_detector | frailty29_trained_to_ptt22 (frozen_cross_dataset) | window | 100.0 ± 0.0 | [100.0, 100.0] | Reference |
| ptt22_trained_motion_detector | ptt22_outer_oof (source_grouped_oof) | file | 100.0 ± 0.0 | [100.0, 100.0] | 1.0000 |
| ptt22_trained_motion_detector | ptt22_outer_oof (source_grouped_oof) | window | 100.0 ± 0.0 | [100.0, 100.0] | 0.7448 |
| ptt22_trained_motion_detector | ptt22_trained_to_frailty29 (frozen_cross_dataset) | file | 50.8 ± 12.6 | [46.5, 55.5] | 6.00e-05 |
| ptt22_trained_motion_detector | ptt22_trained_to_frailty29 (frozen_cross_dataset) | window | 9.5 ± 11.9 | [5.7, 14.2] | 6.00e-05 |
...
| --- | --- | --- | --- |
| frailty29_trained_motion_detector | frailty29_outer_oof (source_grouped_oof) | window | 85.8 |
| frailty29_trained_motion_detector | frailty29_outer_oof (source_grouped_oof) | file | 90.8 |
| ptt22_trained_motion_detector | ptt22_outer_oof (source_grouped_oof) | window | 100.0 |
| ptt22_trained_motion_detector | ptt22_outer_oof (source_grouped_oof) | file | 100.0 |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| frailty29_trained_motion_detector | frailty29_outer_oof | file | static | 97.2 | 96.4 | 96.8 | 97.2 |
| frailty29_trained_motion_detector | frailty29_outer_oof | file | motion | 96.4 | 97.2 | 96.8 | 96.4 |
| frailty29_trained_motion_detector | frailty29_outer_oof | window | static | 95.2 | 94.6 | 94.9 | 97.5 |
| frailty29_trained_motion_detector | frailty29_outer_oof | window | motion | 94.6 | 95.2 | 94.9 | 48.3 |
...
| frailty29_trained_motion_detector | frailty29_trained_to_ptt22 | window | static | 100.0 | 57.8 | 78.9 | 70.3 |
| frailty29_trained_motion_detector | frailty29_trained_to_ptt22 | window | motion | 57.8 | 100.0 | 78.9 | 73.2 |
| ptt22_trained_motion_detector | ptt22_outer_oof | file | static | 100.0 | 100.0 | 100.0 | 100.0 |
| ptt22_trained_motion_detector | ptt22_outer_oof | file | motion | 100.0 | 100.0 | 100.0 | 100.0 |
| ptt22_trained_motion_detector | ptt22_outer_oof | window | static | 100.0 | 100.0 | 100.0 | 100.0 |
| ptt22_trained_motion_detector | ptt22_outer_oof | window | motion | 100.0 | 100.0 | 100.0 | 100.0 |
| ptt22_trained_motion_detector | ptt22_trained_to_frailty29 | file | static | 0.0 | 100.0 | 50.0 | 0.0 |
| ptt22_trained_motion_detector | ptt22_trained_to_frailty29 | file | motion | 100.0 | 0.0 | 50.0 | 60.5 |
| ptt22_trained_motion_detector | ptt22_trained_to_frailty29 | window | static | 0.0 | 100.0 | 50.0 | 0.0 |
| ptt22_trained_motion_detector | ptt22_trained_to_frailty29 | window | motion | 100.0 | 0.0 | 50.0 | 4.6 |
...
| --- | --- | --- | --- | --- | --- | --- |
| frailty29_trained_motion_detector | frailty29_outer_oof | file | static | 97.2 | 96.3 | 96.8 |
| frailty29_trained_motion_detector | frailty29_outer_oof | file | motion | 96.4 | 96.3 | 90.4 |
| frailty29_trained_motion_detector | frailty29_outer_oof | window | static | 99.9 | 96.1 | 99.9 |
| frailty29_trained_motion_detector | frailty29_outer_oof | window | motion | 32.4 | 96.1 | 29.5 |
| frailty29_trained_motion_detector | frailty29_trained_to_ptt22 | file | static | 53.7 | 100.0 | 100.0 |
| frailty29_trained_motion_detector | frailty29_trained_to_ptt22 | file | motion | 100.0 | 100.0 | 100.0 |
| frailty29_trained_motion_detector | frailty29_trained_to_ptt22 | window | static | 54.2 | 99.7 | 99.4 |
| frailty29_trained_motion_detector | frailty29_trained_to_ptt22 | window | motion | 100.0 | 99.7 | 99.9 |
| ptt22_trained_motion_detector | ptt22_outer_oof | file | static | 100.0 | 100.0 | 100.0 |
| ptt22_trained_motion_detector | ptt22_outer_oof | file | motion | 100.0 | 100.0 | 100.0 |
| ptt22_trained_motion_detector | ptt22_outer_oof | window | static | 100.0 | 100.0 | 100.0 |
| ptt22_trained_motion_detector | ptt22_outer_oof | window | motion | 100.0 | 100.0 | 100.0 |
| ptt22_trained_motion_detector | ptt22_trained_to_frailty29 | file | static | 0.0 | 56.7 | 60.9 |
| ptt22_trained_motion_detector | ptt22_trained_to_frailty29 | file | motion | 43.4 | 56.7 | 47.8 |
| ptt22_trained_motion_detector | ptt22_trained_to_frailty29 | window | static | 0.0 | 56.3 | 97.9 |
| ptt22_trained_motion_detector | ptt22_trained_to_frailty29 | window | motion | 2.4 | 56.3 | 3.0 |
...
| --- | --- | --- | --- | --- | --- | --- |
| frailty29_trained_motion_detector | frailty29_outer_oof | window | 20244 | 1023 | 28 | 490 |
| frailty29_trained_motion_detector | frailty29_trained_to_ptt22 | window | 5331 | 0 | 4498 | 6154 |
| ptt22_trained_motion_detector | ptt22_outer_oof | window | 5331 | 0 | 0 | 10652 |
| ptt22_trained_motion_detector | ptt22_trained_to_frailty29 | window | 0 | 21267 | 0 | 518 |
...
| model_id | dataset | aggregation_level | true_static_predicted_static | true_static_predicted_motion | true_motion_predicted_static | true_motion_predicted_motion |
| --- | --- | --- | --- | --- | --- | --- |
| frailty29_trained_motion_detector | frailty29_outer_oof | file_median_window_probability | 141 | 4 | 4 | 107 |
| frailty29_trained_motion_detector | frailty29_trained_to_ptt22 | file_median_window_probability | 22 | 0 | 19 | 25 |
| ptt22_trained_motion_detector | ptt22_outer_oof | file_median_window_probability | 22 | 0 | 0 | 44 |
| ptt22_trained_motion_detector | ptt22_trained_to_frailty29 | file_median_window_probability | 0 | 145 | 0 | 111 |
...
Detector P is the Holm-adjusted, two-sided participant-paired Monte-Carlo sign-flip P value for the retrospective training-source comparison (`ptt22_trained_motion_detector` minus `frailty29_trained_motion_detector`) on an identical target/level roster. Holm correction covers all detector endpoints within each target × level family. This asymmetric grouped-OOF-versus-frozen comparison is exploratory.
...
| candidate_id | training_dataset | training_participant_count | model_path | threshold_path |
| --- | --- | --- | --- | --- |
| frailty29_trained_legacy_reference | frailty29 | 29 | frailty29_trained_legacy_reference/formal_motion_model.pt | frailty29_trained_legacy_reference/deployment_threshold.json |
| ptt22_trained_reverse_ablation | ptt_ppg_1_1_0_local | 22 | ptt22_trained_reverse_ablation/formal_motion_model.pt | ptt22_trained_reverse_ablation/deployment_threshold.json |

<details><summary>Column definitions and formulas</summary>

- **candidate_id** (`candidate_id`): Direct identifier, provenance, configuration, grouping, or status value. Formula: `N/A — identifier, provenance, configuration, status, or other non-arithmetic field.`
- **training_dataset** (`training_dataset`): Direct identifier, provenance, configuration, grouping, or status value. Formula: `N/A — identifier, provenance, configuration, status, or other non-arithmetic field.`
- **training_participant_count** (`training_participant_count`): Number of units satisfying the column's stated inclusion condition. Formula: `count = sum_i 1[unit i satisfies the stated condition]`
- **model_path** (`model_path`): Direct identifier, provenance, configuration, grouping, or status value. Formula: `N/A — identifier, provenance, configuration, status, or other non-arithmetic field.`
- **threshold_path** (`threshold_path`): Direct identifier, provenance, configuration, grouping, or status value. Formula: `N/A — identifier, provenance, configuration, status, or other non-arithmetic field.`
