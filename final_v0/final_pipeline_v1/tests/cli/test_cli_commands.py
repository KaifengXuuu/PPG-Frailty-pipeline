...
def run_cli(*arguments: str) -> tuple[subprocess.CompletedProcess[str], dict[str, object]]:
    """在冻结 V1 source 上运行命令 / Run a command against the frozen V1 source."""
...
class CliCommandTests(unittest.TestCase):
    ...
    def test_validate_all_configs_passes_four_of_four(self) -> None:
        """四份配置必须全部通过 / All four configurations must pass."""
    ...
    def test_shapeformer_machine_id_builds_strict_discovery_provenance(self) -> None:
        ...
        completed, payload = run_cli(
            "compare", "models", "--models", "shapeformer_effect_size", "--seed", "42"
        )
        self.assertEqual(completed.returncode, 0, completed.stderr)
        self.assertEqual(payload["status"], "passed")
        self.assertEqual(len(payload["results"]), 1)
        row = payload["results"][0]
        self.assertEqual(row["canonical_model_id"], "ShapeFormerEffectSize")
        self.assertEqual(row["machine_model_id"], "shapeformer_effect_size")
        self.assertEqual(row["discovery_method"], "effect_size_shapelets_v1")
    ...
    def test_run_experiment_dispatch_preserves_public_budgets_and_cleans_temp(self) -> None:
        """mock dispatch 验证预算隔离；仅清理精确 V1 temp / Dispatch budgets."""
        ...
        with tempfile.TemporaryDirectory(
            prefix="ppg_frailty_v1_experiment_cli_", dir=temporary_root
        ) as directory:
            ...
            def outcome(scope: str, target: Path) -> ExperimentResult:
                ...
                return ExperimentResult(
                    ...
                    config_id="motion_benchmark_spectral_rate_only_v1",
            ...
            with patch(
                ...
                code = main(
                    [
                        "run-experiment", "--config", "motion_benchmark_v1",
    ...
    def test_smoke_output_can_use_and_clean_exact_v1_temporary_directory(self) -> None:
        """仅清理由本测试创建的 V1 temp / Clean only the exact V1 temp created here."""

        temporary_root = (ROOT / "artifacts" / "tmp").resolve()
        temporary_root.mkdir(parents=True, exist_ok=True)
        created_path: Path | None = None
        with tempfile.TemporaryDirectory(
            prefix="ppg_frailty_v1_cli_", dir=temporary_root
        ) as directory:
            created_path = Path(directory).resolve()
            created_path.relative_to(temporary_root)
