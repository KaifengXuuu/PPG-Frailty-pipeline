"""Frozen window-to-file-to-role-to-participant aggregation.

冻结的 window→file→role→participant 聚合。
"""
...
def _coverage_summaries(rows: tuple[OofPredictionRow, ...]) -> tuple[CoverageSummary, ...]:
    """Count retained units without discarding rejected descendants.

    统计各层保留单元，同时保留被拒绝的下级单元作为分母。file/role/participant
    只要至少一个下级结果被保留，就视为该层可用。
    """
    ...
    for _, grouped in groupby(ordered, key=_identity_key):
        ...
        unit_keys: dict[str, list[tuple[object, ...]]] = {
            ...
            "role": [(row.participant_id, row.role) for row in values],
        ...
        retained_keys: dict[str, set[tuple[object, ...]]] = {
            ...
            "role": {
                (row.participant_id, row.role) for row in values if row.retained
            },
            "participant": {(row.participant_id,) for row in values if row.retained},
        }
        for level in ("source", "file", "role", "participant"):
...
def aggregate_hierarchy(
    rows: Iterable[OofPredictionRow], *, quality_weighted: bool = False
) -> HierarchyAggregation:
    """Apply the only canonical hierarchy: window→file→role→participant.

    应用唯一主协议层级：window→file→role→participant。文件之间、role 之间均等权；
    SQI 加权必须显式设置并作为消融报告。
    """
    ...
    # English: Materialise and validate identity before filtering; otherwise a
    # rejected row with mismatched provenance could disappear unnoticed.
    # 中文：过滤前先物化并校验身份；否则 provenance 不一致的拒绝行会悄然消失。
    ...
    if windows:
        generated_files = _group_aggregate(
            ...
            role_factory=lambda values: values[0].role,
        )
    ...
    role_rows = _group_aggregate(
        ...
        file_id_factory=lambda values: f"role::{values[0].participant_id}::{values[0].role}",
        role_factory=lambda values: values[0].role,
    )
    ...
    return HierarchyAggregation(
        ...
        coverage=_coverage_summaries(source_rows),
    )
