"""400 Hz PPG 质量检查与零相位预处理 / PPG QC and zero-phase preprocessing.

M3 数学迁移来源（复制并按 V1 合同改写，绝不 import）：
...
M3 mathematical source was copied conceptually and rewritten for the V1 contract;
...
def inspect_and_repair(
    ...
    *,
    ...
    if float(fs_hz) != CANONICAL_FS_HZ:
        raise ValueError("V1 input QC requires exactly 400 Hz")
...
def build_signal_views(
    ...
    """从记录构建 V1 公共四视图 / Build the public V1 signal views.
    ...
    if any(ppg_filter.get(key) != value for key, value in expected_filter.items()):
        raise ValueError("resolved PPG filter differs from the frozen V1 profile")
    if set(ppg_filter) != set(expected_filter):
    ...
    if not isinstance(analysis_view, Mapping) or dict(analysis_view) != expected_analysis:
        raise ValueError("resolved signal.analysis_view differs from the frozen V1 contract")
    ...
    if imu_config.get("gravity_method") != "quaternion_error_state_ekf":
        raise ValueError("formal V1 primary profile requires quaternion_error_state_ekf")
    if (
        imu_config.get("initialization") != "online_no_precalibration"
        or float(imu_config.get("sensor_lowpass_acc_hz", float("nan"))) != 20.0
        ...
        raise ValueError("resolved signal.imu parameters differ from frozen V1")
    expected_imu_keys = {
    ...
    if set(imu_config) != expected_imu_keys:
        raise ValueError("resolved signal.imu contains missing or unknown keys")
    if (
        imu_config.get("comparison_method") != "lowpass_0p3hz"
        or imu_config.get("required_axes") != 6
        or imu_config.get("output_units")
        != {"acceleration": "m/s^2", "gyroscope": "rad/s", "jerk": "m/s^3"}
    ):
        raise ValueError("resolved signal.imu auxiliary contract differs from frozen V1")
    expected_resampling = {
        "enabled": False,
        "target_fs_hz": 400.0,
    ...
    if signal_config.get("dl_resampling") != expected_resampling:
        raise ValueError("resolved signal.dl_resampling differs from frozen V1")
    ...
    if signal_config.get("normalization") != expected_normalization:
        raise ValueError("resolved signal.normalization differs from frozen V1")
    ...
    views = CanonicalSignalViews(
        x_native=native,
        x_filter=filtered,
        # 中文：V1 direct rate 输入按 ADR-003 明确等于 x_filter。
