"""Dual-wavelength PCA/FastICA/NMF blind source separation.

Every BSS method requires two synchronized PPG channels; single-channel inputs fail.
Outputs support rate recovery only, without claiming amplitude or morphology preservation.
"""
...
class BssConfig:
    ...
    def validate(self) -> None:
        """Validate numerical BSS configuration."""
...
def _cardiac_fraction(values: np.ndarray, fs_hz: float) -> float:
    """Component power fraction in the 0.5–3.5 Hz cardiac band."""
...
def _select_source(
    ...
    *,
    ...
    """Select a rate source by cardiac concentration minus motion correlation."""
...
class _LinearBssReducer(ArtifactReducer):
    """Shared time-domain reconstruction."""
    ...
    def _fit(self, source: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray, dict[str, object]]:
        """Return fitted sources, mixing matrix, mean and diagnostics."""
...
class PcaBssReducer(_LinearBssReducer):
    """Dual-channel PCA comparator."""

    reducer_id = "pca_bss"
    reducer_version = "pca_component_select_v2"
    algorithm_kernel_description = ("Deterministic two-source PCA on synchronized RED/IR; select one component by cardiac concentration minus maximum IMU-correlation penalty, then project to both channels. "
                                    "Kernel: full-SVD PCA, Welch spectral scoring and linear mixing reconstruction.")
    method = "pca"


class FastIcaBssReducer(_LinearBssReducer):
    """Fixed-seed FastICA comparator."""

    reducer_id = "fastica_bss"
    reducer_version = "fastica_component_select_v2"
    algorithm_kernel_description = ("Fixed-seed two-source FastICA on synchronized RED/IR; select by cardiac concentration minus IMU-correlation penalty, then project back. "
                                    "Kernel: unit-variance whitening, FastICA fixed-point iterations and linear mixing reconstruction.")
    method = "fastica"


class NmfBssReducer(ArtifactReducer):
    """NMF with shared dual-channel spectral bases."""

    reducer_id = "nmf_bss"
    reducer_version = "nmf_shared_spectral_basis_v1"
    algorithm_kernel_description = ("Concatenate nonnegative RED/IR STFT magnitudes and fit shared spectral bases; select the strongest cardiac-band concentration and reconstruct using original phase. "
                                    "Kernel: Hann STFT, coordinate-descent NMF with NNDSVDA initialization, and ISTFT.")
    ...
    def reduce(
        ...
        *,
        ...
        """Reconstruct selected NMF magnitude with phase."""
