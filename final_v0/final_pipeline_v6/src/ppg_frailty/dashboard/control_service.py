...
class V5ControlService:
    ...
    def __init__(self, pipeline_root: str | Path | None = None) -> None:
        ...
        self.repository_root = resolve_repository_root(self.pipeline_root)
    ...
    def safe_input(self, value: str | Path, *, label: str, must_exist: bool = True, suffixes: Iterable[str] | None = None) -> Path:
        ...
        if not any(target.is_relative_to(root) for root in (self.pipeline_root, self.repository_root)):
            raise ValueError(f'{label} must remain inside {self.pipeline_root} or {self.repository_root}')
