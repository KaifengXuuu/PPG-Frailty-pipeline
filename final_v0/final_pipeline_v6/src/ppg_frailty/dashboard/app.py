...
PARAMETER_PANELS = {
    'imu': ('Gravity parameters', {'signal.imu.gravity_method'}),
    'quality': ('Calibrator / SQI parameters', {'quality.mode', 'quality.calibrator'}),
    'denoiser': ('Reducer parameters', {'artifact.denoiser_enabled', 'artifact.reducer'}),
    'features': ('Feature parameters', set()),
    'representation': ('Representation parameters', {'representation_mode'}),
    'model': ('Model / Train parameters', {'model.model_id'}),
}
    ...
    for traces, group_key, heading, frequency in (
        ...
        if frequency == 'psd':
            output.append(html.Details([html.Summary(heading), graph,
                html.Small('Positive PSD values use a logarithmic y-axis. Zeros remain in the data but are not displayed on that axis; all-zero panels use a linear axis. The frequency axis includes 0 Hz.')],
                open=False, className='parameter-panel'))
        else:
            output.extend([html.H4(heading), graph])
        if frequency == 'fft':
            output.append(html.Small('FFT amplitude = abs(rFFT)/N, without squaring, dB conversion, doubling or extra detrending. The initial view is 0.01–8 Hz; DC through Nyquist is retained and accessible with Autoscale. Incomplete records use the longest continuous valid segment; see Stage details → fft.'))
            ...
            if not traces or rate.get('valid_window_count') == 0 or rate.get('status') == 'unavailable':
                ...
                explanation = {'peak_detection_unavailable': 'No usable peaks in the current extraction',
                               'no_windows_planned': 'Current window length, hop and boundary settings produced no windows',
                               'no_windows_with_sufficient_route_eligible_ppi': 'Windows exist but none provide statistics meeting routing and interval-validity requirements',
                               'window_rate_not_computed': 'Window statistics have not been computed'}.get(reason, 'Window statistics unavailable')
                output.append(html.Div(f'No valid window PPI/HR: {explanation} ({reason}). Values were not imputed and algorithm validity conditions were not relaxed.', role='status'))
            output.append(html.Small('Reuses feature-matrix windowing and PPI/HR statistics in every representation mode. Outside matrix mode this is preview-only and leaves model inputs unchanged. Shows the full record regardless of the time preview range; x positions are window centers. All curves start visible, with gaps at invalid windows and no imputation.'))
        elif heading == 'Beatwise PPI / HR':
            output.append(html.Small('X positions are midpoints between adjacent peaks; HR = 60/PPI. Consecutive valid intervals are connected; invalid intervals are crosses, hidden until their legend is selected. Gaps and source changes break lines. used and matrix_used identify intervals selected by the actual feature algorithms.'))
...
def create_app(pipeline_root: str | Path | None = None, *, control_service=None,
    ...
    *{box-sizing:border-box}h1{font-size:27px}h2{font-size:21px}h3{font-size:16px}h4{font-size:14px}
        ...
        return html.Main([
            ...
            *[stage_panel(stage, title, i) for i, (stage, title) in enumerate(STAGES, 1)],
            ...
            html.Section([
                ...
                html.Div('Save workflow_config.yaml beside the downloaded stage command; run from the V6 directory.', className='notice'),
    ...
    def tool_cli(_, request, *args):
        ...
        return dcc.send_string(_tool_download_command(request), 'v6_tool.sh') if request else no_update
    ...
    def tool_yaml(_, request, *args):
        if args:
            try:
                ...
                return no_update
        name = f"{request['config_sha256']}.yaml" if request and request.get('config_sha256') else 'v6_tool_request.yaml'
    ...
    def report_cli(_, request):
        return dcc.send_string(request['display']+'\n', 'v6_report.sh') if request else no_update
