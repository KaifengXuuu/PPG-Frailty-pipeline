"""Canonical 400-Hz signal views and the shared window contract."""
...
def _matrix(value: np.ndarray, *, name: str, columns: int = 2) -> np.ndarray:
    """Convert to a finite two-dimensional matrix."""
...
class CanonicalSignalViews:
    """Direct and identity views keep x_filter; non-identity x_ar is rate-only."""
    x_native: np.ndarray
    x_filter: np.ndarray
    ...
    def x_analysis(self) -> np.ndarray:
        """Return amplitude-preserving x_filter, never the rate-only artifact view."""
    ...
    def validate(self) -> None:
        ...
        if self.route is SignalRoute.ARTIFACT_RATE_ONLY:
            ...
            artifact_valid = np.asarray(self.metadata.get("artifact_output_valid_mask"), dtype=bool)
    ...
    def analysis_signal(self) -> np.ndarray:
        """Return the legal rate input without fallback."""
        self.validate()
        if self.route is SignalRoute.ARTIFACT_RATE_ONLY:
            # validate already proves existence; assert only narrows static types.
    ...
    def to_contract(self) -> SignalViews:
        """Adapt to the repository-wide contract."""
        self.validate()
        metadata = dict(self.metadata)
        metadata.update(
            {
                "route": self.route.value,
                "non_identity_artifact_reduction": self.route is SignalRoute.ARTIFACT_RATE_ONLY,
    ...
    def with_artifact_result(self, result: ArtifactReductionResult) -> "CanonicalSignalViews":
        """Bind without fallback."""
        if result.status != "success" or result.x_ar is None:
            raise RuntimeError("artifact reducer did not succeed; direct fallback is scientifically forbidden")
