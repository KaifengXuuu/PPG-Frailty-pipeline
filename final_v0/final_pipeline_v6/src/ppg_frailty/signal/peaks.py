"""Dual-polarity pulse peaks, PPI and adjacency contracts.

Rejected intervals remain on the timeline; every endpoint refers to the original peak ordinal.
...
class _Candidate:
    """Internal polarity and wavelength candidate."""
...
def _robust_scale(values: np.ndarray) -> float:
    """Robust IQR scale with SD fallback."""
...
def _candidate(
    ...
    *,
    ...
    """Score one channel-polarity alternative."""
...
def _detect_pulses_dual_polarity_ablation(
    ...
    *,
    ...
    """Public pulse-detection entry point.

    `CanonicalSignalViews` chooses a valid rate signal; failed reducers cannot create that view.
    There is no silent direct fallback. A canonical view automatically selects its
    ...
    if isinstance(values, CanonicalSignalViews):
        ...
        resolved_route = SignalRoute.DIRECT if source_route is None else SignalRoute(source_route)
        record_id = ""
    ...
    if run_id is None:
        if record_id:
            resolved_run_id = f"{record_id}::{resolved_route.value}::{DETECTOR_ID}"
        else:
            digest = hashlib.sha256(np.ascontiguousarray(matrix, dtype="<f8").tobytes(order="C")).hexdigest()[:20]
            resolved_run_id = f"array::{resolved_route.value}::{DETECTOR_ID}::{digest}"
    else:
    ...
    if matrix.shape[0] / fs_hz < min_observation_sec:
        raise ValueError("HR/PPI requires at least " f"{min_observation_sec:g} seconds of observation")
    sample_offset = 0
    if not np.all(valid_samples):
        # Choose the longest artifact-valid run; never concatenate across invalid gaps.
        ...
        if (stop - start) / fs_hz < min_observation_sec:
            raise ValueError("longest artifact-valid run is shorter than configured "
    ...
    candidates = [
        ...
        for channel in channel_indices for polarity in (1, -1)
    ]
    ...
    # Consecutive original peaks imply true adjacency; validity separately records rejected intervals.
    ...
    detector_coverage = float(np.clip(np.sum(valid_ppi) / peak_span, 0.0, 1.0)) if peak_span > 0.0 else 0.0
    provenance = ({
    ...
    result = PulseResult(
        ...
        interval_rejection_reasons=tuple("" if valid else "outside_legacy_0p30_to_2p00_ppi"
