"""V2 outer-train-only vector transform.

English: One fitted artifact serves ordered matrices and finite fusion tensors.
The shared fitted artifact avoids data leakage and implicit loss of
validity.
"""
...
def fusion_tensor_schema_version(feature_names: Sequence[str]) -> str:
    ...
    return f"feature_vector_values_plus_validity_{width}_" f"registry-{registry.sha256[:12]}_v4"
...
class FoldTransformedFeatureBatch:
    ...
    def validate(self) -> None:
        ...
        expected_schema = registry.names + tuple(f"{name}.validity" for name in registry.names)
        if (
            ...
            or self.provenance.get("feature_vector_transform_schema") != TRANSFORM_SCHEMA_VERSION
        ):
...
def transform_feature_vector(
    ...
    values[valid] = (np.asarray(vector.values, dtype=np.float64)[valid] - transform.center[valid]) / transform.scale[
        valid
    ]
    ...
    provenance.update(
        {
            ...
            "feature_vector_transform_fitted_on_participant_ids": list(transform.fitted_on_participant_ids),
...
def transform_feature_vector_batch(
    ...
    values = np.vstack([np.where(context.validity, context.values, 0.0) for context in contexts])
    validity = np.vstack([context.validity.astype(np.float64) for context in contexts])
    tensor = np.column_stack((values, validity))
    schema = transform.feature_names + tuple(f"{name}.validity" for name in transform.feature_names)
