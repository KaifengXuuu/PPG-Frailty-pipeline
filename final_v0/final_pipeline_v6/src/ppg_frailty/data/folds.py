"""Read-only import and materialization of frozen outer-fold memberships.

Read materialized membership from the M2 corrected SGKF JSON. Runtime splitter
calls are not part of the public API. Each CSV row identifies a participant OOF fold;
training membership is its complement within the same repeat.
...
M2_SPLIT_RELATIVE_PATH = Path("assets/authority/splits/frailty3_future_corrected_sgkf5_v2.json")
M2_SPLIT_FILE_SHA256 = "c80e780d60162ff1132101ee368ee3a55d95f3c7a6d51a5c9a8feec8605d388c"
M2_SPLIT_PAYLOAD_SHA256 = "0bca827fa8e4f9679b6ee9435b5497a60d91e74ddb366e058fc69614fb007f46"
M2_SUBJECT_INPUT_SHA256 = "6a9c5e057200f32fefa44ed7c77cd6c5ef4f1aa61af36dc2e5f8d786861a5e10"
...
def outer_cv_single_model_training_seed(
    ...
    if observed != expected:
        raise ValueError(f"outer CV split seed drift for repeat {repeat}: {observed} != {expected}")
    return observed

@dataclass(frozen=True)
class FrozenFoldAudit:
    """Machine audit of frozen memberships."""
...
class FrozenFoldRegistry:
        ...
        with Path(path).open("r", encoding="utf-8", newline="") as handle:
            ...
            for line_number, raw in enumerate(reader, start=2):
                try:
                    ...
                    raise ValueError(f"invalid fold row at line {line_number}: {exc}") from exc
        if not rows:
    ...
    def participant_ids(self) -> tuple[str, ...]:
        """Return stable participant roster."""
    ...
    def get_split(self, repeat_index: int, fold_index: int) -> dict[str, Any]:
        """Resolve one train/OOF partition."""

        repeat_rows = [row for row in self.assignments if row.repeat_index == int(repeat_index)]
        if not repeat_rows:
            raise KeyError(f"repeat_index_not_found:{repeat_index}")
        oof = sorted(row.participant_id for row in repeat_rows if row.fold_index == int(fold_index))
        if not oof:
...
def _canonical_registry_payload(registry: Mapping[str, Any]) -> dict[str, Any]:
    """Remove the self-referential hash field."""
...
def load_frozen_memberships(path: str | Path) -> dict[str, Any]:
    """Load the authoritative M2 JSON."""

    source = Path(path)
    file_digest = sha256_file(source)
    if file_digest != M2_SPLIT_FILE_SHA256:
        raise ValueError(f"frozen registry file SHA drift: {file_digest} != {M2_SPLIT_FILE_SHA256}")
    registry = json.loads(source.read_text(encoding="utf-8"))
    # M2 payload identity uses its builder pretty strict-JSON bytes, not the
    # compact V1 provenance.py encoding. Reproduce the source identity rule.
...
def load_m2_frozen_registry(repository_root: str | Path) -> dict[str, Any]:
    """Load the packaged memberships; the data-root argument remains API-compatible."""

    return load_frozen_memberships(pipeline_resource(M2_SPLIT_RELATIVE_PATH))

def _participant_class_map(rows: Iterable[ManifestRow]) -> dict[str, int]:
    """Build a conflict-free class map."""
    ...
    for expected_repeat_index, repeat in enumerate(repeats):
        ...
        for expected_fold_index, fold in enumerate(folds):
            ...
            for class_id in CANONICAL_CLASS_NAMES:
                class_counts_by_fold[class_id].append(sum(classes[participant] == class_id for participant in oof))
            # Verify JSON file membership inherits the participant fold.
            # English: Verify file membership is inherited from participant membership.
            expected_oof_files = {
                record_id for participant in oof for record_id in expected_files_by_participant[participant]
            }
...
def materialize_assignments(
    ...
    """Convert JSON to long assignments."""
...
def _validate_assignment_rows(rows: Iterable[FoldAssignment]) -> None:
    """Validate long-table uniqueness."""
...
def materialize_fold_csvs(
    ...
    *,
    ...
    """Write primary and complete repeat tables."""
