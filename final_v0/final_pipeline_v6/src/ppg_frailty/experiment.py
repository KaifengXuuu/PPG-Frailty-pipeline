...
class ExperimentResult:
    """Strictly serializable experiment outcome."""
    ...
    def to_dict(self) -> dict[str, Any]:
        """Convert to a strict-JSON value."""
        payload = asdict(self)
...
class _RuntimeRecord:
    """Per-record state inside one fold."""
...
class _ExperimentProtocolError(RuntimeError):
    """Fail-closed protocol exception."""
...
@dataclass(frozen=True)
class _CellResult:
    """One cell summary and OOF tables."""
...
def _runtime_imports() -> dict[str, Any]:
    """Lazily import experiment dependencies."""
...
def _choose_records(rows: Iterable[Any], participant_ids: Iterable[str], roles: Iterable[str],
    ...
    """Select longest files per participant."""
...
def _dataset(states: Iterable[_RuntimeRecord]) -> Any:
    """Materialise a file-level vector dataset."""
    api = _runtime_imports()
...
def _extract_raw(
    ...
    """Build configured mask-aware raw windows."""
    if not state.retained:
        return
    api = _runtime_imports()
    try:
...
def _retained_states(states: Iterable[_RuntimeRecord],
    ...
    """Return deterministic retained states."""
...
def _sample_identity(
    ...
    """Create one canonical physiological identity."""
    api = _runtime_imports()
...
def _materialize_representation_dataset(
    ...
    """Materialise exactly one typed dataset."""
    api = _runtime_imports()
...
def _resolved_model_config(config: Any, *, training_seed: int,
    ...
    """Map archive metadata to strict factory options."""
    api = _runtime_imports()
...
def _model_input_spec(dataset: Any, mode: str) -> Any:
    """Resolve dimensions from materialised train data."""
    api = _runtime_imports()
    if mode == "feature_vector":
