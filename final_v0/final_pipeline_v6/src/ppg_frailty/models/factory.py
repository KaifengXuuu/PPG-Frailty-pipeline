...
Strict model factory covering four representation modes.
"""
...
# The optional deep profile is a V2 dependency tier; each execution environment still requires probing.
...
# Map specification display names to stable machine IDs only here. Manifests
# preserve both; aliases not listed in this table are rejected.
...
def normalize_model_id(value: str) -> tuple[str, str]:
    ...
    Return canonical names and machine IDs; fail on unknown names.
    """
...
def normalize_model_config(model_config: Mapping[str, Any]) -> dict[str, Any]:
    """Canonicalise the ID while preserving options."""
...
class ModelInputSpec:
    """Frozen dimensions and schema identifiers."""
    ...
    def from_value(cls, value: "ModelInputSpec | Mapping[str, Any]") -> "ModelInputSpec":
        """Normalise a dataclass or mapping."""
...
def _torch_seed(seed: int) -> None:
    """Seed PyTorch only when a deep model is requested."""
...
class PreparedModelFactory:
    ...
    Callable model factory bound to outer-training membership with serializable provenance.
    ShapeFormer discovery is already complete; repeated calls create new models from the same frozen configuration.
    """
    ...
    def factory(self) -> Callable[[], Any]:
        """Expose the stable public factory handle."""

        return self

def _shapelet_bank_hash(bank: Any) -> str:
    """Hash learned bank arrays and metadata."""
...
# Descriptive compatibility alias used by internal configuration code.
