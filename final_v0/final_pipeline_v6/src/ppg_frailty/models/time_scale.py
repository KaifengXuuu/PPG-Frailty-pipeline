...
V2 fixed-kernel-sample resampling ablation applies only to CompactCNN/InceptionTimeFull.
Kernel sample counts stay fixed when input sampling changes, intentionally changing physical duration;
this is not time-matched. Register reference/single-factor conditions only; never launch training/evaluation.
"""
...
def _condition_name(dl_fs_hz: float, raw_window_seconds: float, dilation: int) -> str:
    """Resolve one allowed factor change."""
...
def build_fixed_kernel_resampling_cases() -> tuple[FixedKernelResamplingCase, ...]:
    """Materialise the fixed 12-case registry."""
