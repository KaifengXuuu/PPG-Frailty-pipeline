...
class LoadedBundle:
    """Validated bundle held in memory."""
...
def _jsonable(value: Any) -> Any:
    """Convert dataclasses/arrays/scalars to strict JSON."""
...
def _predict_model(model: Any, inputs: Mapping[str, np.ndarray]) -> np.ndarray:
    """Representation-aware probability prediction."""
...
def _apply_transforms(transforms: Any, inputs: Mapping[str, np.ndarray]) -> dict[str, np.ndarray]:
    ...
    Apply bundle-fitted transforms through their explicit interface, preferring transform_inputs;
    single-array transforms affect x only and preserve structural fields such as masks.
    """
...
def _save_bundle_impl(
    ...
    *,
    ...
    Saving an immutable bundle immediately reloads it and checks golden parity. Missing
    provenance, mismatching hashes or round-trip prediction deviations fail.
    """
    normalized_metadata = validate_bundle_metadata(metadata)
...
def predict_bundle(bundle: LoadedBundle | str | Path, inputs: Mapping[str, np.ndarray]) -> np.ndarray:
    """Predict through a validated loaded bundle."""
...
def assert_golden_parity(bundle: LoadedBundle | str | Path, *, atol: float | None = None) -> None:
    """Assert saved and reloaded probabilities agree."""
    loaded = load_bundle(bundle) if isinstance(bundle, (str, Path)) else bundle
