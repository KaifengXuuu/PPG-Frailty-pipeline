...
def test_execution_fields_match_frozen_configuration(expansion, case_id):
    expected = EXPECTED["legacy_cases"][case_id]
...
def test_bridge_effective_controls_match_historical_run(expansion):
    expected = EXPECTED["legacy_controls"]
    ...
    assert list(actual.profiles) == expected["profiles"]
