...
def test_discovered_paths_use_distribution_root_not_src(monkeypatch):
    ...
    assert paths.pipeline_root == ROOT
    expected_data_root = ROOT.parent.parent if ROOT.parent.name == "final_v0" else ROOT
    assert paths.repository_root == expected_data_root
    assert paths.input_path('configs/presets/finalcase.yaml') == ROOT / 'configs/presets/finalcase.yaml'
...
def test_baseline_is_default_and_byte_identical_to_v2() -> None:
    ...
    # Frozen V2 reference bytes; standalone V6 does not need the V2 source tree.
    assert rows["baseline"]["sha256"] == "9124935b59021ac1b5bba17189e98d0eae1003d1f176402ed19c8108e809733f"
