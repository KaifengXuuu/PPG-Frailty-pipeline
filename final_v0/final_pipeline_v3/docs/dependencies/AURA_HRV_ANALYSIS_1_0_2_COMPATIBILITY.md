...
- The exact profile freezes a 22-package dependency closure and all 24 installed
  distribution records (including pip and setuptools). Missing or additional
  distributions close the formal gate.

## Frozen conclusion

The Aura comparison profile is available only in its isolated exact
...
classifier or clinical-performance evidence. Direct calls to the function-level
adapter are explicitly diagnostic and cannot claim exact-profile evidence. The
formal CLI route first binds the clean tracked source snapshot, adapter and
validator hashes, fixed-PPI output artifact, isolated prefix, complete installed
distribution set and `pip check`; only that wrapper may emit a formal `passed`
status.
...
- the `prv_aura_compare` row in `locks/profiles.lock.json`
