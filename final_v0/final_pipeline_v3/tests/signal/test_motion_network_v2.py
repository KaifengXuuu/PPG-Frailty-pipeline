"""Frozen 11-channel motion tensor/scaler tests; no model training or CV."""
...
class MotionNetworkTests(unittest.TestCase):
    def test_exact_schema_window_hop_and_ppg_only_window_scaling(self) -> None:
        ...
        self.assertEqual(len(MOTION_NETWORK_CHANNEL_SCHEMA), 11)
        self.assertEqual(windows.schema_sha256, MOTION_NETWORK_SCHEMA_SHA256)
        self.assertEqual(windows.values.shape, (2, 11, 3200))
        ...
        self.assertTrue(
            np.array_equal(
                windows.values[0, 2:],
                imu_values[:3200].T.astype(np.float32),
            )
        )

    def test_scaler_uses_train_participants_and_covers_all_nine_imu_channels(self) -> None:
        values = np.zeros((3, 11, 3200), dtype=np.float32)
        ...
        self.assertEqual(transform.valid_count.tolist(), [6400] * 9)
