...
def vector(q_rate: float | None):
    values = {} if q_rate is None else {"sqi.q_rate": q_rate}
    validity = {} if q_rate is None else {"sqi.q_rate": True}
...
class VectorTransformTests(unittest.TestCase):
    ...
    def test_matrix_context_and_fusion_tensor_share_one_artifact(self) -> None:
        ...
        index = registry.names.index("sqi.q_rate")
