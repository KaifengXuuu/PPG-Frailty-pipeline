...
class ModelCardTests(unittest.TestCase):
    ...
    def test_shapeformer_card_locks_experimental_patch_identity(self) -> None:
        ...
        text = (V1_ROOT / "model_cards" / "shapeformer_effect_size_fixed_v1.md").read_text(
            encoding="utf-8"
        )
    ...
    def test_cards_state_evaluation_and_claim_limits(self) -> None:
        ...
        for canonical_name, machine_id in CANONICAL_MODEL_REGISTRY.items():
            ...
            with self.subTest(canonical_name=canonical_name):
                ...
                self.assertIn(
                    'participant after window→file, then config-dependent Line A equal-files or Line B equal-role-families aggregation',
                    text,
                )
                self.assertIn('SQI off; only B and R are admitted', text)
                self.assertIn('scientific benchmark not run', text)
