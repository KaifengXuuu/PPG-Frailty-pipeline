...
def test_formal_builder_needs_explicit_schema_and_historical_model_is_exact() -> None:
    ...
    assert formal.architecture.in_channels == 11
...
def test_yaml_and_python_contract_agree_without_running_science() -> None:
    ...
    assert yaml_payload["scientific_execution_gate"] == (
        python_payload["scientific_execution_gate"]
    )
    assert yaml_payload["scientific_execution_gate"][
    ...
    assert yaml_payload["formal_model"]["network_tensor_schema"]["implied_channel_count"] == 11
...
def test_ptt_unit_conflict_is_structured_and_official_metadata_alone_cannot_bypass(
    ...
    with patch(
        "ppg_frailty.quality.motion_reference.establish_motion_scientific_gate",
        return_value=object(),
    ):
