...
from ppg_frailty.scientific_gate import (
    MOTION_INTERNAL_GATE_PHASES,
    SCIENTIFIC_GATE_CAPTURE_SCHEMA,
    SCIENTIFIC_SOURCE_GATE_SCHEMA,
    ScientificGateError,
...
import ppg_frailty.scientific_gate as scientific_gate
...
class MotionReferenceBoundaryTests(unittest.TestCase):
    ...
    def test_shared_source_gate_rejects_dirty_and_untracked_v2(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            repository = Path(directory).resolve()
            pipeline = repository / "final_v0" / "final_pipeline_v2"
            pipeline.mkdir(parents=True)
            pyproject = pipeline / "pyproject.toml"
            original = b"[project]\nname='gate-fixture'\nversion='0.0.0'\n"
            ...
            clean = git_source_state(repository, pipeline)
            self.assertTrue(
                require_clean_tracked_source_state(clean)["tracked_and_clean"]
            )

            pyproject.write_bytes(original + b"# dirty\n")
            with self.assertRaisesRegex(ScientificGateError, "clean_tracked"):
                require_clean_tracked_source_state(
                    git_source_state(repository, pipeline)
                )
            pyproject.write_bytes(original)
            (pipeline / "untracked.py").write_text("value = 1\n", encoding="utf-8")
            with self.assertRaisesRegex(ScientificGateError, "clean_tracked"):
                require_clean_tracked_source_state(
                    git_source_state(repository, pipeline)
                )

    def test_formal_entries_stop_at_scientific_gate_before_source_loading(
        self,
    ) -> None:
        closed = ScientificGateError(
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch.object(
            ...
            ) as internal_materialize, patch.object(
                motion_reference,
                "load_m2_external_manifest",
            ) as external_manifest:
                with self.assertRaisesRegex(ScientificGateError, "clean_tracked"):
                    run_formal_internal_motion_reference(
                        REPOSITORY_ROOT,
                ...
                with self.assertRaisesRegex(ScientificGateError, "clean_tracked"):
                    run_formal_ptt_motion_reference(
                        REPOSITORY_ROOT,
                        internal_evidence_path=root / "never.json",
                        expected_internal_evidence_sha256="0" * 64,
                        output_dir=root / "ptt",
                    )
            internal_materialize.assert_not_called()
            external_manifest.assert_not_called()

    def test_dependency_drift_closes_capture_and_toctou_checkpoint(self) -> None:
        clean_state = {
            "schema_version": SCIENTIFIC_SOURCE_GATE_SCHEMA,
            "v2_path": "final_v0/final_pipeline_v2",
            "git_head_commit": "1" * 40,
        ...
        gate_config = {
            "contract_id": "motion_fixture",
            "formal_model_id": "formal_local_supervised_motion_detector_v2",
            "config_relative_path": "configs/motion_detector_contract_v2.yaml",
            "config_sha256": "2" * 64,
            "schema_version":
            ...
            "source_snapshot_roots":
                ["src", "configs", "tools", "requirements", "locks"],
            "required_dependency_profile_ids": ["core", "deep"],
        ...
        with patch.object(
            ...
            scientific_gate,
            "_load_motion_gate_config",
            return_value=gate_config,
        ), patch.object(
            scientific_gate,
            "dependency_gate_report_for_profiles",
            side_effect=RuntimeError("deep drift"),
        ):
            with self.assertRaisesRegex(
                ScientificGateError,
                "exact_dependency_gate_closed",
            ):
                capture_motion_scientific_gate(REPOSITORY_ROOT)

        dependency = {
            "schema_version": "ppg_frailty.dependency_gate.v2",
            "operation": "motion_formal_reference",
            "config_id":
                "motion_fixture:formal_local_supervised_motion_detector_v2",
            "required_profile_ids": ["core", "deep"],
        ...
        baseline = {
            ...
            "source_tree_state": clean_state,
            "source_snapshot_sha256": "3" * 64,
            "dependency_gate": dependency,
            "dependency_gate_sha256": stable_payload_sha256(dependency),
        }
        changed = copy.deepcopy(baseline)
        changed["dependency_gate"]["profiles"][1]["drift"] = True
        changed["dependency_gate_sha256"] = stable_payload_sha256(
            changed["dependency_gate"]
        )
        with patch.object(
            scientific_gate,
            "capture_motion_scientific_gate",
            ...
            session = ScientificGateSession.establish(REPOSITORY_ROOT)
            with self.assertRaisesRegex(ScientificGateError, "changed_at_fit_pre"):
                session.checkpoint("fit_pre")
        with patch.object(
            scientific_gate,
            "capture_motion_scientific_gate",
            return_value=baseline,
        ):
            archived_session = ScientificGateSession.establish(REPOSITORY_ROOT)
        with patch.object(
            scientific_gate,
            "capture_motion_scientific_gate",
            return_value=changed,
        ):
            live_ptt_session = ScientificGateSession.establish(REPOSITORY_ROOT)
        with self.assertRaisesRegex(
            ScientificGateError,
            "differs_from_live_ptt_gate",
        ):
            require_archived_gate_matches_session(
                archived_session.evidence(),
                live_ptt_session,
        ...
        with patch.object(
            ...
            full_internal = ScientificGateSession.establish(REPOSITORY_ROOT)
            for phase in MOTION_INTERNAL_GATE_PHASES[1:]:
                full_internal.checkpoint(phase)
        full_evidence = full_internal.evidence()
        self.assertEqual(
            verify_scientific_gate_evidence(
                full_evidence,
        ...
        tampered = copy.deepcopy(full_evidence)
        tampered["checkpoints"][-1]["source_snapshot_sha256"] = "9" * 64
        unsigned = {
            key: value
            for key, value in tampered.items()
            if key != "scientific_gate_evidence_sha256"
        }
        tampered["scientific_gate_evidence_sha256"] = stable_payload_sha256(
            unsigned
        )
        self.assertIn(
            "scientific_gate_checkpoint_hash_drift",
            verify_scientific_gate_evidence(
                tampered,
                expected_phases=MOTION_INTERNAL_GATE_PHASES,
            ),
    ...
    def test_runner_contains_all_required_fit_and_predict_rechecks(self) -> None:
        internal = inspect.getsource(motion_runner._run_internal_motion_oof_impl)
        external = inspect.getsource(
            motion_runner._run_ptt_external_evaluation_impl
        )
        self.assertLess(
            internal.index("_fit_pre"),
            internal.index("fitted = fit_model"),
        )
        self.assertLess(
            internal.index("fitted = fit_model"),
            internal.index("_fit_post"),
        )
        self.assertLess(
            external.index("ptt_predict_pre"),
            external.index("probabilities = _probabilities"),
        )
        self.assertLess(
            external.index("probabilities = _probabilities"),
            external.index("ptt_predict_post"),
        )
