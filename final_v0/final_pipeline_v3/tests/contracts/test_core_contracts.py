...
class ConfigContractTests(unittest.TestCase):
    ...
    def test_all_formal_v2_configs_load_and_have_unique_ids(self) -> None:
        """正式V2配置通过唯一加载器 / Load every formal V2 config."""

        configs = [
            load_config(ROOT / "configs" / name) for name in FORMAL_CONFIG_NAMES
        ]
        self.assertEqual(len(configs), 5)
        self.assertEqual(len({config.config_id for config in configs}), 5)
        self.assertEqual(
    ...
    def test_outer_labels_are_never_visible(self) -> None:
        ...
        for name in FORMAL_CONFIG_NAMES:
