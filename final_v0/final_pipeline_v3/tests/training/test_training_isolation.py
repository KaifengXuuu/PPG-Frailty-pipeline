...
class TrainingIsolationTests(unittest.TestCase):
    ...
    def test_actual_reference_yaml_constructs_training_config(self) -> None:
        ...
        payload = yaml.safe_load(
            (pipeline_root / "configs" / "reference_static_line_a_v2.yaml").read_text(encoding="utf-8")
        )
    ...
    def test_inner_selection_is_not_executable_in_v2(self) -> None:
        """English: V2 rejects inner selection and early stopping at config load.

        中文：V2 在配置加载时拒绝 inner selection 与 early stopping。
        """

        with self.assertRaisesRegex(ValueError, "fixed_epoch only"):
            TrainingConfig(
