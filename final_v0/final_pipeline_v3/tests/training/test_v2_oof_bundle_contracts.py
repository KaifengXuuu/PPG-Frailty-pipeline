...
class BundleContracts(unittest.TestCase):
    def test_exact_environment_gate_precedes_deserialization_and_golden_is_stable(self) -> None:
        spec = ModelInputSpec(
            "raw",
            n_channels=2,
            n_classes=3,
        ...
        golden = {
            "x": np.random.default_rng(42).normal(size=(1, 2, 64)).astype(np.float32)
        }
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with patch("ppg_frailty.training.bundle.joblib.load") as deserialize:
...
class FinalRefitContracts(unittest.TestCase):
    def test_scientific_source_gate_rejects_untracked_or_dirty_v2(self) -> None:
        from ppg_frailty.experiment import _require_scientific_source_gate

        state = {
            "schema_version": "ppg_frailty.scientific_source_gate.v1",
            "v2_path": "final_v0/final_pipeline_v2",
            "git_head_commit": "0" * 40,
            "v2_pyproject_tracked": False,
            "status_entry_count": 1,
            "status_sha256": "0" * 64,
            "status_preview": ["?? final_v0/final_pipeline_v2/"],
            "tracked_and_clean": False,
        }
        with patch(
            "ppg_frailty.experiment._git_source_state",
            return_value=state,
        ):
            with self.assertRaisesRegex(
                RuntimeError,
                "clean_tracked_final_pipeline_v2",
            ):
                _require_scientific_source_gate(SimpleNamespace())
    ...
    def test_classical_executor_fits_exact_all_29_with_seed42_and_no_epoch(self) -> None:
        ...
        publication = experiment_module._canonical_final_bundle_materialization(
            ...
            SimpleNamespace(manifest_hash="manifest"),
            {
                ...
                "scientific_source_gate": source_gate,
                "scientific_source_gate_sha256":
                    stable_payload_sha256(source_gate),
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            attestation = loaded.directory / "final_refit_attestation.json"
            ...
            self.assertEqual(
                ...
                hashlib.sha256(
                    (loaded.directory / "manifest.json").read_bytes()
                ).hexdigest(),
            )
