...
def build_signal_views(
    ...
    if gravity_profile not in {"quaternion_error_state_ekf", "low_pass_0p3hz"}:
    ...
    if (
        ...
        raise ValueError("resolved signal.imu parameters differ from the V2 canonical profile")
    expected_imu_keys = {
    ...
    if (
        ...
        raise ValueError("resolved signal.imu auxiliary contract differs from V2")
    ...
    imu = preprocess_imu(
        ...
        gravity_method=(
            ...
            if gravity_profile == "quaternion_error_state_ekf" else "lpf_0p3"
        ),
        timestamps_s=timestamps,
    )
