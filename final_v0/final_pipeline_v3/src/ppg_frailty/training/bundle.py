...
ONNX_WINNER_OPSET_VERSION = 17
ONNX_WINNER_ABSOLUTE_TOLERANCE = 1e-5
ONNX_WINNER_RELATIVE_TOLERANCE = 1e-5
GENERIC_BUNDLE_KIND = "generic_research"
TRUSTED_FINAL_REFIT_BUNDLE_KIND = "trusted_final_refit_v2"
FINAL_REFIT_ATTESTATION_SCHEMA = "ppg_frailty.final_refit_publication_attestation.v2"
_TRUSTED_FINAL_IDENTITY_FIELDS = frozenset(
    {
        ...
        "scientific_source_gate_sha256", "resolved_model_config_hash",
...
def current_runtime_environment() -> dict[str, str]:
    """Return exact executable/runtime versions used by a V2 bundle."""
...
def assert_runtime_environment_exact(environment: Mapping[str, Any]) -> None:
    """Fail before executable deserialisation when any exact version differs."""

    expected = current_runtime_environment()
    missing = sorted(set(REQUIRED_RUNTIME_ENVIRONMENT_KEYS) - set(environment))
    if missing:
        raise ValueError(f"bundle environment is missing exact runtime fields: {missing}")
    mismatches = {
        name: {"bundle": str(environment[name]), "runtime": expected[name]}
        for name in REQUIRED_RUNTIME_ENVIRONMENT_KEYS
        if str(environment[name]) != expected[name]
    }
    if mismatches:
        raise RuntimeError(f"bundle runtime environment mismatch: {mismatches}")
    ...
    *,
    ...
    final_identity = {
        ...
        "scientific_source_gate_sha256":
            materialization.metadata["scientific_source_gate_sha256"],
...
@dataclass(frozen=True)
class OnnxWinnerGateSpec:
    """Deferred winner gate at the frozen model-input-to-probability boundary."""

    winner_config_hash: str
    input_names: tuple[str, ...]
    output_name: str = "probabilities"
    opset_version: int = ONNX_WINNER_OPSET_VERSION
    absolute_tolerance: float = ONNX_WINNER_ABSOLUTE_TOLERANCE
    relative_tolerance: float = ONNX_WINNER_RELATIVE_TOLERANCE
    boundary: str = "model_input_to_probabilities"

    def __post_init__(self) -> None:
        if len(self.winner_config_hash) != 64:
            raise ValueError("winner_config_hash must be SHA-256")
        if not self.input_names or len(self.input_names) != len(set(self.input_names)):
            raise ValueError("ONNX input_names must be non-empty and unique")
        if self.boundary != "model_input_to_probabilities":
            raise ValueError("V2 hardware-deferred ONNX gate covers model input to probabilities only")
        if (
            self.opset_version != ONNX_WINNER_OPSET_VERSION
            or self.absolute_tolerance != ONNX_WINNER_ABSOLUTE_TOLERANCE
            or self.relative_tolerance != ONNX_WINNER_RELATIVE_TOLERANCE
        ):
            raise ValueError("ONNX winner opset/tolerances must match frozen V2 policy")


def assert_onnx_probability_parity(
    python_probabilities: np.ndarray,
    onnx_probabilities: np.ndarray,
    spec: OnnxWinnerGateSpec,
) -> dict[str, float | str]:
    """Validate an exporter/runtime result without selecting or exporting a winner."""
    ...
    if reference.ndim != 2 or reference.shape != candidate.shape or reference.shape[1] < 2:
        raise ValueError("Python and ONNX outputs must share [sample,class] shape")
    for name, values in (("python", reference), ("onnx", candidate)):
    ...
    if not np.allclose(
        reference,
        candidate,
        rtol=float(spec.relative_tolerance),
        atol=float(spec.absolute_tolerance),
    ):
        raise RuntimeError("winner ONNX probability parity gate failed")
    return {
...
def validate_bundle_metadata(metadata: Mapping[str, Any]) -> dict[str, Any]:
    ...
    assert_runtime_environment_exact(normalized["environment"])
    return normalized
...
def _validate_trusted_final_refit_manifest(
    ...
    for name in (
        ...
        "source_snapshot_hash", "scientific_source_gate_sha256",
    ...
    source_gate = metadata.get("scientific_source_gate")
    if (
        not isinstance(source_gate, Mapping)
        or source_gate.get("tracked_and_clean") is not True
        or stable_payload_sha256(dict(source_gate))
        != normalized["scientific_source_gate_sha256"]
        or manifest.get("config_hash") != normalized["config_hash"]
        ...
        raise ValueError("trusted final-refit manifest/source identity mismatch")
    return normalized


def _validate_trusted_final_refit_attestation(
    directory: Path,
    manifest: Mapping[str, Any],
) -> dict[str, Any]:
    """Verify the attestation committed atomically inside a trusted bundle."""

    identity = _validate_trusted_final_refit_manifest(manifest)
    manifest_path = directory / "manifest.json"
    attestation_path = directory / "final_refit_attestation.json"
    if not attestation_path.is_file():
        raise ValueError("trusted final-refit attestation missing")
    ...
    if (
        ...
        or attestation.get("bundle_manifest_relative_path") != "manifest.json"
        or attestation.get("bundle_manifest_sha256")
        != sha256_file(manifest_path)
        or attestation.get("bundle_manifest_bytes")
        != manifest_path.stat().st_size
        or attestation.get("selection_record_file_sha256")
        != identity["selection_record_file_sha256"]
        or attestation.get("manual_selection_sha256")
        != identity["manual_selection_hash"]
        or attestation.get("config_hash") != identity["config_hash"]
        or attestation.get("model_id") != identity["model_id"]
        or attestation.get("execution_hash") != identity["execution_hash"]
        or attestation.get("dataset_hash") != identity["dataset_hash"]
        or attestation.get("bundle_materialization_hash")
        != identity["bundle_materialization_hash"]
        or attestation.get("source_snapshot_sha256")
        != identity["source_snapshot_hash"]
        or attestation.get("scientific_source_gate_sha256")
        != identity["scientific_source_gate_sha256"]
        or attestation.get("participant_count") != 29
        or attestation.get("participant_ids") != identity["participant_ids"]
        or attestation.get("training_seeds") != identity["training_seeds"]
        or attestation.get("attestation_payload_sha256")
        != stable_payload_sha256(payload)
    ):
        raise ValueError("trusted final-refit attestation identity drift")
    return attestation


def _write_trusted_final_refit_attestation(
    directory: Path,
    manifest: Mapping[str, Any],
) -> None:
    """Write the source-bound attestation before the directory commit point."""

    identity = _validate_trusted_final_refit_manifest(manifest)
    manifest_path = directory / "manifest.json"
    attestation = {
        "schema_version": FINAL_REFIT_ATTESTATION_SCHEMA,
        "pipeline_generation": "final_pipeline_v2",
        "status": "verified_trusted_final_refit_publication",
        "bundle_kind": TRUSTED_FINAL_REFIT_BUNDLE_KIND,
        "bundle_manifest_relative_path": "manifest.json",
        "bundle_manifest_sha256": sha256_file(manifest_path),
        "bundle_manifest_bytes": manifest_path.stat().st_size,
        "selection_record_file_sha256":
            identity["selection_record_file_sha256"],
        "manual_selection_sha256": identity["manual_selection_hash"],
        "config_hash": identity["config_hash"],
        "model_id": identity["model_id"],
        "execution_hash": identity["execution_hash"],
        "dataset_hash": identity["dataset_hash"],
        "bundle_materialization_hash":
            identity["bundle_materialization_hash"],
        "source_snapshot_sha256": identity["source_snapshot_hash"],
        "scientific_source_gate_sha256":
            identity["scientific_source_gate_sha256"],
        "participant_count": identity["participant_count"],
        "participant_ids": identity["participant_ids"],
        "training_seeds": identity["training_seeds"],
    }
    attestation["attestation_payload_sha256"] = stable_payload_sha256(
        attestation
    )
    _atomic_json(directory / "final_refit_attestation.json", attestation)
    _validate_trusted_final_refit_attestation(directory, manifest)
...
def _save_bundle_impl(
    ...
    *,
    ...
    if (
        bundle_kind == TRUSTED_FINAL_REFIT_BUNDLE_KIND
        and prepublish_guard is None
    ):
        raise ValueError("trusted final-refit prepublish guard missing")
    if bundle_kind == GENERIC_BUNDLE_KIND and prepublish_guard is not None:
        raise ValueError("generic bundle cannot accept formal prepublish guard")
    ...
    try:
        ...
        if bundle_kind == TRUSTED_FINAL_REFIT_BUNDLE_KIND:
            ...
            _write_trusted_final_refit_attestation(staging, manifest)
...
def load_bundle(
    ...
    *,
    ...
    if bundle_kind == GENERIC_BUNDLE_KIND:
        ...
        _validate_trusted_final_refit_manifest(manifest)
        _validate_trusted_final_refit_attestation(target, manifest)
    ...
    if bundle_kind == TRUSTED_FINAL_REFIT_BUNDLE_KIND:
        expected_names.add("final_refit_attestation.json")
