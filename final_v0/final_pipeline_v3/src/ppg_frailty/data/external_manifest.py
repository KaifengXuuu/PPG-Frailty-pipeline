...
PTT_IMU_UNIT_EVIDENCE_RELATIVE_PATH = Path(
    "final_v0/final_pipeline_v2/manifests/ptt_imu_unit_evidence_v2_036.json"
)
PTT_IMU_UNIT_EVIDENCE_SHA256 = (
    "c03669029f9e58e53ae24e5e9208aa47541b93d8fcc0b9dea2f215165dc70ee5"
)
...
PTT_IMU_UNIT_CONFLICT_PROVENANCE = {
    ...
    "historical_code_transform": {
        ...
        "acceleration": (
            "legacy multiply source values by 9.80665; prohibited for V2-036 PTT"
        ),
    ...
    "formal_action": (
        "require exact V2-036 artifact path/hash before conversion, EKF, "
...
class PttSynchronizedSignals:
    ...
    def validate(self) -> None:
        ...
        expected_hashes = {
            "source_values_sha256": _array_identity_sha256(
                source, self.source_channel_schema
            ),
            "output_values_sha256": _array_identity_sha256(
                matrix, self.channel_schema
            ),
            "source_channel_schema_sha256": _payload_identity_sha256(
                list(self.source_channel_schema)
            ),
            "target_channel_schema_sha256": _payload_identity_sha256(
                list(self.channel_schema)
            ),
            ...
            "mapping_sha256": _payload_identity_sha256(
                dict(self.mapping_provenance)
            ),
        }
...
def select_ptt_distal_red_ir(channels: Mapping[str, np.ndarray]) -> np.ndarray:
    ...
    missing = [
        source
        for source in PTT_DISTAL_CHANNEL_MAPPING.values()
        if source not in channels
    ]
    if missing:
        raise ValueError("PTT distal mapping missing channels: " + ",".join(missing))
    red = np.asarray(
        channels[PTT_DISTAL_CHANNEL_MAPPING["RED"]], dtype=np.float64
    ).reshape(-1)
    infrared = np.asarray(
        channels[PTT_DISTAL_CHANNEL_MAPPING["IR"]], dtype=np.float64
    ).reshape(-1)
...
def _adapt_m2_row(raw: Mapping[str, str]) -> ExternalRecord:
    ...
    wavelength_status = (
        ...
        if raw["dataset_id"] == PTT_DATASET_ID
        else str(raw["ppg_wavelength_status"])
    )
    return ExternalRecord(
        ...
        ppg_wavelength_status=wavelength_status,
...
def audit_external_manifest(
    ...
    return {
        ...
        "ptt_imu_unit_conflict_provenance": dict(
            PTT_IMU_UNIT_CONFLICT_PROVENANCE
        ),
        "independence_claim": INDEPENDENCE_CLAIM,
    }
