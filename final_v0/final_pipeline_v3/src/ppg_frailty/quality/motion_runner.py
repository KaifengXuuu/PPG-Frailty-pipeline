...
def _run_internal_motion_oof_impl(
    ...
    *,
    ...
    formal_scientific_gate: ScientificGateSession | None = None,
    ...
    if execution_mode == "formal":
        ...
        if not isinstance(formal_scientific_gate, ScientificGateSession):
            raise ValueError(
                "formal internal motion scientific source/dependency gate is required"
            )
        ...
        or formal_scientific_gate is not None
    ...
    for job in sorted(jobs, key=lambda item: (item.repeat_index, item.fold_index)):
        ...
        fit_pre_checkpoint = (
            formal_scientific_gate.checkpoint(
                f"fold_{job.repeat_index}_{job.fold_index}_fit_pre"
            )
            if execution_mode == "formal"
            else None
        )
        ...
        fit_post_checkpoint = (
            formal_scientific_gate.checkpoint(
                f"fold_{job.repeat_index}_{job.fold_index}_fit_post"
            )
            if execution_mode == "formal"
            else None
        )
        ...
        if execution_mode == "formal":
            cell_row.update(
                {
                    "scientific_gate_fit_pre_checkpoint_sha256":
                        fit_pre_checkpoint["checkpoint_sha256"],
                    "scientific_gate_fit_post_checkpoint_sha256":
                        fit_post_checkpoint["checkpoint_sha256"],
                }
            )
    ...
    final_fit_pre_checkpoint = (
        formal_scientific_gate.checkpoint("final_fit_pre")
        if execution_mode == "formal"
        else None
    )
    final_fitted = fit_model(records, final_context)
    final_fit_post_checkpoint = (
        formal_scientific_gate.checkpoint("final_fit_post")
        if execution_mode == "formal"
        else None
    )
    ...
    if formal:
        scientific_gate_evidence = formal_scientific_gate.evidence()
        scientific_gate_reasons = verify_scientific_gate_evidence(
            scientific_gate_evidence,
            expected_phases=MOTION_INTERNAL_GATE_PHASES,
        )
        if scientific_gate_reasons:
            raise RuntimeError(
                "formal internal scientific gate evidence rejected: "
                + ";".join(scientific_gate_reasons)
            )
        ...
        evidence["scientific_execution_gate"] = scientific_gate_evidence
        evidence["final_model"].update(
            {
                "scientific_gate_fit_pre_checkpoint_sha256":
                    final_fit_pre_checkpoint["checkpoint_sha256"],
                "scientific_gate_fit_post_checkpoint_sha256":
                    final_fit_post_checkpoint["checkpoint_sha256"],
            }
        )
...
def _run_ptt_external_evaluation_impl(
    ...
    *,
    ...
    formal_scientific_gate: ScientificGateSession,
    _canonical_entry_token: object | None = None,
    ...
    if not isinstance(formal_scientific_gate, ScientificGateSession):
        raise ValueError("formal PTT scientific source/dependency gate is required")
    if (
    ...
    require_archived_gate_matches_session(
        evidence.get("scientific_execution_gate"),
        formal_scientific_gate,
        expected_phases=MOTION_INTERNAL_GATE_PHASES,
    )
    ...
    predict_pre_checkpoint = formal_scientific_gate.checkpoint("ptt_predict_pre")
    ...
    predict_post_checkpoint = formal_scientific_gate.checkpoint("ptt_predict_post")
    ...
    scientific_gate_reasons = verify_scientific_gate_evidence(
        scientific_gate_evidence,
        expected_phases=MOTION_PTT_GATE_PHASES,
    )
    if scientific_gate_reasons:
        raise RuntimeError(
            "formal PTT scientific gate evidence rejected: "
            + ";".join(scientific_gate_reasons)
        )
    report = {
        "schema_version": MOTION_EXTERNAL_REPORT_SCHEMA,
        ...
        "scientific_execution_gate": scientific_gate_evidence,
        "scientific_gate_predict_pre_checkpoint_sha256":
            predict_pre_checkpoint["checkpoint_sha256"],
        "scientific_gate_predict_post_checkpoint_sha256":
            predict_post_checkpoint["checkpoint_sha256"],
    }
    root = Path(output_dir).resolve()
