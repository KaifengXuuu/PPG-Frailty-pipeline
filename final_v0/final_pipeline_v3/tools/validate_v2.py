...
def _required_paths() -> str:
    ...
    required = [
        ...
        "configs/reference_static_feature_vector_line_b_v2.yaml",
        "configs/reference_static_feature_matrix_v2.yaml",
        "configs/reference_static_fusion_v2.yaml",
        "configs/reference_static_line_a_v2.yaml",
...
def _profiles_and_locks() -> str:
    """验证确认/延期档案及六 profile / Validate decision and dependency profiles."""
    ...
    if decision["confirmed_defaults"]["balance"]["default_line"]["line_id"] != "line_a_equal_files":
        raise AssertionError("Line A must remain the V2 default")
    if len(profiles["profiles"]) != 6 or len(locks["profiles"]) != 6:
        raise AssertionError("exactly six dependency profiles are required")
    ...
    return "decision_profile=1,dependency_profiles=6,ablation_profiles=19"
...
def main(argv: list[str] | None = None) -> int:
    ...
    checks = [
        ...
        Check("profiles_and_locks", _profiles_and_locks),
