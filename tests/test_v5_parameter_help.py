...
def test_all_registered_core_control_paths_have_visible_english_help(all_core_specs):
    ...
    for path, spec in all_core_specs.items():
        ...
        assert isinstance(help_text, str) and re.search(r'[A-Za-z]', help_text), path
        assert not re.search(r'[\u4e00-\u9fff]', help_text), path
        assert not help_text.startswith(('Plan field ', 'CLI parameter ')), path
...
def test_explicit_description_is_displayed_for_additional_parameters():
    spec = {'path': 'extension.depth', 'kind': 'integer', 'value': 3,
            'min': 0, 'max': 20, 'step': 1, 'description': 'Number of repeated processing passes in the extension module.'}
    widget = _parameter_control(spec)
...
def test_advanced_cli_and_plan_controls_also_display_parameter_help():
    ...
    for spec, namespace in specifications:
        ...
        explanation = next(item for item in components(widget)
                           if getattr(item, 'className', None) == 'parameter-help')
        assert re.search(r'[A-Za-z]', explanation.children), (namespace, spec['path'])
        assert not re.search(r'[\u4e00-\u9fff]', explanation.children), (namespace, spec['path'])
