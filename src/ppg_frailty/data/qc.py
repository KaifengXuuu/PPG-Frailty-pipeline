"""Fail-closed recording-level quality control.

QC determines whether a recording can enter preprocessing; it is not SQI or a class label.
Callers provide thresholds explicitly. Missing channels are not zero-filled and parsing errors are not hidden.
...
class QCThresholds:
    """Fully explicit recording QC thresholds."""
    ...
    def validate(self, *, timestamps_present: bool | None = None) -> None:
        """Reject incomplete or invalid thresholds."""
...
@dataclass(frozen=True)
class QCAssessment:
    """Serializable recording QC result."""
...
def parse_failure_assessment(message: str) -> QCAssessment:
    """Represent a parse failure without dropping it."""
...
def assess_numeric_record(
    ...
    *,
    ...
    """Assess one parsed numeric recording."""
    ...
    # Audit present columns even when the count is wrong; never invent missing columns.
    ...
    for index, channel in enumerate(channels[:matrix.shape[1]]):
        if channel not in CANONICAL_CHANNEL_SCHEMA:
            # Unknown columns already have missing_required_channel diagnostics. A missing
            # threshold key must not prevent visible QC results for the whole recording.
