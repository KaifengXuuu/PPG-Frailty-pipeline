"""External heartbeat-motion data contract.

This module adapts only the reviewed M2 external_record_manifest.csv, preserving
original PTT source status and explicitly recording the project distal-channel mapping:
``pleth_1=RED`` and ``pleth_2=IR``. The PhysioNet v1.0.0 documentation identifies
pleth_1 as distal red and pleth_2 as distal infrared; this pair was confirmed for the project.
Earlier M2 unresolved/conflict states remain as historical provenance.
...
M2_EXTERNAL_RELATIVE_PATH = Path("assets/authority/manifests/external_record_manifest.csv")
...
PTT_IMU_UNIT_EVIDENCE_RELATIVE_PATH = Path("manifests/ptt_imu_unit_evidence_v2_036.json")
...
class ExternalManifestError(ValueError):
    """Aggregate external-contract failures."""
...
class ExternalRecord:
    """One source-semantics-preserving record."""
...
def _strict_json(value: object) -> str:
    """Render a stable JSON-valued CSV cell."""
...
def _parse_source_files(value: str) -> tuple[str, ...]:
    """Parse and reject empty source lists."""
...
def _evaluation_role(raw: Mapping[str, str]) -> str:
    """Register use without test claims."""
...
def _adapt_m2_row(raw: Mapping[str, str]) -> ExternalRecord:
    """Map one M2 row into the V2 contract."""
...
def _validate_external_record(row: ExternalRecord) -> None:
    """Validate one row and forbid inference."""
    ...
    # PTT uses one canonical CSV hash; the authoritative SIM table stores a
    # complete file-path -> SHA-256 snapshot JSON. Preserve both encodings losslessly.
...
def _record_to_csv(row: ExternalRecord) -> dict[str, str]:
    """Encode one typed external record."""
...
def _record_from_csv(raw: Mapping[str, str]) -> ExternalRecord:
    """Restore one materialized external record."""
...
def load_m2_external_manifest(source_csv: str | Path) -> list[ExternalRecord]:
    """Load the authoritative M2 external table."""
...
def load_external_manifest(path: str | Path) -> list[ExternalRecord]:
    """Load the materialized V2 external manifest."""
...
def _checked_target(path: str | Path, *, output_root: str | Path) -> Path:
    """Restrict every generated write."""
...
def write_external_manifest_csv(
    ...
    *,
    ...
    """Atomically write the external manifest."""
...
def build_external_manifest(
    ...
    """Build the external manifest from the packaged M2 snapshot."""

    source = Path(source_csv).resolve()
    pipeline_root = Path(__file__).resolve().parents[3]
    expected = pipeline_resource(M2_EXTERNAL_RELATIVE_PATH)
    if source != expected:
