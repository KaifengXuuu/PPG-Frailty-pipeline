"""Unified engineering and deep-learning WindowPlan.

All window behavior is configured: duration, hop, end alignment, short-record handling,
tail padding and caps. WindowPlan plans indexes only; extract_window reads values,
so amplitude-preserving signal views cannot be modified inadvertently.
...
class ShortRecordError(ValueError):
    """Explicit short-record rejection."""


@dataclass(frozen=True)
class WindowSlice:
    """One explicitly masked time window."""
...
@dataclass(frozen=True)
class WindowPlan:
    """Window planning with no hidden defaults."""
    ...
    def _validate(self) -> None:
        """Validate every explicit policy."""
    ...
    def plan(self, n_samples: int, fs: float) -> tuple[WindowSlice, ...]:
        """Generate deterministic indices and masks."""
        ...
        if self.end_alignment in {
            ...
            # Align backward from the final complete window, then restore chronological order.
...
def _uniform_indices(length: int, count: int) -> tuple[int, ...]:
    """Uniformly select ordered positions."""
...
def extract_window(
    ...
    *,
    ...
    """Copy and right-pad one planned window."""
