"""Machine contracts for the data layer.

Central definitions for manifest CSV encoding, QC reason codes and materialized fold rows.
Compound CSV fields use strict JSON; reading restores typed values and validates immediately.
...
def canonicalize_role_family(role: str) -> str:
    """Map one file role to the physiological B/R/S/W family.

    ``R1``..``R4`` are file instances of one Relax/Recovery role. The numeric
    suffix never creates a new physiological role and no cross-file temporal
    ...
    if normalized in ROLE_FAMILIES:
        return normalized
    try:
        return ROLE_FAMILY_BY_ROLE[normalized]
    except KeyError as exc:
        raise ValueError(f"unregistered role: {role}") from exc

def is_default_classifier_role(role: str) -> bool:
    """Return whether a file enters the SQI-off default classifier path."""

    return canonicalize_role_family(role) in DEFAULT_CLASSIFIER_ROLE_FAMILIES

class QCStatus(str, Enum):
    """Recording-level QC disposition."""
...
class QCReason(str, Enum):
    """Contract-required QC reason codes."""
...
def validate_manifest_row(row: ManifestRow) -> None:
    """Validate one canonical recording row."""
    ...
    if row.class_name_provenance_alias != CLASS_NAME_PROVENANCE_ALIASES[row.class_id]:
        raise ValueError("class_name provenance alias mismatch")
    if row.class_source != CLASS_SOURCE_BY_ID[row.class_id]:
        raise ValueError("class_source does not match the frozen label origin")
    if row.class_id in {0, 1} and not row.label_record_id.strip():
        raise ValueError("frailty-status labels require a non-empty label_record_id")
...
def manifest_row_to_csv(row: ManifestRow) -> dict[str, str]:
    """Encode a typed row for portable CSV."""
    ...
    return {
        ...
        "class_name_provenance_alias": row.class_name_provenance_alias,
...
def manifest_row_from_csv(raw: Mapping[str, str]) -> ManifestRow:
    """Restore and validate a typed row."""
    ...
    row = ManifestRow(
        ...
        class_name_provenance_alias=str(raw["class_name_provenance_alias"]),
        ...
        channel_schema=tuple(
            str(value) for value in _parse_json_field(raw["channel_schema"], field_name="channel_schema")
        ),
        channel_units={
            str(key): str(value)
            for key, value in dict(_parse_json_field(raw["channel_units"], field_name="channel_units")).items()
        },
        synchrony_status=str(raw["synchrony_status"]),
        reference_available=reference_text == "true",
        qc_status=str(raw["qc_status"]),
        qc_reasons=tuple(str(value) for value in _parse_json_field(raw["qc_reasons"], field_name="qc_reasons")),
        manifest_version=str(raw["manifest_version"]),
    )
    validate_manifest_row(row)
    return row
def fold_assignment_to_csv(row: FoldAssignment) -> dict[str, str]:
    """Encode one materialized fold assignment."""
...
def fold_assignment_from_csv(raw: Mapping[str, str]) -> FoldAssignment:
    """Restore one materialized fold assignment."""
    ...
    row = FoldAssignment(
        ...
        source_registry_payload_sha256=str(raw["source_registry_payload_sha256"]),
    ...
    # Reuse every encoder consistency check rather than maintaining separate read/write rules.
