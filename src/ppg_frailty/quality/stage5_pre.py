...
from ..paths import pipeline_resource, resolve_repository_root
...
def load_motion_peak_plan(path: str | Path) -> StudyPlan:
    ...
    if schema == STAGE5_SCHEMA and data.get('execution', {}).get('motion', True):
        ...
        if comparison and comparison.get('enabled', True):
...
def _ptt_records(repository_root: Path) -> tuple[ExternalRecord, ...]:
    records = tuple((row for row in load_m2_external_manifest(pipeline_resource(M2_EXTERNAL_RELATIVE_PATH)) if row.dataset_id == PTT_DATASET_ID))
...
def run_ptt_denoiser_benchmark(repository_root: str | Path, *, reducer_ids: Sequence[str], segment_s: float, validation: Mapping[str, Any],
    ...
    unit_evidence = load_ptt_imu_unit_evidence(pipeline_resource(PTT_IMU_UNIT_EVIDENCE_RELATIVE_PATH), expected_sha256=PTT_IMU_UNIT_EVIDENCE_SHA256, expected_records=records)
...
def run_motion_peak_study(plan_path: str | Path, *, pipeline_root: str | Path, output_root: str | Path, resume: str | Path | None = None, progress_sink: ProgressSink | None = None,
    ...
    comparison = plan.payload.get('motion_model_comparison', {})
    comparison_enabled = motion_enabled and bool(comparison) and bool(comparison.get('enabled', True))
    ...
    progress_total = (4 * motion_enabled + comparison_enabled + denoiser_enabled) if plan.schema_version == STAGE5_SCHEMA else 1
    progress_current = 0
    progress(
    ...
    repository = resolve_repository_root(pipeline)
        ...
        if motion_enabled:
            ...
            if not external_complete:
                ...
                external = run_formal_ptt_motion_reference(repository, internal_evidence_path=evidence_path, expected_internal_evidence_sha256=evidence_sha,
                                                           output_dir=external_dir, unit_evidence_path=pipeline_resource(PTT_IMU_UNIT_EVIDENCE_RELATIVE_PATH),
            ...
            if not ptt_training_complete:
                ...
                subprogress(0, 1, 'load PTT source manifest')
                ptt_training = run_formal_ptt_motion_training_ablation(repository, output_dir=ptt_training_dir, unit_evidence_path=pipeline_resource(PTT_IMU_UNIT_EVIDENCE_RELATIVE_PATH),
                                                                       expected_unit_evidence_sha256=PTT_IMU_UNIT_EVIDENCE_SHA256, progress_callback=subprogress,
                                                                       training_device=str(plan.payload['motion_detector']['training_device']))
            ...
            if comparison_enabled:
                ...
                comparison_manifest = _write_motion_model_comparison_package(pipeline_root=pipeline, output_dir=comparison_dir,
                                                                             legacy_study_relative=str(comparison['legacy_frailty29_stage5_study']),
