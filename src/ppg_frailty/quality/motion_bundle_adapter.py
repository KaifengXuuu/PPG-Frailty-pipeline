...
from ..signal.motion_imu import MOTION_IMU_CHANNEL_SCHEMA, MOTION_IMU_CHANNEL_UNITS, MotionImuResult
...
_CUDA_DEVICE = re.compile('^cuda(?::[0-9]+)?$')
_SHA256 = re.compile('^[0-9a-f]{64}$')

# A loaded detector binds model bytes, threshold evidence, split, and training roster.
...
class ReusedMotionDetectorConfig:
    ...
    def validate(self) -> None:
        ...
        if self.evidence_path is not None and (not isinstance(self.evidence_path, Path)):
            raise TypeError('motion detector evidence_path must resolve to pathlib.Path')
        if self.enabled and self.expected_evidence_sha256 is None:
            raise ValueError('enabled reused motion detector requires expected_evidence_sha256')
        if self.expected_evidence_sha256 is not None and (not isinstance(self.expected_evidence_sha256, str)
                                                          or not _SHA256.fullmatch(self.expected_evidence_sha256)):
            raise ValueError('motion detector expected_evidence_sha256 must be lowercase SHA-256')
        if not isinstance(self.device, str) or (self.device != 'cpu' and (not _CUDA_DEVICE.fullmatch(self.device))):
        ...
        if self.window_probability_aggregation not in {'median', 'native_windows_file_median_diagnostics_only'}:
            raise ValueError('motion output must retain native windows and diagnostic file median')
        if not isinstance(self.threshold_source, str) or self.threshold_source != 'bundle_frozen':
            raise ValueError('reused motion detector requires bundle_frozen threshold')
        if self.reuse_scope not in {
                'all29_smoke_or_final_only', 'all29_frozen_in_sample_auxiliary', 'matching_outer_fold_or_all29_final'
        }:
...
def resolve_reused_motion_detector_config(payload: Mapping[str, Any] | None = None) -> ReusedMotionDetectorConfig:
    values = {} if payload is None else dict(payload)
    allowed = {
        'enabled', 'evidence_path', 'expected_evidence_sha256', 'device', 'batch_size',
        'window_probability_aggregation', 'threshold_source', 'reuse_scope', 'expected_split_registry_sha256'
    }
    unknown = sorted(set(values) - allowed)
    if unknown:
        raise ValueError('unknown reused motion detector configuration fields: ' + ', '.join(unknown))
...
class MotionRecordDecision:
    ...
    def validate(self) -> None:
        ...
        elif self.record_probability is None or not np.isfinite(float(self.record_probability)) or (
                not 0.0 <= float(self.record_probability) <= 1.0) or (self.window_count < 1):
...
class MotionWindowSeries:
    ...
    def validate(self) -> None:
        ...
        for row in self.decisions:
            if (row.start_sample_400 < 0 or row.stop_sample_400 - row.start_sample_400 != MOTION_WINDOW_SAMPLES
                    or row.centre_sample_400 != row.start_sample_400 + MOTION_WINDOW_SAMPLES // 2
                    or (previous is not None and row.start_sample_400 - previous != MOTION_HOP_SAMPLES)):
            ...
            if row.threshold != self.threshold or (row.motion_state == 'unavailable' and row.probability is not None
                                                   ) or (row.motion_state != 'unavailable' and
                                                         (row.probability is None or not np.isfinite(row.probability) or
                                                          (not 0.0 <= row.probability <= 1.0) or
                                                          ((row.motion_state == 'low') !=
...
def _resolve_model_path(evidence_path: Path, value: object, *, directory: Path) -> Path:
    """Read a bundle-local weight; archived absolute paths are provenance only."""
    declared = Path(str(value)).expanduser()
    relative = directory / declared.name if declared.is_absolute() else declared
...
def _validated_frozen_threshold(evidence: Mapping[str, Any], training_participant_ids: tuple[str,
                                                                                             ...]) -> tuple[float, str]:
    payload = evidence.get('final_threshold')
    if not isinstance(payload, Mapping):
        raise ValueError('motion bundle final_threshold is missing')
    if payload.get('schema_version') != MOTION_DEPLOYMENT_THRESHOLD_SCHEMA or payload.get(
            'score_origin') != MOTION_DEPLOYMENT_THRESHOLD_SCORE_ORIGIN or payload.get(
                'fit_scope') != MOTION_DEPLOYMENT_THRESHOLD_FIT_SCOPE:
        raise ValueError('motion bundle threshold is not frozen from internal OOF')
    participant_ids = tuple(sorted((str(value) for value in payload.get('participant_ids', ()))))
...
def _validate_matching_split_rosters(evidence: Mapping[str, Any], evidence_path: Path, *, repeat_index: int,
                                     fold_index: int, train_roster: tuple[str, ...], oof_roster: tuple[str, ...],
                                     expected_sha256: str) -> Path:
    declared = Path(str(evidence.get('split_registry_csv_path', ''))).expanduser()
    relative = Path(declared.name) if declared.is_absolute() else declared
    split_path = (evidence_path.parent / relative).resolve()
    if not split_path.is_file():
        # The distributed split has identical bytes; retain the existing hash
        # and participant-roster checks before reusing any matching-fold model.
        split_path = pipeline_resource(Path('splits') / declared.name)
    ...
    all_participants = tuple(
        sorted({str(row['participant_id'])
                for row in rows if int(row['repeat_index']) == repeat_index}))
    expected_oof = tuple(
        sorted((str(row['participant_id']) for row in rows
                if int(row['repeat_index']) == repeat_index and int(row['fold_index']) == fold_index)))
...
def load_reused_motion_detector(
    config: ReusedMotionDetectorConfig, *, outer_train_participant_ids: tuple[str, ...] = (),
    outer_oof_participant_ids: tuple[str, ...] = ()
) -> LoadedReusedMotionDetector:
    config.validate()
    if not config.enabled or config.evidence_path is None or config.expected_evidence_sha256 is None:
        raise ValueError('cannot load a disabled reused motion detector')
    evidence_path = config.evidence_path.expanduser().resolve()
    evidence, evidence_sha256 = load_motion_internal_evidence(evidence_path,
    ...
    if evidence.get('execution_status') != 'completed_formal_not_smoke' or evidence.get(
            'scientific_scope') != 'frailty29_single_sgkf5_oof' or evidence.get(
    ...
    if not isinstance(trained_ekf_config_sha256, str) or not _SHA256.fullmatch(trained_ekf_config_sha256):
    ...
    if (config.expected_split_registry_sha256 is not None
    ...
    use_matching_fold = formal_outer_oof and config.reuse_scope == 'matching_outer_fold_or_all29_final'
    if formal_outer_oof and config.reuse_scope == 'all29_smoke_or_final_only':
        raise ValueError('all29_smoke_or_final_only cannot be used inside outer OOF routing')
    if use_matching_fold:
        ...
        for raw_cell in evidence.get('cell_evidence', ()):
            ...
            if not isinstance(threshold_payload, Mapping):
                continue
            threshold_roster = tuple(sorted(map(str, threshold_payload.get('participant_ids', ()))))
        ...
        split_path = _validate_matching_split_rosters(evidence, evidence_path, repeat_index=int(cell['repeat_index']),
                                                      fold_index=int(cell['fold_index']), train_roster=train_roster,
                                                      oof_roster=oof_roster, expected_sha256=str(evidence_split_sha256))
        ...
        model_directory = Path(f"repeat_{cell['repeat_index']}") / f"fold_{cell['fold_index']}"
    else:
        model = evidence.get('final_model')
        if not isinstance(model, Mapping):
            raise ValueError('motion bundle final_model is missing')
        final_training_roster = tuple(sorted((str(value) for value in model.get('training_participant_ids', ()))))
        if len(final_training_roster) != MOTION_PARTICIPANT_COUNT or len(
                set(final_training_roster)) != len(final_training_roster):
            raise ValueError('reused final motion model must contain the exact 29-person roster')
        if formal_outer_oof and set(train_roster) | set(oof_roster) != set(final_training_roster):
            raise ValueError('frailty outer cell roster differs from the all-29 motion roster')
        train_roster = final_training_roster
        frozen_threshold, threshold_sha256 = _validated_frozen_threshold(evidence, train_roster)
        training_scope = 'frailty29_all_participants'
        reuse_scope = 'all29_reused_in_sample_auxiliary' if formal_outer_oof else 'all29_reused_final_or_smoke'
        evaluation_relation = 'in_sample_for_frailty29'
        valid_outer_oof_claim = False
        fold_identity = {'repeat_index': None, 'fold_index': None, 'outer_oof_participant_ids': list(oof_roster)}
        model_directory = Path('final_all_internal')
    ...
    model_path = _resolve_model_path(evidence_path, model.get('artifact_path'), directory=model_directory)
    metadata = {
    ...
    runtime = load_formal_motion_model(model_path, metadata, runtime_device=config.device)
    ...
    provenance = {
        ...
        'model_input_schema_sha256': model.get('model_input_schema_sha256'),
        'training_participant_ids': list(train_roster),
        **fold_identity, 'split_registry_csv_sha256': evidence_split_sha256,
        ...
        'window_probability_aggregation': 'native_windows_file_median_diagnostics_only',
        'expected_ppg_input_source': 'CanonicalSignalViews.x_native',
        'stage5_training_ppg_source': 'manifest_numeric_values_columns_RED_IR',
        'ppg_source_equivalence_requirement': 'x_native_on_same_source_with_no_gap_repair',
        'source_qc_difference': (
            'canonical_ppg_qc_precedes_inference_whereas_stage5_used_physical_recording_qc_'
            'without_signal_repair'),
        'window_seconds': 8.0,
        'hop_seconds': 2.0
    }
    return LoadedReusedMotionDetector(runtime=runtime, threshold=selected_threshold,
                                      ekf_config_sha256=trained_ekf_config_sha256, provenance=provenance)

def _fail_closed_unfit(detector: LoadedReusedMotionDetector, *, reason: str) -> MotionRecordDecision:
    result = MotionRecordDecision(motion_state='unfit', record_probability=None, threshold=detector.threshold,
                                  window_count=0, reason=reason)
    result.validate()
    return result

def motion_recording_from_signal_views(views: CanonicalSignalViews, *, detector: LoadedReusedMotionDetector,
                                       record_id: str, participant_id: str, role: str) -> MotionRecordingInput:
    views.validate()
    ...
    if np.any(np.asarray(views.repair_mask, dtype=bool)):
        raise ValueError('reused Stage5 motion detector forbids gap-repaired native PPG')
    required = {
        'dynamic_acc_mps2', 'gyro_rads', 'dynamic_magnitude', 'gyro_magnitude', 'jerk_magnitude', 'roll_rad',
        'pitch_rad', 'gravity_mps2', 'imu_valid_mask'
    }
    missing = sorted(required - set(views.imu_processed))
    if missing:
        raise ValueError('CanonicalSignalViews lacks formal motion IMU fields: ' + ', '.join(missing))
    ...
    values = np.column_stack(
        (dynamic, gyro, np.asarray(views.imu_processed['dynamic_magnitude'], dtype=np.float64),
         np.asarray(views.imu_processed['gyro_magnitude'],
                    dtype=np.float64), np.asarray(views.imu_processed['jerk_magnitude'], dtype=np.float64)))
    ...
    if not isinstance(runtime_ekf_config_sha256, str) or not _SHA256.fullmatch(runtime_ekf_config_sha256):
    ...
    motion_imu = MotionImuResult(values=values, channel_schema=MOTION_IMU_CHANNEL_SCHEMA,
                                 ...
                                 gravity_mps2=np.asarray(views.imu_processed['gravity_mps2'], dtype=np.float64),
                                 valid_mask=np.asarray(views.imu_processed['imu_valid_mask'], dtype=bool),
                                 profile_id=profile_id, diagnostics=dict(diagnostics))
    result = MotionRecordingInput(ppg_red_ir=np.asarray(views.x_native, dtype=np.float64), motion_imu=motion_imu,
                                  record_id=record_id, participant_id=participant_id, role_or_activity=role,
                                  dataset_id=M2_DATASET_VERSION_ID, fs_hz=CANONICAL_FS_HZ)
    result.validate()
    return result

def infer_reused_motion_recording(detector: LoadedReusedMotionDetector,
                                  recording: MotionRecordingInput | None) -> MotionRecordDecision:
    ...
    result = MotionRecordDecision(
        motion_state='low_motion' if low_motion else 'high_motion', record_probability=probability,
        threshold=detector.threshold, window_count=len(series.decisions),
        reason='diagnostic_record_median_below_frozen_threshold'
        if low_motion else 'diagnostic_record_median_at_or_above_frozen_threshold')
    result.validate()
    return result

# Inference accepts either canonical recording input or already materialized windows.
def infer_reused_motion_windows(detector: LoadedReusedMotionDetector, recording: MotionRecordingInput | None, *,
                                precomputed_windows: MotionWindowTensors | None = None) -> MotionWindowSeries:
    if recording is None:
        result = MotionWindowSeries(decisions=(), threshold=detector.threshold, file_median_probability_diagnostic=None,
    ...
    if ppg.ndim != 2 or ppg.shape[1] != 2 or imu_values.ndim != 2 or (imu_values.shape[1] < 6):
        result = MotionWindowSeries(decisions=(), threshold=detector.threshold, file_median_probability_diagnostic=None,
                                    reason='motion_signal_missing')
        result.validate()
        return result
    ...
    if precomputed_windows is None:
        windows = build_motion_window_tensors(recording.ppg_red_ir, recording.motion_imu, record_id=recording.record_id,
                                              participant_id=recording.participant_id,
                                              role_or_activity=recording.role_or_activity,
                                              dataset_id=recording.dataset_id, fs_hz=recording.fs_hz,
                                              profile_id=MOTION_REFERENCE_PROFILE_ID)
    else:
        precomputed_windows.validate()
        if (precomputed_windows.record_id != recording.record_id
            ...
            raise ValueError('precomputed motion windows differ from the recording/bundle identity')
    ...
    if probabilities.shape != (len(rows), ) or not np.isfinite(probabilities).all() or np.any((probabilities < 0.0)
                                                                                              | (probabilities > 1.0)):
        result = MotionWindowSeries(
            decisions=tuple(
                (MotionWindowDecision(routing_window_id=f'{recording.record_id}::routing_{index:06d}',
                                      start_sample_400=int(start), stop_sample_400=int(start + MOTION_WINDOW_SAMPLES),
                                      centre_sample_400=int(start + MOTION_WINDOW_SAMPLES // 2), probability=None,
                                      threshold=detector.threshold, motion_state='unavailable',
                                      reason='motion_probability_unavailable')
                 for index, start in enumerate(windows.start_samples))), threshold=detector.threshold,
            file_median_probability_diagnostic=None, reason='motion_probability_unavailable')
        result.validate()
        return result
    decisions = tuple((MotionWindowDecision(
        routing_window_id=f'{recording.record_id}::routing_{index:06d}', start_sample_400=int(start),
        stop_sample_400=int(start + MOTION_WINDOW_SAMPLES), centre_sample_400=int(start + MOTION_WINDOW_SAMPLES // 2),
        probability=float(probability), threshold=detector.threshold,
        motion_state='low' if probability < detector.threshold else 'high', reason='window_below_frozen_threshold'
        if probability < detector.threshold else 'window_at_or_above_frozen_threshold')
                       for index, (start, probability) in enumerate(zip(windows.start_samples, probabilities))))
    result = MotionWindowSeries(decisions=decisions, threshold=detector.threshold,
