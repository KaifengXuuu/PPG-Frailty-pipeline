"""Repeat summaries and paired deltas."""
...
def _summary(values: np.ndarray) -> dict[str, float | list[float]]:
    """Mean, sample SD, and t confidence interval."""
...
def summarize_repeats(
    metrics_by_repeat: Mapping[str, Mapping[str, float]]
) -> dict[str, dict[str, float | list[float]]]:
    """Summarise complete repeats per metric."""
    ...
    return {
        name: _summary(
            np.asarray([metrics_by_repeat[key][name] for key in sorted(metrics_by_repeat)], dtype=np.float64)
        )
        for name in sorted(names)
    }

def paired_metric_delta(reference: Mapping[str, float], candidate: Mapping[str, float]) -> PairedMetricDelta:
    """Require identical fold/seed keys."""
