"""Fixed-PPI PRV backend comparison only.
...
Aura and rhenan adapters receive exactly the same uncleaned PPI vectors as the formal local implementation.
Missing dependencies affect only that backend; this module neither cleans intervals nor feeds a classifier.
"""
...
class PrvBackendResult:
    ...
    def to_dict(self) -> dict[str, object]:
        """Return strict-JSON-safe payload"""
...
def fixed_ppi_fixtures() -> dict[str, np.ndarray]:
    """Deterministic vectors; no preprocessing"""
...
def _local_features(ppi_ms: np.ndarray) -> dict[str, float | None]:
    """Local formal equations evaluated directly on fixed PPI"""
...
def evaluate_prv_backend(
    ...
    """Evaluate one backend without cleaning"""
...
def run_prv_backend_comparison(
    ...
    """Run only function-level fixed-vector comparisons"""
