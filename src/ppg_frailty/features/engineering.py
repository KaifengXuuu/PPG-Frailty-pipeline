...
ENGINEERING_SCHEMA_VERSION = "engineering_10s_hop2s_thesis_115_v3"
WELCH_WINDOW = "hann"
...
class EngineeringExtraction:
    """Sequence plus per-value validity."""
...
def validate_engineering_extraction(
    ...
    *,
    ...
    expected_version = ENGINEERING_SCHEMA_VERSION + ("+fold_robust_v1" if fold_transformed else "")
    if (tuple(sequence.channel_schema) != names or sequence.schema_version != expected_version or values.ndim != 2
            or values.shape[1] != len(names) or validity.shape != values.shape or starts.shape != (values.shape[0], )
...
def _entropy(power: np.ndarray) -> float:
    ...
    return float(-np.sum(distribution[positive] * np.log(distribution[positive])) / np.log(distribution.size))
...
def _band_power(frequencies: np.ndarray, power: np.ndarray, low: float, high: float) -> float:
    ...
    return float(integrate.trapezoid(power[mask], frequencies[mask])) if np.count_nonzero(mask) >= 2 else float("nan")
...
def _one_channel_features(
    ...
    *,
    ...
    """Extract one channel descriptor."""

    source = np.asarray(values, dtype=np.float64)
    finite = np.isfinite(source)
    if np.mean(finite) < 0.80:
        count = len(TIME_STATISTICS) + (len(PPG_SPECTRAL_STATISTICS) if include_spectral_summary else 0) + len(bands)
        return [float("nan")] * count, [False] * count
    x = source[finite]
    q25, q75 = np.percentile(x, [25.0, 75.0])
    median = float(np.median(x))
    # Welch uses only the longest finite run; never concatenate across invalid gaps.
    ...
    if include_spectral_summary:
        total_power = float(integrate.trapezoid(power, frequencies)) if frequencies.size >= 2 else float("nan")
        positive_power = power.size > 1 and np.isfinite(power).all() and float(np.sum(power)) > 0.0
        dominant_frequency = float(frequencies[int(np.argmax(power))]) if positive_power else float("nan")
        spectral_centroid = (float(np.sum(frequencies * power) / (float(np.sum(power)) + np.finfo(np.float64).eps))
                             if positive_power else float("nan"))
        features.extend((
            total_power,
            _entropy(power),
            dominant_frequency,
...
def engineering_feature_names() -> tuple[str, ...]:
    """Return the frozen ordered engineering schema."""

    names: list[str] = []
    for channel in ("ppg_red", "ppg_ir"):
        names.extend(f"{channel}.{statistic}" for statistic in TIME_STATISTICS)
        names.extend(f"{channel}.{statistic}" for statistic in PPG_SPECTRAL_STATISTICS)
...
def _imu_columns(views: CanonicalSignalViews) -> list[tuple[np.ndarray, bool]]:
    """Read A/Omega/J followed by six measured axes."""
    ...
    acc_magnitude = np.asarray(views.imu_processed["dynamic_magnitude"], dtype=np.float64)
    gyro_magnitude = np.asarray(views.imu_processed["gyro_magnitude"], dtype=np.float64)
    jerk_magnitude = np.asarray(views.imu_processed["jerk_magnitude"], dtype=np.float64)
    ...
    if any(item.shape != expected_scalar_shape for item in (acc_magnitude, gyro_magnitude, jerk_magnitude)):
...
def extract_engineering_features(
    ...
    *,
    ...
    """Extract chronological engineering rows using the shared plan.

    Non-identity `x_ar` PPG predictor slots are explicitly NaN/false; retain only IMU
    ...
    if selected_plan.short_record_action != "reject" or selected_plan.include_padded_tail:
    ...
    for item in windows:
        ...
        if views.route in {SignalRoute.DIRECT, SignalRoute.IDENTITY}:
            ...
            unavailable = len(TIME_STATISTICS) + len(PPG_SPECTRAL_STATISTICS) + len(PPG_BANDS)
...
def fit_fold_feature_transform(
    ...
    *,
    ...
    """Fit median/IQR on outer-train rows only."""

    fitted = assert_training_only(fitted_on_participant_ids, outer_train_participant_ids, outer_oof_participant_ids)
...
def transform_engineering(
    ...
    """Apply without imputing unavailable slots."""

    validate_engineering_extraction(extraction, fold_transformed=False)
    expected_names = engineering_feature_names()
    if (extraction.sequence.channel_schema != transform.feature_names or transform.feature_names != expected_names
            or np.asarray(transform.center).shape != (len(expected_names), ) or np.asarray(transform.scale).shape !=
        (len(expected_names), ) or not np.isfinite(transform.center).all() or not np.isfinite(transform.scale).all()
