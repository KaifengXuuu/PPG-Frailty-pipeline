"""Endpoint-aware Q_rate/Q_morph SQI components and diagnostics."""
...
from ..contracts import PulseResult, QualityComponent, QualityEndpoint, QualityResult, QualityState, SignalRoute
from ..provenance import assert_training_only
from .views import CANONICAL_FS_HZ, CanonicalSignalViews

_RATE_WEIGHTS = {
    "cardiac_concentration": 0.20, "autocorrelation_periodicity": 0.15, "normalized_spectral_entropy": 0.10,
    "peak_density_bpm": 0.08, "ppi_physiological_fraction": 0.15, "ppi_stability": 0.12,
    "red_ir_agreement": 0.08, "motion_energy_rms": 0.05, "nonflat_scale": 0.02, "source_coverage": 0.04,
    "flatline": 0.02, "clipping": 0.015, "saturation": 0.015, "long_gap": 0.01,
}
_MORPH_WEIGHTS = {
    "template_correlation": 0.30, "skewness": 0.08, "pearson_kurtosis": 0.08, "red_ir_agreement": 0.18,
    "cardiac_concentration": 0.16, "nonflat_scale": 0.04, "source_coverage": 0.12, "flatline": 0.03,
    "clipping": 0.02, "saturation": 0.02, "long_gap": 0.02,
}
RATE_COMPONENT_NAMES = tuple(_RATE_WEIGHTS)
MORPH_COMPONENT_NAMES = tuple(_MORPH_WEIGHTS)

def _default_rate_component_weights() -> dict[str, float]:
    return dict(_RATE_WEIGHTS)

def _default_morph_component_weights() -> dict[str, float]:
    return dict(_MORPH_WEIGHTS)

def _value_or_default(mapping: Mapping[str, object], name: str, default: Any) -> Any:
...
def _weights(
    ...
    return {component: float(raw.get(component, default)) for component, default in defaults.items()}

_NORMALIZATION_FIELDS = (
    "cardiac_concentration_reference", "autocorrelation_reference", "ppi_cv_scale", "motion_rms_scale",
    "nonflat_std_threshold", "clipping_fraction_reference", "saturation_fraction_reference", "morph_skewness_scale",
    "morph_kurtosis_center", "morph_kurtosis_scale", "component_pass_threshold", "template_half_width_s",
)
_PHYSICAL_INTEGER_LIMITS = {
    "welch_max_nperseg": 2, "template_min_peaks": 2, "template_min_beats": 2,
    "template_resample_points": 3, "ppi_stability_min_intervals": 2,
}
_DIAGNOSTIC_FIELDS = (
    "cardiac_low_hz", "cardiac_high_hz", "peak_density_min_bpm", "peak_density_max_bpm", "ppi_min_s", "ppi_max_s",
    "long_gap_max_samples", "flatline_duration_s", "template_half_width_s", "spectral_analysis_low_hz",
    "spectral_analysis_high_hz",
    *_PHYSICAL_INTEGER_LIMITS,
)

def _validate_physical_sqi(config: Any, *, diagnostic: bool = False) -> None:
    finite = (
        config.cardiac_low_hz,
        config.cardiac_high_hz,
        config.peak_density_min_bpm,
        config.peak_density_max_bpm,
        config.ppi_min_s,
        config.ppi_max_s,
        config.flatline_duration_s,
        config.template_half_width_s,
        config.spectral_analysis_low_hz,
        config.spectral_analysis_high_hz,
    )
    prefix = "diagnostic " if diagnostic else ""
    if not np.isfinite(finite).all():
        raise ValueError(f"all {prefix}SQI parameters must be finite")
    if not 0.0 < config.cardiac_low_hz < config.cardiac_high_hz < CANONICAL_FS_HZ / 2.0:
        raise ValueError(f"invalid {prefix}cardiac band")
    if not (
        0.0
        <= config.spectral_analysis_low_hz
        <= config.cardiac_low_hz
        ...
        raise ValueError(f"invalid {prefix}spectral analysis band")
    if not 0.0 < config.peak_density_min_bpm < config.peak_density_max_bpm:
        raise ValueError(f"invalid {prefix}peak-density range")
    if not 0.0 < config.ppi_min_s < config.ppi_max_s:
        raise ValueError(f"invalid {prefix}PPI range")
    if (
        isinstance(config.long_gap_max_samples, bool)
        or not isinstance(config.long_gap_max_samples, (int, np.integer))
        or config.long_gap_max_samples < 0
        or config.flatline_duration_s <= 0.0
        or config.template_half_width_s <= 0.0
    ):
        raise ValueError(f"invalid {prefix}gap/flatline parameters")
    for name, minimum in _PHYSICAL_INTEGER_LIMITS.items():
        value = getattr(config, name)
        if isinstance(value, bool) or not isinstance(value, (int, np.integer)) or int(value) < minimum:
            raise ValueError(f"{name} must be an integer of at least {minimum}")
    if config.template_min_peaks < config.template_min_beats:
        raise ValueError("template_min_peaks must be at least template_min_beats")
    if config.template_half_width_s * CANONICAL_FS_HZ < 1.0:
        raise ValueError("template_half_width_s must span at least one sample")

@dataclass(frozen=True)
class SqiConfig:
    """Complete SQI endpoint and component policy."""
    ...
    rate_component_weights: Mapping[str, float] = field(default_factory=_default_rate_component_weights)
    morph_component_weights: Mapping[str, float] = field(default_factory=_default_morph_component_weights)

    def validate(self) -> None:
        _validate_physical_sqi(self)
        finite = (
            self.q_rate_threshold,
            self.q_morph_threshold,
            self.minimum_coverage,
            self.calibrator_lower_quantile,
            self.calibrator_upper_quantile,
            *(getattr(self, name) for name in _NORMALIZATION_FIELDS),
        )
        if not np.isfinite(finite).all():
            raise ValueError("all SQI numerical parameters must be finite")
        if not all(
            0.0 <= value <= 1.0
            for value in (self.q_rate_threshold, self.q_morph_threshold, self.component_pass_threshold)
        ):
            raise ValueError("SQI thresholds must lie in [0,1]")
        ...
        positive = set(_NORMALIZATION_FIELDS) - {"morph_kurtosis_center", "component_pass_threshold"}
        if any(getattr(self, name) <= 0.0 for name in positive):
            raise ValueError("SQI normalization scales must be positive")
        if self.calibrator not in {"outer_train_empirical_quantiles_v1", "fixed_formula_thresholds_v1"}:
        ...
        for name, values, expected in (
            ...
            weights = (
                np.asarray([values[item] for item in expected], dtype=np.float64)
                if set(values) == set(expected)
                else np.asarray([])
            )
            if (
                weights.size != len(expected)
                or not np.isfinite(weights).all()
                or np.any(weights < 0.0)
                or not np.any(weights > 0.0)
            ):
                raise ValueError(f"{name} must resolve finite nonnegative registered weights with positive mass")
    ...
    def from_quality_mapping(cls, quality: Mapping[str, object] | None) -> "SqiConfig":
        ...
        unknown = set(quality) - allowed
        if unknown:
            raise ValueError(f"quality has unknown fields: {sorted(unknown)}")
        defaults = cls()
        pairs = {
            "cardiac_band_hz": ("cardiac_low_hz", "cardiac_high_hz"),
            "peak_density_bpm_range": ("peak_density_min_bpm", "peak_density_max_bpm"),
            "ppi_range_s": ("ppi_min_s", "ppi_max_s"),
            "spectral_analysis_band_hz": ("spectral_analysis_low_hz", "spectral_analysis_high_hz"),
            "calibrator_quantiles": ("calibrator_lower_quantile", "calibrator_upper_quantile"),
        }
        kwargs: dict[str, Any] = {}
        for public, names in pairs.items():
            values = _pair(quality, public, tuple(getattr(defaults, name) for name in names))
            kwargs.update(zip(names, values))
        direct = {
            "q_rate_threshold": "rate_threshold",
            "q_morph_threshold": "morph_threshold",
            "minimum_coverage": "minimum_coverage",
            "long_gap_max_samples": "long_gap_max_samples",
            "flatline_duration_s": "flatline_duration_s",
            **{name: name for name in _PHYSICAL_INTEGER_LIMITS},
        }
        for internal, public in direct.items():
            value = _value_or_default(quality, public, getattr(defaults, internal))
            kwargs[internal] = (
                value if internal == "long_gap_max_samples" or internal in _PHYSICAL_INTEGER_LIMITS else float(value)
            )
        normalization = quality.get("component_normalization") or {}
        if not isinstance(normalization, Mapping):
            raise ValueError("quality.component_normalization must be a mapping")
        unknown = set(normalization) - set(_NORMALIZATION_FIELDS)
        if unknown:
            raise ValueError(f"quality.component_normalization has unknown fields: {sorted(unknown)}")
        kwargs.update(
            {
                name: float(_value_or_default(normalization, name, getattr(defaults, name)))
                for name in _NORMALIZATION_FIELDS
            }
        )
        raw_calibrator = quality.get("calibrator")
        kwargs["calibrator"] = (
            "outer_train_empirical_quantiles_v1"
            if raw_calibrator is None
            or (isinstance(raw_calibrator, str) and raw_calibrator in {"", "deferred_supervised_design"})
            else str(raw_calibrator)
        )
        kwargs["rate_component_weights"] = _weights(quality, "rate_component_weights", defaults.rate_component_weights)
        kwargs["morph_component_weights"] = _weights(
            quality, "morph_component_weights", defaults.morph_component_weights
        )
        result = cls(**kwargs)
        result.validate()
        return result
    ...
    def to_dict(self) -> dict[str, object]:
        ...
        return {
            ...
            "peak_density_bpm_range": [float(self.peak_density_min_bpm), float(self.peak_density_max_bpm)],
            ...
            "spectral_analysis_band_hz": [float(self.spectral_analysis_low_hz), float(self.spectral_analysis_high_hz)],
            **{name: int(getattr(self, name)) for name in _PHYSICAL_INTEGER_LIMITS},
            "calibrator": self.calibrator,
            "calibrator_quantiles": [float(self.calibrator_lower_quantile), float(self.calibrator_upper_quantile)],
            "rate_component_weights": dict(self.rate_component_weights),
            "morph_component_weights": dict(self.morph_component_weights),
            "component_normalization": {name: float(getattr(self, name)) for name in _NORMALIZATION_FIELDS},
        }
...
class SqiDiagnosticConfig:
    """Physical component parameters only; no endpoint policy."""
    cardiac_low_hz: float = 0.5
    cardiac_high_hz: float = 3.0
    peak_density_min_bpm: float = 30.0
    peak_density_max_bpm: float = 200.0
    ...
    def validate(self) -> None:
        _validate_physical_sqi(self, diagnostic=True)

    @classmethod
    def from_resolved(cls, config: Mapping[str, object]) -> "SqiDiagnosticConfig":
        quality = config.get("quality")
        if not isinstance(quality, Mapping):
            raise ValueError("resolved config['quality'] is required")
        defaults = cls()
        ...
        kwargs: dict[str, Any] = {}
        for public, names in pairs.items():
            kwargs.update(zip(names, _pair(quality, public, tuple(getattr(defaults, name) for name in names))))
        for name in ("long_gap_max_samples", "flatline_duration_s", *_PHYSICAL_INTEGER_LIMITS):
            kwargs[name] = _value_or_default(quality, name, getattr(defaults, name))
        normalization = quality.get("component_normalization") or {}
        if not isinstance(normalization, Mapping):
            raise ValueError("quality.component_normalization must be a mapping")
        kwargs["template_half_width_s"] = float(
            _value_or_default(normalization, "template_half_width_s", defaults.template_half_width_s)
        )
        result = cls(**kwargs)
        result.validate()
        return result
    ...
    def from_sqi_config(cls, config: SqiConfig) -> "SqiDiagnosticConfig":
        result = cls(**{name: getattr(config, name) for name in _DIAGNOSTIC_FIELDS})
        result.validate()
        return result
...
@dataclass(frozen=True)
class SqiCalibrator:
    """Train-only empirical-quantile map."""
...
def fit_sqi_calibrator(
    ...
    *,
    ...
    """Fit using declared outer-train rows only."""
    if (
    ...
    selected = [(row, participant) for row, participant in zip(rows, ids) if participant in fitted_set]
    if not selected:
    ...
    for name in names:
        observed = [
            (float(row[name]), participant) for row, participant in selected if name in row and np.isfinite(row[name])
        ]
        values = np.asarray([value for value, _ in observed], dtype=np.float64)
        if values.size:
            valid_count_by_participant = {
                participant: sum(candidate == participant for _, candidate in observed) for participant in fitted
            }
            ...
            weights = np.asarray(
                [1.0 / valid_count_by_participant[participant] for _, participant in observed],
                dtype=np.float64,
            )
            order = np.argsort(values, kind="stable")
            ordered_values = values[order]
            ordered_weights = weights[order]
            positions = (np.cumsum(ordered_weights) - 0.5 * ordered_weights) / np.sum(ordered_weights)
            bounds[name] = tuple(
                float(np.interp(quantile, positions, ordered_values, left=ordered_values[0], right=ordered_values[-1]))
...
def _component(
    ...
    *,
    ...
    """Build a finite or unavailable component."""
...
def _components(
    specs: Mapping[str, tuple[float | None, float | None, str]], pass_threshold: float
) -> dict[str, QualityComponent]:
    return {
        name: _component(raw, normalized, reason, pass_threshold=pass_threshold)
        for name, (raw, normalized, reason) in specs.items()
    }
...
def _welch_metrics(
    ...
    for column in range(values.shape[1]):
        ...
        usable = (frequencies >= config.spectral_analysis_low_hz) & (frequencies <= config.spectral_analysis_high_hz)
...
def _endpoint(
    ...
    *,
    ...
    usable = [
        ...
        if name in weights and weights[name] > 0.0 and component.normalized_value is not None
    ]
    if not usable:
    ...
    score = float(sum(weight * float(value) for weight, value in scaled) / denominator)
    reasons: list[str] = []
...
def _calibrate_components(
    ...
    *,
    ...
    """Apply a train-only component map."""
    if calibrator is None:
        return components
    calibrated: dict[str, QualityComponent] = {}
    for name, component in components.items():
        normalized = calibrator.transform(f"{prefix}.{name}", component.normalized_value)
        if normalized is None:
            ...
            calibrated[name] = QualityComponent(
                ...
                state=(QualityState.PASS if normalized >= pass_threshold else QualityState.FAIL),
...
def _qc_components(
    ...
    *,
    ...
    unavailable = {
        ...
        "saturation": _component(None, None, "adc_rails_unknown_saturation_not_inferred"),
    ...
    if not rows:
        return unavailable
    longest_flat = max(float(row.get("longest_constant_run", 0.0)) for row in rows)
    ...
    longest_gap = max(float(row.get("longest_nonfinite_gap_samples", 0.0)) for row in rows)
    output = {
        "flatline": _component(
            longest_flat / fs_hz,
            1.0 - min(longest_flat / max(flatline_duration_s * fs_hz, 1.0), 1.0),
...
def quality_component_scores(result: QualityResult) -> dict[str, float]:
    """Export one row of base normalized components."""
    return {
...
@dataclass(frozen=True)
class _RawQualityObservations:
    matrix: np.ndarray
    route: SignalRoute
    pulse: PulseResult
    coverage: float
    qc_evidence: Mapping[str, object] | None
    values: dict[str, float]

def _raw_quality_observations(
    source: np.ndarray | CanonicalSignalViews,
    *,
    route: SignalRoute | None,
    pulse: PulseResult | None,
    imu: dict[str, np.ndarray] | None,
    source_valid_mask: np.ndarray | None,
    qc_evidence: Mapping[str, object] | None,
    fs_hz: float,
    physical: SqiDiagnosticConfig | SqiConfig,
    detector_id: str | None,
    min_observation_sec: float,
    min_peaks: int,
    detector_parameters: Mapping[str, object] | None,
    diagnostics_only: bool,
) -> _RawQualityObservations:
    """Compute shared raw observations once; endpoint policy remains in callers."""
    canonical = source if isinstance(source, CanonicalSignalViews) else None
    coverage_override: float | None = None
    if canonical is not None:
        route = canonical.route
        matrix = np.asarray(canonical.analysis_signal, dtype=np.float64)
        imu = canonical.imu_processed
        source_valid_mask = canonical.source_valid_mask
        if qc_evidence is None:
            candidate = canonical.metadata.get("ppg_qc_metrics")
            qc_evidence = candidate if isinstance(candidate, Mapping) else None
        rate_valid = np.asarray(canonical.rate_valid_mask, dtype=bool)
        if diagnostics_only or not np.all(rate_valid):
            coverage_override = float(np.mean(rate_valid))
        if not np.all(rate_valid):
            changes = np.diff(np.concatenate(([False], rate_valid, [False])).astype(np.int8))
            ...
            imu = {
                name: np.asarray(item)[start:stop]
                if np.asarray(item).ndim >= 1 and np.asarray(item).shape[0] == rate_valid.size
                else np.asarray(item)
                for name, item in (imu or {}).items()
            }
    else:
    ...
    if coverage_override is not None:
        ...
        if diagnostics_only and valid.shape == (matrix.shape[0],):
            coverage = float(np.mean(valid))
        elif valid.shape == matrix.shape:
            coverage = float(np.mean(np.all(valid, axis=1)))
        else:
            raise ValueError("source_valid_mask must align with signal samples")

    cardiac, entropy = _welch_metrics(matrix, fs_hz, physical)
    periodicity = _autocorrelation_periodicity(
        matrix, fs_hz, lag_min_s=physical.ppi_min_s, lag_max_s=physical.ppi_max_s
    )
    red_ir = (
        abs(float(np.corrcoef(matrix[:, 0], matrix[:, 1])[0, 1]))
        if matrix.shape[1] == 2 and np.std(matrix[:, 0]) > 0.0 and np.std(matrix[:, 1]) > 0.0
        else float("nan")
    )
    if pulse is None:
        if detector_id is None:
            raise ValueError("SQI requires a persisted detector_id when pulse is not supplied")
        from ..peaks import detect_pulses

        pulse = detect_pulses(
            matrix if diagnostics_only or canonical is None else canonical,
    ...
    ppi_cv = (
        float(np.std(selected) / np.mean(selected))
        if selected.size >= physical.ppi_stability_min_intervals and float(np.mean(selected)) > 0.0
        else float("nan")
    )
    motion_rms = float("nan")
    if imu is not None and "dynamic_magnitude" in imu:
        motion = np.asarray(imu["dynamic_magnitude"], dtype=np.float64)
        if motion.shape == (matrix.shape[0],):
            finite = motion[np.isfinite(motion)]
            if finite.size:
                motion_rms = float(np.sqrt(np.mean(np.square(finite))))
    raw = {
        "cardiac": cardiac,
        ...
        "nonflat": float(min(np.std(matrix[:, column]) for column in range(matrix.shape[1]))),
    }
    if route is not SignalRoute.ARTIFACT_RATE_ONLY:
        raw.update(
            {
                ...
                "template": _template_correlation(
                    matrix,
                    pulse,
                    fs_hz,
    ...
    return _RawQualityObservations(matrix, route, pulse, coverage, qc_evidence, raw)
...
def evaluate_quality_diagnostics(
    ...
    *,
    ...
    """Compute physical SQI observations without weights, thresholds, or routing."""
    ...
    observations = _raw_quality_observations(
        values,
        route=route,
        pulse=pulse,
        imu=imu_processed,
    ...
    route, coverage, qc_evidence, raw = (
        observations.route,
        observations.coverage,
        observations.qc_evidence,
        observations.values,
    )
    components: dict[str, SqiDiagnosticComponent] = {
        "rate.cardiac_concentration": _diagnostic_component(raw["cardiac"], "cardiac_band_power_fraction"),
        "rate.autocorrelation_periodicity": _diagnostic_component(raw["periodicity"], "physiological_lag_acf"),
        "rate.normalized_spectral_entropy": _diagnostic_component(raw["entropy"], "normalized_welch_entropy"),
        "rate.peak_density_bpm": _diagnostic_component(raw["peak_density"], "detected_peaks_per_minute"),
        "rate.ppi_physiological_fraction": _diagnostic_component(
            raw["ppi_fraction"], "ppi_inside_explicit_physical_range"
        ),
        "rate.ppi_cv": _diagnostic_component(raw["ppi_cv"], "valid_ppi_coefficient_of_variation"),
        "rate.red_ir_agreement": _diagnostic_component(raw["red_ir"], "absolute_zero_lag_correlation"),
        "rate.motion_energy_rms": _diagnostic_component(raw["motion_rms"], "dynamic_acceleration_rms_mps2"),
        "rate.nonflat_scale": _diagnostic_component(raw["nonflat"], "minimum_channel_standard_deviation"),
        "rate.source_coverage": _diagnostic_component(coverage, "finite_and_route_valid_source_fraction"),
    }
    qc = _qc_components(
        qc_evidence,
        fs_hz=fs_hz,
    ...
    components.update({f"rate.{name}": _diagnostic_component(item.raw_value, item.reason) for name, item in qc.items()})
    reasons: list[str] = []
        ...
        components.update(
            {
                "morph.template_correlation": _diagnostic_component(
                    raw["template"], "median_beat_template_correlation"
                ),
                "morph.skewness": _diagnostic_component(raw["skewness"], "mean_channel_skewness"),
                "morph.pearson_kurtosis": _diagnostic_component(raw["kurtosis"], "mean_channel_pearson_kurtosis"),
            }
        )
    reasons.extend(f"{name}:unavailable" for name, item in sorted(components.items()) if not item.available)
...
def evaluate_quality(
    ...
    *,
    ...
    """Public endpoint-SQI entry point."""
    if float(fs_hz) != CANONICAL_FS_HZ:
        raise ValueError("SQI requires exactly 400 Hz")
    config.validate()
    if config.calibrator == "outer_train_empirical_quantiles_v1" and calibrator is None:
    ...
    peak_density = raw["peak_density"]
    ppi_stability = (
        float(np.exp(-max(raw["ppi_cv"], 0.0) / config.ppi_cv_scale)) if np.isfinite(raw["ppi_cv"]) else float("nan")
    )
    density_score = (
        1.0
        if config.peak_density_min_bpm <= peak_density <= config.peak_density_max_bpm
        else float(
            np.exp(
                -abs(
                    peak_density
    ...
    rate_specs = {
        "cardiac_concentration": (
            raw["cardiac"],
            raw["cardiac"] / config.cardiac_concentration_reference,
            "cardiac_band_power_fraction",
        ),
        "autocorrelation_periodicity": (
            raw["periodicity"],
            raw["periodicity"] / config.autocorrelation_reference,
            "physiological_lag_acf",
        ),
        "normalized_spectral_entropy": (raw["entropy"], 1.0 - raw["entropy"], "lower_entropy_is_better"),
        "peak_density_bpm": (peak_density, density_score, "physiological_peak_density"),
        "ppi_physiological_fraction": (
            raw["ppi_fraction"],
            raw["ppi_fraction"],
            "ppi_inside_configured_physical_range",
        ),
        "ppi_stability": (ppi_stability, ppi_stability, "low_ppi_cv"),
        "red_ir_agreement": (raw["red_ir"], raw["red_ir"], "absolute_zero_lag_correlation"),
        "motion_energy_rms": (
            raw["motion_rms"],
            np.exp(-raw["motion_rms"] / config.motion_rms_scale) if np.isfinite(raw["motion_rms"]) else None,
            "lower_dynamic_acc_is_better",
        ),
        "nonflat_scale": (
            raw["nonflat"],
            1.0 if raw["nonflat"] > config.nonflat_std_threshold else 0.0,
            "nonzero_channel_variance",
        ),
        "source_coverage": (coverage, coverage, "finite_source_coverage"),
    }
    rate_components = {**_components(rate_specs, config.component_pass_threshold), **qc_components}
    ...
    if route is SignalRoute.ARTIFACT_RATE_ONLY:
            ...
            **_components(
                {
                    "template_correlation": (
                        raw["template"],
                        max(raw["template"], 0.0) if np.isfinite(raw["template"]) else None,
                        "median_beat_template",
                    ),
                    "skewness": (
                        raw["skewness"],
                        np.exp(-abs(raw["skewness"]) / config.morph_skewness_scale),
                        "moderate_skewness",
                    ),
                    "pearson_kurtosis": (
                        raw["kurtosis"],
                        np.exp(-abs(raw["kurtosis"] - config.morph_kurtosis_center) / config.morph_kurtosis_scale),
                        "moderate_kurtosis",
                    ),
                },
                config.component_pass_threshold,
            ),
        ...
        morph_pass = q_rate.state is QualityState.PASS and q_shape.state is QualityState.PASS
        q_morph = QualityEndpoint(
            ...
            reasons=tuple(
                dict.fromkeys(
                    (
                        *q_shape.reasons,
                        *(() if q_rate.state is QualityState.PASS else ("q_morph_requires_q_rate_pass",)),
                    )
                )
            ),
    ...
    state = (
        "pass"
        if q_rate.state is QualityState.PASS and q_morph.state in {QualityState.PASS, QualityState.NOT_APPLICABLE}
        else "fail"
    )
    result = QualityResult(
        ...
        coverage=coverage,
        fitted_on_participant_ids=(calibrator.fitted_on_participant_ids if calibrator is not None else ()),
        q_shape=q_shape,
    )
