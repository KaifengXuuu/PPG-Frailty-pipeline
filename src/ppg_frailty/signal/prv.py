"""Time- and adjacency-preserving PRV."""
...
class PrvConfig:
    ...
    def from_mapping(cls, features: Mapping[str, Any] | None) -> "PrvConfig":
        ...
        defaults = cls()
        if ("rate_prv_min_peaks" in features and "time_prv_min_accepted_peaks" in features
                and features["rate_prv_min_peaks"] != features["time_prv_min_accepted_peaks"]):
            raise ValueError("features.rate_prv_min_peaks conflicts with its deprecated "
        ...
        if unknown_bands:
            raise ValueError("features.spectral_bands_hz contains unknown bands: " f"{sorted(unknown_bands)}")
        ...
        if unknown_entropy:
            raise ValueError("features.sample_entropy contains unknown fields: " f"{sorted(unknown_entropy)}")
        ...
        result = cls(
            ...
            tachogram_fs_hz=float(features.get("tachogram_fs_hz", defaults.tachogram_fs_hz)),
            ...
            sample_entropy_r_sd_fraction=float(entropy_raw.get(
                "r_sd_fraction",
    ...
    def validated(self) -> "PrvConfig":
        ...
        if self.sample_entropy_min_intervals < self.sample_entropy_m + 2:
            raise ValueError("sample_entropy_min_intervals must be at least sample_entropy_m + 2")
        for name, raw in (
        ...
        for name, band_edges in named_bands:
            ...
            if not np.isfinite([low, high]).all() or low < 0.0 or low >= high or high > nyquist:
                raise ValueError(f"{name} spectral band must be finite, ordered, and within tachogram Nyquist")
...
@dataclass(frozen=True)
class PrvResult:
    """Values, field validity, and eligibility."""
...
def _nan_payload() -> tuple[dict[str, float], dict[str, bool]]:
    """Build NaN/false unavailable placeholders."""
...
def _sample_entropy(values: np.ndarray, m: int = 2, tolerance: float | None = None) -> float:
    """Deterministic sample entropy implementation."""
    ...
    def matches(length: int) -> int:
        """Count non-self Chebyshev matches."""
    ...
    # SampEn uses a ratio of matching probabilities. Length m and m+1 have different
    # numbers of possible template pairs; a ratio of raw counts would systematically bias the definition.
...
def _band_integral(frequencies: np.ndarray, power: np.ndarray, low: float, high: float) -> float:
    """Integrate a frequency band including its upper edge."""
...
def _true_runs(mask: np.ndarray) -> list[tuple[int, int]]:
    """Contiguous valid interval runs."""
...
def compute_prv(
    ...
    *,
    ...
    """Compute complete rate/time/spectral PRV.

    RMSSD/SDSD/NN50 use originally adjacent interval pairs with both valid; never bridge gaps.
    ...
    pulse_route = (pulse.source_route if isinstance(pulse.source_route, SignalRoute) else SignalRoute(
        str(pulse.source_route)))
    ...
    reasons: list[str] = []
    rate_eligible = (observation_duration_s >= resolved.rate_prv_min_duration_s
    ...
    time_eligible = bool(observation_duration_s >= resolved.time_prv_min_duration_s
    ...
    if not time_eligible:
        ...
        # Pair masks refer to ppi[i] and ppi[i+1]: both must be valid and originally adjacent.
        # English: Each pair requires two valid intervals and preserved original adjacency.
        endpoint_contiguous = stops[:-1] == starts[1:]
        pair_mask = valid[:-1] & valid[1:] & adjacency[:-1] & adjacency[1:] & endpoint_contiguous
        ...
        if differences.size:
            ...
            pair_values = {
                ...
                "pnn50": float(nn50 / differences.size),
                "sd1_s": sd1,
                "sd2_s": sd2,
    ...
    # Do not bridge gaps into adjacent templates; SampEn uses the longest contiguous interval run.
    # English: SampEn never concatenates templates across a rejected interval.
    runs = _true_runs(valid & adjacency)
    longest_run = max(
        runs,
        key=lambda bounds: float(np.sum(ppi_all[bounds[0]:bounds[1]])),
        default=(0, 0),
    )
    run_start, run_stop = longest_run
    contiguous_ppi = ppi_all[run_start:run_stop]
    entropy_eligible = contiguous_ppi.size >= resolved.sample_entropy_min_intervals
    if entropy_eligible:
        entropy = _sample_entropy(
            contiguous_ppi,
            m=resolved.sample_entropy_m,
            tolerance=(resolved.sample_entropy_r_sd_fraction * np.std(contiguous_ppi, ddof=1)),
        )
    ...
    # V2 callers supply canonical role families; B/R support long static PRV. Numeric
    # file suffixes and free-text aliases are not accepted as roles here.
    allowed_roles = {"B", "R"}
    ...
    frequency_eligible = bool(route_eligible and q_rate_qualified and role.strip().upper() in allowed_roles
    ...
    if not frequency_eligible:
        ...
        # Sample the configured tachogram using actual endpoint times, then linearly detrend.
