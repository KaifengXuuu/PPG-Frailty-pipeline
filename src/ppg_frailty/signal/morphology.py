"""Beat morphology for direct/identity only."""
...
class MorphologyResult:
    """Beat arrays, validity, and robust summaries."""
...
def require_direct_route(route: SignalRoute) -> None:
    """Block rate-only morphology before access."""

    if route not in {SignalRoute.DIRECT, SignalRoute.IDENTITY}:
        raise PermissionError("morphology is forbidden for non-identity x_ar; Q_morph is not_applicable")
...
def _crossing_time(samples: np.ndarray, values: np.ndarray, target: float, *, first: bool) -> float | None:
...
def extract_morphology(
    ...
    *,
    ...
    """Extract beat amplitude, width, rise/decay, slope and area.

    Use a linear valley-to-valley baseline per beat to reduce residual-drift bias in amplitude/area.
    ...
    for ordinal in range(1, peaks.size - 1):
        ...
        if not np.isfinite(amplitude) or amplitude <= 0.0:
            continue
        half = amplitude / 2.0
        left_cross = _crossing_time(segment_samples[: peak_offset + 1], detrended[: peak_offset + 1], half, first=False)
        right_cross = _crossing_time(segment_samples[peak_offset:], detrended[peak_offset:], half, first=True)
    ...
    for name in MORPHOLOGY_NAMES:
        ...
        for statistic, value in (
            ...
            (
                "mad",
                float(np.median(np.abs(selected - np.median(selected)))) if selected.size else float("nan"),
            ),
        ):
