"""Canonical 400-Hz PPG/IMU input checks and zero-phase preprocessing."""
...
M3_PPG_SOURCE_SHA256 = "d68a807893b02b590341199822425e83ec3fcba5e44e2f6597b944e21001abe5"
M3_QUALITY_SOURCE_SHA256 = "b246d0bfae4afdf6275b92d9579ded7fde063f3d0d33b49c77d6af43d5386bdc"
REFERENCE_PPG_FILTER_PROFILE_ID = "butterworth_0p2_8hz_reference"
ABLATION_PPG_FILTER_PROFILE_ID = "butterworth_0p5_5hz_ablation"
_CALIBRATED_MOTION_METHODS = frozenset(
    {"calibrated_roll_pitch_ekf", "profile_a_lowpass_0p3hz", "sensor_filter_only_no_gravity_removal"}
)
_LEGACY_MOTION_METHODS = frozenset({"quaternion_error_state_ekf", "low_pass_0p3hz"})
_IMU_COMPARISON = {
...
_IMU_OUTPUT_UNITS = {"acceleration": "m/s^2", "gyroscope": "rad/s", "jerk": "m/s^3"}
...
_PPG_FILTER_PROFILES = {
    REFERENCE_PPG_FILTER_PROFILE_ID: PpgFilterProfile(REFERENCE_PPG_FILTER_PROFILE_ID, 0.2, 8.0, 3, "reference"),
    ABLATION_PPG_FILTER_PROFILE_ID: PpgFilterProfile(ABLATION_PPG_FILTER_PROFILE_ID, 0.5, 5.0, 3, "ablation"),
}
...
@dataclass(frozen=True)
class InputQC:
    """Result of pre-filter input inspection."""
    repaired: np.ndarray
...
def validate_timestamp_grid(
    ...
    *,
    ...
    """Validate an explicit synchronized time grid."""
...
def _true_runs(mask: np.ndarray) -> list[tuple[int, int]]:
    """Return right-open runs of true values."""
...
def _longest_constant_run(values: np.ndarray) -> int:
    """Compute the longest exactly constant run."""
...
def inspect_and_repair(
    ...
    *,
    ...
    """Repair bounded internal gaps; reject boundary/long gaps and flatlines."""
    ...
    for column in range(source.shape[1]):
        ...
        # No hidden total-missingness cutoff: each gap has an explicit maximum length.
        # Pass original coverage to SQI, evaluated against its registered 0.80 coverage threshold.
        if not finite.any():
            fatal = True
        ...
        for start, stop in gap_runs:
            ...
            if (
                internal
                and length <= max_gap
                and np.isfinite([repaired[start - 1, column], repaired[stop, column]]).all()
            ):
                ...
                fatal = True
                reasons.append(f"channel_{column}:{'boundary_gap' if not internal else 'long_gap'}:{length}")
...
def design_ppg_sos(
    ...
    *,
    ...
    """Design runtime-configured SOS."""
    if float(fs_hz) != CANONICAL_FS_HZ:
        raise ValueError("PPG filter design requires the canonical 400 Hz grid")
    if not np.isfinite([low_hz, high_hz]).all() or not 0.0 < float(low_hz) < float(high_hz) < fs_hz / 2.0:
...
def preprocess_ppg_pair(
    ...
    *,
    ...
    # Offline analysis uses linear detrend + sosfiltfilt; never fall back to causal filtering for short inputs.
...
def roll_pitch_ekf_config_from_resolved(imu_config: Mapping[str, Any]) -> RollPitchEkfConfig:
    ...
    aliases = {"accelerometer_lowpass_hz": "sensor_lowpass_acc_hz", "gyroscope_lowpass_hz": "sensor_lowpass_gyro_hz"}
    vectors = {
        "process_covariance_diagonal_per_second",
        "observation_covariance_diagonal_rad2",
        "initial_covariance_diagonal",
    }
    integers = {"sensor_filter_order", "gravity_filter_order"}
    names = (
        *vectors,
    ...
    kwargs: dict[str, Any] = {}
    for name in names:
        raw = imu_config.get(aliases.get(name, name), getattr(defaults, name))
        kwargs[name] = (
            tuple(float(value) for value in raw) if name in vectors else raw if name in integers else float(raw)
        )
    result = RollPitchEkfConfig(**kwargs)
    result.validate()
    return result
...
def _mapping_section(value: Any, name: str, allowed: set[str] | tuple[str, ...]) -> dict[str, Any]:
    value = {} if value is None else value
    if not isinstance(value, Mapping):
        raise ValueError(f"signal.{name} must be a mapping")
    unknown = sorted(set(value) - set(allowed))
    if unknown:
        raise ValueError(f"signal.{name} contains unknown fields: {unknown}")
    return dict(value)
...
def _materialize_imu_profile(
    ...
    *,
    ...
    declared = _mapping_section(value, "imu", structural_keys | set(numeric_keys))

    method = str(declared.get("gravity_method", "profile_a_lowpass_0p3hz"))
    if method not in _CALIBRATED_MOTION_METHODS | _LEGACY_MOTION_METHODS:
        raise ValueError("signal.imu.gravity_method is not a registered V2 profile")

    derived_initialization = (
        "same_participant_static_calibration" if method in _CALIBRATED_MOTION_METHODS else "online_no_precalibration"
    )
    derived_comparison = _IMU_COMPARISON[method]
    if "initialization" in declared and declared["initialization"] != derived_initialization:
        raise ValueError("signal.imu.initialization disagrees with the selected gravity_method")
    if "comparison_method" in declared and declared["comparison_method"] != derived_comparison:
        raise ValueError("signal.imu.comparison_method disagrees with the selected gravity_method")
    if "output_units" in declared and declared["output_units"] != _IMU_OUTPUT_UNITS:
    ...
    numeric_defaults: dict[str, Any] = {
        ...
        "gravity_filter_order": (2 if method in _LEGACY_MOTION_METHODS else defaults.gravity_filter_order),
        "calibration_start_s": defaults.calibration_start_s,
        "calibration_stop_s": defaults.calibration_stop_s,
        "process_covariance_diagonal_per_second": list(defaults.process_covariance_diagonal_per_second),
        "observation_covariance_diagonal_rad2": list(defaults.observation_covariance_diagonal_rad2),
    ...
    sensor_fields = {"sensor_lowpass_acc_hz", "sensor_lowpass_gyro_hz", "sensor_filter_order"}
    static_fields = {"calibration_start_s", "calibration_stop_s", "gravity_mps2"}
    gravity_fields = {"gravity_lowpass_hz", "gravity_filter_order"}
    active_by_method = {
        "calibrated_roll_pitch_ekf": sensor_fields
        | static_fields
        | {
            "process_covariance_diagonal_per_second",
            "observation_covariance_diagonal_rad2",
        ...
        "profile_a_lowpass_0p3hz": sensor_fields | static_fields | gravity_fields,
        "sensor_filter_only_no_gravity_removal": sensor_fields | static_fields,
        "quaternion_error_state_ekf": sensor_fields,
        "low_pass_0p3hz": sensor_fields | gravity_fields,
    }
    ...
    for name in numeric_keys:
        ...
        if name in vector_lengths:
            ...
            if isinstance(raw, (bool, np.bool_)) or not isinstance(raw, (int, np.integer)) or not 1 <= int(raw) <= 20:
        ...
        if name not in active:
            if resolved != numeric_defaults[name]:
                raise ValueError(
                    f"signal.imu.{name} is inactive for gravity_method={method}; " "remove the non-default value"
                )
            continue
    ...
    effective = {
        ...
        **normalized,
        "output_units": _IMU_OUTPUT_UNITS,
        "required_axes": 6,
        "failure_action": "fail_closed",
    }
    if method in _CALIBRATED_MOTION_METHODS:
        roll_pitch_ekf_config_from_resolved(effective).validate(fs_hz)
    else:
        from .imu import ImuProfile

        ImuProfile(
            gravity_method=("no_precalibration_ekf" if method == "quaternion_error_state_ekf" else "lpf_0p3"),
...
def materialize_signal_preprocessing_config(
    ...
    channel_order = declared.get("channel_order", ["RED", "IR", "AX", "AY", "AZ", "GX", "GY", "GZ"])
    ...
    raw_filter = _mapping_section(declared.get("ppg_filter"), "ppg_filter", tuple(filter_defaults))
    ...
    raw_analysis = _mapping_section(
        declared.get("analysis_view"), "analysis_view", {"direct_source", *analysis_defaults}
    )
    expected_direct_source = canonical_ppg_direct_source(ppg_filter["low_hz"], ppg_filter["high_hz"])
    ...
    if direct_source is not None and str(direct_source) not in {
        ...
        raise ValueError("signal.analysis_view.direct_source numeric alias disagrees with " "signal.ppg_filter")
    ...
    raw_gap = _mapping_section(declared.get("gap_repair"), "gap_repair", tuple(gap_defaults))
    ...
    if isinstance(max_gap, (bool, np.bool_)) or not isinstance(max_gap, (int, np.integer)) or int(max_gap) < 0:
    ...
    effective.update(
        {
            ...
            "accelerometer_input_unit": _canonical_acceleration_unit(declared.get("accelerometer_input_unit", "g")),
            "gyroscope_input_unit": _canonical_gyroscope_unit(declared.get("gyroscope_input_unit", "deg/s")),
...
def build_signal_views(
    ...
    *,
    ...
    timestamps = record.get("timestamps_s") if isinstance(record, Mapping) else getattr(record, "timestamps_s", None)
    ppg_filter = signal_config["ppg_filter"]
    analysis_view = signal_config["analysis_view"]
    gap_repair = signal_config["gap_repair"]
    ...
    acc_unit = signal_config["accelerometer_input_unit"]
    gyro_unit = signal_config["gyroscope_input_unit"]
    imu_config = signal_config["imu"]
    gravity_profile = str(imu_config["gravity_method"])
    ...
    resolved_normalization = RawNormalizationConfig.from_mapping(signal_config.get("normalization")).to_mapping()
    for name, expected in (("acc_unit", acc_unit), ("gyro_unit", gyro_unit)):
        observed = record.get(name) if isinstance(record, Mapping) else getattr(record, name, None)
        if observed is not None:
            canonical_observed = (
                _canonical_acceleration_unit(observed) if name == "acc_unit" else _canonical_gyroscope_unit(observed)
            )
        else:
    ...
    if gravity_profile in _CALIBRATED_MOTION_METHODS:
        ...
        if not isinstance(calibration, MotionImuCalibration):
            raise ValueError(
                "calibrated EKF requires an explicit same-participant " "MotionImuCalibration; no fallback is permitted"
            )
        ...
        processor = {
            ...
            "sensor_filter_only_no_gravity_removal": (preprocess_motion_imu_without_gravity_removal),
        }[gravity_profile]
        ...
        imu = preprocess_imu(
            ...
            acc_unit=acc_unit,
            gyro_unit=gyro_unit,
            gravity_method=("no_precalibration_ekf" if gravity_profile == "quaternion_error_state_ekf" else "lpf_0p3"),
            timestamps_s=timestamps,
            acceleration_lowpass_hz=float(imu_config.get("sensor_lowpass_acc_hz", 20.0)),
            gyroscope_lowpass_hz=float(imu_config.get("sensor_lowpass_gyro_hz", 40.0)),
            sensor_filter_order=imu_config.get("sensor_filter_order", 3),
            gravity_lowpass_hz=float(imu_config.get("gravity_lowpass_hz", 0.3)),
    ...
    views = CanonicalSignalViews(
        x_native=native,
        x_filter=filtered,
        # The frozen V2 contract defines direct rate input as x_filter.
