...
class EskfConfiguration:
    """Frozen MEKF engineering parameters."""
...
@dataclass(frozen=True)
class ImuProfile:
    """Explicit versioned runtime profile."""
    gravity_method: str
    ...
    def validate(self, fs_hz: float = CANONICAL_FS_HZ) -> None:
        """Reject unimplemented methods and bad ranges."""
...
class ImuPreprocessResult:
    """Synchronized result, masks, and diagnostics."""
...
def convert_acceleration(values: np.ndarray, unit: str) -> np.ndarray:
    """Convert ACC only from explicit metadata."""
    factors = {
        "g": STANDARD_GRAVITY,
        "m/s2": 1.0,
        "m/s^2": 1.0,
...
def convert_gyro(values: np.ndarray, unit: str) -> np.ndarray:
    """Convert gyro only from explicit metadata."""
...
def quat_to_rotation(quaternion: np.ndarray) -> np.ndarray:
    """Convert q_NB to body-to-navigation R_NB."""
...
def quat_from_two_vectors(source: np.ndarray, target: np.ndarray) -> np.ndarray:
    """Stable shortest-arc quaternion."""
...
def tangent_basis(direction: np.ndarray) -> np.ndarray:
    """Build a deterministic two-dimensional tangent plane."""
...
def _causal_filter_axes(
    ...
    *,
    ...
    """Shared causal SOS with persistent state."""
...
class NoPrecalibrationEskf:
    """Quaternion MEKF without precalibration."""
    def __init__(self, fs_hz: float, config: EskfConfiguration | None = None) -> None:
        """Create initialization-pending state."""
    ...
    def _initialize(self, acceleration: np.ndarray) -> None:
        """Initialize tilt from the first eligible ACC observation, with yaw zero."""
    ...
    def step(self, acceleration: np.ndarray, gyroscope: np.ndarray, sample_index: int) -> dict[str, Any]:
        """Predict one sample and update using adaptive NIS gating."""
        ...
        if not initialized_now:
            ...
            # Do not omit bias-random-walk attitude integration or cross-covariance.
        ...
        if state == "no_estimate":
            # Terminal status remains latched until an explicit session reset.
...
def _timestamp_ok(
    ...
    """Validate chunk and boundary time grids."""
...
def _repair_chunk(values: np.ndarray, fs_hz: float) -> tuple[np.ndarray, np.ndarray, tuple[str, ...]]:
    """Repair only bounded internal gaps no longer than 0.25 s."""
    source = np.asarray(values, dtype=np.float64)
...
def _jerk(
    ...
    """Continuous jerk and common mask."""
...
class CausalImuProcessor:
    """Persistent causal IMU processor."""
    def __init__(
        self,
        *,
        fs_hz: float,
        ...
        """Bind explicit profile and state."""
        ...
        # Validate units during construction, not after processing the first chunk.
    ...
    def process_chunk(
        ...
        *,
        ...
        """Process one transactional chunk."""
        ...
        try:
            ...
            # Advance a copy so exceptions cannot contaminate the committed session.
        ...
        # Commit state atomically only after complete success; failed chunks do not affect the next chunk.
...
def preprocess_imu(
    ...
    *,
    ...
    """One-shot facade with explicit parameters."""
    profile = ImuProfile(
...
def estimate_gravity_no_precalibration_ekf(
    ...
    *,
    ...
    """Compatibility wrapper around stateful MEKF."""
...
def estimate_gravity_lpf(
    ...
    *,
    ...
    """Shared ACC20 followed by the causal second-order 0.3 Hz LPF comparator."""
