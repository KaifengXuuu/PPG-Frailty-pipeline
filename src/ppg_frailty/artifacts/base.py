"""Common reducer interface and fail-closed rules."""
...
IMU_REFERENCE_DERIVED9_AUGMENTATION_PROFILE_ID = "imu_axes6_plus_derived3_augmentation_ablation_v2"

class ArtifactReducer(ABC):
    """Deterministic CPU interface for reducers."""
    ...
    def reduce(
        ...
        *,
        ...
        """Return aligned output or explicit failure."""
...
def validate_ppg(
    ...
    *,
    ...
    """Validate without implicit resampling."""
...
def imu_reference_matrix(
    ...
    *,
    ...
    """Build a standardized IMU reference matrix.

    The reference uses six dynamic-acceleration and gyro axes; three derived channels
    are added only by a named augmentation profile. Derived channels require
    ...
    if "imu_valid_mask" in imu_processed:
        valid_rows = np.asarray(imu_processed["imu_valid_mask"], dtype=bool).ravel()
    ...
    standardized[valid_rows] = (matrix[valid_rows][:, keep] - center[keep]) / scale[keep]
    ...
    # Return the mask: reducers must propagate validity or fail. Zero-filled rows are not evidence.
    return np.asarray(standardized, dtype=np.float64), kept_names, valid_rows.copy()

def parameters_dict(config: Any) -> dict[str, Any]:
    """Convert frozen config to audit dictionary."""
...
def failure_result(
    ...
    *,
    ...
    """Failure results must never contain a waveform."""
...
def success_result(
    ...
    *,
    ...
    """Construct and validate a successful result."""
...
def validate_result(input_ppg: np.ndarray, result: ArtifactReductionResult) -> None:
    """Audit a reducer result at the router boundary."""
