"""V2 named historical-derived reducers.
...
These reducers preserve auditable historical algorithms and parameters. Successful
outputs are non-identity and rate-only; they are not promoted to defaults or claimed to preserve morphology.
"""
...
class CeemdLiteNlmsLegacyConfig:
    """Frozen CEEMD-lite and leaky-NLMS parameters"""
...
def _local_extrema(values: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    """Match frozen derivative-sign extrema"""
...
def _welch_peak(values: np.ndarray, fs_hz: float, low: float, high: float) -> float:
    """Return dominant Welch frequency"""
...
def _ceemd_reference(
    ...
    """Build frozen complementary-noise reference"""
...
def _nlms_clean(
    ...
    """Run frozen leaky normalized LMS"""
...
class EmdSiftingRateOnlyReducer(ArtifactReducer):
    ...
    def reduce(
        self, ppg: np.ndarray, imu_processed: Mapping[str, np.ndarray] | None, *, fs_hz: float = CANONICAL_FS_HZ
    ) -> ArtifactReductionResult:
        """Remove each residual; fail if no IMF"""
...
class CeemdLiteNlmsLegacyReducer(ArtifactReducer):
    ...
    def reduce(
        ...
        """Run channels independently, matching history"""
        ...
        try:
            ...
            return failure_result(
                ...
                str(exc),
                parameters=parameters_dict(self.config),
                diagnostics={"historical_source": HISTORICAL_CEEMD_NLMS_SOURCE, "rate_only": True},
            )

class DwtA2LegacyReducer(ArtifactReducer):
    """Legacy db4 level-2 approximation interpolation"""
    ...
    def reduce(
        ...
        """Fail unavailable without PyWavelets"""
        ...
        try:
            ...
            try:
                ...
                return failure_result(
                    self,
                    f"optional dependency PyWavelets is unavailable: {exc}",
                    status="unsupported",
                    parameters=parameters_dict(self.config),
                    diagnostics={
