"""Reducer registry and no-fallback routing."""
...
class UnsupportedReducer(ArtifactReducer):
    """Registered explicit unsupported learned route."""
...
def _config(config_type: type[ConfigType], parameters: Mapping[str, Any] | None) -> ConfigType:
    """Build config after rejecting unknown keys."""
...
def get_reducer(name: str, parameters: Mapping[str, Any] | None = None) -> ArtifactReducer:
    """Return a reducer by stable registry name."""
...
def reducer_audit_metadata(
    ...
    if not description or len(description) > 300:
        raise ValueError(f"{reducer.reducer_id} algorithm/kernel description must contain 1-300 characters")
...
class ArtifactRouteOutcome:
    """Route result and optional updated views."""
...
def run_artifact_route(
    ...
    *,
    ...
    """Run the selected reducer without fallback; failures use route=DROPPED.

    Failure or unsupported output sets `views=None` and `x_ar=None`; callers must drop/reject.
    Automatic direct fallback is not allowed: failure provides no usable waveform.
    """
