...
Typed datasets for four frozen representation modes.
"""
...
try:
    ...
    class Dataset:  # type: ignore[no-redef]
        ...
        Minimal dependency-free base class used only by estimator datasets.
        """
...
def _require_torch() -> None:
    """Fail only when tensor materialisation is requested"""
...
class SampleIdentity:
    """Immutable identity and grouping metadata"""
...
def _validate_identities(identities: list[SampleIdentity] | tuple[SampleIdentity, ...],
    ...
    """Freeze and validate identity rows"""
...
class RawWindowDataset(_IdentityDataset):
    """Raw fixed-length windows ``[N,C,T]``"""
...
class FeatureVectorDataset(_IdentityDataset):
    """One engineered feature vector per file"""
...
class FeatureMatrixDataset(_IdentityDataset):
    ...
    Ordered ``[N,D,K]`` feature matrices with mandatory column masks.
    """
    ...
    def from_contracts(
        ...
        Construct batches only from fully schema-compatible OrderedFeatureMatrixV1 objects.
        """
...
class FileBagDataset(_IdentityDataset):
    ...
    Variable-length window bags with exactly one vector per file; file-feature storage has no window dimension.
    """
...
def collate_samples(samples: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    Collate all four representations while preserving identity metadata.
    """
