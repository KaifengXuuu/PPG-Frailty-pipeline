...
Strict OOF prediction rows and Parquet writer.
"""
...
class OofPredictionRow:
    ...
    One auditable prediction before or after hierarchical aggregation.
    """
    ...
    def __post_init__(self) -> None:
        ...
        if self.retained or probability.size:
            ...
            if not np.isclose(probability.sum(), 1.0, atol=1e-06):
                raise ValueError("probabilities must sum to one")
        if self.class_order and (len(self.class_order) != probability.size
        ...
        if self.training_seed is not None:
            object.__setattr__(self, "training_seed", _normalized_oof_seed(self.training_seed, field="training_seed"))
        ...
        if not isinstance(self.member_training_seeds, (list, tuple)):
            raise ValueError("member_training_seeds must be an ordered list or tuple")
        member_seeds = tuple(
            (_normalized_oof_seed(value, field="member_training_seeds") for value in self.member_training_seeds))
        if self.prediction_kind == "single_model":
            ...
            if not member_seeds or len(member_seeds) != len(set(member_seeds)):
                raise ValueError("ensemble_average requires a non-empty unique member seed roster")
...
def validate_unique_subject_oof(rows: Iterable[OofPredictionRow]) -> None:
    ...
    Ensure each subject belongs to only one fold per repeat/configuration.
    """
...
FORMAL_TRACE_FIELDS = tuple(
    "config_hash manifest_hash fold_hash preprocessing_hash feature_hash model_hash representation_mode signal_route "
    "code_commit data_schema_id feature_schema_id model_version aggregation_rule environment_hash manifest_version "
    "fold_registry_version artifact_reducer_name artifact_reducer_version route_status source_snapshot_hash".split())
...
def validate_expected_oof_roster(
    ...
    *,
    ...
    For every (repeat, fold, seed), configuration and frozen held-out subject, single models
    require exactly one row; ensembles require precisely member IDs 0..N-1. Rejected subjects
    remain as retained=False rows, keeping completeness separate from coverage statistics.
    """
    frozen = tuple(rows)
    roster = {
    ...
    if (isinstance(expected_member_count, (bool, np.bool_)) or not isinstance(expected_member_count, (int, np.integer))
    ...
    ensemble_expected = expected_member_count != 1 if expect_ensemble is None else bool(expect_ensemble)
    ...
    expected_combinations = {(repeat, fold, seed, config_hash)
                             for repeat, fold, seed in roster for config_hash in configurations}
    observed_combinations = {(row.repeat, row.fold, row.seed, row.config_hash) for row in selected}
    ...
    for row in selected:
        ...
        if require_trace:
            missing_trace = [name for name in FORMAL_TRACE_FIELDS if not str(getattr(row, name) or "").strip()]
            if missing_trace:
        ...
        for config_hash in configurations:
            for participant in roster[repeat, fold, seed]:
                ...
                if len(values) != expected_rows:
                    raise ValueError(
                        f"OOF completeness failed for {key}: expected {expected_rows}, observed {len(values)}")
                if not ensemble_expected:
                    ...
                    observed_member_seeds = tuple((int(row.training_seed) for row in ordered_members))
                    if (len(average.member_training_seeds) != expected_member_count
                            or observed_member_seeds != average.member_training_seeds):
                        raise ValueError("OOF ensemble average roster does not match indexed member seeds")
                    member_fields = {
                        "probabilities",
                        "window_id",
                        "member_index",
                    ...
                    coherent_fields = set(OofPredictionRow.__dataclass_fields__) - member_fields
                    for member in members:
                        drift = [name for name in coherent_fields if getattr(member, name) != getattr(average, name)]
                        if drift:
                            raise ValueError(f"OOF ensemble member/average truth or trace drift for {key}: {drift}")
                    ...
                    if True in retained_states:
                        expected_average = np.asarray([row.probabilities for row in ordered_members],
                        ...
                        if not np.allclose(
                            ...
                            raise ValueError(f"ensemble average OOF row is not the exact member mean for {key}")
                    else:
...
_GENERIC_SEED_SEMANTICS = (
    b"single_model_rows_store_the_executed_training_seed;ensemble_member_rows_store_indexed_explicit_member_seeds;"
    b"ensemble_average_rows_store_the_complete_ordered_member_roster")
_GENERIC_ENSEMBLE_SEMANTICS = b"n_member_rows_plus_exact_probability_average"
_LEGACY_SEED_SEMANTICS = (
    b"ordinary_single_outer_cv_training_seed_equals_repeat_split_seed;ensemble_member_seeds_are_fixed_and_"
    b"split_seed_independent;matched_single_comparator_uses_member0_seed_50042")
_LEGACY_ENSEMBLE_SEMANTICS = b"five_member_rows_plus_exact_probability_average"


def _arrow_schema(pa: object, *, empty_reason: str | None = None, legacy_ensemble_metadata: bool = False) -> object:
    ...
    fields = [
        ...
        pa.field("probabilities", pa.list_(pa.field("element", pa.float64(), nullable=False)), nullable=False),
        ...
        pa.field("member_training_seeds", pa.list_(pa.field("element", pa.int64(), nullable=False)), nullable=False),
        pa.field("ensemble_base_model_id", pa.string(), nullable=False),
        pa.field("class_order", pa.list_(pa.field("element", pa.int64(), nullable=False)), nullable=False),
    ...
    metadata = {
        ...
        b"seed_semantics": _LEGACY_SEED_SEMANTICS if legacy_ensemble_metadata else _GENERIC_SEED_SEMANTICS,
        b"ensemble_semantics": _LEGACY_ENSEMBLE_SEMANTICS if legacy_ensemble_metadata else _GENERIC_ENSEMBLE_SEMANTICS,
        b"artifact_state": b"empty" if empty_reason is not None else b"populated",
        b"empty_reason": str(empty_reason).strip().encode("utf-8") if empty_reason is not None else b"",
    }
...
def _schema_for_readback(pa: object, table: object) -> object:
    ...
    semantics = (metadata.get(b"seed_semantics"), metadata.get(b"ensemble_semantics"))
    ...
    if state == b"populated":
        ...
        return _arrow_schema(pa, legacy_ensemble_metadata=legacy_ensemble_metadata)
    if state == b"empty":
        try:
        ...
        return _arrow_schema(pa, empty_reason=reason, legacy_ensemble_metadata=legacy_ensemble_metadata)
...
class OofWriter:
    """Fail-closed optional-pyarrow Parquet writer."""
    def write(self, rows: Iterable[OofPredictionRow], path: str | Path) -> Path:
        """Atomically write validated rows."""
        frozen = tuple(rows)
        if not frozen:
        ...
        try:
            ...
            raise RuntimeError(
                "OOF Parquet output requires optional dependency pyarrow; CSV fallback is forbidden") from exc
        target = Path(path)
...
def read_oof_parquet_metadata(path: str | Path) -> dict[str, str]:
    ...
    if not table.schema.equals(expected, check_metadata=True):
        raise ValueError("OOF Parquet does not match the exact V2 schema/metadata")
    return {key.decode("ascii"): value.decode("utf-8") for key, value in (table.schema.metadata or {}).items()}
