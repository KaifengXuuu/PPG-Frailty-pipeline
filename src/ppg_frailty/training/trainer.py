...
from .datasets import FeatureMatrixDataset, collate_samples


def _require_torch() -> None:
    """Fail only for deep operations."""
    if torch is None or nn is None or DataLoader is None:
        raise ImportError("deep training requires torch; fit_estimator remains available without it")
...
def resolve_torch_training_device(requested: str, *, deterministic_algorithms: bool) -> Any:
    ...
    if deterministic_algorithms:
        ...
        if workspace is None:
            ...
            raise RuntimeError("deterministic CUDA training requires CUBLAS_WORKSPACE_CONFIG to be :4096:8 or :16:8")
    if not torch.cuda.is_available():
        raise RuntimeError(
            "CUDA training was requested but torch.cuda.is_available() is false; CPU fallback is forbidden")
    if device.index is not None and device.index >= torch.cuda.device_count():
        raise RuntimeError(
            f"CUDA device index {device.index} is unavailable; visible device count={torch.cuda.device_count()}")
    return device


@dataclass(frozen=True)
class FrozenOuterSplit:
    """Immutable subject membership for one repeat/fold."""
    ...
    def assert_train_only(self, participant_ids: list[str] | tuple[str, ...], *, exact: bool = False) -> None:
        """Reject outer leakage and optionally require the complete train roster."""
    ...
    def bind_training_dataset(self, dataset: Dataset) -> "FrozenOuterSplit":
        """Bind this membership to the training rows and exposed input values."""
    ...
    def membership_hash(self) -> str:
        """Stable hash of exact membership."""
...
class InnerGroupedSplit:
    """Participant identities for one deterministic grouped inner fold."""
    ...
    def __post_init__(self) -> None:
        ...
        if isinstance(self.n_folds, bool) or not isinstance(self.n_folds, (int, np.integer)):
            raise ValueError("inner n_folds must be an integer")
        if isinstance(self.validation_fold_index, bool) or not isinstance(self.validation_fold_index,
                                                                          (int, np.integer)):
        ...
        if not 0 <= int(self.seed) <= 4294967295:
    ...
    def validate(self, outer: FrozenOuterSplit) -> None:
        """Require disjoint subsets of outer-train."""
    ...
    def membership_hash(self) -> str:
        ...
        return stable_payload_sha256({
            ...
            "validation_participant_ids": tuple(sorted(self.validation_participant_ids)),
...
DEEP_EPOCH_PROFILES: dict[str, int] = {"default_10": 10, "ablation_7": 7, "ablation_15": 15}
...
def derived_epoch_profile(epoch_rule: str, fixed_epochs: int) -> str:
    ...
    named_profiles = {epoch_count: profile_name for profile_name, epoch_count in DEEP_EPOCH_PROFILES.items()}
...
TRAINING_CLASS_WEIGHTINGS = frozenset({"inverse_frequency", "effective_number", "none"})
...
TRAINING_LOSSES = frozenset({"cross_entropy", "balanced_softmax", "focal_loss"})
...
def _finite_optimizer_float(
    ...
    *,
    ...
    if not strictly_positive and resolved < minimum:
        raise ValueError(f"optimizer_parameters.{field} must be at least {minimum:g}")
    if maximum_exclusive is not None and resolved >= maximum_exclusive:
        raise ValueError(f"optimizer_parameters.{field} must be below {maximum_exclusive:g}")
    return resolved


def resolve_optimizer_parameters(optimizer: str, parameters: Any) -> dict[str, Any]:
    """Materialize every parameter accepted by the selected optimizer."""
    ...
    if unknown:
        raise ValueError(f"optimizer_parameters for {optimizer} contain unsupported keys: {unknown}")
    ...
    if optimizer in {"adam", "adamw"}:
        ...
        beta1 = _finite_optimizer_float(betas[0], field="betas[0]", maximum_exclusive=1.0)
        beta2 = _finite_optimizer_float(betas[1], field="betas[1]", maximum_exclusive=1.0)
        resolved["betas"] = [beta1, beta2]
        resolved["eps"] = _finite_optimizer_float(resolved["eps"], field="eps", strictly_positive=True)
        for field_name in ("amsgrad", "maximize"):
            if not isinstance(resolved[field_name], bool):
                raise ValueError(f"optimizer_parameters.{field_name} must be boolean")
    elif optimizer == "sgd":
        resolved["momentum"] = _finite_optimizer_float(resolved["momentum"], field="momentum")
        resolved["dampening"] = _finite_optimizer_float(resolved["dampening"], field="dampening")
        for field_name in ("nesterov", "maximize"):
            if not isinstance(resolved[field_name], bool):
                raise ValueError(f"optimizer_parameters.{field_name} must be boolean")
        if resolved["nesterov"] and (resolved["momentum"] <= 0.0 or resolved["dampening"] != 0.0):
            raise ValueError("SGD nesterov requires positive momentum and zero dampening")
    else:
        resolved["alpha"] = _finite_optimizer_float(resolved["alpha"], field="alpha", maximum_exclusive=1.0)
        resolved["eps"] = _finite_optimizer_float(resolved["eps"], field="eps", strictly_positive=True)
        resolved["momentum"] = _finite_optimizer_float(resolved["momentum"], field="momentum")
    ...
    if isinstance(value, str):
        ...
        if text.endswith("%"):
            try:
                ...
                raise ValueError("participant_window_quota percentage is not numeric") from exc
            if not np.isfinite(percentage) or not 0.0 < percentage <= 100.0:
                raise ValueError("participant_window_quota percentage must be in (0,100]")
            return f"{percentage:g}%"
        try:
            value = float(text)
        except ValueError as exc:
            raise ValueError("participant_window_quota must be all, a positive count, or a fraction") from exc
    if (isinstance(value, bool) or not isinstance(value, (int, float, np.integer, np.floating))
            or (not np.isfinite(value))):
        raise ValueError("participant_window_quota must be all, a positive count, or a fraction")
    numeric = float(value)
    if 0.0 < numeric <= 1.0 and (not numeric.is_integer()):
        return f"{numeric * 100.0:g}%"
    if numeric <= 0.0 or not numeric.is_integer():
        raise ValueError("participant_window_quota must be a positive integer or fraction in (0,1]")
    return str(int(numeric))


@dataclass(frozen=True)
class TrainingConfig:
    """Executable epoch, optimizer, sampling, weighting and loss controls."""
    ...
    def __post_init__(self) -> None:
        ...
        if self.epoch_rule == "fixed_epoch":
            if self.maximum_inner_epochs != 0 or self.inner_patience != 0 or self.inner_grouped_folds != 0:
                raise ValueError("fixed_epoch requires zero inner epochs, patience and folds")
        else:
            if self.maximum_inner_epochs <= 0:
                raise ValueError("inner_grouped_selection requires positive maximum_inner_epochs")
            if self.inner_patience <= 0:
                raise ValueError("inner_grouped_selection requires positive inner_patience")
        ...
        object.__setattr__(self, "epoch_profile", derived_epoch_profile(self.epoch_rule, int(self.fixed_epochs)))
        if self.batch_size <= 0 or self.num_workers < 0:
            raise ValueError("batch_size must be positive and num_workers non-negative")
        if self.seed < 0 or self.seed > 4294967295:
            raise ValueError("seed must be in [0, 2^32-1]")
        _finite_optimizer_float(self.learning_rate, field="learning_rate", strictly_positive=True)
        _finite_optimizer_float(self.weight_decay, field="weight_decay")
        _finite_optimizer_float(self.focal_gamma, field="focal_gamma")
        _finite_optimizer_float(self.class_weight_beta, field="class_weight_beta", maximum_exclusive=1.0)
        if not isinstance(self.optimizer, str) or self.optimizer not in TRAINING_OPTIMIZERS:
            raise ValueError(f"optimizer must be one of {sorted(TRAINING_OPTIMIZERS)}")
        object.__setattr__(self, "optimizer_parameters",
        ...
        if self.class_weighting in _CLASS_WEIGHTING_INPUT_ALIASES:
            canonical_weighting, implied_basis = _CLASS_WEIGHTING_INPUT_ALIASES[self.class_weighting]
            object.__setattr__(self, "class_weighting", canonical_weighting)
            object.__setattr__(self, "class_count_basis", implied_basis)
        if not isinstance(self.class_weighting, str) or self.class_weighting not in TRAINING_CLASS_WEIGHTINGS:
            raise ValueError(f"class_weighting must be one of {sorted(TRAINING_CLASS_WEIGHTINGS)}")
        if not isinstance(self.class_count_basis, str) or self.class_count_basis not in TRAINING_CLASS_COUNT_BASES:
            raise ValueError(f"class_count_basis must be one of {sorted(TRAINING_CLASS_COUNT_BASES)}")
        _finite_optimizer_float(self.label_smoothing, field="label_smoothing", maximum_exclusive=1.0)
        if self.gradient_clip_norm is not None:
            _finite_optimizer_float(self.gradient_clip_norm, field="gradient_clip_norm", strictly_positive=True)
        ...
        if self.samples_per_epoch is not None and (isinstance(
                self.samples_per_epoch, bool) or not isinstance(self.samples_per_epoch,
                                                                (int, np.integer)) or int(self.samples_per_epoch) <= 0):
        ...
        if self.samples_per_epoch is not None and self.sampler not in {
            ...
            raise ValueError(
                "samples_per_epoch applies only to replacement samplers; subject samplers use participant_window_quota")
        quota = normalize_participant_window_quota(self.participant_window_quota)
        if quota != "all" and self.sampler not in {"subject_balanced", "class_subject_balanced"}:
            raise ValueError("participant_window_quota applies only to subject samplers")
        ...
        object.__setattr__(self, "classifier_role_families",
        ...
        if self.loss in _TRAINING_LOSS_INPUT_ALIASES:
            if self.class_weighting == "none":
                raise ValueError("weighted_ce compatibility alias requires an active class_weighting strategy")
            object.__setattr__(self, "legacy_loss_alias", str(self.loss))
            object.__setattr__(self, "loss", _TRAINING_LOSS_INPUT_ALIASES[str(self.loss)])
        ...
        if self.loss == "balanced_softmax" and self.class_weighting != "none":
            raise ValueError(
                "balanced_softmax owns its configured count-basis correction and requires class_weighting=none")
        if self.loss != "focal_loss" and float(self.focal_gamma) != 2.0:
            raise ValueError("focal_gamma is configurable only when loss=focal_loss")
        if self.class_weighting != "effective_number" and float(self.class_weight_beta) != 0.999:
            raise ValueError("class_weight_beta is configurable only when class_weighting=effective_number")
        if not isinstance(self.cache_policy, str) or self.cache_policy not in TRAINING_CACHE_POLICIES:
            raise ValueError(f"cache_policy must be one of {sorted(TRAINING_CACHE_POLICIES)}")
        ...
        if not self.refit_on_all_outer_training:
            raise ValueError("refit_on_all_outer_training must remain true to prevent partial refits")
        if self.n_classes <= 1:
    ...
    def _with_epoch_override(self, fixed_epochs: int) -> "TrainingConfig":
        ...
        if isinstance(fixed_epochs, bool) or not isinstance(fixed_epochs, (int, np.integer)) or int(fixed_epochs) <= 0:
    ...
    def from_mapping(cls, value: dict[str, Any]) -> "TrainingConfig":
        """Construct from a resolved training block without ignored keys."""
        accepted = {field_name for field_name, definition in cls.__dataclass_fields__.items() if definition.init}
    ...
    def validate_for_execution_backend(self, execution_backend: str) -> None:
        """Validate the selected execution backend; shared controls execute in both paths."""
        if execution_backend not in {"torch", "estimator"}:
            raise ValueError(f"unknown training execution_backend: {execution_backend!r}")
        if execution_backend == "estimator" and self.samples_per_epoch is not None:
            raise ValueError("execution_backend=estimator does not support samples_per_epoch replacement draws")
...
def materialize_deep_epoch_config(model_id: str,
                                  config_id: str,
                                  *,
                                  base_config: TrainingConfig | None = None) -> TrainingConfig:
    ...
    if model_factory_contract(machine_id)["execution_backend"] != "torch":
        raise ValueError("epoch 7/10/15 profiles are deep-only; non-iterative estimators do not execute epoch controls")
    try:
...
def materialize_all_deep_epoch_configs(model_id: str,
                                       *,
                                       base_config: TrainingConfig | None = None) -> dict[str, TrainingConfig]:
    """Return the three named materializable identities without executing them."""
    return {
        config_id: materialize_deep_epoch_config(model_id, config_id, base_config=base_config)
...
class FittedObjectProvenance:
    """Fold-local audit record for every fitted object."""
...
class TrainingResult:
    """Fitted model plus auditable history."""
...
def dataset_participant_ids(dataset: Dataset) -> tuple[str, ...]:
    """Read row identities through nested ``Subset`` objects."""
...
def dataset_identities(dataset: Dataset) -> tuple[Any, ...]:
    """Return row identities through nested subsets."""
    if Subset is not None and isinstance(dataset, Subset):
...
def validate_dataset_identity_coherence(dataset: Dataset) -> None:
    ...
    Reject inconsistent participant, file, window or label identities; providing only
    participant IDs cannot bypass validation.
    """
...
def _hash_dataset_value(digest: "hashlib._Hash", name: str, value: Any) -> None:
    """Update one content digest deterministically."""
...
def dataset_binding_hash(dataset: Dataset) -> str:
    ...
    found = False
    for name in ("values", "sample_mask", "sample_masks", "row_mask", "window_bags", "window_masks", "file_features"):
...
def participant_file_window_sampling_weights(dataset: Dataset,
                                             *,
                                             training_balance: str = "equal_files",
                                             allowed_role_families: tuple[str, ...] = ("B", "R")) -> np.ndarray:
    ...
    if outside:
        raise ValueError(f"classifier rows contain role families outside the declared input set: {outside}")
    files_by_participant: dict[str, set[str]] = {participant: set() for participant in participants}
    families_by_participant: dict[str, set[str]] = {participant: set() for participant in participants}
    ...
    for identity, family in zip(identities, family_by_row):
        ...
        if training_balance == "equal_files":
            denominator = len(participants) * len(files_by_participant[participant]) * row_counts[participant, file_id]
        else:
            denominator = (len(participants) * len(families_by_participant[participant]) *
                           len(files_by_participant_family[participant, family]) * row_counts[participant, file_id])
...
def outer_train_class_counts(dataset: Dataset, n_classes: int, *, class_count_basis: str) -> np.ndarray:
    ...
    if class_count_basis == "row":
        labels = np.asarray([int(identity.label) for identity in dataset_identities(dataset)], dtype=np.int64)
...
def outer_train_inverse_frequency_weights(dataset: Dataset,
                                          n_classes: int,
                                          *,
                                          class_count_basis: str = "row") -> np.ndarray:
    """Compute inverse-frequency weights from the configured outer-train unit."""
    counts = outer_train_class_counts(dataset, n_classes, class_count_basis=class_count_basis)
    present = counts > 0
...
def outer_train_participant_class_counts(dataset: Dataset, n_classes: int) -> np.ndarray:
...
def outer_train_effective_number_weights(dataset: Dataset,
                                         n_classes: int,
                                         *,
                                         beta: float,
                                         class_count_basis: str = "row") -> np.ndarray:
    """Return effective-number weights from the configured train-only counts."""
    if (isinstance(beta, bool) or not isinstance(beta, (int, float, np.integer, np.floating)) or (not np.isfinite(beta))
...
def outer_train_window_inverse_frequency_weights(dataset: Dataset, n_classes: int = 3) -> np.ndarray:
    """Compute the historical row-count inverse-frequency class weights."""
    return outer_train_inverse_frequency_weights(dataset, n_classes, class_count_basis="row")
...
def configured_class_weight_vector(
    ...
    *,
    ...
    if class_weighting in _CLASS_WEIGHTING_INPUT_ALIASES:
        class_weighting, class_count_basis = _CLASS_WEIGHTING_INPUT_ALIASES[class_weighting]
    if class_weighting == "inverse_frequency":
        return outer_train_inverse_frequency_weights(dataset, n_classes, class_count_basis=class_count_basis)
    if class_weighting == "effective_number":
        return outer_train_effective_number_weights(dataset,
                                                    n_classes,
...
def _subject_sampling_structure(
    dataset: Dataset, ) -> tuple[dict[str, np.ndarray], dict[str, int], dict[int, tuple[str, ...]]]:
    ...
    indices = {subject: np.asarray(rows_by_subject[subject], dtype=np.int64) for subject in sorted(rows_by_subject)}
    subjects_by_class: dict[int, tuple[str, ...]] = {}
    for label in sorted(set(label_by_subject.values())):
        subjects_by_class[label] = tuple((subject for subject in sorted(indices) if label_by_subject[subject] == label))
    return (indices, label_by_subject, subjects_by_class)


def participant_window_sampling_weights(dataset: Dataset,
                                        *,
                                        class_balanced: bool,
                                        participant_window_quota: Any = "all") -> np.ndarray:
    """Return the expected row mass of the migrated legacy subject sampler."""
    rows_by_subject, label_by_subject, subjects_by_class = _subject_sampling_structure(dataset)
...
def _torch_choice(candidates: np.ndarray, size: int, *, replacement: bool, generator: torch.Generator) -> list[int]:
    """Draw deterministic integer candidates with an explicit torch generator."""
    if size <= 0 or candidates.size <= 0:
        return []
    if replacement:
        positions = torch.randint(int(candidates.size), (int(size), ), generator=generator).cpu().numpy()
    else:
        if size > candidates.size:
            raise ValueError("non-replacement sample exceeds available candidates")
        positions = torch.randperm(int(candidates.size), generator=generator)[:int(size)].cpu().numpy()
    return [int(candidates[int(position)]) for position in positions]


def subject_epoch_sampling_indices(dataset: Dataset, *, sampler: str, participant_window_quota: Any,
    ...
    rows_by_subject, _label_by_subject, subjects_by_class = _subject_sampling_structure(dataset)
    ...
    if sampler == "subject_balanced":
        ...
        for subjects in nonempty:
            ...
            chosen_positions = _torch_choice(candidate_positions,
                                             target_slots,
    ...
    for subject in selected_subjects:
        ...
        sampled.extend(_torch_choice(candidates, quota, replacement=quota > len(candidates), generator=generator))
    if not sampled:
...
def configured_row_sampling_weights(
    ...
    *,
    ...
    if sampler == "balance_line_weighted_v2":
        return participant_file_window_sampling_weights(dataset,
                                                        training_balance=training_balance,
                                                        allowed_role_families=allowed_role_families)
    if sampler in {"uniform_replacement", "exhaustive_shuffle_without_replacement"}:
        if len(dataset) <= 0:
...
def subset_by_participants(dataset: Dataset, participant_ids: set[str]) -> Subset:
    """Create a deterministic row subset."""
    _require_torch()
    indices = [
        index for index, participant_id in enumerate(dataset_participant_ids(dataset))
...
def build_inner_grouped_split(
    ...
    *,
    ...
    if n_folds > smallest_class:
        raise ValueError("inner n_folds cannot exceed the smallest outer-train participant class")
    fold_index = ((int(outer.repeat) * 1000 + int(outer.fold)) %
                  n_folds if validation_fold_index is None else int(validation_fold_index))
    if not 0 <= fold_index < n_folds:
        raise ValueError("inner validation_fold_index is outside n_folds")
    generator = np.random.default_rng(seed)
    validation: list[str] = []
    for label in sorted(participants_by_class):
        participants = np.asarray(sorted(participants_by_class[label]), dtype=object)
        generator.shuffle(participants)
        validation.extend((str(value) for value in participants[fold_index::n_folds]))
...
def forward_batch(model: nn.Module, batch: dict[str, Any], device: torch.device) -> torch.Tensor:
    """Dispatch a typed batch without representation guessing."""
    _require_torch()
    ...
    class TrainingClassificationLoss(nn.Module):
        ...
        def __init__(
            ...
            *,
            ...
            self.loss = str(loss)
            self.register_buffer("weight", None if class_weight is None else class_weight.detach().clone())
        ...
        def forward(self, logits: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
            ...
            if self.loss == "focal_loss":
                weighted_ce = nn.functional.cross_entropy(logits,
                                                          target,
                                                          weight=self.weight,
                                                          reduction="none",
                                                          label_smoothing=self.label_smoothing)
                unweighted_ce = nn.functional.cross_entropy(logits, target, reduction="none")
                probability_true_class = torch.exp(-unweighted_ce)
                return torch.mean(torch.pow(1.0 - probability_true_class, self.focal_gamma) * weighted_ce)
            return nn.functional.cross_entropy(logits, target, weight=self.weight, label_smoothing=self.label_smoothing)


def _state_hash(model: nn.Module) -> str:
    """Hash tensor names, dtypes, shapes and bytes."""
    _require_torch()
    digest = hashlib.sha256()
...
class UnifiedTrainer:
    ...
    Unified trainer with frozen membership and no outer-label visibility. Public ``fit`` has
    neither ``outer_oof_dataset`` nor ``outer_y``; epochs are fixed or selected by grouped
    validation within outer-training data, followed by full outer-training refitting from scratch.
    """
    def __init__(self, config: TrainingConfig) -> None:
        self.config = config
        self.device = (resolve_torch_training_device(
            config.device, deterministic_algorithms=config.deterministic_algorithms) if torch is not None else None)
    ...
    def _loader(self,
                ...
                *,
                shuffle: bool,
                seed_offset: int = 0,
                absolute_seed: int | None = None) -> DataLoader:
        """Build a seeded deterministic loader."""
        _require_torch()
        generator = torch.Generator()
        generator.manual_seed(self.config.seed + seed_offset if absolute_seed is None else int(absolute_seed))
        sampler = None
        if shuffle and self.config.sampler in {"balance_line_weighted_v2", "uniform_replacement"}:
            sampler = WeightedRandomSampler(
                ...
                if self.config.samples_per_epoch is None else int(self.config.samples_per_epoch),
                replacement=True,
                generator=generator,
            )
        elif shuffle and self.config.sampler in {"subject_balanced", "class_subject_balanced"}:
            sampler = list(
    ...
    def _criterion(self, dataset: Dataset) -> nn.Module:
        ...
        torch_weight = (None if self.config.class_weighting == "none" else torch.as_tensor(
            weights, dtype=torch.float32, device=self.device))
        class_counts = outer_train_class_counts(dataset,
                                                self.config.n_classes,
                                                class_count_basis=self.config.class_count_basis)
        return TrainingClassificationLoss(
            loss=self.config.loss,
            class_weight=torch_weight,
            class_counts=torch.as_tensor(class_counts, dtype=torch.float32, device=self.device),
    ...
    def _optimizer(self, member: nn.Module) -> torch.optim.Optimizer:
        ...
        parameters = resolve_optimizer_parameters(self.config.optimizer, getattr(self.config, "optimizer_parameters",
                                                                                 {}))
        if "betas" in parameters:
            parameters["betas"] = tuple(parameters["betas"])
        return optimizer_type(member.parameters(),
                              lr=self.config.learning_rate,
                              weight_decay=self.config.weight_decay,
                              **parameters)

    def _train_member_epoch(self, model: nn.Module, loader: DataLoader, optimizer: torch.optim.Optimizer,
                            criterion: nn.Module) -> float:
        """Train one independent member for one epoch."""
        model.train()
        ...
        for batch in loader:
            ...
            if self.config.gradient_clip_norm is not None:
                torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=float(self.config.gradient_clip_norm))
            optimizer.step()
            total_loss += float(loss.detach()) * labels.shape[0]
            total_rows += int(labels.shape[0])
        return total_loss / max(total_rows, 1)

    def _train_epochs(self, model: nn.Module, dataset: Dataset, epochs: int, *,
                      training_data_scope: str) -> list[dict[str, bool | float | int | str]]:
        ...
        member_seeds = tuple((int(value) for value in getattr(model, "member_seeds", (self.config.seed, ))))
        ...
        for epoch in range(1, epochs + 1):
            for member_index, (member, optimizer) in enumerate(zip(members, optimizers)):
                ...
                epoch_seed = member_seed + epoch * 1000000
            ...
            probability, labels, identities = self._predict_probabilities_with_identities(model, dataset)
            participant_probability, participant_labels = self._participant_training_predictions(
                probability, labels, identities, balance_line=self.config.expected_aggregation_rule)
            score = balanced_accuracy_score(participant_labels, participant_probability.argmax(axis=1))
            history.append({
                "epoch": epoch,
                "training_participant_balanced_accuracy": float(score),
                "training_balanced_accuracy_unit": "participant",
                "training_balanced_accuracy_aggregation_rule": self.config.expected_aggregation_rule,
    ...
    def _predict_probabilities_with_identities(self, model: nn.Module,
                                               dataset: Dataset) -> tuple[np.ndarray, np.ndarray, tuple[Any, ...]]:
        """Predict inner probabilities with row-aligned immutable identities.

        Predict inner probabilities together with immutable row identities.
        """
        model.to(self.device)
        model.eval()
        ...
        with torch.no_grad():
            for batch in self._loader(dataset, shuffle=False):
                if hasattr(model, "predict_probabilities"):
                    ...
                    mask = batch.get("mask")
                    probability = model.predict_probabilities(x, None if mask is None else mask.to(self.device))
                else:
    ...
    @staticmethod
    def _validate_row_aligned_probabilities(probability: np.ndarray, labels: np.ndarray, identities: tuple[Any, ...], *,
                                            n_classes: int) -> tuple[np.ndarray, np.ndarray, tuple[Any, ...]]:
        """Validate public prediction output."""
        values = np.asarray(probability, dtype=np.float64)
        targets = np.asarray(labels, dtype=np.int64)
        if values.ndim != 2 or values.shape != (targets.size, int(n_classes)) or targets.size != len(identities):
            raise ValueError("probabilities, labels and identities are not row-aligned")
        if (not np.isfinite(values).all() or np.any(values < 0.0)
                or (not np.allclose(values.sum(axis=1), 1.0, rtol=0.0, atol=1e-06))):
            raise ValueError("predict_probabilities must return finite normalised probabilities")
        return (values, targets, tuple(identities))

    def predict_probabilities(self, model: nn.Module,
                              dataset: Dataset) -> tuple[np.ndarray, np.ndarray, tuple[Any, ...]]:
        """Predict a deep/ensemble/FileBag dataset without fitting.

        Perform fit-free prediction for deep models, five-member ensembles or FileBags, returning
        probabilities, labels and identities aligned exactly to input rows.
        """
        _require_torch()
        probability, labels, identities = self._predict_probabilities_with_identities(model, dataset)
        return self._validate_row_aligned_probabilities(probability,
                                                        labels,
                                                        identities,
                                                        n_classes=self.config.n_classes)

    def predict_ensemble_members(self, model: nn.Module,
                                 dataset: Dataset) -> tuple[np.ndarray, np.ndarray, np.ndarray, tuple[Any, ...]]:
        ...
        with torch.no_grad():
            for batch in self._loader(dataset, shuffle=False):
                ...
                x = batch["x"].to(self.device)
                mask = batch.get("mask")
                members = model.member_probabilities(x, None if mask is None else mask.to(self.device))
        ...
        self._validate_row_aligned_probabilities(averaged, targets, tuple(identities), n_classes=self.config.n_classes)
        expected_member_count = len(tuple(model.member_seeds))
        if (expected_member_count <= 0 or member_probability.shape[0] != expected_member_count
                or (not np.allclose(member_probability.sum(axis=-1), 1.0, rtol=0.0, atol=1e-06))):
            raise RuntimeError("ensemble member probabilities violate the declared roster contract")
        return (member_probability, averaged, targets, tuple(identities))

    def predict_estimator_probabilities(self, estimator: Any,
                                        dataset: Dataset) -> tuple[np.ndarray, np.ndarray, tuple[Any, ...]]:
        """Predict estimator probabilities with the dataset mask when supported.

        Predict estimators through the shared interface; supply FeatureMatrix row masks or
        raw sample masks if declared by the estimator. Never fit in this method.
        """
        if Subset is not None and isinstance(dataset, Subset):
    ...
    def _predict_probabilities(self, model: nn.Module, dataset: Dataset) -> tuple[np.ndarray, np.ndarray]:
        """Backward-compatible probability-only helper."""
        probability, labels, _ = self.predict_probabilities(model, dataset)
        return (probability, labels)

    @staticmethod
    def _participant_validation_predictions(probability: np.ndarray, labels: np.ndarray, identities: tuple[Any, ...], *,
                                            balance_line: str) -> tuple[np.ndarray, np.ndarray]:
        """Aggregate inner validation with the configured Line A/Line B rule."""
        return UnifiedTrainer._participant_training_predictions(probability,
                                                                labels,
                                                                identities,
                                                                balance_line=balance_line)

    @staticmethod
    def _participant_training_predictions(probability: np.ndarray, labels: np.ndarray, identities: tuple[Any, ...], *,
                                          balance_line: str) -> tuple[np.ndarray, np.ndarray]:
        ...
        for index, identity in enumerate(identities):
            ...
            file_groups.setdefault((participant, role, str(identity.file_id)), []).append(index)
        file_probabilities: dict[tuple[str, str], list[np.ndarray]] = {}
        for (participant, role, _), indices in file_groups.items():
            file_probabilities.setdefault((participant, role),
                                          []).append(probability[np.asarray(indices, dtype=np.int64)].mean(axis=0))
        ...
        if balance_line == LINE_A_EQUAL_FILES:
            ...
            for (participant, _), values in file_probabilities.items():
                participant_groups.setdefault(participant, []).append(np.asarray(values, dtype=np.float64).mean(axis=0))
        ...
        aggregated = np.asarray([
            np.asarray(participant_groups[participant], dtype=np.float64).mean(axis=0) for participant in participants
        ])
        ...
        participant_labels = np.asarray([labels_by_participant[participant] for participant in participants],
                                        dtype=np.int64)
        return (aggregated, participant_labels)

    def _select_epoch(self, model: nn.Module, inner_train: Dataset,
                      inner_validation: Dataset) -> tuple[int, list[dict[str, bool | float | int | str]]]:
        """Select an epoch using inner labels only."""
        model.to(self.device)
        members = list(model.members) if hasattr(model, "members") else [model]
        member_seeds = tuple((int(value) for value in getattr(model, "member_seeds", (self.config.seed, ))))
        if len(member_seeds) != len(members):
        ...
        for epoch in range(1, self.config.maximum_inner_epochs + 1):
            for member_index, (member, optimizer) in enumerate(zip(members, optimizers)):
                ...
                epoch_seed = member_seed + epoch * 1000000
                self._set_absolute_seed(epoch_seed)
                loss = self._train_member_epoch(member, self._loader(inner_train,
                                                                     shuffle=True,
                                                                     absolute_seed=epoch_seed), optimizer, criterion)
                history.append({
                    "epoch": epoch,
                    "member": member_index,
                    "training_seed": member_seed,
            ...
            probability, labels, identities = self._predict_probabilities_with_identities(model, inner_validation)
            participant_probability, participant_labels = self._participant_validation_predictions(
                probability, labels, identities, balance_line=self.config.expected_aggregation_rule)
            score = balanced_accuracy_score(participant_labels, participant_probability.argmax(axis=1))
            ...
            history.append({
                ...
                "inner_selection_aggregation_rule": self.config.expected_aggregation_rule,
                "outer_heldout_used": False,
            })
    ...
    def fit(
        ...
        *,
        ...
        Fit without accepting/reading outer-OOF labels. After inner epoch selection, recreate
        the model and retrain on all outer-training data, without omitting training participants.
        """
        _require_torch()
        ...
        if self.config.epoch_rule == "fixed_epoch":
            ...
            if isinstance(frozen_split, FullCohortRefitScope):
                raise ValueError("inner epoch selection belongs to an outer fold, not final all-cohort refit")
            if inner_split is None:
                raise ValueError("inner_grouped_selection requires an explicit train-only inner split")
            ...
            inner_train = subset_by_participants(outer_train_dataset, set(inner_split.train_participant_ids))
            inner_validation = subset_by_participants(outer_train_dataset, set(inner_split.validation_participant_ids))
            ...
            outer_classes = set((int(identity.label) for identity in dataset_identities(outer_train_dataset)))
            inner_train_classes = set((int(identity.label) for identity in dataset_identities(inner_train)))
            inner_validation_classes = set((int(identity.label) for identity in dataset_identities(inner_validation)))
            if inner_train_classes != outer_classes or inner_validation_classes != outer_classes:
                raise ValueError("inner train and validation must each retain every outer-train class")
            selection_model = model_factory()
            selected_epoch, selection_history = self._select_epoch(selection_model, inner_train, inner_validation)
        self._set_seed()
        model = model_factory()
        if selection_model is not None and model is selection_model:
            raise RuntimeError("model_factory must return a fresh model for full outer-train refit")
        ...
        training_data_scope = ("full_cohort_refit_all_29"
                               if isinstance(frozen_split, FullCohortRefitScope) else "full_outer_train_only")
        fit_history = self._train_epochs(model,
                                         outer_train_dataset,
                                         selected_epoch,
                                         training_data_scope=training_data_scope)
        member_training_seeds = tuple((int(value) for value in getattr(model, "member_seeds", ())))
        ...
        provenance = FittedObjectProvenance(
            ...
            optimizer_parameters=resolve_optimizer_parameters(self.config.optimizer,
            ...
            class_weight_beta=float(getattr(self.config, "class_weight_beta", 0.999)),
            focal_gamma=float(getattr(self.config, "focal_gamma", 2.0)),
            class_count_basis=self.config.class_count_basis if self.config.loss == "balanced_softmax"
            or self.config.class_weighting != "none" else "archived_train_only_counts_not_used_by_loss",
            class_counts=tuple((float(value) for value in outer_train_class_counts(
                outer_train_dataset, self.config.n_classes, class_count_basis=self.config.class_count_basis))),
            class_weight_vector=tuple((float(value) for value in configured_class_weight_vector(
                outer_train_dataset,
                class_weighting=self.config.class_weighting,
                n_classes=self.config.n_classes,
                class_weight_beta=float(getattr(self.config, "class_weight_beta", 0.999)),
            ...
            class_weight_count_basis=self.config.class_count_basis
            if self.config.class_weighting != "none" else "not_applicable_uniform",
            sampler=self.config.sampler,
            samples_per_epoch=getattr(self.config, "samples_per_epoch", None),
            participant_window_quota=str(getattr(self.config, "participant_window_quota", "all")),
            label_smoothing=float(self.config.label_smoothing),
            ...
            inner_validation_fold_index=None if inner_split is None else int(inner_split.validation_fold_index),
            inner_membership_hash="" if inner_split is None else inner_split.membership_hash,
            inner_train_participant_ids=() if inner_split is None else tuple(sorted(inner_split.train_participant_ids)),
            inner_validation_participant_ids=() if inner_split is None else tuple(
                sorted(inner_split.validation_participant_ids)),
            refit_on_all_outer_training=bool(self.config.refit_on_all_outer_training),
            notes=("outer_labels_not_accepted_by_fit_api", ),
        )
        return TrainingResult(model=model,
                              selected_epoch=selected_epoch,
                              history=selection_history + fit_history,
                              provenance=provenance)

    def fit_estimator(self, estimator: Any, outer_train_dataset: Dataset,
                      frozen_split: FrozenOuterSplit | FullCohortRefitScope) -> TrainingResult:
        """Fit estimators under the same membership guard.

        Fit an estimator under the same membership checks.
        """
        ...
        if "sample_weight" not in inspect.signature(estimator.fit).parameters:
            raise TypeError("estimator must accept sample_weight for the declared balanced training protocol")
        ...
        state_hash = hashlib.sha256(pickle.dumps(estimator, protocol=pickle.HIGHEST_PROTOCOL)).hexdigest()
        provenance = FittedObjectProvenance(
