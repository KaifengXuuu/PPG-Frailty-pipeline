"""Prediction-only evaluation utilities."""
...
class PerClassMetrics:
    """Metrics for one declared class."""
...
class EvaluationMetrics:
    """Participant/file/window metric summary."""
...
class RepeatMetricSummary:
    ...
    Repeat-level means, population/sample standard deviations and Student-t intervals.
    """
...
class PairedDeltaSummary:
    """Paired candidate-minus-baseline fold/seed deltas."""
...
def _multiclass_brier(labels: np.ndarray, probability: np.ndarray, classes: np.ndarray) -> float:
    ...
    Multiclass Brier score: sum over classes, then average over samples.
    """
...
def _expected_calibration_error(
    ...
    ECE uses maximum-probability classes, equal-width bins and fixed boundary rules.
    """
    ...
    # Clip digitized indexes so confidence=1 falls in the final valid bin.
...
def evaluate_predictions(
    ...
    *,
    ...
    Evaluate existing predictions without fitting any object.
    """
    ...
    # Precisely renormalize float32 probability rows before sklearn metrics to avoid
    # version-dependent warnings, preserving class order.
...
def summarize_repeat_metric(values: np.ndarray | list[float] | tuple[float, ...]) -> RepeatMetricSummary:
    ...
    Calculate means, population/sample SD and explicit Student-t 95% CIs from independent repeats.
    One repeat cannot estimate an interval; return NaN for CI and sample SD.
    """
...
def paired_fold_seed_deltas(
    ...
    Calculate paired candidate-baseline differences only for identical (repeat, fold, seed) keys.
    """
...
def predict_torch_dataset(
    ...
    *,
    ...
    Predict a frozen OOF dataset and validate membership; no optimizer or fit calls occur here.
    """
