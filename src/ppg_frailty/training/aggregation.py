...
V2 balance-line aggregation; window-to-file is always the fixed first step.
"""
...
def canonical_role_family(role: str) -> str:
    ...
    if match is None:
        raise ValueError(f"unsupported role; expected B/R/S/W with optional file suffix: {role!r}")
...
class HierarchyAggregation:
    ...
    source_rows deliberately retains rejected/unavailable rows so downstream reporting
    cannot silently turn rejected samples into a smaller dataset.
    """
...
class ExperimentIdentity:
    ...
    Complete experiment identity accompanies each aggregation level. Omitting a hash or
    route field could mix different experiments, so every field belongs to the group key.
    """
...
class CoverageSummary:
    """Coverage for one experiment and one hierarchy level."""
...
def experiment_identity(row: OofPredictionRow) -> ExperimentIdentity:
    ...
    Return the shared fail-closed identity key for every grouping operation.
    """
...
def _identity_key(row: OofPredictionRow) -> tuple[object, ...]:
    """Tuple form used for deterministic sorting."""

    identity = experiment_identity(row)
    return tuple(identity.__dict__.values())


def _coverage_summaries(rows: tuple[OofPredictionRow, ...], *, balance_line: str) -> tuple[CoverageSummary, ...]:
    ...
    for _, grouped in groupby(ordered, key=_identity_key):
        ...
        retained_keys: dict[str, set[tuple[object, ...]]] = {
            "source": {unit_keys["source"][index]
                       for index, row in enumerate(values) if row.retained},
            "file": {(row.participant_id, row.file_id)
                     for row in values if row.retained},
            "participant": {(row.participant_id, )
                            for row in values if row.retained},
        }
        if balance_line == LINE_B_EQUAL_ROLE_FAMILIES:
            unit_keys["role"] = [(row.participant_id, canonical_role_family(row.role)) for row in values]
            retained_keys["role"] = {(row.participant_id, canonical_role_family(row.role))
                                     for row in values if row.retained}
        levels = (("source", "file", "participant") if balance_line == LINE_A_EQUAL_FILES else
                  ("source", "file", "role", "participant"))
        for level in levels:
...
def _mean_probabilities(
    ...
    *,
    ...
    Average probabilities; use SQI weights only when explicitly requested.
    """
...
def _group_aggregate(
    ...
    *,
    ...
    """Aggregate deterministic sorted groups."""
    ...
    for _, grouped in groupby(ordered, key=key):
        ...
        if require_constant_quality and not np.allclose(
            ...
            raise ValueError("route_file_q_rate must be constant across windows of one file")
        reference = values[0]
        output.append(
            replace(
                reference,
...
def aggregate_hierarchy(
    ...
    *,
    ...
    if quality_weight_source is None:
        resolved_weight_source = "row_quality_all_levels" if quality_weighted else QUALITY_WEIGHT_SOURCE_NONE
    else:
        resolved_weight_source = str(quality_weight_source)
        if resolved_weight_source not in QUALITY_WEIGHT_SOURCES:
            raise ValueError("quality_weight_source must be one of " f"{list(QUALITY_WEIGHT_SOURCES)}")
    if quality_weighted == (resolved_weight_source == QUALITY_WEIGHT_SOURCE_NONE):
        raise ValueError("quality_weighted and quality_weight_source disagree")
    for row in source_rows:
        experiment_identity(row)
        canonical_role_family(row.role)
        if row.aggregation_rule and row.aggregation_rule != balance_line:
            raise ValueError("OOF aggregation_rule metadata disagrees with the requested balance_line")
    ...
    duplicate_keys = [(
        *_identity_key(row),
        ...
        row.window_id,
        row.member_index,
    ) for row in retained if row.level == "window"]
    ...
    if resolved_weight_source == QUALITY_WEIGHT_SOURCE_LEGACY_WINDOW and direct_files:
        if windows or any(row.representation_mode != "raw" for row in direct_files):
            raise ValueError("legacy_window_sqi requires raw window predictions or their "
    ...
    if windows:
        ...
        generated_files = _group_aggregate(
            ...
            zero_weight_fallback=(resolved_weight_source == QUALITY_WEIGHT_SOURCE_LEGACY_WINDOW),
            require_constant_quality=(resolved_weight_source == QUALITY_WEIGHT_SOURCE_ROUTE_FILE),
            file_id_factory=lambda values: values[0].file_id,
            role_factory=lambda values: canonical_role_family(values[0].role),
        )
    direct_files = [
        replace(row, role=canonical_role_family(row.role), aggregation_rule=balance_line) for row in direct_files
    ]
    generated_files = tuple(replace(row, aggregation_rule=balance_line) for row in generated_files)
    file_rows = tuple(
        sorted(
            (*generated_files, *direct_files),
            key=lambda row: (*_identity_key(row), row.participant_id, row.role, row.file_id),
        ))
    unique_file_keys = [(*_identity_key(row), row.participant_id, row.file_id) for row in file_rows]
    ...
    participant_rows = tuple(replace(row, aggregation_rule=balance_line) for row in participant_rows)
...
def hierarchy_oof_rows(
    ...
    *,
    ...
    if aggregation.balance_line == CANONICAL_BALANCE_LINE and not aggregation.role_rows:
        raise ValueError("canonical role-aware aggregation requires role OOF rows")
    rows = (
        *(aggregation.source_rows if include_source_rows else ()),
