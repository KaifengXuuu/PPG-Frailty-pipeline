...
def _count_learned_arrays(value: Any, visited: set[int]) -> int:
    ...
    if isinstance(attributes, dict):
        ...
        return sum(
            _count_learned_arrays(item, visited) for name, item in attributes.items()
...
def model_parameter_count(model: Any) -> tuple[int, str]:
    ...
    try:
        ...
        if isinstance(model, torch.nn.Module):
            count = sum(int(parameter.numel()) for parameter in model.parameters() if parameter.requires_grad)
            if count <= 0:
        ...
        if isinstance(model, torch.nn.Module):
            ...
            with torch.inference_mode():
                if isinstance(model_input, Mapping):
                    ...
                    output = model(model_input)
                probability = (output if torch.all(output >= 0) and torch.allclose(
                    output.sum(dim=-1),
    ...
    *,
    ...
    try:
        with native_context:
            ...
            model_p50, model_p95 = _measure(lambda: _invoke_probability(model, model_input))
            native_info = tuple(
                {key: item.get(key)
                 for key in (
                     "user_api",
            ...
            if preprocessing is None:
                ...
                preprocess_p50, preprocess_p95 = _measure(lambda: preprocessing(preprocessing_input))
    finally:
    ...
    return OperationalMetrics(
        ...
        bundle_bytes=(None if bundle_path is None else measure_bundle_bytes(bundle_path)),
    )
