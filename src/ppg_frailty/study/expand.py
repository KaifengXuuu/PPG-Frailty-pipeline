...
_SLUG_RE = re.compile("[^A-Za-z0-9_-]+")
_IDENTITY_ONLY_PATHS = frozenset({"config_id"})
_NAMED_EPOCH_PROFILES = {7: "ablation_7", 10: "default_10", 15: "ablation_15"}
...
def _derived_axis_values(path: str, value: Any) -> dict[str, Any]:
    ...
    if path == "training.optimizer":
        ...
        return {"training.optimizer_parameters": resolve_optimizer_parameters(str(value), {})}
    ...
    return {"training.epoch_profile": derived_epoch_profile("fixed_epoch", int(value))}
...
def _is_reportable_parameter_path(path: str) -> bool:
    ...
    return (path not in _IDENTITY_ONLY_PATHS and path != "model.ensemble_size"
...
def _apply_execution_device(payload: dict[str, Any], requested: str | None) -> None:
    ...
    _set_dotted(payload, "training.device", str(requested).strip() if backend == "torch" else "cpu")


def _epoch_materialization_identity(*, base_path: Path, pipeline_root: Path, base_config_sha256: str,
                                    fixed_epochs: int) -> dict[str, Any]:
    ...
    matches = [dict(row) for row in entries if int(row["fixed_epochs"]) == int(fixed_epochs)]
    if len(matches) != 1:
...
def _set_dotted(payload: dict[str, Any], path: str, value: Any, *, allow_new_leaf: bool = False) -> None:
    fields = path.split(".")
...
def validate_canonical_expansion(expansion: StudyExpansion) -> StudyExpansion:
    ...
    for case in expansion.cases:
        try:
            ...
            raise ValueError(
                f"resolved study case {case.case_id!r} is not a valid canonical pipeline configuration: {error}"
            ) from error
    return expansion


def parse_study_plan(payload: Mapping[str, Any], *, plan_path: str | Path | None = None) -> StudyPlan:
    raw = dict(payload)
    study_raw = dict(raw.get("study") or {})
    kind = str(study_raw.get("kind", "")).strip()
    if kind == "catalog_sweep":
        expected_top = {"schema_version", "study", "catalog", "search", "cases", "execution", "output", "report"}
        if "legacy_bridge" in raw:
            expected_top.add("legacy_bridge")
        if set(raw) != expected_top:
            raise ValueError(
                f"catalog_sweep plan key mismatch: missing={sorted(expected_top - set(raw))}, unknown={sorted(set(raw) - expected_top)}"
            )
        expected_study = {
            "study_id",
            "kind",
        ...
        if set(study_raw) != expected_study:
            raise ValueError(
                f"catalog_sweep study key mismatch: missing={sorted(expected_study - set(study_raw))}, unknown={sorted(set(study_raw) - expected_study)}"
            )
    default_role = {"single": "single_run", "ablation": "ablation", "grid": "screening"}.get(kind, "screening")
    study = StudyInfo(
        ...
        reference_case_id=None if study_raw.get("reference_case_id") in (None,
                                                                         "") else str(study_raw["reference_case_id"]),
        thesis_sections=tuple((str(value) for value in study_raw.get("thesis_sections", ()))),
    )
    axes = tuple((AxisSpec(path=str(item["path"]),
                           values=tuple(item.get("values", ())),
                           reference=item.get("reference")) for item in raw.get("axes", ())))
    ...
    report = ReportSpec(
        ...
        detailed_configuration_top_k=int(report_raw.get("detailed_configuration_top_k", 0)),
        write_html=bool(report_raw.get("write_html", True)),
        write_static_figures=bool(report_raw.get("write_static_figures", True)),
        calibration_bins=int(report_raw.get("calibration_bins", 10)),
        figure_modules=tuple(map(str, raw_figure_modules)),
        compact_mean_sd=bool(report_raw.get("compact_mean_sd", True)),
        write_excel_workbook=bool(report_raw.get("write_excel_workbook", True)),
        classification_tsne_random_state=int(report_raw.get("classification_tsne_random_state", 42)),
        classification_tsne_perplexity=float(report_raw.get("classification_tsne_perplexity", 30.0)),
        classification_tsne_max_samples=int(report_raw.get("classification_tsne_max_samples", 5000)),
        classification_roc_macro_grid_points=int(report_raw.get("classification_roc_macro_grid_points", 201)),
        classification_score_histogram_bins=int(report_raw.get("classification_score_histogram_bins", 40)),
    )
    return StudyPlan(
        schema_version=str(raw.get("schema_version", "")),
        study=study,
        base_config=None if kind == "catalog_sweep" else str(raw.get("base_config", "")),
        axes=axes,
        catalog=catalog_spec_from_mapping(raw["catalog"]) if kind == "catalog_sweep" else None,
        search=sparse_search_spec_from_mapping(raw["search"]) if kind == "catalog_sweep" else None,
        cases=catalog_cases_from_mapping(raw["cases"]) if kind == "catalog_sweep" else (),
        legacy_bridge=legacy_bridge_spec_from_mapping(raw["legacy_bridge"]) if "legacy_bridge" in raw else None,
...
def _apply_fixed_kernel_profile(payload: dict[str, Any], *, profile_id: str, pipeline_root: Path, catalog_path: Path,
                                catalog_entry: str) -> None:
    """Apply one registered raw-DL sampling profile without writing side files."""
    from ppg_frailty.config import canonical_json_bytes, load_formal_ablation_profiles
    ...
    expected_machine = "compact_cnn" if selected.model_name == "CompactCNN1D" else "inception_full"
    if payload["representation_mode"] != "raw" or machine_id != expected_machine:
        raise ValueError(f"formal profile {profile_id} is incompatible with {catalog_entry}")
    payload["windows"]["raw_dl"]["length_s"] = float(selected.raw_window_seconds)
    ...
    profiles = load_formal_ablation_profiles(pipeline_root / "configs" / "formal_ablation_profiles_v2.yaml")
    payload["output"]["formal_ablation_materialization"] = {
        "schema_version": "ppg_frailty.formal_ablation_materialization.v2",
        "family": "fixed_kernel_samples",
        "profile_id": selected.case_id,
        "catalog_role": "reference" if selected.case_id.endswith("__reference") else "ablation",
        "base_config_path": f"{catalog_path.relative_to(pipeline_root).as_posix()}#{catalog_entry}",
        "base_config_sha256": hashlib.sha256(canonical_json_bytes(base_payload)).hexdigest(),
...
def _catalog_varied_rows(cases: Iterable[ResolvedCase]) -> tuple[Mapping[str, Any], ...]:
    values = tuple(cases)
    paths = sorted({path for case in values for path in case.changed_values})
    return tuple(({
        "parameter_path": path,
        "case_values":
        {case.case_id: copy.deepcopy(case.changed_values.get(path, "not_applicable"))
         for case in values},
    } for path in paths))


def _expand_catalog_sweep(plan: StudyPlan, *, pipeline_root: Path) -> StudyExpansion:
    from ppg_frailty.catalog import resolved_catalog_payloads
    from ppg_frailty.config import load_formal_experiment_catalog, validate_config_payload
    ...
    if unknown_ids:
        raise ValueError(f"catalog cases contain unknown entries: {sorted(unknown_ids)}")
    if plan.catalog.scope in {"ordinary_active", "ordinary_13"} and requested_ids != ordinary_ids:
        raise ValueError(
            f"ordinary cases must cover the exact active registered candidate set: missing={sorted(ordinary_ids - requested_ids)}, unknown={sorted(requested_ids - ordinary_ids)}"
        )
    if plan.catalog.scope == "selected_ordinary" and (not requested_ids <= ordinary_ids):
        raise ValueError(
            f"selected_ordinary cases must use only registered ordinary entries: unknown={sorted(requested_ids - ordinary_ids)}"
        )
    payloads = resolved_catalog_payloads(pipeline_root=pipeline_root,
    ...
    payload_by_entry = {
        ...
        for entry_id, payload in ((
            str(row["entry_id"]),
            next((value for value in payloads
                  if value["config_id"] == f"{row['config_stem']}_{plan.catalog.balance_line}_v2")),
    ...
    for case in plan.cases:
        ...
        allowed_roles = ({"matched_comparator", "ensemble_comparison"} if plan.catalog.scope == "matched_ensemble_pair"
                         else {"reference_candidate", "ablation_candidate"})
        if entry["catalog_role"] not in allowed_roles:
            raise ValueError(f"catalog case cannot use role {entry['catalog_role']}")
        ...
        if case.output_group != expected_group:
            raise ValueError(
                f"{case.case_id} output_group={case.output_group!r} differs from representation_mode={expected_group!r}"
            )
        ...
        if case.formal_profile is not None:
            ...
            if case.overrides:
                raise ValueError("fixed-kernel single-factor profiles cannot add search overrides")
        ...
        payload["config_id"] = f"{payload['config_id']}__{case.screen_profile_id}"
        ...
        if plan.legacy_bridge is not None:
            profile_matches = tuple(
                (profile for profile in plan.legacy_bridge.profiles if str(profile["catalog_case_id"]) == case.case_id))
            if len(profile_matches) != 1:
                raise ValueError(f"legacy bridge case must bind exactly one frozen profile: {case.case_id}")
            changed["study.legacy_bridge_profile"] = str(profile_matches[0]["profile_id"])
            changed["study.legacy_bridge_runtime"] = plan.legacy_bridge.runtime_status
            changed["study.legacy_bridge_design"] = plan.legacy_bridge.design
            if plan.legacy_bridge.uses_inline_profiles:
                changed["study.legacy_bridge_profile_definition_sha256"] = plan.legacy_bridge.controls_sha256(
                    profile_matches[0])
        changed.update(copy.deepcopy(dict(case.overrides)))
        if case.formal_profile is not None:
            changed["study.formal_profile"] = f"{case.formal_profile.family}/{case.formal_profile.profile_id}"
            changed["signal.dl_resampling.target_fs_hz"] = checked["signal"]["dl_resampling"]["target_fs_hz"]
            changed["signal.dl_resampling.enabled"] = checked["signal"]["dl_resampling"]["enabled"]
            if "dilations" in checked["model"]:
                changed["model.dilations"] = copy.deepcopy(checked["model"]["dilations"])
    ...
    if plan.catalog.scope == "matched_ensemble_pair":
        if sorted(selected_roles) != ["ensemble_comparison", "matched_comparator"]:
            raise ValueError("matched_ensemble_pair requires one matched comparator and one ensemble comparison")
        if len(set(selected_groups)) != 1:
            raise ValueError("matched_ensemble_pair entries must use one representation route")
...
def _rows_for_parameters(cases: Iterable[ResolvedCase], *, varied: bool) -> tuple[Mapping[str, Any], ...]:
    case_values = {
        case.case_id:
        {path: value
         for path, value in flatten_mapping(case.config).items() if _is_reportable_parameter_path(path)}
        for case in cases
    }
    if not case_values:
        return ()
    ...
    for path in all_paths:
        ...
        if varied:
            ...
            rows.append({"parameter_path": path, "value": next(iter(values.values()))})
    return tuple(rows)
...
def expand_study(plan: StudyPlan, *, pipeline_root: str | Path) -> StudyExpansion:
    ...
    if base_payload.get("schema_version") == "ppg_frailty.pipeline_config.v2":
        ...
        if plan.execution.device is not None:
            base_payload = validate_config_payload(base_payload)
    combinations = [()] if not plan.axes else itertools.product(*(axis.values for axis in plan.axes))
    cases: list[ResolvedCase] = []
    base_flat = flatten_mapping(base_payload)
    for index, values in enumerate(combinations):
        changes = {axis.path: _normalize_axis_value(axis.path, value) for axis, value in zip(plan.axes, values)}
        config = copy.deepcopy(base_payload)
        derived_changes: dict[str, Any] = {}
        for path, value in changes.items():
            _set_dotted(config, path, value)
            for derived_path, derived_value in _derived_axis_values(path, value).items():
        ...
        if (not isinstance(changed_epoch, bool) and isinstance(changed_epoch, int)
            ...
            derived_changes.update(flatten_mapping({"output": {"formal_ablation_materialization": identity}}))
        if config.get("schema_version") == "ppg_frailty.pipeline_config.v2":
            ...
            if set(changes) & {"features.enabled_groups", "features.matrix_k"}:
                for field in ("registry_id", "file_vector_schema", "matrix_schema"):
                    path = f"features.{field}"
                    derived_changes[path] = config["features"][field]
            derived_changes.update(
                flatten_mapping({
                    "model": {
                        "architecture_parameters": config["model"]["architecture_parameters"],
                        "ensemble_size": config["model"]["ensemble_size"],
                    }
                }))
        case_id = (plan.study.reference_case_id if not changes and plan.study.reference_case_id
                   else _case_id(changes, index))
        original_id = str(base_payload.get("config_id", "study_config"))
        config["config_id"] = original_id if not plan.axes else f"{original_id}__{case_id}"
        changed_paths = {
            path
            for path, value in flatten_mapping(config).items()
            if path not in _IDENTITY_ONLY_PATHS and _strict_value_key(value) != _strict_value_key(base_flat.get(path))
        }
        allowed_changed_paths = set(changes) | set(derived_changes)
        allowed_changed_prefixes = tuple(
            (f"{path}." for path, value in derived_changes.items() if isinstance(value, Mapping)))
        undeclared_changed_paths = {
            path
            for path in changed_paths
            if path not in allowed_changed_paths and (not path.startswith(allowed_changed_prefixes))
        }
        if undeclared_changed_paths:
            raise ValueError(
                f"case {case_id} changed undeclared fields: expected={sorted(allowed_changed_paths)}, observed={sorted(undeclared_changed_paths)}"
            )
        resolved_flat = flatten_mapping(config)
        mismatches = {
            path: {
                "requested": value,
                "resolved": resolved_flat.get(path)
            }
            for path, value in changes.items() if _strict_value_key(resolved_flat.get(path)) != _strict_value_key(value)
        }
        if mismatches:
            raise ValueError(f"case {case_id} did not preserve requested axis values: {mismatches}")
        derived_mismatches = {
            path: {
                "derived": value,
                "resolved": _get_dotted(config, path)
            }
            for path, value in derived_changes.items()
            if _strict_value_key(_get_dotted(config, path)) != _strict_value_key(value)
        }
        if derived_mismatches:
            raise ValueError(f"case {case_id} did not preserve derived axis values: {derived_mismatches}")
        inferred_reference = bool(plan.axes) and all(
            (axis.reference is not None and value == axis.reference for axis, value in zip(plan.axes, values)))
    ...
    if len(case_ids) != len(set(case_ids)):
        raise ValueError(
            "study values produced colliding case identifiers; use values with distinct filesystem-safe representations"
        )
    explicit_reference = plan.study.reference_case_id
    if explicit_reference is not None and explicit_reference not in {case.case_id for case in cases}:
    ...
    for axis in plan.axes:
        for value in axis.values:
            for derived_path, derived_value in _derived_axis_values(axis.path, value).items():
                derived_axis_paths.add(derived_path)
                if isinstance(derived_value, Mapping):
                    derived_axis_paths.update(flatten_mapping(derived_value, derived_path))
        if axis.path in {"features.enabled_groups", "features.matrix_k"}:
            derived_axis_paths.update({"features.registry_id", "features.file_vector_schema", "features.matrix_schema"})
    if any((axis.path == "training.fixed_epochs" for axis in plan.axes)):
        derived_axis_paths.update((row["parameter_path"] for row in _rows_for_parameters(cases, varied=True)
                                   if str(row["parameter_path"]).startswith("output.formal_ablation_materialization.")))
    varied = tuple(
        (row for row in _rows_for_parameters(cases, varied=True) if row["parameter_path"] not in derived_axis_paths))
    declared = set((axis.path for axis in plan.axes))
    observed = {str(row["parameter_path"]) for row in varied}
    if observed != declared:
        raise ValueError(
            f"resolved varied parameters differ from declared axes: declared={sorted(declared)}, observed={sorted(observed)}"
        )
    return StudyExpansion(
        plan=plan,
