...
class ProgressEvent:
    ...
    def from_value(cls, value: "ProgressEvent | Mapping[str, Any] | str") -> "ProgressEvent":
        ...
        return cls(
            ...
            unit_current=(None if payload.get("unit_current") is None else int(payload.pop("unit_current"))),
            unit_total=(None if payload.get("unit_total") is None else int(payload.pop("unit_total"))),
            detail_current=(None if payload.get("detail_current") is None else int(payload.pop("detail_current"))),
            detail_total=(None if payload.get("detail_total") is None else int(payload.pop("detail_total"))),
...
class CompositeProgressSink:
    ...
    def __init__(
        ...
        *,
        ...
        self._close_sinks = self._sinks if close_sinks is None else tuple(close_sinks)
...
class TerminalProgressSink:
    ...
    def __call__(self, event: ProgressEvent) -> None:
        with self._lock:
            ...
            self._apply(event)
            self._render_locked()
            if self._ansi and self._refresh_interval > 0.0 and self._thread is None:
    ...
    def _apply(self, event: ProgressEvent) -> None:
        ...
        if event.case_id and event.unit_current is not None:
            ...
            self._units_by_case[event.case_id] = max(previous, int(event.unit_current))
            if self._study_mode:
                self._overall_current = min(self._overall_total, sum(self._units_by_case.values()))
        ...
        if name == "case_finished" and event.case_id:
            ...
            self._overall_current = min(self._overall_total, sum(self._units_by_case.values()))
            self._active.pop(event.case_id, None)
            self._overall_label = (f"{event.current}/{event.total} cases · last {event.message}"
                                   if event.total else event.message or self._overall_label)
            return
        if name == "study_finished":
            ...
            self._overall_label = " · ".join(part for part in (self._study_status, summary) if part)
            self._active.clear()
            return
        ...
        if name == "report_finished":
            ...
            self._overall_label = event.message or ("report complete" if self._study_status is None else
                                                    f"report complete · study {self._study_status}")
            return
    ...
    def _lines(self) -> tuple[str, str | None]:
        ...
        count = (f"{self._overall_current}/{self._overall_total} case-repeats"
                 if self._study_mode and self._overall_total else
                 f"{self._overall_current}/{self._overall_total}" if self._overall_total else "working")
        total_line = (f"TOTAL [{self._bar(self._overall_current, self._overall_total)}] "
        ...
        detail_count = f"{detail.current}/{detail.total}" if detail.total else "working"
        detail_line = (f" SUB  [{self._bar(detail.current, detail.total)}] {detail_count}  "
