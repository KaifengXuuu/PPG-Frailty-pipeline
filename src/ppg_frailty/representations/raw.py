"""Raw multichannel window construction."""
...
class RawWindows:
    """Fixed windows and validity masks."""
...
def build_raw_windows(
    ...
    *,
    ...
    """Build eight-channel windows from the shared plan.

    PPG always uses amplitude-preserving x_filter; non-identity x_ar serves only
    rate/PPI/PRV. Copy dynamic acceleration and gyro from the physical-unit IMU view.
    Normalize each of eight channels only within DL-window copies, never overwriting
    upstream views. Complete windows are default; explicit padding normalizes valid prefixes only.
    Padded suffixes remain neutral zeros, excluded by valid_mask from pooling and fold transforms.
    ...
    for item in windows:
        ...
        # EKF startup samples without estimates drop only affected windows; never fill
        # them with zeros or reject whole records for startup outside a window. Startup rows
