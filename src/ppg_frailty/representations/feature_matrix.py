"""OrderedFeatureMatrixV1 validation."""
...
def validate_feature_matrix(matrix: OrderedFeatureMatrixV1) -> OrderedFeatureMatrixV1:
    """Enforce D-by-K and neutral padding."""
    ...
    channel_hash = hashlib.sha256("\n".join(expected_channel_schema).encode("utf-8")).hexdigest()
    provenance = matrix.provenance
    if (matrix.schema_version != expected_schema_version or provenance.get("matrix_k") != matrix_k
            or provenance.get("matrix_length_policy") != "all_complete_windows_variable_k"
            or provenance.get("matrix_schema_version") != expected_schema_version
            or tuple(matrix.channel_schema) != expected_channel_schema or tuple(matrix.context_schema)
            or provenance.get("matrix_channel_schema_sha256") != channel_hash
            or provenance.get("validity_encoding") != "provenance_only_not_predictor_channels_v1"
            or provenance.get("padding_policy") != "none_at_record_storage_batch_only"
