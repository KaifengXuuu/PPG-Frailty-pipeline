"""Leakage-safe feature-vector baseline models."""
...
class FeatureVectorBaseline:
    ...
    Allowlisted sklearn baselines with fold-local preprocessing. Freeze feature names at
    construction and reject prediction-time column-width mismatches.
    """
    ...
    def __init__(
        ...
        *,
        ...
        if class_weight is not None:
            raise ValueError("model.class_weight is not an independent weighting capability; "
        ...
        if model_id == "logistic_regression":
            if (isinstance(logistic_c, (bool, np.bool_)) or not isinstance(logistic_c,
                                                                           (int, float, np.integer, np.floating))
                    or not np.isfinite(float(logistic_c)) or float(logistic_c) <= 0.0):
                raise ValueError("logistic_c must be finite and positive")
            if (isinstance(logistic_max_iter, (bool, np.bool_)) or not isinstance(logistic_max_iter, (int, np.integer))
            ...
            if str(logistic_solver) not in supported_solvers:
                raise ValueError(f"logistic_solver must be one of {sorted(supported_solvers)}")
            self.logistic_c = float(logistic_c)
            self.logistic_max_iter = int(logistic_max_iter)
            self.logistic_solver = str(logistic_solver)
        if model_id == "rbf_svm" and (svm_kernel != "rbf" or svm_probability is not True or float(svm_c) <= 0.0):
            raise ValueError("rbf_svm requires kernel=rbf, probability=true and positive C")
        if model_id == "extra_trees":
            if (isinstance(extra_trees_n_estimators,
                           (bool, np.bool_)) or not isinstance(extra_trees_n_estimators, (int, np.integer))
                    or int(extra_trees_n_estimators) <= 0):
                raise ValueError("extra_trees_n_estimators must be a positive integer")
            if (isinstance(extra_trees_n_jobs, (bool, np.bool_))
                    or not isinstance(extra_trees_n_jobs, (int, np.integer)) or int(extra_trees_n_jobs) == 0):
            ...
            if extra_trees_max_features is None:
                ...
                if not np.isfinite(float(extra_trees_max_features)) or not 0.0 < float(extra_trees_max_features) <= 1.0:
            ...
            if isinstance(extra_trees_min_samples_leaf, (int, np.integer)):
                ...
                normalized_min_samples_leaf: int | float = int(extra_trees_min_samples_leaf)
            elif isinstance(extra_trees_min_samples_leaf, (float, np.floating)):
                if (not np.isfinite(float(extra_trees_min_samples_leaf))
    ...
    def _make_pipeline(self) -> Pipeline:
        """Construct an unfitted allow-listed pipeline."""
        ...
        if self.model_id == "logistic_regression":
            ...
            return Pipeline([
                ...
                # Preserve frozen width even when an outer-training fold has entirely missing route features.
    ...
    def _validate_x(self, x: np.ndarray) -> np.ndarray:
        """Validate the frozen feature width."""
    ...
    def fit(
        ...
        *,
        ...
        """Fit only on explicitly supplied training rows."""
        ...
        # Record identical outer-fold membership for every fitted step so imputer/scaler
