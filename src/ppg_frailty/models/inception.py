...
Mask-aware InceptionTime single networks and probability ensembles.
"""
...
try:
    ...
    raise ImportError("InceptionTime models require the optional 'deep' dependency: pip install .[deep]") from exc
...
def _positive_integer(value: object, *, field: str) -> int:
    ...
    if isinstance(value, float) and (not math.isfinite(value) or float(normalized) != value):
...
def masked_global_average(encoded: torch.Tensor, mask: torch.Tensor | None) -> torch.Tensor:
    """Average features over valid columns only."""
...
class InceptionModule(nn.Module):
    """One multi-scale temporal Inception module."""
    ...
    def __init__(
        ...
        bottleneck_channels = _positive_integer(bottleneck_channels, field="bottleneck_channels")
...
class InceptionBlock(nn.Module):
    ...
    Six-module block with a residual connection after every three modules.
    """
    ...
    def __init__(
        ...
        residual_interval = _positive_integer(residual_interval, field="residual_interval")
        ...
        for index in range(depth):
            ...
            if (index + 1) % residual_interval == 0:
                if residual_channels == module_channels:
                    ...
                    # With matching widths the reviewed shortcut uses BN only; adding a 1x1
                    # convolution would incorrectly add 16,384 parameters to the full model.
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Apply modules and residual paths."""
...
class InceptionTimeSingleNetwork(nn.Module):
    ...
    Reviewed full/small InceptionTime network. After all convolutions, feature-matrix
    column masks enter final pooling so padded columns do not affect global means.
    """
    ...
    def __init__(
        ...
        kernels = _positive_odd_kernel_sizes(settings["kernels"] if kernel_sizes is None else kernel_sizes)
        dilation = _positive_integer(dilation, field="dilation")
        pool_size = _positive_integer(pool_size, field="pool_size")
        if pool_size % 2 == 0:
            raise ValueError("pool_size must be a positive odd integer")
        ...
        resolved_bottleneck = _positive_integer(
            settings["bottleneck"] if bottleneck_channels is None else bottleneck_channels,
            field="bottleneck_channels",
        )
        resolved_depth = _positive_integer(settings["depth"] if depth is None else depth, field="depth")
        residual_interval = _positive_integer(residual_interval, field="residual_interval")
    ...
    def forward_features(self, x: torch.Tensor, mask: torch.Tensor | None = None) -> torch.Tensor:
        """Return mask-aware pooled features."""
        ...
        if mask is not None:
            ...
            # Zero invalid columns before convolution so padding cannot contaminate adjacent valid representations.
...
class FullInceptionTimeSingleNetwork(InceptionTimeSingleNetwork):
    """Full-capacity reviewed configuration."""
...
class SmallInceptionTimeSingleNetwork(InceptionTimeSingleNetwork):
    """Reduced-capacity reviewed configuration."""
...
class InceptionTimeProbabilityEnsemble(nn.Module):
    ...
    def __init__(self, members: Sequence[nn.Module], member_seeds: Sequence[int]) -> None:
        ...
        for seed in member_seeds:
            ...
            if isinstance(seed, float) and (not math.isfinite(seed) or float(normalized) != seed):
    ...
    def member_provenance(self) -> tuple[dict[str, int], ...]:
        ...
        return tuple({"member_index": index, "training_seed": seed} for index, seed in enumerate(self.member_seeds))

    def member_probabilities(self, x: torch.Tensor, mask: torch.Tensor | None = None) -> torch.Tensor:
        """Return ``[member,batch,class]`` probabilities."""

        return torch.stack([F.softmax(member(x, mask), dim=-1) for member in self.members], dim=0)

    def predict_probabilities(self, x: torch.Tensor, mask: torch.Tensor | None = None) -> torch.Tensor:
        """Average member probabilities."""

        return self.average_member_probabilities(self.member_probabilities(x, mask))

    def forward(self, x: torch.Tensor, mask: torch.Tensor | None = None) -> torch.Tensor:
        """Return log-probabilities suitable for NLL loss."""
