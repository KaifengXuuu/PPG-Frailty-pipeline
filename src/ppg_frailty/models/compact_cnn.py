...
Compact one-dimensional convolutional reference architecture.
...
The reviewed eight-channel, three-class architecture has 79,139 trainable parameters.
Inputs are ``[batch, channel, time]`` tensors; ``forward_features`` lets file-bag fusion
reuse the same signal encoder.
"""
...
try:
    ...
    raise ImportError("CompactCNN1D requires the optional 'deep' dependency: pip install .[deep]") from exc

class CompactCNN1D(nn.Module):
    """Reviewed compact CNN for raw-window frailty classification.

    Reviewed compact CNN for raw-window frailty classification. Global average pooling
    supports variable sequence length without assigning fixed meaning to window positions.
    """
    ...
    def __init__(
        ...
        try:
            channels = tuple(operator.index(value) for value in raw_stage_channels if not isinstance(value, bool))
        ...
        if len(dropout_values) != 2 or any(
            not math.isfinite(value) or not 0.0 <= value < 1.0 for value in dropout_values
        ):
        ...
        # Three compact convolution stages support the embedded-CPU deployment target.
    ...
    def forward_features(self, x: torch.Tensor, mask: torch.Tensor | None = None) -> torch.Tensor:
        """Return one feature vector per window.
        ...
        Accept ``mask`` for the common encoder interface. Raw windows must be presegmented
        without padded columns; reject non-all-true masks to avoid silently biased pooling.
        """
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor | None = None) -> torch.Tensor:
        """Return unnormalised class logits."""
