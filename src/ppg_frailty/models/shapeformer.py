...
No external PISD import or machine-specific paths. Shapelet discovery fits only
the explicitly provided training partition, binding its results to outer-fold provenance.
"""
...
try:
    ...
    raise ImportError("ExperimentalShapeFormer requires the optional 'deep' dependency: pip install .[deep]") from exc
...
class EffectSizeShapelets:
    ...
    Immutable outer-training-bound shapelet bank with physical time scales. Discovery
    method, sampling rate and outer-fold identity intentionally have no defaults: missing
    fields fail instead of silently replacing a PISD request with effect-size discovery.
    """
    ...
    def __post_init__(self) -> None:
        """Validate scientific identity and physical time."""
        ...
        if self.discovery_method not in _PARAMETERIZED_EFFECT_SIZE_METHODS and (
                self.shapelet_length_samples != expected_length or self.discovery_stride_samples != expected_stride):
            raise ValueError(f"{self.discovery_method} requires length/stride "
...
def _z_normalise_window(window: np.ndarray) -> np.ndarray:
    """Channel-wise stable z-normalisation."""
...
def _minimum_distance(series: np.ndarray, candidate: np.ndarray, stride: int) -> float:
    """Minimum normalised window distance."""
...
def discover_effect_size_shapelets(
    ...
    *,
    ...
    Discover candidates by absolute standardized class differences. Compute each candidate
    minimum distance to every training sequence, then rank its class against the rest by Cohen-type effect size.
    Only ``x_train/y_train`` are read; callers provide outer-training data and repeat/fold identity.
    """

    # English: This named function implements exactly one discovery method.
    # This named function implements one discovery method; unknown/PISD requests never fall back implicitly.
    if discovery_method not in EFFECT_SIZE_DISCOVERY_SPECS:
        raise ValueError("this route implements only explicitly named fixed effect-size methods; "
    ...
    if discovery_method not in _PARAMETERIZED_EFFECT_SIZE_METHODS and (shapelet_length != expected_length
                                                                       or stride != expected_stride):
        raise ValueError(f"{discovery_method} requires shapelet_length={expected_length} and stride={expected_stride}")
    ...
    for class_value in sorted(np.unique(y).tolist()):
        ...
        for sample_index in np.flatnonzero(y == class_value):
            for start in range(0, x.shape[-1] - shapelet_length + 1, stride):
                candidate_windows.append(_z_normalise_window(x[sample_index, :, start:start + shapelet_length]))
        ...
        for candidate_index, candidate in enumerate(candidate_windows):
            distances = np.asarray([_minimum_distance(series, candidate, stride) for series in x], dtype=np.float64)
            positive_values = distances[positive]
            negative_values = distances[~positive]
            if positive_values.size == 0 or negative_values.size == 0:
                continue
            pooled_variance = 0.5 * (float(np.var(positive_values, ddof=0)) + float(np.var(negative_values, ddof=0)))
...
class ExperimentalShapeFormer(nn.Module):
    ...
    def __init__(
        ...
        Build only from a fold-local shapelet bank with complete provenance.
        """

        super().__init__()
        if not isinstance(shapelets, (EffectSizeShapelets, PisdShapelets)):
            raise TypeError("ShapeFormer requires a provenance-complete EffectSizeShapelets or " "PisdShapelets bank")
        if isinstance(shapelets, EffectSizeShapelets) and (shapelets.discovery_method
                                                           not in EFFECT_SIZE_DISCOVERY_SPECS):
            raise ValueError("effect-size bank identity mismatch")
        if isinstance(shapelets, PisdShapelets) and (shapelets.discovery_method != PISD_DISCOVERY_METHOD):
        ...
        if patch_size_samples < 2:
            raise ValueError("patch_size_samples must be at least 2; raw sample-token attention is forbidden")
        ...
        feedforward_channels = (int(hidden_channels) *
                                2 if attention_feedforward_channels is None else int(attention_feedforward_channels))
        if attention_layers <= 0 or feedforward_channels <= 0:
            raise ValueError("attention_layers and attention_feedforward_channels must be positive")
        ...
        # Nonoverlapping Conv1d patches form tokens before attention. Reject patch_size=1
        # above so attention cannot operate on raw sample-point tokens.
    ...
    def provenance(self) -> dict[str, object]:
        """Return required model identity fields."""
    ...
    def _channel_specific_similarity(self, x: torch.Tensor, mask: torch.Tensor | None) -> torch.Tensor:
        ...
        for index, candidate in enumerate(self.shapelets):
            ...
            best = torch.full((x.shape[0], ), torch.inf, dtype=x.dtype, device=x.device)
            for offset in range(0, windows.shape[1], self.distance_position_chunk_size):
                ...
                correction = torch.maximum(complexity, candidate_complexity) / torch.minimum(
                    complexity, candidate_complexity).clamp_min(1e-8)
    ...
    def _fixed_shapelet_similarity(self, x: torch.Tensor, mask: torch.Tensor | None) -> torch.Tensor:
    ...
    def _sinusoidal_position(tokens: torch.Tensor) -> torch.Tensor:
        """Return deterministic physical-order encoding."""

        token_count, width = tokens.shape[1], tokens.shape[2]
        position = torch.arange(token_count, device=tokens.device, dtype=tokens.dtype)[:, None]
        ...
        encoding = torch.zeros(token_count, width, device=tokens.device, dtype=tokens.dtype)
        encoding[:, 0::2] = torch.sin(angles[:, :encoding[:, 0::2].shape[1]])
        encoding[:, 1::2] = torch.cos(angles[:, :encoding[:, 1::2].shape[1]])
        return encoding.unsqueeze(0)

    def _patch_features(self, x: torch.Tensor, mask: torch.Tensor | None) -> torch.Tensor:
        """Encode fully valid patches with masked attention and pooling.

        Encode fully valid patches only, preserving masks through attention and pooling.
        """
        ...
        if mask is not None:
            patch_valid = (mask.to(dtype=torch.float32).unfold(-1, self.patch_size_samples,
                                                               self.patch_size_samples).amin(-1) > 0)
    ...
    def forward_features(self, x: torch.Tensor, mask: torch.Tensor | None = None) -> torch.Tensor:
        """Concatenate patch and shapelet features."""
        ...
        if mask is not None:
            ...
            # Clear padding placeholders before either branch.
