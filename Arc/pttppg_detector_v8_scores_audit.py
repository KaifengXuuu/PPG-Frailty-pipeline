...
def main():
    ...
    def _plot_hist_kde(scores, y, title, out_png):
    ...
    if y_true_all.size:
        plt.figure()
        disp = ConfusionMatrixDisplay(confusion_matrix=np.asarray(cm_cv), display_labels=["sit","walk/run"])
        disp.plot(values_format="d")
    ...
    used_cols_ppg = list(ppg_feat_cols)
    used_cols_imu = list(imu_feat_cols)
    leakage_findings = {
    ...
    # choose threshold on training to maximize balanced accuracy
    imu_tr = df_tr2["imu_score_shift"].to_numpy(np.float32)
    y_tr = y_train_final
    thr_grid = np.quantile(imu_tr[np.isfinite(imu_tr)], np.linspace(0.01, 0.99, 99))
    best_thr = float(thr_grid[0]); best_ba = -1.0
    for thr in thr_grid:
        pred = (imu_tr >= thr).astype(int)
        ba = metrics.balanced_accuracy_score(y_tr, pred)
        if ba > best_ba:
    ...
    metrics_ho_imu_thr.update({"thr": best_thr, "train_bal_acc_at_thr": best_ba})
    ...
    audit_summary = {
        ...
        "F": lag_audit,
        "cm_cv": cm_cv,
        "cm_holdout": cm_ho.tolist(),
    }
    (audit_dir/"audit_summary.json").write_text(json.dumps(audit_summary, ensure_ascii=False, indent=2), encoding="utf-8")
    ...
    bundle = {
        ...
        "ppg_feature_names": list(ppg_feat_cols),
        "imu_feature_names": list(imu_feat_cols),
